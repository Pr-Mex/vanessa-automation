...
Сценарий: Передача параметров во вложенный сценарий
	...
	Теперь напишем простой feature файл по созданию элемента справочника
			...
			И я пишу "Функционал: Создание элемента Склады "
			...
			И я пишу 'Сценарий: Я создаю элемент справочника Склады с наименованием "Элемент" '
			И я пишу 'Когда В панели разделов я выбираю "Склады" '
			И я пишу 'И     В панели функций я выбираю "Склады" '
	...
	И я сохраняю файл открытый в редакторе как "Support\Instructions\Temp\СозданиеОдногоЭлементаСправочникаСклады.feature"
	...
	Теперь рядом создадим второй feature файл, который будет вызывать только что созданный сценарий
			...
			И в текстовом редакторе я выбираю подсветку языка Gherkin
			И я пишу "Функционал: Создание нескольких элементов справочника склады "
			И я пишу "Как Разработчик "
			И я пишу "Я хочу создавать сразу элемент справочника склады одной строкой "
			И я пишу "Чтобы я экономил своё время "
			И я пишу 'Сценарий: Создание нескольких элементов справочника склады '
			...
			И я пишу 'Когда Я создаю элемент справочника склады с наименованием "Элемент1" '
			И я пишу 'И     Я создаю элемент справочника склады с наименованием "Второй элемент" '
			И я пишу 'И     Я создаю элемент справочника склады с наименованием "Другой элемент" '
			И я пишу 'И     Я закрываю сеанс TestClient '
	И я сохраняю файл открытый в редакторе как "Support\Instructions\Temp\СозданиеНесколькихЭлементовСправочникаСклады.feature"
	...
	Теперь проверим работу второго feature файла
			...
			Тогда в открывшемся окне я указываю путь к фиче "Support\Instructions\Temp\СозданиеНесколькихЭлементовСправочникаСклады.feature"
			И пауза 5
			И Я нажимаю на кнопку Выполнить Сценарии
			И я ожидаю окончания выполнения сценариев в сеансе служебной базы
			И обратите внимание 'Таким образом было создано три элемента склады'
