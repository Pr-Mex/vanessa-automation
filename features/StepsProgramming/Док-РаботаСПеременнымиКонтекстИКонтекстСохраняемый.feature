...
#			И я сворачиваю область Служебные функции
	И я сохраняю в переменную Контекст значение первого слагаемого
			...
			И я добавляю строку текста 'Контекст.Вставить("ПервоеСлагаемое",Парам01);'
	И я сохраняю в переменную Контекст значение второго слагаемого
			И я перехожу к процедуре "ЯУказалВтороеСлагаемое"
			И я перехожу внутрь свернутой процедуры
			И я добавляю строку текста 'Контекст.Вставить("ВтороеСлагаемое",Парам01);'
	И я использую значения из переменной Контекст
			И я перехожу к процедуре "ЯПолучуСумму"
			И я перехожу внутрь свернутой процедуры
			И я добавляю строку текста 'Сумма = Контекст.ПервоеСлагаемое + Контекст.ВтороеСлагаемое;'
			И я добавляю строку текста 'Если Сумма <> Парам01 Тогда'
			И я добавляю строку текста '	ВызватьИсключение "Ошибка. Ожидали сумму = " + Парам01 + ", а получили " + Сумма;'
			И я добавляю строку текста 'КонецЕсли;'
	...
	И я вывожу финальную секцию видео		
			И обратите внимание 'Обратите внимание, что во втором сценарии содержимое переменной Контекст очистилось'
			...
			И обратите внимание 'А КонтекстСохраняемый cуществует, пока открыта форма Vanessa-Behavior.'
