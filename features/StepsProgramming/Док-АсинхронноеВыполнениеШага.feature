...
Функционал: Инструкция по ассинхронному выполнению шагов
	Как Разработчик
...
Сценарий: Работа с таймером
	...
	Сначала мы напишем простой feature файл, который будет ожидать какого-то события
			...
			И я пишу "Я хочу чтобы у меня была шаг, который длится неопределенное время "
	...
	Далее мы напишем шаги, которые создают и проводят документ. И мы заранее не знаем сколько будет проводится наш документ.
			...
			И обратите внимание 'Этот шаг сохранит в пременную Контекст значение поля Номер, т.е. в коде можно будет писать: Контекст.НомерДокумента'
			И я пишу 'И     я нажимаю на кнопку "Провести и закрыть" '
			И я пишу 'И     я ожидаю окончания проведения созданного документа '	
			И обратите внимание 'В этом шаге мы будем использовать таймер, т.к. мы предполагаем, что документ может проводится достаточно долго. '
	...
	Далее надо написать код в полученных процедурах 
			...
			И обратите внимание 'В модуле была создана только одна процедура, хотя шагов в фиче было больше.'
			И обратите внимание 'Это происходит благодаря механизму повторного использования шагов.'
			
			И я перехожу внутрь процедуры "ЯОжидаюОкончанияПроведенияСозданногоДокумента"
			...
			Напишем обработку таймера
					...
					И я добавляю строку 'Если ДокументСуществует(Контекст.НомерДокумента) Тогда'
			...
			Напишем запрос, проверяющий существование документа		
					...
					И я добавляю строку 'Функция ДокументСуществует(Номер)'
