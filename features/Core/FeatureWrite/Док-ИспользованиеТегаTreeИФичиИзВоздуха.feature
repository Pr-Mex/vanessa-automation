...
Функционал: Использование тэга tree
	Как Разработчик
	Я Хочу Иметь возможность писать feature-файлы с использованием иерархии шагов
	Чтобы работать с feature-файлом как с деревом шагов
...
Сценарий: Написание требований бизнес аналитиком и их детализация
	...
	Теперь добавим в наш feature файл полученные шаги.
			...
			И я выделяю 3 строки начиная с "В панели разделов" и вставляю их после "Когда я открываю журнал"
			И я выделяю 1 строки начиная с 'я нажимаю на кнопку "Создать"' и вставляю их после "И я создаю новый документ"
			И я выделяю 2 строки начиная с 'в поле "Склад"' и вставляю их после "я указываю склад"
			И я выделяю 6 строки начиная с 'я нажимаю на кнопку "Добавить"' и вставляю их после "я указываю товары"
			И я выделяю 1 строки начиная с 'Провести и закрыть' и вставляю их после "я записываю документ"
