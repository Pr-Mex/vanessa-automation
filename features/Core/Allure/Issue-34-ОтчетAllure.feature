...
Сценарий: Проверка заполнения suite
	...
	И я устанавливаю флаг с именем 'ДелатьОтчетВФорматеАллюр'
	И в поле каталог отчета аллюр я указываю путь к относительному каталогу "tools\Allure"
	И в поле с именем 'ГруппировкаTestParentSuites' я ввожу текст 'ИмяСценария'
	И в поле с именем 'ГруппировкаTestSuites' я ввожу текст 'ИмяСценария'
	И в поле с именем 'ГруппировкаTestSubSuites' я ввожу текст 'ИмяСценария'
	...
	И для каждого файла "ТекФайл" из каталога "$КаталогИнструментов$\tools\Allure"	
		Если файл "$ТекФайл$" содержит строки тогда
			|'"name": "parent suite",'|
			|'"value": "Тестовая фича, проверяющая заполнение parent suite"'|
		...
		Если файл "$ТекФайл$" содержит строки тогда
			|'"name": "sub suite",'|
			|'"value": "Тестовая фича, проверяющая заполнение sub suite"'|
