...
Сценарий: Проверка, что сработала попытка исключение с регистрацией ошибки в секции исключение
	...
	//Текст первого шага
	И выражение внутреннего языка 'Найти($ДанныеJson$.steps[2].name, "Первая ошибка для теста Allure") > 0' Истинно
	
	//количество файлов приложенных к первому шагу, регистрирующему ошибку
	И выражение внутреннего языка '$ДанныеJson$.steps[2].attachments.Количество() = 10' Истинно
	//количество файлов приложенных ко второму шагу, регистрирующему ошибку
	И выражение внутреннего языка '$ДанныеJson$.steps[7].attachments.Количество() = 10' Истинно
	
	//количество файлов приложенных к самому сценарию
	И выражение внутреннего языка '$ДанныеJson$.attachments.Количество() = 20' Истинно
	...
	Тогда таблица "ДеревоТестов" стала равной:
		...
		| 'И я регистрирую ошибку "Первая ошибка для теста Allure"'                 | 'Failed'  |
		...
		| 'Исключение'                                                              | 'Failed'  |
		| 'И Я запоминаю значение выражения \'1+1\' в переменную "ИмяПеременной"'   | 'Success' |
		| 'И я регистрирую ошибку "Вторая ошибка для теста Allure"'                 | 'Failed'  |
		| 'И Я запоминаю значение выражения \'2+2\' в переменную "ИмяПеременной"'   | 'Success' |
...
Сценарий: Проверка, что шаг условие имеет статус успешный, а вложенный шаг упал
	...
	// второй шаг упал
	И выражение внутреннего языка 'Контекст.НастройкиJSON.steps[1].status = "failed"' Истинно
