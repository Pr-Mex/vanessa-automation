...
Сценарий: Я запускаю клиент тестирования с эмуляцией запуска из командной строки "ИмяJson" "ДопПараметрКоманднойСтроки" "ОставитьЗагрузчикНастроек"
	...
	И Я запоминаю значение выражения '" /TESTCLIENT -TPort 48111 /Execute " + Ванесса.Объект.КаталогИнструментов + "/" + $ИмяEpf$ + " /CStartFeaturePlayer;DisableUserSettingsLoader;NoLoadConfig;VBParams=" + $TestJson$ + ";"' в переменную "ДопПараметр"
