...
Функция ДополнитьПараметрыОтрисовкиПереданныеПользователем(НастройкиРисования, ПараметрыОтрисовки)
	...
	Ванесса.ДополнитьПараметрыОтрисовкиПереданныеПользователем(НастройкиРисования, ПараметрыОтрисовки);
...
Функция ЯДелаюКликПоЭлементуФормыUIAutomation(Знач PID, Заголовок, Тип = "", ДвойнойКлик = Ложь) Экспорт
	...
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		...
		Попытка
			...
			Ванесса.КликПоЭлементуБезПеремещенияМышкиUIAutomation(ДанныеЭлементов[0].size, ДвойнойКлик);
		Исключение
			СтрокаОшибки = ОписаниеОшибки();
			БылаОшибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
...
Функция НарисоватТеньПоКоординатам(Left, Top, Right, Bottom, ПараметрыОтрисовки, ОбработчикПродолжения = Неопределено)
	...
	Ванесса.НарисоватТеньПоКоординатам(Left, Top, Right, Bottom, ПараметрыОтрисовки, ОбработчикПродолжения);
...
Функция ЯДелаюКликПоЭлементуФормыVAUIAutomationUIAutomation(PID, ИмяЭлемента) Экспорт
	...
	Ванесса.КликПоЭлементуБезПеремещенияМышкиUIAutomation(Элемент.size);
