...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|`Тип` — это ControlType из UI Automation: `Button`, `MenuItem`, `Edit`, `Text`, `ListItem` и так далее. Если тип не указан, берётся первый подходящий элемент с таким заголовком.
		...
		|В обычном приложении меню недоступно обычным шагам, а через UI Automation по нему можно кликать.
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Пустой `Тип` допустим: тогда ищется первый элемент с таким заголовком. Правая кнопка и двойной клик — в парных шагах <И я делаю клик по элементу формы 'PID' 'Заголовок' 'Тип' правой кнопкой UI Automation> и <И я делаю двойной клик по элементу формы 'PID' 'Заголовок' 'Тип' UI Automation>.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Элемент ищется по всплывающей подсказке, а не по заголовку. Незаменимо для кнопок с одной картинкой и без подписи: заголовка у них нет, а подсказка есть.
		|### Пример. Кнопка без подписи, но с подсказкой
		|Текст подсказки кладут в переменную и подставляют в шаг — так его удобно менять под язык интерфейса.
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Отсюда `32768` из примера — чистый зелёный (0 + 128 * 256 + 0). Красный будет `255`, синий — `16711680`, белый — `16777215`.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|То же, что <И я делаю эффект затемнения у элементов формы UI Automation 'PID'>, но элементы ищутся по всплывающей подсказке. Нужен для кнопок без подписи — у них нет заголовка, по которому можно искать.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. ID панели, на которой лежит кнопка
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Найти элемент один раз и кликнуть по ID
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Элементы ищутся по именам, как они заданы в форме VA, а не по надписям на экране. Этим шаг отличается от эффектов над клиентом тестирования вроде <И я делаю эффект затемнения у элементов формы UI Automation ""PID"">: тот работает с произвольным приложением, этот — с самой VA.
		...
		|Первая строка таблицы — заголовок колонки, дальше идут имена элементов.
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|То же, что <И я делаю подсветку элементов VA UI Automation>, но в подсветку попадает и заголовок элемента, а не только сам элемент.
		|Разница видна на полях ввода: без заголовка подсвечивается только поле, с заголовком — поле вместе с надписью слева от него. Реализация та же, отличается одним флагом.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Ввод текста в поле формы VA через буфер обмена
		|Текст кладут в буфер, находят поле по имени, кликают в него по ID и вставляют.
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Находит элемент собственной формы Vanessa Automation по имени и кладёт его идентификатор UI Automation в переменную.
		|Делает ровно то же, что <И я запоминаю ID элемента формы Vanessa Automation с именем ""ИмяПоля"" в переменную ""ИмяПеременной"">, — это две формулировки одного действия, реализация у них общая. В новых сценариях удобнее та, что короче.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Если элемент — поле ввода и нужно поставить в него курсор, ровно то же делает <И я делаю клик в поле ""ID"" UI Automation>.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Служебный шаг библиотеки: подключает обработчик ожидания и на этом всё. Полезной работы не выполняет.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Отладочный шаг разработчиков Vanessa Automation. Считает экранные координаты девятой строки редактора сценариев и выводит их в окно сообщений в виде `(x,y)`.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Единственный проверяющий шаг библиотеки — остальные кликают, ищут или рисуют. Чаще всего применяется как условие: интерфейс в разных версиях платформы и в разных языках выглядит по-разному, и сценарий выбирает ветку по тому, что реально есть на экране.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Самый точный из шагов подсветки редактора: им показывают отдельное ключевое слово или параметр шага. Подсвечивается первое найденное вхождение. Если нужно ограничить поиск конкретной строкой, есть <И я делаю подсветку фрагмента текста в редакторе VA в строке 1 в режиме обучения ""Функциональность:"" ""Ключевое слово Функциональность"">.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Подсвечивает элемент, выводит рядом текст и ждёт, пока **пользователь** сам по нему не нажмёт. Ожидание заканчивается, когда на экране появляется элемент, указанный второй парой параметров.
		...
		|Первая пара параметров — что подсветить, третий — текст подсказки, последняя пара — признак того, что действие выполнено.
		|### Пример. Урок: дождаться, пока ученик откроет настройки
		|Подсвечивается кнопка настроек, а ожидание закончится, когда откроется её окно.
		|```Gherkin
		|	И я ожидаю нажатие на элемент с именем 'ФормаНастройки' и типом 'Button' показывая текст 'Нажмите сюда, чтобы открыть настройки' пока не появится элемент с именем 'ГруппаНастройки' и типом 'Pane'
		|```
		|",
		"");
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Ждёт, пока в указанном поле не появится нужное значение. Поле ищется механизмом UI Automation по имени и типу.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Запуск после описания состояний
		|```Gherkin
		|	И я создаю состояние Onboarding
		|		| 'Имя'                | 'Номер'           |
		|		| 'ИмяЭлемента'        | 'Номер'           |
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Поиск идёт вглубь по всему поддереву, а не только по прямым потомкам, — этим шаг отличается от <И я запоминаю элемент с типом ""Тип"" c родителем ""$IDЭлемента$"" в переменную ""IDЭлемента"" Vanessa Automation>. Берите его, когда точная вложенность неизвестна.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Подсвечивает не сам элемент формы Vanessa Automation, а его **расширенную подсказку** — то всплывающее окно, которое платформа показывает по наведению, — и выводит рядом свой текст.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Так добираются до безымянных элементов, которые в дереве UI Automation идут сразу за известным: кнопка выбора рядом с полем, значок рядом с надписью. Опереться на имя у них нельзя, а на соседство — можно.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Кнопки не нажимаются — только наведение. Нужно в двух случаях: показать на видео, о чём речь, и вызвать всплывающую подсказку. Клик по тому же элементу делает <И я делаю клик по элементу формы 'PID' 'Заголовок' 'Тип' UI Automation>.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Курсор ставят в нужное поле, после чего в окно сообщений выводятся его имя и тип — их дальше подставляют в шаги поиска.
		|```Gherkin
		|	И получаю данные текущего элемента формы активного окна UI Automation
