...
&НаКлиенте
Функция ОпределитьКоординатыПоМассивуЭлементовUIAutomation(МассивЭлементов, Left, Top, Bottom, Right)
...
&НаКлиенте
Функция РисоватьЗатемнениеПоНастроящему()
	
	РисоватьЗатемнениеПоНастроящему = Истина;
	Если КонтекстСохраняемый.Свойство("ОтрисовыватьЭффектЗатемнения") Тогда
		...
		РисоватьЗатемнениеПоНастроящему = КонтекстСохраняемый.ОтрисовыватьЭффектЗатемнения;
	КонецЕсли;	 
	
	Возврат РисоватьЗатемнениеПоНастроящему; 
	
КонецФункции	 

&НаКлиенте
Функция НарисоватТеньПоКоординатам(Left, Top, Right, Bottom, ПараметрыОтрисовки, ОбработчикПродолжения = Неопределено)
	Ванесса.ЗапретитьВыполнениеШагов();
	...
	Если РисоватьЗатемнениеПоНастроящему() Тогда
		...
		Если ЗначениеЗаполнено(Подсказка) Тогда
			...
			Если ВсеПоля = Неопределено Тогда
				...
				Если ТекТаблица <> Неопределено Тогда
					...
					Если ВсеПоля = Неопределено Тогда
						Если Ванесса.Версия8319ИлиВыше Тогда 
							НоваяСтрокаПоиска = ТекущееЗначениеЯчейкиТаблицы + " " + Подсказка;
							ОтборЭлементов = Ванесса.ЗаписатьОбъектJSON(Новый Структура("ProcessId, Name", Число(PID), НоваяСтрокаПоиска));
						Иначе	
							НоваяСтрокаПоиска = ПерваяЧастьПодсказки + " " + Подсказка;
							ОтборЭлементов = Ванесса.ЗаписатьОбъектJSON(Новый Структура("ProcessId, Name", Число(PID), НоваяСтрокаПоиска));
...
Процедура ЯЗапускаюOnboardingОбработчик()
	...
	Если ТекущееСостояниеOnboarding = Неопределено Тогда
		...
		Если ТекущееСостояниеOnboarding.Окно <> Неопределено Тогда
			Контекст.Вставить("_ЗаголовокОкнаOnboarding", ТекущееСостояниеOnboarding.Окно);
		КонецЕсли;
		
	КонецЕсли;	
	
	Контекст.Вставить("_ТекущееСостояниеOnboarding", ТекущееСостояниеOnboarding);
	СделатьЭффектЗатемненияOnboarding(ТекущееСостояниеOnboarding);
	
КонецПроцедуры

&НаКлиенте
Функция ТекущееСостояниеOnboarding()
	
	Если НЕ Контекст.Свойство("_ТекущееСостояниеOnboarding") Тогда
		Возврат Неопределено;
	КонецЕсли;	             
	
	Возврат Контекст._ТекущееСостояниеOnboarding;
	
КонецФункции	

&НаКлиенте
Функция НачальноеСосояниеOnboarding()
	
	Если НЕ Контекст.Свойство("СостоянияOnboarding") Тогда
		Возврат Неопределено;
	КонецЕсли;	             
	
	Для Каждого Элем Из Контекст.СостоянияOnboarding Цикл
...
&НаКлиенте
Функция СостоянияOnboarding()
	
	Если НЕ Контекст.Свойство("СостоянияOnboarding") Тогда
		Контекст.Вставить("СостоянияOnboarding", Новый Соответствие);
	КонецЕсли;	
	
	Возврат Контекст.СостоянияOnboarding;
	
КонецФункции

&НаКлиенте
Функция НовоеСостояниеOnboarding()
...
Процедура СделатьЭффектЗатемненияOnboarding(Состояние)
	...
	Если НЕ Контекст.Свойство("_ЗаголовокОкнаOnboarding") Тогда
		ВызватьИсключение Ванесса.Локализовать("Не указано текущее окно Onboarding.");
	КонецЕсли;		
	
	ФормаOnboarding = ФормаOnboarding(Контекст._ЗаголовокОкнаOnboarding);
	...
	ЭлементUIAutomationOnboarding(ДанныеРеквизитовФормыOnboarding, ЭлементФормы);
...
&НаКлиенте
Функция ДанныеРеквизитовФормыOnboarding(ФормаOnboarding)
	
	ИмяФормыOnboarding = ФормаOnboarding.ИмяФормы;
	
	Если НЕ Контекст.Свойство("_КешДанныхФормOnboarding") Тогда
		Контекст.Вставить("_КешДанныхФормOnboarding", Новый Соответствие);
	КонецЕсли;
	
	Если Контекст._КешДанныхФормOnboarding[ИмяФормыOnboarding] <> Неопределено Тогда
		Возврат Контекст._КешДанныхФормOnboarding[ИмяФормыOnboarding];
	КонецЕсли;
	
	Результат = ФормаOnboarding.ДанныеРеквизитовФормыOnboarding();
	
	Контекст._КешДанныхФормOnboarding.Вставить(ИмяФормыOnboarding, Результат);
...
&НаКлиенте
Функция ЭлементUIAutomationOnboarding(ДанныеРеквизитовФормыOnboarding, ЭлементФормы)
	PID = ПолучитьКорректныйPID("ЭтотСеанс"); 
	...
	Контекст.Вставить("_ИмяЭлементаФормы", ЭлементФормы.Имя);
	Контекст.Вставить("_ПодсказкаЭлементаФормы", ТекПодсказка);
	Контекст.Вставить("_ОтборЭлементов", ОтборЭлементов);
	Контекст.Вставить("_ЭлементФормы", ЭлементФормы);
...
Процедура ЭлементUIAutomationOnboardingПродолжение1()
	...
	ДанныеПолей = Ванесса.ПрочитатьОбъектJSON(Ванесса.ВнешняяКомпонентаДляСкриншотов.НайтиЭлементы(Контекст._ОтборЭлементов));
	Контекст._ЭлементФормы.Подсказка = Контекст._ПодсказкаЭлементаФормы;
	Если ДанныеПолей = Неопределено ИЛИ ДанныеПолей.Количество() = 0 Тогда
		ВызватьИсключение Ванесса.ПодставитьПараметрыВСтроку(Ванесса.Локализовать("Не найден элемент <%1>."), Контекст._ИмяЭлементаФормы);
	КонецЕсли;	
	
	Контекст.Вставить("_ЭлементUIAutomationOnboarding", ДанныеПолей[0]);
...
Процедура НарисоватьЗатемнениеOnboardingОбработчик()
	...
	ЭлементUIAutomation = Контекст._ЭлементUIAutomationOnboarding;
	Координаты  = КоординатыЭлементаUIAutomation(ЭлементUIAutomation);
	
	ПараметрыОтрисовки = Новый Массив;
	
	ДобавитьПараметрОтрисовки("text", Контекст._ТекущееСостояниеOnboarding.Текст, ПараметрыОтрисовки);
	
	МассивКнопок = Новый Массив;
	Если НЕ Контекст._ТекущееСостояниеOnboarding.НачальноеСостояние Тогда
		МассивКнопок.Добавить(Новый Структура("title", "Назад"));
	КонецЕсли;	
	Если НЕ Контекст._ТекущееСостояниеOnboarding.КонечноеСостояние Тогда
		МассивКнопок.Добавить(Новый Структура("title", "Далее"));
	КонецЕсли;	
	МассивКнопок.Добавить(Новый Структура("title", "Завершить"));
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "SHADOW_BUTTON" Тогда
		Если Параметр = "Далее" Тогда
			Переход = Контекст._ТекущееСостояниеOnboarding.Переход;
			УстановитьПредыдущееСостояниеOnboarding(Контекст._ТекущееСостояниеOnboarding.Имя, Переход);
			//Контекст.Вставить("_ПредыдущееСостояниеOnboarding", Контекст._ТекущееСостояниеOnboarding.Имя);
			НовоеСостояние = Контекст.СостоянияOnboarding[НРег(Переход)];
			Если НовоеСостояние = Неопределено Тогда
				ВызватьИсключение Ванесса.ПодставитьПараметрыВСтроку(Ванесса.Локализовать("Не найдено состояние Onboarding <%1>."), Переход);
			КонецЕсли;	
			Контекст.Вставить("_ТекущееСостояниеOnboarding", НовоеСостояние);
			ЯЗапускаюOnboardingОбработчик();
			//ПодключитьОбработчикОжидания("ЯЗапускаюOnboardingОбработчик", 0.1, Истина);
		ИначеЕсли Параметр = "Назад" Тогда
			ТекИмя = Контекст._ТекущееСостояниеOnboarding.Имя;
			ПредыдущееИмя = Контекст._ПредыдущееСостояниеOnboarding[НРег(ТекИмя)];
			Контекст.Вставить("_ТекущееСостояниеOnboarding", Контекст.СостоянияOnboarding[НРег(ПредыдущееИмя)]);
			ЯЗапускаюOnboardingОбработчик();
			//ПодключитьОбработчикОжидания("ЯЗапускаюOnboardingОбработчик", 0.1, Истина);
		ИначеЕсли Параметр = "Завершить" Тогда
			Ванесса.ПродолжитьВыполнениеШагов();
		КонецЕсли;	
	КонецЕсли;	
...
Процедура УстановитьПредыдущееСостояниеOnboarding(ТекИмя, Переход)
	...
	Если НЕ Контекст.Свойство("_ПредыдущееСостояниеOnboarding") Тогда
		Контекст.Вставить("_ПредыдущееСостояниеOnboarding", Новый Соответствие);
	КонецЕсли;
	
	Контекст._ПредыдущееСостояниеOnboarding.Вставить(НРег(Переход), ТекИмя);
