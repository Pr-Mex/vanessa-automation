...
Функция СохранитьЗаписьВПеременную(ВидОбъекта, ИмяОбъекта, ЗначенияПоиска, ИмяПеременной)
	...
	Контекст.Вставить(ИмяПеременной, Запись);
...
Функция ЗаписатьЭлементСправочника(Знач АдресДанныеСервера, Знач ИмяОбъекта, Знач ЗначенияРеквизитов, Знач ЗначенияПоиска = Неопределено)
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(Ссылка) Тогда 
			...
			ВызватьИсключение "Не удалось найти ссылку по реквизитам " + ПредставлениеРеквизиты(ЗначенияПоиска);
...
Функция СоздатьЗаписьРегистра(Знач АдресДанныеСервера, Знач ВидОбъекта, Знач ИмяОбъекта, Знач ЗначенияРеквизитов, Знач Перезаписать = Истина)
	...
	Иначе
		...
		КлючЗаписи = КлючРегистраПоОтбору(ВидОбъекта, ИмяОбъекта, ЗначенияРеквизитов);
		
		Если НЕ ЗначениеЗаполнено(КлючЗаписи) Тогда 
			
			КлючевыеЗначения = ДанныеКлюча(ВидОбъекта, ИмяОбъекта, ЗначенияРеквизитов);
			ВызватьИсключение "Не удалось найти запись регистра по ключу " + ПредставлениеРеквизиты(КлючевыеЗначения);
...
Функция ТекстОшибкиПоиска(ИмяОбъекта, РезультатПоиска, ЗначенияПоиска, НЕ_ = Ложь)
	...
	ТекстЗначенияПоиска = ПредставлениеРеквизиты(ЗначенияПоиска);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПредставлениеРеквизиты(ЗначенияРеквизитов, Разделитель = "; ")
...
Процедура ВыполнитьПриведениеЗначений(АдресДанныеСервера, Значения, ТипыПолей)
	...
	Для Каждого Элемент Из Значения Цикл
		...
		Если ОписаниеТипаПоля.Типы().Количество() = 1 Тогда 
			...
			Если Справочники.ТипВсеСсылки().СодержитТип(ТипПоля) Тогда 
			
				МетаданныеОбъекта = Метаданные.НайтиПоТипу(ТипПоля);
				
				ПолеПредставления = РеквизитПредставления(МетаданныеОбъекта);
				
				Значение = СсылкаПоОтбору("Справочник", МетаданныеОбъекта.Имя, Новый Структура(ПолеПредставления, Элемент.Значение));
				
				Если ЗначениеЗаполнено И НЕ ЗначениеЗаполнено(Значение) Тогда 
					
					ВызватьИсключение СтрШаблон("Не удалось найти значение справочника '%1' = '%2'", МетаданныеОбъекта.Имя, Элемент.Значение);
					
				КонецЕсли;
				
			ИначеЕсли Перечисления.ТипВсеСсылки().СодержитТип(ТипПоля) Тогда 
			
				МетаданныеОбъекта = Метаданные.НайтиПоТипу(ТипПоля);
				
				Значение = Перечисления[МетаданныеОбъекта.Имя][Значение];
