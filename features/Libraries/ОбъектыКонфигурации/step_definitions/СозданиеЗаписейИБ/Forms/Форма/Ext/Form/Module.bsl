...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Указывается только имя справочника — `Справочник1`, а не `Справочник.Справочник1`. Работает на стороне менеджера тестирования, интерфейс при этом не задействован, поэтому подготовка данных идёт заметно быстрее, чем через клиент тестирования.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Указывается только имя документа — `ПриходнаяНакладная`, а не `Документ.ПриходнаяНакладная`. Ссылочные реквизиты заполняют переменной, в которой уже лежит ссылка.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Элемент ищется по паре «имя реквизита» — «значение», поэтому искать удобнее по наименованию или коду. Если подходящих элементов несколько, изменения получит каждый.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Тот же приём, что и для справочников, только для документов: сначала документ создают программно, потом забирают на него ссылку и подставляют её дальше через `$ИмяПеременной$`.
		|### Пример. Ссылка на только что созданный документ
		|Документ ищется по складу, ссылка кладётся в переменную. В окно сообщений попадёт представление найденного документа.
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Удаляет из базы только те записи, которые отвечают условиям отбора. Имя указывается полное, как в конфигураторе, условия передаются таблицей Gherkin под шагом.
		|Точечный вариант шага <И я удаляю все элементы таблицы 'ИмяОбъектаМетаданных'>: остальные записи остаются на месте. Удаление непосредственное, без пометки на удаление.
