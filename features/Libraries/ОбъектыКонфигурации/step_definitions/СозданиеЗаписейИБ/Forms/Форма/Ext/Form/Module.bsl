	...
	Для Каждого Элемент Из Значения Цикл
		...
		Если ОписаниеТипаПоля.Типы().Количество() = 1 Тогда 
			...
			Если ТипПоля <> ТипЗначения Тогда 
				...
				Ссылка = ПолучитьСсылочныйОбъектПоПредставлению(ТипПоля, Значение);
...
&НаСервереБезКонтекста
Функция ПолучитьСсылочныйОбъектПоПредставлению(ТипОбъека, Представление)
	...
	ИначеЕсли ЭтоПеречисление(ТипОбъека) Тогда 
		...
		Значение = Перечисления[МетаданныеОбъекта.Имя][Представление];
