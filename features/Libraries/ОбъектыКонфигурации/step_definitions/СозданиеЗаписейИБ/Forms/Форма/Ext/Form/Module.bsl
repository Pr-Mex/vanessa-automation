...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Указывается только имя документа — `ПриходнаяНакладая`, а не `Документ.ПриходнаяНакладая`. Ссылочные реквизиты заполняют переменной, в которой уже лежит ссылка.
		|### Пример. Документ со ссылкой на созданный справочник
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Создаёт запись независимого регистра сведений напрямую в базе. Измерения, ресурсы и реквизиты передаются таблицей Gherkin под шагом.
		...
		|Создаётся запись с одним измерением и двумя значениями, проверка подтверждает, что она появилась. В окно сообщений попадёт `Ключ 1`.
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Ссылка на элемент для составного реквизита
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Удаляет из базы все записи объекта метаданных. Имя здесь указывается **полное**, как в конфигураторе: `Справочник.Справочник1`, `Документ.ПриходнаяНакладная`, — в отличие от шагов создания, которым достаточно короткого имени.
