...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Цикл. Выполняет шаги, пока истинно условие. Условие вычисляется перед каждой итерацией, переменные сценария доступны через `Контекст.ИмяПеременной`.
	...
	,"### Описание
	|Цикл. Выполняет шаги для каждой строки таблицы. При этом будет происходить явный переход по номеру строки по указанной колонке. Колонка, по которой будет происходить переход, задаётся заголовком, а не именем.
	|Если передано имя таблицы, то будет попытка перебрать все строки в таблице клиента тестирования.
	|Если передана таблица Gherkin, то будут перебираться её строки. Первая строка должна содержать имена колонок.
	|При переходе к следующей строке будет создаваться переменная с именем каждой колонки.
	...
	,"### Описание
	|Цикл. Выполняет шаги для каждой строки таблицы. При этом будет происходить явный переход по номеру строки снизу вверх по указанной колонке. Колонка, по которой будет происходить переход, задаётся заголовком, а не именем.
	|Если передано имя таблицы, то будет попытка перебрать все строки в таблице клиента тестирования.
	|Если передана таблица Gherkin, то будут перебираться её строки. Первая строка должна содержать имена колонок.
	|При переходе к следующей строке будет создаваться переменная с именем каждой колонки.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Для условий в своих сценариях используйте шаг <Если 'Истина' тогда>.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Цикл. Выполняет вложенные шаги в течение заданного числа секунд. По истечении времени вызывает исключение, если не был вызван шаг <Тогда я прерываю цикл>.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Прерывается ближайший цикл, внутри которого стоит шаг. Шаги, стоящие в цикле после этого шага, не выполняются.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Продолжает выполнение цикла аналогично оператору `Продолжить` из встроенного языка 1С.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Перебирает строки файла и создаёт переменную `_Строка`.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Цикл. Сначала ищется файл табличного документа в каталоге проекта, затем — макет обработки-библиотеки.
		|И только потом берётся табличный документ с текущей формы клиента тестирования.
		...
		|Имена колонок берутся из первой строки табличного документа, поэтому на каждой итерации доступна переменная `Наименование`. В окно сообщений попадёт по одному наименованию на каждую строку макета.
		|```Gherkin
