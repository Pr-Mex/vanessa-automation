...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯВыгружаюВСистемуОтката(Парам01)",
		...
		|шаг сообщает об этом и молча выходит.
		|### Пример. Точка возврата перед разрушающей проверкой
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯЗагружаюДанныеСистемыОткатаИзФайла(Парам01)",
		...
		"### Описание
		|Загружает из файла данные системы отката — то же, что загрузка переменных из табло, только без диалога выбора файла.
		...
		|```Gherkin
		|	И я загружаю данные системы отката из Файла 'C:\Temp\Переменные.dat'
		|	И я вывожу значение переменной 'Итого'
		|```
		|",
		"Прочее.Система отката");
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯВыгружаюДанныеСистемыОткатаВФайл(Парам01)",
		...
		|Файл двоичный, читать его глазами не получится: он нужен, чтобы вернуть переменные шагом
		|<И я загружаю данные системы отката из Файла 'ПутьКФайлу'>.
		|Путь указывают полный, каталог должен существовать. Существующий файл перезаписывается.
		|В отличие от <И я выгружаю в систему отката 'ИмяЗакладки'>, здесь состояние переживает перезапуск Vanessa Automation.
		...
		|```Gherkin
		|	И я выгружаю данные системы отката в Файл 'C:\Temp\Переменные.dat'
		|	И Файл 'C:\Temp\Переменные.dat' существует
		|```
		|",
		"Прочее.Система отката");
