...
Процедура ОчиститьКаталогиОтчетов()
	Ванесса.ОчиститьСодержимоеКаталога(Ванесса.Объект.КаталогВыгрузкиAllure);
КонецПроцедуры
...
Процедура ЯЗагрузилВРаннерТекущийФичаФайлИЗапустилНаВыполнение() Экспорт
	...
	Если ЗначениеЗаполнено(Ванесса.Объект.КаталогВыгрузкиAllure) Тогда
		...
		Путь = Ванесса.Объект.КаталогВыгрузкиAllure + "\" + Ванесса.Транслит(ТекущаяФичаФайл.ИмяБезРасширения)
