...
Процедура ОбновитьСтатусВыполненияФич()
	...
	Если ОкноVB = Неопределено Тогда
		ТекстСообщения = Ванесса.ПолучитьТекстСообщенияПользователю("Не найдено окно VanessaBehavior в раннере %1");
...
Процедура ЯЗагрузилВРаннерТекущийФичаФайлИЗапустилНаВыполнение() Экспорт
	...
	ОкноVB = Ванесса.НайтиОкноVB();
	Если ОкноVB = Неопределено Тогда
		ВызватьИсключение Ванесса.ПолучитьТекстСообщенияПользователю("Не найдено окно VanessaBehavior.");
	КонецЕсли;
