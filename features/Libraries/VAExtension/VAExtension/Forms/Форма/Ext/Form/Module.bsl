...
Функция ЯЭмулируюСканированиеШтрихкодаБПОЧерезФайлСобытияРасширение(Знач Штрихкод) Экспорт
	...
	Если Контекст._ЗапущенМониторингКаталогаСобытий Тогда
		...
		ТекстСобытия.Вставить("Событие","СканерШтрихкода");
		ТекстСобытия.Вставить("Данные",Штрихкод);
		
		УникальныйИдентификаторСобытия = Новый УникальныйИдентификатор;
		ФайлСобытия = Контекст._ПутьДоКаталогаСобытий + ПолучитьРазделительПути() + "Event_ШК_"+Строка(УникальныйИдентификаторСобытия)+".json";
...
//И я запускаю мониторинг каталога ""C:\temp""для внешних событий(Расширение)
//@ЯЗапускаюМониторингКаталогаДляВнешнихСобытийРасширение(Парам)
Функция ЯЗапускаюМониторингКаталогаДляВнешнихСобытийРасширение(Парам) Экспорт
	...
	РеквизитКаталог.ВвестиТекст(Парам);
	...
	Контекст.Вставить("_ПутьДоКаталогаСобытий", Парам);	
...
Функция ЯВыполняюКодВстроенногоЯзыкаЧерезФайлСобытияРасширениеСлужебный(Код, НаКлиенте, ПривилегированныйРежим = Ложь)
	...
	Если Контекст._ЗапущенМониторингКаталогаСобытий Тогда
		...
		ТекстСобытия = Новый Структура;
		Если НаКлиенте Тогда
			ТекстСобытия.Вставить("Событие","ВыполнитьКодНаКлиенте");
		Иначе
			Если ПривилегированныйРежим тогда
				ТекстСобытия.Вставить("Событие","ВыполнитьКодНаСервереПривилегировано");
			Иначе
				ТекстСобытия.Вставить("Событие","ВыполнитьКодНаСервере");
			КонецЕсли;	
		КонецЕсли;	
