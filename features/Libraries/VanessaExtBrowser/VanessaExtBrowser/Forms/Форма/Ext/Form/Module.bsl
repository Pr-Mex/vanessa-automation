
///////////////////////////////////////////////////
//Служебные функции и процедуры
///////////////////////////////////////////////////

&НаКлиенте
// контекст фреймворка Vanessa-Automation
Перем Ванесса;

&НаКлиенте
// Структура, в которой хранится состояние сценария между выполнением шагов. Очищается перед выполнением каждого сценария.
Перем Контекст Экспорт;
 
&НаКлиенте
// Структура, в которой можно хранить служебные данные между запусками сценариев. Существует, пока открыта форма Vanessa-Automation.
Перем КонтекстСохраняемый Экспорт;

&НаКлиенте
// Функция экспортирует список шагов, которые реализованы в данной внешней обработке.
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	Ванесса = КонтекстФреймворкаBDD;
	
	ВсеТесты = Новый Массив;

	//описание параметров
	//Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,Снипет,ИмяПроцедуры,ПредставлениеТеста,ОписаниеШага,ТипШага,Транзакция,Параметр);

	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"НужноСброситьБраузер()","НужноСброситьБраузер","И Нужно сбросить браузер",
		"### Описание
		|Закрывает и заново открывает браузер (см. Сервис->Настройки клиентов тестирования->Запуск браузера)
		|Шаг помечен как устаревший — в новых сценариях просто запускайте браузер шагом <И я запускаю браузер>, а лишние вкладки закрывайте шагом <И я закрываю все вкладки браузера>.
		|### Пример. Перезапуск браузера перед открытием ссылки
		|```Gherkin
		|	И Нужно сбросить браузер
		|	И я открываю ссылку в браузере 'https://ya.ru'
		|```
		|",
		"Устаревшее.Прочее.Браузер");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаЯУстанавливаюТекстПоля(ID,ТекстПоля)","ВСтраницеБраузераДляЭлементаЯУстанавливаюТекстПоля","И в странице браузера для элемента ""ID"" я устанавливаю текст поля ""ТекстПоля""",
		"### Описание
		|В странице браузера для элемента с указанным ""ID"" устанавливает заданный текст поля ввода
		|`ID` — это атрибут `id` в HTML-разметке страницы, его смотрят в инструментах разработчика браузера. Если у поля нет `id`, ищите по атрибуту `name` шагом <И в странице браузера для элемента с именем ""Name"" я устанавливаю текст поля ""ТекстПоля""> или по классу — <И в странице браузера для элемента с именем класса ""className"" я устанавливаю текст поля ""ТекстПоля"">.
		|Текст именно устанавливается в поле, а не набирается посимвольно: обработчики нажатия клавиш на странице не сработают.
		|### Пример. Заполнение поиска на странице
		|```Gherkin
		|	Дано я запускаю браузер
		|	И я открываю ссылку в браузере 'https://ya.ru'
		|	И в странице браузера для элемента 'text' я устанавливаю текст поля 'Ванесса! Тесты есть!'
		|```
		|",
		"Прочее.Браузер.Элементы.Установить текст поля");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"СтраницаБраузераСодержитТекст(ТекстHTML)","СтраницаБраузераСодержитТекст",
		"И страница браузера содержит текст ""ТекстHTML""",
		"### Описание
		|Ищет заданный текст в странице браузера. Если текст не найден, шаг падает.
		|Поиск идёт по **видимому тексту** страницы (`document.body.innerText`), а не по её исходному коду: то, что спрятано в разметке, в атрибутах или в скриптах, этим шагом не найдётся. Регистр не важен — сравнение идёт без учёта регистра.
		|### Пример. Проверка, что загрузилась нужная страница
		|```Gherkin
		|	И я открываю ссылку в браузере 'https://ya.ru'
		|	И страница браузера содержит текст 'Яндекс'
		|```
		|",
		"Прочее.Браузер.Страница содержит текст");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаЯПолучаюТекстВПеременную(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаЯПолучаюТекстВПеременную","И в странице браузера для элемента ""ID"" я получаю текст в переменную ""ИмяПеременной""",
		"### Описание
		|В странице браузера для элемента с указанным ""ID"" запоминает текст в переменную
		|Берётся значение элемента — свойство `value`; исключение сделано для ссылки (`<a>`), у неё берётся видимый текст. Дальше переменную проверяют обычными шагами или подставляют в другие через `$ИмяПеременной$`.
		|Аналоги для других способов поиска: <И в странице браузера для элемента с именем ""Name"" я получаю текст в переменную ""ИмяПеременной""> и <И в странице браузера для элемента с именем класса ""className"" я получаю текст в переменную ""ИмяПеременной"">.
		|### Пример. Забрать со страницы номер заказа
		|В окно сообщений попадёт текст элемента, а следом он проверяется.
		|```Gherkin
		|	И в странице браузера для элемента 'order-number' я получаю текст в переменную 'НомерЗаказа'
		|	И я вывожу значение переменной 'НомерЗаказа'
		|	Тогда переменная 'НомерЗаказа' имеет значение 'ЗК-*' по шаблону
		|```
		|",
		"Прочее.Браузер.Элементы.Текст элемента в переменную");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаЯПолучаюСсылкуВПеременную(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаЯПолучаюСсылкуВПеременную","И в странице браузера для элемента ""ID"" я получаю ссылку в переменную ""ИмяПеременной""",
		"### Описание
		|В странице браузера для элемента-ссылки с указанным ""ID"" запоминает ссылку(href) в переменную
		|Берётся адрес из атрибута `href`, а не видимая подпись ссылки — для подписи есть <И в странице браузера для элемента ""ID"" я получаю текст в переменную ""ИмяПеременной"">.
		|Удобно, когда по ссылке не надо переходить: достаточно проверить, куда она ведёт, или открыть её потом отдельным шагом.
		|### Пример. Адрес ссылки по id
		|В окно сообщений попадёт полный адрес из `href`.
		|```Gherkin
		|	И в странице браузера для элемента 'download-link' я получаю ссылку в переменную 'АдресФайла'
		|	И я вывожу значение переменной 'АдресФайла'
		|	И я открываю ссылку в браузере '$АдресФайла$'
		|```
		|",
		"Прочее.Браузер.Элементы.Ссылку в переменную");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаЯПолучаюТекстКнопкиВПеременную(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаЯПолучаюТекстКнопкиВПеременную","И в странице браузера для элемента ""ID"" я получаю текст кнопки в переменную ""ИмяПеременной""",
		"### Описание
		|В странице браузера для элемента ""submit"" с указанным ""ID"" запоминает текст кнопки в переменную
		|У кнопок `input type=submit` подпись лежит не в тексте элемента, а в атрибуте `value` — этот шаг читает именно его.
		|### Пример. Подпись кнопки по id
		|В окно сообщений попадёт подпись кнопки, например `Отправить`.
		|```Gherkin
		|	И в странице браузера для элемента 'submit-btn' я получаю текст кнопки в переменную 'ПодписьКнопки'
		|	И я вывожу значение переменной 'ПодписьКнопки'
		|```
		|",
		"Прочее.Браузер.Элементы.Текст кнопки в переменную");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаЯПолучаюСписокЗначенийВПеременную(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаЯПолучаюСписокЗначенийВПеременную","И в странице браузера для элемента ""ID"" я получаю список значений в переменную ""ИмяПеременной""",
		"### Описание
		|В странице браузера для элемента-выпадающего списка с указанным ""ID"" получаем список в переменную типа ""СписокЗначений""
		|Шаг читает у элемента свойство `options`, но в переменную ничего не кладёт: обработчик результат не сохраняет. Пока это не исправлено, состав списка достают шагом <И я Выполняю код Javascript в странице браузера>.
		|Шаг помечен как устаревший, но замены в библиотеке нет: если нужен состав выпадающего списка, берите его.
		|### Пример. Состав списка по id
		|В окно сообщений попадёт количество пунктов списка.
		|```Gherkin
		|	И в странице браузера для элемента 'month_card' я получаю список значений в переменную 'СписокМесяцев'
		|	И Я запоминаю значение выражения 'Контекст.СписокМесяцев.Количество()' в переменную 'КоличествоПунктов'
		|	И я вывожу значение переменной 'КоличествоПунктов'
		|```
		|",
		"Устаревшее.Прочее.Браузер.Элементы.Данные списка");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаЯДелаюПравыйКлик(Парам01)","ВСтраницеБраузераДляЭлементаЯДелаюПравыйКлик","И в странице браузера для элемента ""ID"" я делаю правый клик",
		"### Описание
		|В странице браузера для элемента с указанным ""ID"" делает правый клик (контекстное меню)
		|Вызывает контекстное меню **страницы**, а не браузера. Учтите: собственное меню самого Chrome через страницу не управляется — по его пунктам кликают средствами UI Automation.
		|### Пример. Контекстное меню элемента страницы
		|```Gherkin
		|	И в странице браузера для элемента 'grid-row-1' я делаю правый клик
		|	И Пауза 1
		|```
		|",
		"Прочее.Браузер.Клик правой кнопкой");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемЯПолучаюТекстВПеременную(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаСИменемЯПолучаюТекстВПеременную","И в странице браузера для элемента с именем ""Name"" я получаю текст в переменную ""ИмяПеременной""",
		"### Описание
		|В странице браузера для элемента с указанным ""Name"" запоминает текст в переменную
		|Поиск идёт по HTML-атрибуту `name`. Это второй из трёх способов адресовать элемент: по `id` — <И в странице браузера для элемента ""ID"" я получаю текст в переменную ""ИмяПеременной"">, по классу — <И в странице браузера для элемента с именем класса ""className"" я получаю текст в переменную ""ИмяПеременной"">.
		|`name` есть у полей форм и обычно устойчивее сгенерированного `id`, поэтому для форм это чаще всего лучший выбор.
		|### Пример. Забрать текст поля формы
		|В окно сообщений попадёт текст элемента с именем `login`.
		|```Gherkin
		|	И в странице браузера для элемента с именем 'login' я получаю текст в переменную 'Логин'
		|	И я вывожу значение переменной 'Логин'
		|```
		|",
		"Прочее.Браузер.Элементы.Текст элемента в переменную");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемЯПолучаюСсылкуВПеременную(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаСИменемЯПолучаюСсылкуВПеременную","И в странице браузера для элемента с именем ""Name"" я получаю ссылку в переменную ""ИмяПеременной""",
		"### Описание
		|В странице браузера для элемента-ссылки с указанным ""Name"" запоминает ссылку(href) в переменную
		|То же, что <И в странице браузера для элемента ""ID"" я получаю ссылку в переменную ""ИмяПеременной"">, но элемент ищется по атрибуту `name`. Берётся адрес из `href`, а не подпись ссылки.
		|### Пример. Адрес ссылки, найденной по имени
		|```Gherkin
		|	И в странице браузера для элемента с именем 'report-link' я получаю ссылку в переменную 'АдресОтчета'
		|	И я вывожу значение переменной 'АдресОтчета'
		|```
		|",
		"Прочее.Браузер.Элементы.Ссылку в переменную");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемЯПолучаюТекстКнопкиВПеременную(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаСИменемЯПолучаюТекстКнопкиВПеременную","И в странице браузера для элемента с именем ""Name"" я получаю текст кнопки в переменную ""ИмяПеременной""",
		"### Описание
		|В странице браузера для элемента ""submit"" с указанным ""Name"" запоминает текст кнопки в переменную
		|То же, что <И в странице браузера для элемента ""ID"" я получаю текст кнопки в переменную ""ИмяПеременной"">, но поиск по атрибуту `name`. Подпись кнопки берётся из `value`.
		|### Пример. Подпись кнопки, найденной по имени
		|```Gherkin
		|	И в странице браузера для элемента с именем 'submit' я получаю текст кнопки в переменную 'ПодписьКнопки'
		|	И я вывожу значение переменной 'ПодписьКнопки'
		|```
		|",
		"Прочее.Браузер.Элементы.Текст кнопки в переменную");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемЯУстанавливаюТекстПоля(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаСИменемЯУстанавливаюТекстПоля","И в странице браузера для элемента с именем ""Name"" я устанавливаю текст поля ""ТекстПоля""",
		"### Описание
		|В странице браузера для элемента с указанным ""Name"" устанавливает заданный текст поля ввода
		|У полей ввода атрибут `name` есть почти всегда, а `id` часто генерируется и меняется от сборки к сборке, поэтому для форм поиск по `name` устойчивее.
		|Текст устанавливается напрямую, без эмуляции набора с клавиатуры.
		|### Пример. Заполнение формы входа
		|```Gherkin
		|	И в странице браузера для элемента с именем 'login' я устанавливаю текст поля 'Администратор'
		|	И в странице браузера для элемента с именем 'password' я устанавливаю текст поля '$Пароль$'
		|	И в странице браузера для элемента с именем 'submit' я делаю клик
		|```
		|",
		"Прочее.Браузер.Элементы.Установить текст поля");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемЯПолучаюСписокЗначенийВПеременную(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаСИменемЯПолучаюСписокЗначенийВПеременную","И в странице браузера для элемента с именем ""Name"" я получаю список значений в переменную ""ИмяПеременной""",
		"### Описание
		|В странице браузера для элемента-выпадающего списка с указанным ""Name"" получаем список в переменную типа ""СписокЗначений""
		|То же, что <И в странице браузера для элемента ""ID"" я получаю список значений в переменную ""ИмяПеременной"">, но поиск по атрибуту `name`.
		|### Пример. Состав списка, найденного по имени
		|```Gherkin
		|	И в странице браузера для элемента с именем 'month_card' я получаю список значений в переменную 'СписокМесяцев'
		|	И Я запоминаю значение выражения 'Контекст.СписокМесяцев.Количество()' в переменную 'КоличествоПунктов'
		|	И я вывожу значение переменной 'КоличествоПунктов'
		|```
		|",
		"Устаревшее.Прочее.Браузер.Элементы.Данные списка");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемЯВыбираюЗначениеВСпискеВыбора(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаСИменемЯВыбираюЗначениеВСпискеВыбора","И в странице браузера для элемента с именем ""Name"" я выбираю значение ""ЗначениеЭлемента"" в списке выбора",
		"### Описание
		|В странице браузера для элемента-выпадающего списка с указанным ""Name"" устанавливаем значение из списка
		|Выбирается пункт по его **значению** (`value` в разметке), а не по видимой подписи. Посмотреть состав списка вместе со значениями можно шагом <И в странице браузера для элемента с именем ""Name"" я получаю список значений в переменную ""ИмяПеременной"">.
		|### Пример. Выбор месяца в выпадающем списке
		|```Gherkin
		|	И в странице браузера для элемента с именем 'month_card' я выбираю значение '05' в списке выбора
		|```
		|",
		"Устаревшее.Прочее.Браузер.Элементы.Установить значение списка");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемЯДелаюКлик(Парам01)","ВСтраницеБраузераДляЭлементаСИменемЯДелаюКлик","И в странице браузера для элемента с именем ""Name"" я делаю клик",
		"### Описание
		|В странице браузера для элемента с указанным ""Name"" делает клик мышкой
		|Основной шаг нажатия на странице: кнопки отправки формы, ссылки, флаги. Аналоги для других способов поиска — <И в странице браузера для элемента ""ID"" я делаю клик> и <И в странице браузера для элемента с именем класса ""className"" я делаю клик>.
		|### Пример. Отправка формы
		|```Gherkin
		|	И в странице браузера для элемента с именем 'login' я устанавливаю текст поля 'Администратор'
		|	И в странице браузера для элемента с именем 'submit' я делаю клик
		|	И страница браузера содержит текст 'Личный кабинет'
		|```
		|",
		"Прочее.Браузер.Клик мышкой");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСIDЯДелаюКлик(Парам01)","ВСтраницеБраузераДляЭлементаСIDЯДелаюКлик","И в странице браузера для элемента с ID ""ID"" я делаю клик",
		"### Описание
		|В странице браузера для элемента с указанным ""ID"" делает клик мышкой
		|Шаг помечен как устаревший: то же самое делает <И в странице браузера для элемента ""ID"" я делаю клик>, его и используйте в новых сценариях. Этот оставлен, чтобы не ломать сценарии, написанные раньше.
		|### Пример. Клик по элементу с известным id
		|```Gherkin
		|	И в странице браузера для элемента с ID 'submit-btn' я делаю клик
		|```
		|",
		"Устаревшее.Прочее.Браузер.Клик мышкой");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемЯДелаюПравыйКлик(Парам01)","ВСтраницеБраузераДляЭлементаСИменемЯДелаюПравыйКлик","И в странице браузера для элемента с именем ""Name"" я делаю правый клик",
		"### Описание
		|В странице браузера для элемента с указанным ""Name"" делает правый клик (контекстное меню)
		|То же, что <И в странице браузера для элемента ""ID"" я делаю правый клик>, но поиск по атрибуту `name`. Вызывается контекстное меню страницы, а не браузера.
		|### Пример. Контекстное меню поля формы
		|```Gherkin
		|	И в странице браузера для элемента с именем 'login' я делаю правый клик
		|	И Пауза 1
		|```
		|",
		"Прочее.Браузер.Клик правой кнопкой");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюТекстВПеременную(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюТекстВПеременную","И в странице браузера для элемента с именем класса ""className"" я получаю текст в переменную ""ИмяПеременной""",
		"### Описание
		|В странице браузера для элемента с указанным ""className"" запоминает текст в переменную
		|Третий способ адресовать элемент — по CSS-классу. Нужен, когда у элемента нет ни `id`, ни `name`.
		|Учтите главное отличие от поиска по `id`: класс обычно **не уникален**, элементов с ним на странице много, и шаг возьмёт первый подходящий.
		|### Пример. Забрать заголовок страницы по классу
		|В окно сообщений попадёт значение (`value`) первого элемента с этим классом.
		|```Gherkin
		|	И в странице браузера для элемента с именем класса 'page-title' я получаю текст в переменную 'Заголовок'
		|	И я вывожу значение переменной 'Заголовок'
		|```
		|",
		"Прочее.Браузер.Элементы.Текст элемента в переменную");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюСсылкуВПеременную(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюСсылкуВПеременную","И в странице браузера для элемента с именем класса ""className"" я получаю ссылку в переменную ""ИмяПеременной""",
		"### Описание
		|В странице браузера для элемента-ссылки с указанным ""className"" запоминает ссылку(href) в переменную
		|То же, что <И в странице браузера для элемента ""ID"" я получаю ссылку в переменную ""ИмяПеременной"">, но элемент ищется по CSS-классу. Если элементов с таким классом несколько, берётся первый.
		|### Пример. Адрес первой ссылки нужного класса
		|```Gherkin
		|	И в странице браузера для элемента с именем класса 'doc-link' я получаю ссылку в переменную 'АдресДокумента'
		|	И я вывожу значение переменной 'АдресДокумента'
		|```
		|",
		"Прочее.Браузер.Элементы.Ссылку в переменную");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюТекстКнопкиВПеременную(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюТекстКнопкиВПеременную","И в странице браузера для элемента с именем класса ""className"" я получаю текст кнопки в переменную ""ИмяПеременной""",
		"### Описание
		|В странице браузера для элемента ""submit"" с указанным ""className"" запоминает текст кнопки в переменную
		|То же, что <И в странице браузера для элемента ""ID"" я получаю текст кнопки в переменную ""ИмяПеременной"">, но поиск по CSS-классу. Подпись берётся из атрибута `value`.
		|### Пример. Подпись кнопки по классу
		|```Gherkin
		|	И в странице браузера для элемента с именем класса 'btn-primary' я получаю текст кнопки в переменную 'ПодписьКнопки'
		|	И я вывожу значение переменной 'ПодписьКнопки'
		|```
		|",
		"Прочее.Браузер.Элементы.Текст кнопки в переменную");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемКлассаЯУстанавливаюТекстПоля(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаСИменемКлассаЯУстанавливаюТекстПоля","И в странице браузера для элемента с именем класса ""className"" я устанавливаю текст поля ""ТекстПоля""",
		"### Описание
		|В странице браузера для элемента с указанным ""className"" устанавливает заданный текст поля ввода
		|То же, что <И в странице браузера для элемента с именем ""Name"" я устанавливаю текст поля ""ТекстПоля"">, но поиск по CSS-классу. Для полей форм предпочтительнее искать по `name`: класс редко бывает уникальным, и текст может уехать не в то поле.
		|### Пример. Заполнение поля, у которого есть только класс
		|```Gherkin
		|	И в странице браузера для элемента с именем класса 'search-input' я устанавливаю текст поля 'Ванесса'
		|```
		|",
		"Прочее.Браузер.Элементы.Установить текст поля");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюСписокЗначенийВПеременную(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюСписокЗначенийВПеременную","И в странице браузера для элемента с именем класса ""className"" я получаю список значений в переменную ""ИмяПеременной""",
		"### Описание
		|В странице браузера для элемента-выпадающего списка с указанным ""className"" получает список в переменную типа ""СписокЗначений""
		|То же, что <И в странице браузера для элемента ""ID"" я получаю список значений в переменную ""ИмяПеременной"">, но поиск по CSS-классу.
		|### Пример. Состав списка, найденного по классу
		|```Gherkin
		|	И в странице браузера для элемента с именем класса 'month-select' я получаю список значений в переменную 'СписокМесяцев'
		|	И Я запоминаю значение выражения 'Контекст.СписокМесяцев.Количество()' в переменную 'КоличествоПунктов'
		|	И я вывожу значение переменной 'КоличествоПунктов'
		|```
		|",
		"Устаревшее.Прочее.Браузер.Элементы.Данные списка");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемКлассаЯВыбираюЗначениеВСпискеВыбора(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаСИменемКлассаЯВыбираюЗначениеВСпискеВыбора","И в странице браузера для элемента с именем класса ""className"" я выбираю значение ""ЗначениеЭлемента"" в списке выбора",
		"### Описание
		|В странице браузера для элемента-выпадающего списка с указанным ""className"" устанавливает значение из списка
		|То же, что <И в странице браузера для элемента с именем ""Name"" я выбираю значение ""ЗначениеЭлемента"" в списке выбора>, но поиск по CSS-классу. Пункт выбирается по значению `value`, а не по видимой подписи.
		|### Пример. Выбор значения в списке, найденном по классу
		|```Gherkin
		|	И в странице браузера для элемента с именем класса 'month-select' я выбираю значение '05' в списке выбора
		|```
		|",
		"Устаревшее.Прочее.Браузер.Элементы.Установить значение списка");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемКлассаЯДелаюКлик(Парам01)","ВСтраницеБраузераДляЭлементаСИменемКлассаЯДелаюКлик","И в странице браузера для элемента с именем класса ""className"" я делаю клик",
		"### Описание
		|В странице браузера для элемента с указанным ""className"" делает клик мышкой
		|То же, что <И в странице браузера для элемента с именем ""Name"" я делаю клик>, но поиск по CSS-классу. Если элементов с таким классом несколько, нажатие придётся на первый — для кнопок в списках и таблицах это стоит проверять.
		|### Пример. Нажатие на кнопку по классу
		|```Gherkin
		|	И в странице браузера для элемента с именем класса 'btn-primary' я делаю клик
		|	И Пауза 1
		|```
		|",
		"Прочее.Браузер.Клик мышкой");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаСИменемКлассаЯДелаюПравыйКлик(Парам01)","ВСтраницеБраузераДляЭлементаСИменемКлассаЯДелаюПравыйКлик","И в странице браузера для элемента с именем класса ""className"" я делаю правый клик",
		"### Описание
		|В странице браузера для элемента с указанным ""className"" делает правый клик (контекстное меню)
		|То же, что <И в странице браузера для элемента """"ID"""" я делаю правый клик>, но поиск по CSS-классу. Шаг вызывает у найденного элемента метод `contextMenu()`; в стандартном DOM такого метода нет, поэтому сработает он только там, где страница определяет его сама.
		|### Пример. Контекстное меню строки таблицы
		|```Gherkin
		|	И в странице браузера для элемента с именем класса 'grid-row' я делаю правый клик
		|	И Пауза 1
		|```
		|",
		"Прочее.Браузер.Клик правой кнопкой");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЯВыполняюКодJavascriptВСтраницеБраузера(Парам01)","ЯВыполняюКодJavascriptВСтраницеБраузера","И я Выполняю код Javascript в странице браузера
			|	| '{                                                                               ' |
			|	| '  document.getElementsByClassName(\'mailbox-service svelte-giypkm\')[0].click();' |
			|	| '};                                                                              ' |",
		"### Описание
		|Выполняет произвольный JavaScript код в контексте страницы браузера.
		|Запасной вариант на случай, когда готовых шагов не хватает: сложный селектор, элемент в теневом DOM (Shadow DOM), действие, которого в библиотеке нет. Код передаётся таблицей Gherkin, по строке на строку кода, и склеивается в один текст.
		|Апострофы внутри кода экранируются обратной косой чертой: `\'mailbox-service\'`.
		|Результат выполнения проверяется: если браузер вернул ошибку, шаг упадёт с её текстом.
		|### Пример. Клик по элементу кодом JavaScript
		|Код выполняется в контексте страницы: здесь он находит элементы сразу по двум классам и нажимает на первый найденный.
		|```Gherkin
		|	И я Выполняю код Javascript в странице браузера
		|		| '{                                                                               ' |
		|		| '  document.getElementsByClassName(\'mailbox-service svelte-giypkm\')[0].click();' |
		|		| '};                                                                              ' |
		|```
		|",
		"Прочее.Браузер.Выполнить произвольный JavaScript");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЯРисуюПрямоугольникДляБраузера(ТабПарам)","ЯРисуюПрямоугольникДляБраузера","И я рисую прямоугольник для браузера
			|	| 'ТочкаX'|'450'| 
			|	| 'ТочкаY'|'340'| 
			|	| 'Width'|'400'| 
			|	| 'Height'|'300'| 
			|	| 'borderColor'|'red'| 
			|	| 'borderWidth'|'0.1em'| 
			|	| 'Длительность'|'10000'| ",
		"### Описание
		|Рисует элемент-рамку вокруг области в странице браузера
		|Нужен для видеоинструкций: обвести на записи ту часть страницы, о которой идёт речь. Проверок не делает.
		|Важно: параметры отрисовки здесь **не такие, как в шагах UI Automation**. Рамка — это `div`, который добавляется на страницу, поэтому значения задаются как в CSS: `borderColor` — название или код цвета (`red`, `#ff0000`), `borderWidth` — с единицей измерения (`0.1em`, `3px`). Числовой цвет в упаковке BGR тут не подойдёт.
		|`ТочкаX` и `ТочкаY` — координаты левого верхнего угла рамки в пикселях, от левого верхнего угла окна браузера: позиционирование фиксированное, при прокрутке страницы рамка остаётся на месте. `Width` и `Height` — размеры, `Длительность` — сколько миллисекунд держать рамку, после чего она сама удаляется.
		|### Пример. Обвести область страницы на видео
		|```Gherkin
		|	И я рисую прямоугольник для браузера
		|		| 'ТочкаX'       | '450'   |
		|		| 'ТочкаY'       | '340'   |
		|		| 'Width'        | '400'   |
		|		| 'Height'       | '300'   |
		|		| 'borderColor'  | 'red'   |
		|		| 'borderWidth'  | '0.1em' |
		|		| 'Длительность' | '10000' |
		|```
		|",
		"Прочее.Браузер.Эффекты.Прямоугольник по координатам");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаЯВыбираюЗначениеВСпискеВыбора(Парам01,Парам02)","ВСтраницеБраузераДляЭлементаЯВыбираюЗначениеВСпискеВыбора","И в странице браузера для элемента ""month_card"" я выбираю значение ""05"" в списке выбора",
		"### Описание
		|В странице браузера для элемента-выпадающего списка с указанным ""ID"" устанавливает значение из списка
		|Первый параметр — значение атрибута `id` тега `select`, второй — значение `value` нужного пункта, а не его видимая подпись. Для `<option value=""05"">Май</option>` в шаг передаётся `05`.
		|Шаг отнесён к разделу «Устаревшее» вместе с обоими двойниками — и тем, что ищет по `name`, и тем, что ищет по CSS-классу: устаревшим помечено всё семейство выбора значения в списке.
		|При неудаче шаг повторяет попытку столько раз, сколько задано в настройках VA, с паузой в секунду.
		|### Пример. Выбор месяца в списке по id
		|```Gherkin
		|	И в странице браузера для элемента 'month_card' я выбираю значение '05' в списке выбора
		|```
		|",
		"Устаревшее.Прочее.Браузер.Элементы.Установить значение списка");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераЯПровожуСкроллККоординатам(Парам01,Парам02)","ВСтраницеБраузераЯПровожуСкроллККоординатам","И в странице браузера я провожу скролл к координатам ""500"" ""1000""",
		"### Описание
		|Производит перемотку в странице браузера к координатам: на случай, если страница объёмная и имеет ползунок прокрутки
		|Параметры — координаты в пикселях: первый по горизонтали, второй по вертикали. Отсчёт от левого верхнего угла документа, а не от текущего положения: `'0' '0'` возвращает страницу в самое начало.
		|Для клика и ввода текста скролл не обязателен — они работают через DOM и до невидимой части страницы дотягиваются сами. Шаг нужен там, где важна картинка: запись видеоинструкции, скриншот, эффекты отрисовки.
		|### Пример. Промотать длинную страницу вниз перед скриншотом
		|```Gherkin
		|	И я открываю ссылку в браузере 'https://ya.ru'
		|	И в странице браузера я провожу скролл к координатам '500' '1000'
		|```
		|",
		"Прочее.Браузер.Скролл");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЯРисуюНадписьДляБраузера(ТабПарам)","ЯРисуюНадписьДляБраузера","И я рисую надпись для браузера
			|	| 'ТочкаX'|'200'| 
			|	| 'ТочкаY'|'670'| 
			|	| 'НадписьТекст'|'Для оплаты нажмите сюда ===>'| 
			|	| 'color'|'red'| 
			|	| 'borderWidth'|'0.1em'| ",
		"### Описание
		|Рисует текст-подсказку возле области в странице браузера
		|Пара к <И я рисую прямоугольник для браузера>: рамка показывает место, надпись объясняет, что там делать. Нужна для видеоинструкций, проверок не выполняет.
		|`НадписьТекст` вставляется как HTML, поэтому в нём можно использовать теги — например `<b>` для выделения.
		|`ТочкаX` и `ТочкаY` — координаты левого верхнего угла надписи в пикселях. Позиционирование фиксированное, от окна браузера, поэтому при прокрутке страницы надпись остаётся на месте.
		|`Длительность` — сколько миллисекунд держать надпись; если параметр не задан, берётся 10000, то есть десять секунд.
		|Оформление надписи задано в коде шага и не настраивается: красный текст, прозрачный фон. Параметры `color` и `borderWidth` из шаблона вставки шаг молча игнорирует — если нужно другое оформление, надпись рисуют шагом <И я Выполняю код Javascript в странице браузера>.
		|### Пример. Подписать элемент на видео
		|```Gherkin
		|	И я рисую надпись для браузера
		|		| 'ТочкаX'       | '200'                          |
		|		| 'ТочкаY'       | '670'                          |
		|		| 'НадписьТекст' | 'Для оплаты нажмите сюда ===>' |
		|		| 'Длительность' | '10000'                        |
		|```
		|",
		"Прочее.Браузер.Эффекты.Надпись по координатам");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВСтраницеБраузераДляЭлементаЯДелаюКлик(Парам01)","ВСтраницеБраузераДляЭлементаЯДелаюКлик","И в странице браузера для элемента ""ID"" я делаю клик",
		"### Описание
		|В странице браузера для элемента с указанным ""ID"" делает клик мышкой
		|Параметр — значение атрибута `id` элемента. Клик выполняется методом DOM, а не движением курсора: элемент может находиться за пределами видимой части страницы, скроллить к нему заранее не нужно.
		|`id` уникален в пределах страницы — этим шаг удобнее вариантов поиска по классу, где срабатывает первый совпавший элемент. Но у форм, которые собирает фреймворк, `id` часто генерируется и меняется от сборки к сборке — тогда надёжнее <И в странице браузера для элемента с именем ""Name"" я делаю клик>.
		|При неудаче шаг повторяет попытку столько раз, сколько задано в настройках VA, с паузой в секунду. Так он переживает элемент, который появляется не сразу.
		|### Пример. Нажатие на кнопку по id
		|```Gherkin
		|	И я открываю ссылку в браузере 'https://ya.ru'
		|	И в странице браузера для элемента 'search-button' я делаю клик
		|```
		|",
		"Прочее.Браузер.Клик мышкой");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЯОткрываюСсылкуВБраузере(Парам01)","ЯОткрываюСсылкуВБраузере","И я открываю ссылку в браузере ""Ссылка""",
		"### Описание
		|Открывает ссылку в браузере Chrome.
		|Адрес открывается **в новой вкладке**, и она становится текущей — дальнейшие шаги работы с элементами страницы будут обращаться именно к ней. Идентификатор этой вкладки запоминается, на него опирается шаг <И я жду, что исчезнет вкладка браузера с текущем идентификатором в течение 10 секунд>.
		|Браузер должен быть уже запущен шагом <И я запускаю браузер>.
		|### Пример. Открыть страницу и заполнить поле
		|После открытия адреса сценарий работает с элементами страницы обычными шагами библиотеки.
		|```Gherkin
		|	Дано я запускаю браузер
		|	И я открываю ссылку в браузере 'https://ya.ru'
		|	И в странице браузера для элемента 'text' я устанавливаю текст поля 'Ванесса! Тесты есть!'
		|```
		|",
		"Прочее.Браузер.Вкладки.Открыть новую вкладку");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЯАктивизируюВкладкуБраузераСзаголовком(Парам01)","ЯАктивизируюВкладкуБраузераСзаголовком","И я активизирую вкладку браузера с заголовком ""Заголовок""",
		"### Описание
		|Переключает контекст на уже существующую вкладку в браузере Chrome и активизирует её. При указании заголовка вкладки можно использовать символы *.
		|Нужен, когда вкладок несколько: сайт открыл ссылку в новой вкладке, или сценарий переключается между почтой и тестируемой системой. После переключения все шаги работы с элементами относятся к этой вкладке, и её идентификатор запоминается.
		|Проверить наличие вкладки, не переключаясь на неё, можно шагом <И в браузере есть вкладка с заголовком ""Заголовок"">.
		|### Пример. Переключение на вкладку по части заголовка
		|Заголовок известен неточно, поэтому используется маска.
		|```Gherkin
		|	И я активизирую вкладку браузера с заголовком 'Яндекс*'
		|	И я активизирую текущее окно браузера
		|```
		|",
		"Прочее.Браузер.Вкладки.Активизировать вкладку");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЯЗапускаюБраузер()","ЯЗапускаюБраузер","И я запускаю браузер",
		"### Описание
		|Запускает браузер согласно указанным настройкам (Сервис/Настройки клиентов тестирования/Запуск браузера). Используется в том случае, если нужно выполнять действия во вкладках браузера без использования клиента тестирования, запущенного как web клиент.
		|С этого шага начинается любой сценарий прямой работы с браузером: остальные шаги библиотеки работают с уже запущенным браузером. Требуются три настройки: флаг «Использовать компоненту VanessaExt» на закладке Сервис → Основные, флаг «Использовать браузер + web socket» и заполненная команда запуска Chrome с ключом `--remote-debugging-port` (кнопка «Chrome+debug») — оба на закладке Сервис → Настройки клиентов тестирования.
		|Не путайте с веб-клиентом: там браузером управляет сам клиент тестирования, а эта библиотека нужна, когда надо зайти на **произвольный сайт** — почту, портал контрагента, стороннюю систему.
		|### Пример. Запуск браузера и переход на сайт
		|Браузер поднимается, его окно выводится на передний план, после чего открывается нужный адрес.
		|```Gherkin
		|	Дано я запускаю браузер
		|	И я активизирую вкладку браузера с заголовком 'Новая вкладка'
		|	И я активизирую текущее окно браузера
		|	И я открываю ссылку в браузере 'https://ya.ru'
		|```
		|",
		"Прочее.Браузер.Запуск браузера");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВБраузереЕстьВкладкаСЗаголовком(Парам01)","ВБраузереЕстьВкладкаСЗаголовком","И в браузере есть вкладка с заголовком ""Заголовок""",
		"### Описание
		|Проверяет, что в браузере есть вкладка с нужным заголовком. Можно использовать символы *.
		|Шаг не переключается на вкладку, только проверяет её наличие; если вкладки нет — сценарий падает. Переключение делает <И я активизирую вкладку браузера с заголовком 'Заголовок'>.
		|Типовое применение — убедиться, что нажатие на ссылку и правда открыло новую вкладку.
		|### Пример. Ссылка открыла новую вкладку
		|```Gherkin
		|	И в странице браузера для элемента с именем 'external-link' я делаю клик
		|	И в браузере есть вкладка с заголовком 'Личный кабинет*'
		|```
		|",
		"Прочее.Браузер.Вкладки.Существует вкладка");
	
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЯЖдуЧтоИсчезнетВкладкаБраузераСТекущемИдентификаторомВТечениеСекунд(Парам01)","ЯЖдуЧтоИсчезнетВкладкаБраузераСТекущемИдентификаторомВТечениеСекунд","И я жду, что исчезнет вкладка браузера с текущем идентификатором в течение 10 секунд",
		"### Описание
		|Проверяет, что в браузере исчезнет вкладка с текущим идентификатором. Текущий идентификатор сохраняется при открытии новой вкладки или при активизации существующей вкладки.
		|Идентификатор запоминают шаги <И я открываю ссылку в браузере 'Ссылка'> и <И я активизирую вкладку браузера с заголовком 'Заголовок'>, так что перед ожиданием одна из этих операций должна быть выполнена.
		|Шаг ждёт исчезновения и становится успешным сразу, как вкладка закрылась. Заголовок при этом не важен — сравнение идёт по идентификатору, поэтому шаг не спутает вкладку с другой, у которой такой же заголовок.
		|### Пример. Дождаться, что платёжная страница закрылась
		|Страница закрывает сама себя после завершения операции; сценарий дожидается этого, а не гадает по паузе.
		|```Gherkin
		|	И я открываю ссылку в браузере 'https://payment.example/checkout'
		|	И в странице браузера для элемента с именем 'submit' я делаю клик
		|	И я жду, что исчезнет вкладка браузера с текущем идентификатором в течение 30 секунд
		|```
		|",
		"Прочее.Браузер.Вкладки.Больше нет вкладки с сохраненным идентификатором");
	
	Возврат ВсеТесты;
КонецФункции
	
&НаСервере
// Служебная функция.
Функция ПолучитьМакетСервер(ИмяМакета)
	ОбъектСервер = РеквизитФормыВЗначение("Объект");
	Возврат ОбъектСервер.ПолучитьМакет(ИмяМакета);
КонецФункции
	
&НаКлиенте
// Служебная функция для подключения библиотеки создания fixtures.
Функция ПолучитьМакетОбработки(ИмяМакета) Экспорт
	Возврат ПолучитьМакетСервер(ИмяМакета);
КонецФункции

// Делает отключение модуля
&НаКлиенте
Функция ОтключениеМодуля() Экспорт

	Ванесса = Неопределено;
	Контекст = Неопределено;
	КонтекстСохраняемый = Неопределено;

КонецФункции


///////////////////////////////////////////////////
//Работа со сценариями
///////////////////////////////////////////////////

&НаКлиенте
// Функция выполняется перед началом каждого сценария
Функция ПередНачаломСценария() Экспорт
	
КонецФункции

&НаКлиенте
// Функция выполняется перед окончанием каждого сценария
Функция ПередОкончаниемСценария() Экспорт
	
КонецФункции



///////////////////////////////////////////////////
//Реализация шагов
///////////////////////////////////////////////////

&НаКлиенте
// Функция выполняется перед началом каждого сценария
Функция СброситьБраузер()
	
	Если ПустаяСтрока(Ванесса.Объект.КомандаЗапускаБраузера) Тогда
		ВызватьИсключение "Не заполнена строка запуска браузера (см. Сервис->Настройки клиентов тестирования->Запуск браузера)";
	КонецЕсли;
	
	ФормаНастройкиРаботыСБраузером = Ванесса.МодульНастройкиРаботыСБраузером();
	
	Если Ванесса.ПолучитьМассивPIDПроцессов(Ванесса.Объект.КомандаЗапускаБраузера).Количество() > 0 Тогда
		Ванесса.TASKKILL(Ванесса.Объект.КомандаЗапускаБраузера);
	КонецЕсли;
	
	СтрокаКоманды = Ванесса.Объект.КомандаЗапускаБраузера;
	ЗапуститьПриложение(СтрокаКоманды);
	
КонецФункции

&НаКлиенте
//И Нужно сбросить браузер
//@НужноСброситьБраузер()
Функция НужноСброситьБраузер() Экспорт
	
	СброситьБраузер();
	
КонецФункции

&НаКлиенте
Функция JavascriptДляЭлементаСтраницыБраузера(Идентификатор, Значение, ДопПараметры = Неопределено)
	
	focus = Истина;
	
	Если ТипЗнч(ДопПараметры) = Тип("Структура") Тогда
		Если ДопПараметры.Свойство("focus") Тогда
			focus = ДопПараметры.focus;
		КонецЕсли;	
	КонецЕсли;	
	
	ТабПарам = Новый Структура("Идентификатор,ИмяФункции", Значение,"");
	
	Если ВРег(Идентификатор) = "ID" Тогда
		ТабПарам.ИмяФункции = "getElementById";
	ИначеЕсли ВРег(Идентификатор) = "NAME" Тогда
		ТабПарам.ИмяФункции = "getElementsByName";
	ИначеЕсли ВРег(Идентификатор) = "CLASSNAME" Тогда
		ТабПарам.ИмяФункции = "getElementsByClassName";
	Иначе
		Возврат Неопределено
	КонецЕсли;
	
	Если ТабПарам.ИмяФункции = "getElementById" Тогда
		ТекстШаблонаНачало = "{
		|    let node = document.%ИмяФункции%('%Идентификатор%');
		|    if(node === null) {
		|	    throw new Error('Element ""%Идентификатор%"" not found.');   }
		|	 node.focus();";
	Иначе	
		ТекстШаблонаНачало = "{
		|    let node1 = document.%ИмяФункции%('%Идентификатор%');
		|    if(node1.length === 0) {
		|	    throw new Error('Element ""%Идентификатор%"" not found.');   }
		|    let node = node1[0];
		|	 node.focus();";
	КонецЕсли;
	
	ТекстJavaScript = ТекстШаблонаНачало;
	
	Для Каждого СтрокаПарам Из ТабПарам Цикл
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%"+СтрокаПарам.Ключ+"%", СтрокаПарам.Значение)
	КонецЦикла; 
	
	Если НЕ focus Тогда
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "node.focus();", "");
	КонецЕсли;	
	
	Возврат ТекстJavaScript
КонецФункции

&НаКлиенте
//И я рисую прямоугольник для браузера
//@ЯРисуюПрямоугольникДляБраузера(ТабПарам)
Функция ЯРисуюПрямоугольникДляБраузера(ТабПарам) Экспорт
	
	ТекстШаблона = "{
	|    let node = document.createElement('div');
	|    node.style.fontSize = '100%';
	|    node.style.position = 'fixed';
	|    node.style.zIndex = 999999;
	|    node.style.left = '%ТочкаX%px';
	|    node.style.top = '%ТочкаY%px';
	|    node.style.width = '%Width%px';
	|    node.style.height = '%Height%px';
	|    node.style.borderWidth = '%borderWidth%';
	|    node.style.borderStyle = 'solid';
	|    node.style.borderColor = '%borderColor%';
	|    node.style.backgroundColor = '#11ffee00';
	|    node.style.padding = '1em';
	|    node.style.borderRadius = '1em';
	|    window.top.document.body.appendChild(node);
	|    setTimeout(() => { if (node) node.remove() }, %Длительность%);
	|};";
	
	ТекстJavaScript = ТекстШаблона;
	
	Для Каждого СтрокаПарам Из ТабПарам Цикл
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%"+СтрокаПарам.Кол1+"%", СтрокаПарам.Кол2)
	КонецЦикла;
	
	// Если значения параметров в шаге не заданы - ставим по умолчанию.
	ПараметрыУмолчания = Новый Структура("borderWidth,borderColor,Длительность","0.1em","red","10000");
	Для Каждого СтрокаПарам Из ПараметрыУмолчания Цикл
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%"+СтрокаПарам.Ключ+"%", СтрокаПарам.Значение)
	КонецЦикла;
	
	ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));
	
КонецФункции

&НаКлиенте
//И я Выполняю код Javascript в странице веб-браузера
//@ЯВыполняюКодJavascriptВСтраницеБраузера(ТабПарам)
Функция ЯВыполняюКодJavascriptВСтраницеБраузера(ТабПарам) Экспорт
	
	ТекстJavaScript = Ванесса.МногострочнаяСтрокаИзПараметраШага(ТабПарам);
	
	ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));
	
КонецФункции

&НаКлиенте
//И я рисую надпись для браузера
//@ЯРисуюНадписьДляБраузера(Парам01,ТабПарам)
Функция ЯРисуюНадписьДляБраузера(ТабПарам) Экспорт
	
	//Ванесса.ПосмотретьЗначение(Истина);
	ТекстШаблона = "{
	|    let node = document.createElement('div');
	|    node.innerHTML = 
	|    '%НадписьТекст%';
	|    node.style.fontSize = '100%';
	|    node.style.position = 'fixed';
	|    node.style.zIndex = 999999;
	|    node.style.left = '%ТочкаX%px';
	|    node.style.top = '%ТочкаY%px';
	|    node.style.color = 'red';
	|    node.style.backgroundColor = '#11ffee00';
	|    node.style.padding = '1em';
	|    window.top.document.body.appendChild(node);
	|    setTimeout(() => { if (node) node.remove() }, %Длительность%);
	|};";   
	ТекстJavaScript = ТекстШаблона;
	
	Для Каждого СтрокаПарам Из ТабПарам Цикл
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%"+СтрокаПарам.Кол1+"%", СтрокаПарам.Кол2)
	КонецЦикла;
	
	// Если значения параметров в шаге не заданы - ставим по умолчанию.
	ПараметрыУмолчания = Новый Структура("color,Длительность","red","10000");
	Для Каждого СтрокаПарам Из ПараметрыУмолчания Цикл
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%"+СтрокаПарам.Ключ+"%", СтрокаПарам.Значение)
	КонецЦикла;
	
	
	ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента "ID" я устанавливаю текст поля "ТекстПоля"
//@ВСтраницеБраузераДляЭлементаЯУстанавливаюТекстПоля(ID,ТекстПоля)
Функция ВСтраницеБраузераДляЭлементаЯУстанавливаюТекстПоля(ID,ТекстПоля) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("ID", ID);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.value = '%ТекстПоля%';
			|};";
			ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%ТекстПоля%", ТекстПоля);
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	
КонецФункции

&НаКлиенте
//И страница браузера содержит текст "ТекстHTML"
//@СтраницаБраузераСодержитТекст(ТекстHTML)
Функция СтраницаБраузераСодержитТекст(ИскомыйТекст) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка
			
			ТекстJavaScript = "document.body.innerText";
			Результат = Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript);
			ПроверитьРезультатВыполненияJavaScript(Результат);
			
			Если Найти(НРег(Результат.result.result.value), НРег(ИскомыйТекст)) = 0 Тогда
				ТекстСообщения = Ванесса.Локализовать("Во вкладке браузера не найден текст <%1>.");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", ИскомыйТекст);
				ВызватьИсключение ТекстСообщения;
			КонецЕсли;
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента "ID" я получаю текст в переменную "ИмяПеременной"
//@ВСтраницеБраузераДляЭлементаЯПолучаюТекстВПеременную(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаЯПолучаюТекстВПеременную(ID,ИмяПеременной) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка
			
			Ванесса.СохранитьЗначениеПеременнойВКонтекст(ИмяПеременной, ЗначениеТекстаЭлементаБраузера("ID", ID));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента "ID" я получаю ссылку в переменную "ИмяПеременной"
//@ВСтраницеБраузераДляЭлементаЯПолучаюСсылкуВПеременную(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаЯПолучаюСсылкуВПеременную(ID,ИмяПеременной) Экспорт                    
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка
			
			Ванесса.СохранитьЗначениеПеременнойВКонтекст(ИмяПеременной, ЗначениеАтрибутаЭлементаБраузера("ID", ID, "href"));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента "ID" я получаю текст кнопки в переменную "ИмяПеременной"
//@ВСтраницеБраузераДляЭлементаЯПолучаюТекстКнопкиВПеременную(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаЯПолучаюТекстКнопкиВПеременную(ID,ИмяПеременной) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка
			
			Ванесса.СохранитьЗначениеПеременнойВКонтекст(ИмяПеременной, ЗначениеАтрибутаЭлементаБраузера("ID", ID, "value"));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;			
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента "ID" я получаю список значений в переменную "ИмяПеременной"
//@ВСтраницеБраузераДляЭлементаЯПолучаюСписокЗначенийВПеременную(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаЯПолучаюСписокЗначенийВПеременную(ID,ИмяПеременной) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка
			
			Результат = ЗначениеАтрибутаЭлементаБраузера("ID", ID, "options");
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;			
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента "ID" я выбираю значение "ЗначениеЭлемента" в списке выбора
//@ВСтраницеБраузераДляЭлементаЯВыбираюЗначениеВСпискеВыбора(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаЯВыбираюЗначениеВСпискеВыбора(Парам01,Парам02) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка
			
			ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("ID", Парам01);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.value = '%ТекстПоля%';
			|};";
			
			ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%ТекстПоля%", Парам02);
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));	
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента "ID" я делаю клик
//@ВСтраницеБраузераДляЭлементаЯДелаюКлик(Парам01)
Функция ВСтраницеБраузераДляЭлементаЯДелаюКлик(Парам01) Экспорт
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("ID", Парам01);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.click();
			|};";
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));	
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента "ID" я делаю правый клик
//@ВСтраницеБраузераДляЭлементаЯДелаюПравыйКлик(Парам01)
Функция ВСтраницеБраузераДляЭлементаЯДелаюПравыйКлик(Парам01) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("ID", Парам01);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.contextMenu();
			|};";
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));	
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем "Name" я получаю текст в переменную "ИмяПеременной"
//@ВСтраницеБраузераДляЭлементаСИменемЯПолучаюТекстВПеременную(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаСИменемЯПолучаюТекстВПеременную(Name,ИмяПеременной) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			Ванесса.СохранитьЗначениеПеременнойВКонтекст(ИмяПеременной, ЗначениеТекстаЭлементаБраузера("name", Name));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем "Name" я получаю ссылку в переменную "ИмяПеременной"
//@ВСтраницеБраузераДляЭлементаСИменемЯПолучаюСсылкуВПеременную(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаСИменемЯПолучаюСсылкуВПеременную(name,ИмяПеременной) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			Ванесса.СохранитьЗначениеПеременнойВКонтекст(ИмяПеременной, ЗначениеАтрибутаЭлементаБраузера("name", name, "href"));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем "Name" я получаю текст кнопки в переменную "ИмяПеременной"
//@ВСтраницеБраузераДляЭлементаСИменемЯПолучаюТекстКнопкиВПеременную(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаСИменемЯПолучаюТекстКнопкиВПеременную(Name,ИмяПеременной) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			Ванесса.СохранитьЗначениеПеременнойВКонтекст(ИмяПеременной, ЗначениеАтрибутаЭлементаБраузера("Name", Name, "value"));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем "Name" я устанавливаю текст поля "ТекстПоля"
//@ВСтраницеБраузераДляЭлементаСИменемЯУстанавливаюТекстПоля(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаСИменемЯУстанавливаюТекстПоля(Парам01,Парам02) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("name", Парам01);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.value = '%ТекстПоля%';
			|};";
			ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%ТекстПоля%", Парам02);
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));	
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем "Name" я получаю список значений в переменную "ИмяПеременной"
//@ВСтраницеБраузераДляЭлементаСИменемЯПолучаюСписокЗначенийВПеременную(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаСИменемЯПолучаюСписокЗначенийВПеременную(name,ИмяПеременной) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			Результат = ЗначениеАтрибутаЭлементаБраузера("name", name, "options");
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем "Name" я выбираю значение "ЗначениеЭлемента" в списке выбора
//@ВСтраницеБраузераДляЭлементаСИменемЯВыбираюЗначениеВСпискеВыбора(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаСИменемЯВыбираюЗначениеВСпискеВыбора(Парам01,Парам02) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("name", Парам01);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.value = '%ТекстПоля%';
			|};";
			
			ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%ТекстПоля%", Парам02);
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));	
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем "Name" я делаю клик
//@ВСтраницеБраузераДляЭлементаСИменемЯДелаюКлик(Парам01)
Функция ВСтраницеБраузераДляЭлементаСИменемЯДелаюКлик(Парам01) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("name", Парам01);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.click();
			|};";
			
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем "Name" я делаю клик
//@ВСтраницеБраузераДляЭлементаСИменемЯДелаюКлик(Парам01)
Функция ВСтраницеБраузераДляЭлементаСIDЯДелаюКлик(Парам01) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("id", Парам01);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.click();
			|};";
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем "Name" я делаю правый клик
//@ВСтраницеБраузераДляЭлементаСИменемЯДелаюПравыйКлик(Парам01)
Функция ВСтраницеБраузераДляЭлементаСИменемЯДелаюПравыйКлик(Парам01) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("name", Парам01);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.contextMenu();
			|};";
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем класса "className" я получаю текст в переменную "ИмяПеременной"
//@ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюТекстВПеременную(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюТекстВПеременную(className,ИмяПеременной) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			Ванесса.СохранитьЗначениеПеременнойВКонтекст(ИмяПеременной, ЗначениеТекстаЭлементаБраузера("classname", className));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем класса "className" я получаю ссылку в переменную "ИмяПеременной"
//@ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюСсылкуВПеременную(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюСсылкуВПеременную(classname,ИмяПеременной) Экспорт                     
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка
			
			Ванесса.СохранитьЗначениеПеременнойВКонтекст(ИмяПеременной, ЗначениеАтрибутаЭлементаБраузера("classname", classname, "href"));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем класса "className" я получаю текст кнопки в переменную "ИмяПеременной"
//@ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюТекстКнопкиВПеременную(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюТекстКнопкиВПеременную(classname,ИмяПеременной) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка
			
			Ванесса.СохранитьЗначениеПеременнойВКонтекст(ИмяПеременной, ЗначениеАтрибутаЭлементаБраузера("classname", classname, "value"));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем класса "className" я устанавливаю текст поля "ТекстПоля"
//@ВСтраницеБраузераДляЭлементаСИменемКлассаЯУстанавливаюТекстПоля(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаСИменемКлассаЯУстанавливаюТекстПоля(Парам01,Парам02) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("classname", Парам01);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.value = '%ТекстПоля%';
			|};";
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем класса "className" я получаю список значений в переменную "ИмяПеременной"
//@ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюСписокЗначенийВПеременную(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаСИменемКлассаЯПолучаюСписокЗначенийВПеременную(classname,ИмяПеременной) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка
			
			Результат = ЗначениеАтрибутаЭлементаБраузера("classname", classname, "options");
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем класса "className" я выбираю значение "ЗначениеЭлемента" в списке выбора
//@ВСтраницеБраузераДляЭлементаСИменемКлассаЯВыбираюЗначениеВСпискеВыбора(Парам01,Парам02)
Функция ВСтраницеБраузераДляЭлементаСИменемКлассаЯВыбираюЗначениеВСпискеВыбора(Парам01,Парам02) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("classname", Парам01);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.value = '%ТекстПоля%';
			|};";
			
			ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%ТекстПоля%", Парам02);
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем класса "className" я делаю клик
//@ВСтраницеБраузераДляЭлементаСИменемКлассаЯДелаюКлик(Парам01)
Функция ВСтраницеБраузераДляЭлементаСИменемКлассаЯДелаюКлик(Парам01) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("classname", Парам01);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.click();
			|};";
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера для элемента с именем класса "className" я делаю правый клик
//@ВСтраницеБраузераДляЭлементаСИменемКлассаЯДелаюПравыйКлик(Парам01)
Функция ВСтраницеБраузераДляЭлементаСИменемКлассаЯДелаюПравыйКлик(Парам01) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			ТекстШаблонаНачало = JavascriptДляЭлементаСтраницыБраузера("classname", Парам01);
			ТекстJavaScript = ТекстШаблонаНачало + "
			|    node.contextMenu();
			|};";
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в странице браузера я провожу скролл к координатам "500" "1000"
//@ВСтраницеБраузераЯПровожуСкроллККоординатам(Парам01,Парам02)
Функция ВСтраницеБраузераЯПровожуСкроллККоординатам(Парам01,Парам02) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка	
			
			ТекстШаблона = "{
			|	 window.scrollTo(%ТочкаX%, %ТочкаY%);
			|};";
			
			ТекстJavaScript = ТекстШаблона;
			
			ТабПарам = Новый Структура("ТочкаX,ТочкаY", Парам01,Парам02);
			Для Каждого СтрокаПарам Из ТабПарам Цикл
				ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%"+СтрокаПарам.Ключ+"%", СтрокаПарам.Значение)
			КонецЦикла;
			
			ПроверитьРезультатВыполненияJavaScript(Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript));
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И я открываю ссылку в браузере "Ссылка"
//@ЯОткрываюСсылкуВБраузере(Парам01)
Функция ЯОткрываюСсылкуВБраузере(Адрес) Экспорт
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка
			
			Ванесса.ОткрытьВкладкуБраузера(Адрес);
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
Процедура ПроверитьРезультатВыполненияJavaScript(Результат)
	
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		Если Результат.Свойство("error") Тогда
			ВызватьИсключение Результат.error.message;
		ИначеЕсли Результат.Свойство("result") Тогда	
			Если Результат.result.Свойство("exceptionDetails") Тогда
				ВызватьИсключение Результат.result.exceptionDetails.exception.description;
			КонецЕсли;	
		КонецЕсли;	
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
//И я активизирую вкладку браузера с заголовком "Заголовок"
//@ЯАктивизируюВкладкуБраузераСзаголовком(Парам01)
Функция ЯАктивизируюВкладкуБраузераСзаголовком(ЗаголовокВкладки) Экспорт
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка
			
			Ванесса.АктивизироватьВкладкуБраузераПоЗаголовкуВкладки(ЗаголовокВкладки);
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	
КонецФункции

&НаКлиенте
Функция СтраницаБраузераСодержитТекстСлужебный(ТекстHTML, Данные)
	Результат = Ложь;
	
	СтраницаБраузераСодержитТекстСлужебныйРекурсивно(ТекстHTML, Данные, Результат);
	
	Возврат Результат;
КонецФункции	

&НаКлиенте
Процедура СтраницаБраузераСодержитТекстСлужебныйРекурсивно(ИскомыйТекст, Данные, Результат)
	Если Результат Тогда
		Возврат;
	КонецЕсли;	
	
	Для Каждого Элем Из Данные Цикл
		
		Если Элем.Свойство("NodeValue") Тогда
			Если Найти(НРег(Элем.NodeValue), НРег(ИскомыйТекст)) > 0 Тогда
				Результат = Истина;
				Возврат;
			КонецЕсли;	
		КонецЕсли;	
		
		Если Элем.Свойство("children") Тогда
			СтраницаБраузераСодержитТекстСлужебныйРекурсивно(ИскомыйТекст, Элем.children, Результат);
			Если Результат Тогда
				Прервать;
			КонецЕсли;	
		КонецЕсли;	
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Функция ЗначениеТекстаЭлементаБраузера(Идентификатор, Значение);
	
	ТекстJavaScript = ТекстJavaScriptЗначениеТекстаЭлементаБраузера();
	
	Если ВРег(Идентификатор) = "ID" Тогда
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%function%", "getElementById");
	ИначеЕсли ВРег(Идентификатор) = "NAME" Тогда
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%function%", "getElementsByName");
	ИначеЕсли ВРег(Идентификатор) = "CLASSNAME" Тогда
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%function%", "getElementsByClassName");
	Иначе
		Возврат Неопределено
	КонецЕсли;
	
	ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%Element%", Значение);
	
	Результат = Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript);
	Возврат Результат.result.result.value;
	
КонецФункции	

&НаКлиенте
Функция ЗначениеАтрибутаЭлементаБраузера(Идентификатор, Значение, ИмяАтрибута);
	
	ТекстJavaScript = ТекстJavaScriptЗначениеАтрибутаЭлементаБраузера();
	
	Если ВРег(Идентификатор) = "ID" Тогда
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%function%", "getElementById");
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "[0]", "");
		
	ИначеЕсли ВРег(Идентификатор) = "NAME" Тогда
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%function%", "getElementsByName");
	ИначеЕсли ВРег(Идентификатор) = "CLASSNAME" Тогда
		ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%function%", "getElementsByClassName");
	Иначе
		Возврат Неопределено
	КонецЕсли;
	
	ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%Element%", Значение);
	ТекстJavaScript = СтрЗаменить(ТекстJavaScript, "%ИмяАтрибута%", ИмяАтрибута);
	
	Результат = Ванесса.ВыполнитьJavaScriptБраузер(ТекстJavaScript);
	ПроверитьРезультатВыполненияJavaScript(Результат);
	Возврат Результат.result.result.value;
	
КонецФункции	

&НаКлиенте
Функция ТекстJavaScriptЗначениеТекстаЭлементаБраузера()
	
	Возврат "
	|{
	|function getres(){
	|    let node1 = document.%function%('%Element%');
	|    if(node1.length === 0) {
	|        throw new Error('Element ""%Element%"" not found.');   }
	|    let node = node1[0];
	|     
	|
	| if(node.nodeName === 'A') {
	|  console.log(node.innerText);
	|  return node.innerText;
	|}
	|else {
	|  console.log(node.value);
	| return node.value;
	|}; 
	|
	|};
	|getres()
	|}
	|";
	
КонецФункции
&НаКлиенте
Функция ТекстJavaScriptЗначениеАтрибутаЭлементаБраузера()
	
	Возврат "
	|{
	|function getres(){
	|    let node1 = document.%function%('%Element%');
	|    if(node1.length === 0) {
	|        throw new Error('Element ""%Element%"" not found.');   }
	|    let node = node1[0];
	|     
	| return node.%ИмяАтрибута%;
	|
	|};
	|getres()
	|}
	|";
	
КонецФункции	

&НаКлиенте
Функция УбратьКавычкиАпострофы(Стр)
	Если Лев(Стр, 1) = """" И Прав(Стр, 1) = """" Тогда
		Возврат Сред(Стр, 2, СтрДлина(Стр) - 2);
	КонецЕсли;	 
	
	Если Лев(Стр, 1) = "'" И Прав(Стр, 1) = "'" Тогда
		Возврат Сред(Стр, 2, СтрДлина(Стр) - 2);
	КонецЕсли;	 
	
	Возврат Стр; 
КонецФункции	

&НаКлиенте
//И я запускаю браузер
//@ЯЗапускаюБраузер()
Функция ЯЗапускаюБраузер() Экспорт
	Ванесса.ЗапуститьБраузер(Ложь);

	Если Ванесса.Объект.ИспользоватьБраузерПлюсWebSocket И Ванесса.Объект.ИспользоватьКомпонентуVanessaExt Тогда
		ТекстОшибки = "";
		Ошибка = Ложь;
		КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
		Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
			Ошибка = Ложь;
			ТекстОшибки = "";

			Попытка

				ДанныеВкладокБраузера = Ванесса.ДанныеВкладокБраузера();

			Исключение
				ТекстОшибки = ОписаниеОшибки();
				Ошибка = Истина;
				Если СчетчикПопыток < КоличествоПопыток Тогда
					// Первая попытка часто падает с холодного старта Chrome (--remote-debugging-port:9222
					// поднимается 3+ сек). После первой ошибки даём подольше, далее короткие паузы.
					Ванесса.sleep(?(СчетчикПопыток = 1, 3, 1));
				КонецЕсли;
			КонецПопытки;

			Если НЕ Ошибка Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Если Ошибка Тогда
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;	
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
//И в браузере есть вкладка с заголовком "Заголовок"
//@ВБраузереЕстьВкладкаСЗаголовком(Парам01)
Функция ВБраузереЕстьВкладкаСЗаголовком(ЗаголовокВкладки) Экспорт
	
	ТекстОшибки = "";
	Ошибка = Ложь;
	КоличествоПопыток = Ванесса.ЗначениеКоличествоПопытокВыполненияДействия();
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		Ошибка = Ложь;
		ТекстОшибки = "";
		
		Попытка
			
			Нашли = Ложь;
			ДанныеВкладокБраузера = Ванесса.ДанныеВкладокБраузера();
			Для Каждого Элем Из ДанныеВкладокБраузера Цикл
				Если Ванесса.СтрокаСоответствуетШаблону(НРег(Элем.title), НРег(ЗаголовокВкладки)) Тогда
					Нашли = Истина;
					Прервать;
				КонецЕсли;	
			КонецЦикла;
			
			Если НЕ Нашли Тогда
				ТекстИсключения = Ванесса.Локализовать("Не найдена вкладка браузера <%1>.");
				ТекстИсключения = СтрЗаменить(ТекстИсключения, "%1", ЗаголовокВкладки);
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;    
			
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Ошибка = Истина;
			Если СчетчикПопыток < КоличествоПопыток Тогда
				Ванесса.sleep(1);
			КонецЕсли;	 
		КонецПопытки;
		
		Если НЕ Ошибка Тогда
			Прервать;
		КонецЕсли;	 
	КонецЦикла;	
	
	Если Ошибка Тогда
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;	
	
КонецФункции

&НаКлиенте
Функция ОбработчикОжиданияЯЖдуЧтоИсчезнетВкладкаБраузераСТекущемИдентификаторомВТечениеСекунд()
	
	ИмяОбработчика = "ОбработчикОжиданияЯЖдуЧтоИсчезнетВкладкаБраузераСТекущемИдентификаторомВТечениеСекунд";
	
	Если НЕ Ванесса.ИдетВыполнениеСценариев() Тогда
		ОтключитьОбработчикОжидания(ИмяОбработчика);
		Возврат Неопределено;
	КонецЕсли;	 
	
	Нашли = Ложь;
	БылаОшибка = Ложь;
	Попытка
		ДанныеВкладокБраузера = Ванесса.ДанныеВкладокБраузера();
		Для Каждого Элем Из ДанныеВкладокБраузера Цикл
			Если Элем.id = ИдентификаторВкладкиОбработкаОжидания Тогда
				Нашли = Истина;
				Прервать;
			КонецЕсли;	
		КонецЦикла;	
	Исключение
		БылаОшибка = Истина;
		Ванесса.ЗаписатьЛогВЖРОшибка("ОбработчикОжиданияЯЖдуЧтоИсчезнетВкладкаБраузераСТекущемИдентификаторомВТечениеСекунд", ОписаниеОшибки())
	КонецПопытки;
	
	Если НЕ Нашли И НЕ БылаОшибка Тогда
		ОтключитьОбработчикОжидания(ИмяОбработчика);
		Ванесса.ПродолжитьВыполнениеШагов();
		Возврат Неопределено;
	КонецЕсли;	
	
	Если (ТекущаяДата() - ДатаНачалаОбработкиОжидания) > КоличествоСекундОбработкаОжидания Тогда
		ОтключитьОбработчикОжидания(ИмяОбработчика);
		ТекстСообщения = Ванесса.ПолучитьТекстСообщенияПользователю("Ожидали в течение <%1> секунд, что изменится исчезнет вкладка с идентификатором <%2>.");
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",КоличествоСекундОбработкаОжидания);
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%2",ИдентификаторВкладкиОбработкаОжидания);
		Ванесса.ПродолжитьВыполнениеШагов(Истина,ТекстСообщения);
		Возврат Неопределено;
	КонецЕсли;	 
	
КонецФункции

&НаКлиенте
//И я жду, что исчезнет вкладка браузера с текущем идентификатором в течение 10 секунд
//@ЯЖдуЧтоИсчезнетВкладкаБраузераСТекущемИдентификаторомВТечениеСекунд(Парам01)
Функция ЯЖдуЧтоИсчезнетВкладкаБраузераСТекущемИдентификаторомВТечениеСекунд(КоличествоСекунд) Экспорт
	
	ИдентификаторВкладкиОбработкаОжидания = Ванесса.ИдентификаторТекущейВкладкиБраузера();
	Если НЕ ЗначениеЗаполнено(ИдентификаторВкладкиОбработкаОжидания) Тогда
		Возврат Неопределено;
	КонецЕсли;	
	
	Ванесса.ЗапретитьВыполнениеШагов();
	ДатаНачалаОбработкиОжидания       = ТекущаяДата();
	КоличествоСекундОбработкаОжидания = Ванесса.ЗначениеТаймаутаДляАсинхронногоШага(КоличествоСекунд);
	ПодключитьОбработчикОжидания("ОбработчикОжиданияЯЖдуЧтоИсчезнетВкладкаБраузераСТекущемИдентификаторомВТечениеСекунд",1,Ложь);
	
КонецФункции
