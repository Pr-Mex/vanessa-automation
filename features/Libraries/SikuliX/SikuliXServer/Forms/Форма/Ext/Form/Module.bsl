...
Процедура КликНаКартинку(ИмяФайла) Экспорт
	...
	Если Ванесса.Объект.ПоискКартинокСПомощьюКомпонентыVanessaExt Тогда
...
Процедура НайтиКартинкуНаЭкране(ПутьКФайлу, ИмяФайла, Действие, ДопПараметры = Неопределено)
	...
	Если ДопПараметры = Неопределено Тогда
		ДопПараметры = Новый Структура("ИмяФайла,Действие", ИмяФайла, Действие);
	КонецЕсли;
...
Процедура ОкончаниеВызоваНайтиНаЭкране(Результат, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Действие = "КурсорККартинке"
		...
		ИЛИ ДополнительныеПараметры.Действие = "ДвойнойКликНаКартинку" Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ОкончаниеВызоваЭмуляцияДвиженияМыши", ЭтаФорма, ДополнительныеПараметры);
		Ванесса.ВнешняяКомпонентаДляСкриншотов.НачатьВызовЭмуляцияДвиженияМыши(ОписаниеОповещения, Координаты.X, Координаты.Y, 1000, 3);
