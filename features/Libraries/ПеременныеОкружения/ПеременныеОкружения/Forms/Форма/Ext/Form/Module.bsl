...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Значение читается на той машине, где запущена Vanessa Automation, — под Windows через `set`, под Linux через `env`. Если переменной среды нет, в переменную контекста попадёт пустая строка.
		|Основное применение — не хранить в сценарии логины, пароли и пути к базам: их передают через переменные окружения агента сборки.
		|### Пример. Чтение переменной среды в переменную сценария
		|В окно сообщений попадёт значение переменной среды `MY_ENV_VAR`; если такой переменной в окружении нет, вывод будет пустым. Дальше значение подставляют в любой шаг через `$ЗначениеПеременнойСреды$` — например, вводят в поле пароля.
		|```Gherkin
		|	И Я запоминаю значение переменной среды 'MY_ENV_VAR' в переменную 'ЗначениеПеременнойСреды'
		|	И я вывожу значение переменной 'ЗначениеПеременнойСреды'
		|```
		|",
		"Переменные среды");
