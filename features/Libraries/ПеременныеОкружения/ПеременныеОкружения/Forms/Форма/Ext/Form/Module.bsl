...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Значение читается на той машине, где работает менеджер тестирования, — под Windows через `set`, под Linux через `env`. Если переменной среды нет, в переменную контекста попадёт пустая строка.
		...
		|В окно сообщений попадёт значение переменной среды `MY_ENV_VAR`, а дальше его можно подставлять в шаги через `$ЗначениеПеременнойСреды$`.
		|```Gherkin
		|	И Я запоминаю значение переменной среды 'MY_ENV_VAR' в переменную 'ЗначениеПеременнойСреды'
		|	И я вывожу значение переменной 'ЗначениеПеременнойСреды'
		|	И в поле с именем 'Пароль' я ввожу текст '$ЗначениеПеременнойСреды$'
		|```
		|",
		"Переменные среды");
