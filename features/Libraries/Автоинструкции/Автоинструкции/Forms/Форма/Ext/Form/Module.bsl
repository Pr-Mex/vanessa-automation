...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(
		...
		|```
		|### Пример. Путь и подсказка из переменных сценария
	...
	Ванесса.ДобавитьШагВМассивТестов(
		...
		" |'Уровень  '|'2'|",
		"### Описание
		|Добавляет заголовок в инструкции MD и HTML. Уровень задаёт глубину заголовка, от неё зависит его оформление.
	...
	Ванесса.ДобавитьШагВМассивТестов(
		...
		"### Описание
		|Добавляет горизонтальную разделительную линию в автоинструкцию.
	...
	Ванесса.ДобавитьШагВМассивТестов(
		...
		|Если в поле `Вариант оформления` выбрана `3D Карусель`, то в поле `ID якоря` указывают индекс шага со скриншотом, например `#2`.
		|Вариант оформления задаётся не шагом, а настройкой Vanessa Automation: Сервис → Автоинструкции → Текстовые инструкции → HTML → Стили HTML, поле
		|`Вариант оформления`. При запуске из командной строки — параметр JSON `VariantsHTMLInstructions`.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯВставляюИзображениеИзФайлаВАвтоинструкцию(Парам01)", 
		...
		|Файл ищется в каталоге проекта рекурсивно, в любой его подпапке, а если не нашёлся и идёт прогон — ещё и в каталоге фичи. Имя без расширения дополняется до `.png`; если подошло больше одного файла, шаг завершится ошибкой.
