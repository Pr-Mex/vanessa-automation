...
Функция ЯДобавляюПроизвольныйТекстВАвтоинструкцию(Парам) Экспорт
	...
	УзелТекст = ДокументHTML.СоздатьТекстовыйУзел(Текст);
...
&НаКлиенте
Процедура ЗаполнитьПереданныеВШагТаблицы(Параметр1)
	
	ПараметрыШага = Новый Структура;
	ПараметрыШага.Вставить("Кол1", Параметр1);
	...
	Если ТекущийШаг.Свойство("ПереданныеВШагТаблицы") Тогда
		...
		ПереданныеВШагТаблицы.Добавить(ПараметрыШага);
	...
	Иначе
		...
		МассивПараметров.Добавить(ПараметрыШага);
