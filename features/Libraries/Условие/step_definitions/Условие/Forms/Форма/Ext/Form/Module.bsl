...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ПоявилосьОкноСЗаголовкомТогда(Парам01)","ПоявилосьОкноСЗаголовкомТогда","Если появилось окно с заголовком ""ИмяОкна"" Тогда","Условие. Проверяет, что среди всех окон есть окно с нужным заголовком","UI.Окна","Условие");
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ТекущееОкноИмеетЗаголовокТогда(Парам01)","ТекущееОкноИмеетЗаголовокТогда","Если текущее окно имеет заголовок ""ИмяОкна"" Тогда","Условие. Проверяет, что в данный момент активным окном является окно с нужным заголовком","UI.Окна","Условие");
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВТаблицеПолеИмеетЗначениеТогда(Парам01,Парам02,Парам03)","ВТаблицеПолеИмеетЗначениеТогда","Если в таблице ""ИмяТаблицы"" поле ""ИмяПоля"" имеет значение ""ЗначениеПоля"" тогда","Условие. Проверяет значение ячейки таблицы. Имя ячейки указывается как оно отображено в заголовке.","UI.Таблицы.Значение ячейки таблицы","Условие");
...
Процедура ВТаблицеЕстьСтрокаТогда(ИмяТЧ,ТабПарам) Экспорт
	Если ТабПарам.Количество() <> 2 Тогда
		ТекстСообщения = "Шаг принимает на вход только таблицу Gherkin, в которой две строки. А было передано %1 строк.";
...
Процедура ВТаблицеЕстьКолонкиТогда(ИмяТЧ,ТабПарам) Экспорт
	...
	Если ЗначениеТЧ = Неопределено Тогда
		ТекстСообщения = "Не смог получить состяние таблицы <%1>";
	...
	Для Каждого СтрокаТабПарам Из ТабПарам Цикл
		...
		Если ЗначениеВСоответствии = Неопределено Тогда
			ТекстСообщения = "Не нашел колонку: %1";
