...
Функция ЯПоказываюПодсказкуEnjoyHintУЭлемента(Текст, ИмяЭлемента, ТабПарам = Неопределено) Экспорт
	...
	Попытка
		...
		Если Вычислить("ОкноВнешнегоСайтаДоступно") Тогда
			ЭтоВнешнийСайт = Истина;
		КонецЕсли;
	...
	Если ЭтоВнешнийСайт Тогда
		...
		ТекстСкрипта = СтрШаблон(ШаблонПодсказки, ИмяЭлемента, Текст, shape, timeout);
