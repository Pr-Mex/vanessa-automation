...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯВключаюПолноэкранныйРежимВБраузере()",
		"ЯВключаюПолноэкранныйРежимВБраузере",
		"И я включаю полноэкранный режим в браузере",
		"Браузер переключается в полноэкранный режим",
		"Прочее.Браузер");

	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯОткрываюВБраузереВнешнийФайл(ИмяФайла)",
	"ЯОткрываюВБраузереВнешнийФайл",
	"И я открываю в браузере внешний файл ""ИмяФайла""",
	"Открывает в браузере произвольный внешний файл, например файл внешней обработки.
...
&НаКлиенте
//И я включаю полноэкранный режим в браузере
Процедура ЯВключаюПолноэкранныйРежимВБраузере() Экспорт
	Ванесса.МодульРаботаСБраузером().АктивизироватьОкноБраузера(Истина);
КонецПроцедуры

&НаКлиенте
//И я открываю в браузере внешний файл "ИмяФайла"
Процедура ЯОткрываюВБраузереВнешнийФайл(ИмяФайла) Экспорт

	ВнКомп = Ванесса.ВнешняяКомпонентаДляСкриншотов;
	ВнКомп.ЭмуляцияНажатияКлавиши("[17,79]");
	Ванесса.ВыполнитьJavaScriptБраузер("document.querySelector('#fileSelectButton').click()");
	ВнКомп.Пауза(500);
	ВнКомп.ЭмуляцияВводаТекста(ИмяФайла, 0);
	ВнКомп.Пауза(500);
	ВнКомп.ЭмуляцияНажатияКлавиши("[13]");
	ВнКомп.Пауза(500);
	Ванесса.ВыполнитьJavaScriptБраузер("document.querySelector('#fileSelectDialogOk').click()");
