...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "HTMLЭлементИмеетЗначениеПоШаблонуТогда(Парам01,Парам02)",
		"HTMLЭлементИмеетЗначениеПоШаблонуТогда",
		"Если HTML элемент ""Элемент"" имеет значение по шаблону ""ЗначениеПоля"" тогда",
		"Условие. Проверяет значение HTML элемента.",
		"Прочее.Браузер.Условия",
		"Условие");
...
&НаКлиенте
//Если HTML элемент "Элемент" имеет значение по шаблону "тест" тогда
//@HTMLЭлементИмеетЗначениеПоШаблонуТогда(Парам01,Парам02)
Процедура HTMLЭлементИмеетЗначениеПоШаблонуТогда(Элемент,НужноеЗначение) Экспорт
	
	РезультатВыполнения = Ванесса.ВыполнитьJavaScriptБраузер(Элемент).result;
	Если РезультатВыполнения.Свойство("exceptionDetails") Тогда
		ВызватьИсключение Ванесса.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найден элемент по селектору %1'"), РезультатВыполнения.exceptionDetails.exception.description);
	КонецЕсли;
	
	Значение = РезультатВыполнения.result.value;
