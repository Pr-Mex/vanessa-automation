...
Функция ЯПоказываюПодсказкуEnjoyHintУОбластиТаблицыВБраузере(Текст, ИмяТаблицы, ТабПарам = Неопределено) Экспорт
	...
	КоординатыОбласти = Ванесса.ВыполнитьJavaScriptБраузер(Скрипт).result.result.value;
	СтруктураКоординат = Ванесса.ПрочитатьОбъектJSON(КоординатыОбласти);
	Для каждого КлючИЗначение Из СтруктураКоординат Цикл
		ТабПарам.Добавить(Новый Структура("Кол1, Кол2", КлючИЗначение.Ключ, КлючИЗначение.Значение));	
	КонецЦикла; 
