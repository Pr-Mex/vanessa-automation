...
&НаКлиенте
Функция ВОткрытойФормеЯЖдуПоявленияЭлементаВТеченииСекунд(ИмяЭлемента, КоличествоСекунд = 10, ИскатьПоИмени = Ложь)
...
Функция ПолеНеСуществует(ИмяПоля,ИскатьПоИмени = Ложь) Экспорт
	Нашли = Ложь;
	МассивФорм = Ванесса.НайтиКонтейнерыТекущегоОкнаTestClient();
	Для каждого ТекущаяФорма Из МассивФорм Цикл
...
Функция ЯВывожуЗначениеПеременной(Знач ИмяПеременной) Экспорт
...
Функция ЯЗапоминаюЗначениеТаблицыКак(ИмяТаблицы,ИмяПеременной,ТабПарам,ВыделятьВсеСтроки = Истина) Экспорт
	//ТабПарам = Ванесса.ПолучитьЗначенияТаблицыGherkinИзКонтекста(ТабПарам);
	ТЧ       = Ванесса.НайтиТЧПоИмени(ИмяТаблицы);
