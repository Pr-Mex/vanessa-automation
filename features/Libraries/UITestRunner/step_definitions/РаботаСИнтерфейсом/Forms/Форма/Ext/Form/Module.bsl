		...
		Если ФормаОкна <> Неопределено Тогда
			...
			Если НРег_ИмяФормы = НРег("ОбщаяФорма.Вопрос") Тогда
				...
				Если МногострочныйТекстСообщения <> Неопределено Тогда
					...
					Если Найти(НРег(ПредставлениеДанных),"ошибка") > 0
	...
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		...
		Попытка
			...
			Если Не ПустаяСтрока(ОшибкаНетКолонок) Тогда
				...
				ТекстСообщения = СтрШаблон(ТекстСообщения, ИмяТаблицы, ОшибкаНетКолонок);
				ВызватьИсключение ТекстСообщения;
			КонецЕсли;	 			
			...
			Если ЗначениеТЧ.Количество() <> КолСтрок Тогда
				...
				ТекстСообщения = СтрШаблон(ТекстСообщения, ИмяТаблицы, КолСтрок, ЗначениеТЧ.Количество());
			...
			Для НомерСтроки = 1 По ТабПарам.Количество()-1 Цикл //в первой строке шапка
				...
				Для каждого Колонка Из СтрокаШапки Цикл
					...
					Исключение
						...
						ТекстСообщения = СтрШаблон(ТекстСообщения, ИмяТаблицы, НомерСтроки, ИмяКолонкиСтарое);					
						ВызватьИсключение ТекстСообщения;
					КонецПопытки;
					...
					Если СравнениеПоШаблону Тогда
						Если Не Ванесса.СтрокаСоответствуетШаблону(ЗначНовое, ЗначСтарое) Тогда
							...
							ТекстСообщения = СтрШаблон(ТекстСообщения, ИмяТаблицы, НомерСтроки, ИмяКолонкиСтарое, ЗначСтарое, ЗначНовое);
