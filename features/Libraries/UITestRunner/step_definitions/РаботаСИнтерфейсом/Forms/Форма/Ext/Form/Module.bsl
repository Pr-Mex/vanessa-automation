...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"УПоляЯПерехожуКДате(Парам01,Парам02)","УПоляЯПерехожуКДате","И у поля ""Имя поля"" я перехожу к дате ""08.04.1981""","Делает переход к дате в поля календаря. Если переход делается через переменную контекста, то в ней должно лежать значение типа Дата.","UI.Формы.Календарь");
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВТаблицеУПоляЯЖдуЗначенияВТечениеСекунд(Парам01,Парам02,Парам03,Парам04)","ВТаблицеУПоляЯЖдуЗначенияВТечениеСекунд","И в таблице ""ИмяТаблицы"" у поля ""Имя элемента"" я жду значения ""Значение"" в течение 20 секунд","Ожидает значения в ячейке таблицы в текущей строки в течение указанного числа секунд. Ячейка указывается по её заголовку.","UI.Таблицы.Ожидание значения ячейки таблицы");
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВТаблицеУПоляСИменемЯЖдуЗначенияВТечениеСекунд(Парам01,Парам02,Парам03,Парам04)","ВТаблицеУПоляСИменемЯЖдуЗначенияВТечениеСекунд","И в таблице ""ИмяТаблицы"" у поля с именем ""ИмяЭлемента"" я жду значения ""Значение"" в течение 20 секунд","Ожидает значения в ячейке таблицы в текущей строки в течение указанного числа секунд. Ячейка указывается по её имени.","UI.Таблицы.Ожидание значения ячейки таблицы");
	...
	Если ТаблицаTestclient.Количество() > 0 Тогда
		...
		Для Каждого ЭлементФильтра Из ФильтрПоКолонкам Цикл
			Если ОписаниеСтроки[ЭлементФильтра.Ключ] = Неопределено Тогда
				ТекстСообщения = Ванесса.ПолучитьТекстСообщенияПользователю("В таблице <%1> отсутствует колонка <%2>, указнная в параметрах.");
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ИмяТЧ); 
...
Процедура ВОткрытойФормеЯНажимаюНаКнопкуСЗаголовком(ИмяКнопки,ИскатьПоИмени = Ложь) Экспорт
	...
	Если НЕ Нашли Тогда
		Если ИскатьПоИмени Тогда
			ТекстСообщения = Ванесса.ПолучитьТекстСообщенияПользователю("Кнопка с именем <%1> не найдена. ТекущееОкно=%2");
		Иначе	
			ТекстСообщения = Ванесса.ПолучитьТекстСообщенияПользователю("Кнопка/команда с заголовком <%1> не найдена. ТекущееОкно=%2");
		КонецЕсли;	
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ИмяКнопки); 
...
Процедура ВПанелиРазделовЯВыбираю(ИмяКнопки) Экспорт
	...
	Исключение
		Ванесса.ОбработатьОшибкуПодключенияTestClient();
		ТекстСообщения = Ванесса.ПолучитьТекстСообщенияПользователю("Не подключен Testclient.");
	...
	Для каждого Элем Из ПодчиненныеОбъекты Цикл
		Если Ванесса.ЭтоПанельРазделов(Элем.ТекстЗаголовка) Тогда
			ПанельРазделов = Элем;
			Прервать;
		КонецЕсли;	 
	...
	Если ПанельРазделов = Неопределено Тогда
		ВызватьИсключение Ванесса.ПолучитьТекстСообщенияПользователю("Не нашли панель разделов.");
	КонецЕсли;	 
...
Процедура ВПанелиФункцийЯВыбираю(ИмяКнопки) Экспорт
	...
	Для каждого Элем Из ПодчиненныеОбъекты Цикл
		Если Ванесса.ЭтоМенюФункций(Элем.ТекстЗаголовка) Тогда
			МенюФункций = Элем;
			Прервать;
		КонецЕсли;	 
...
Процедура СделатьВводПоСтрокеВПолеВВода(ТЧ,ИмяТЧ,ПолеРеквизит,ИмяРеквизита,ИскатьПоИмени,ВыбираемоеЗначение,ДопПараметры)
	...
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		...
		Исключение
			...
			//	
			//	ТекстСообщения = Ванесса.ПолучитьТекстСообщенияПользователю("Предыдущая ошибка: %1. ТаймаутОжидания=%2");
...
Процедура ВТаблицеУПоляЯЖдуЗначенияВТечениеСекунд(ИмяТаблицы,ИмяПоля,Значение,КоличествоСекунд,ИскатьПоИмени = Ложь) Экспорт
	Ванесса.ЗапретитьВыполнениеШагов();
	ДатаНачалаОбработкиОжидания       = ТекущаяДата();
	ИмяПоляОбработкаОжидания          = ИмяПоля;
	ИмяТаблицыОбработкаОжидания       = ИмяТаблицы;
