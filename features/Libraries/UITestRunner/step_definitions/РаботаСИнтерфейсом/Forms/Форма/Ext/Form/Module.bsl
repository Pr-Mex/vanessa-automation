...
Процедура ВОткрытойФормеЯНажимаюНаКнопкуСЗаголовком(ИмяКнопки,ИскатьПоИмени = Ложь) Экспорт
	...
	Если НЕ Нашли Тогда
		Если ИскатьПоИмени Тогда
			ТекстСообщения = "Кнопка с именем <%1> не найдена. ТекущееОкно=%2";
		Иначе	
			ТекстСообщения = "Кнопка/команда с заголовком <%1> не найдена. ТекущееОкно=%2";
		КонецЕсли;	
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ИмяКнопки); 
		
		Если ДопПараметры.Свойство("АктивноеОкно") Тогда
			Если ДопПараметры.АктивноеОкно <> Неопределено Тогда
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%2",ДопПараметры.АктивноеОкно.Заголовок); 
			КонецЕсли;	 
		КонецЕсли;	 
...
&НаСервереБезКонтекста
Функция ПроизвестиСравнениеТаблицНаСервере(ИмяТЧ, ТабПарам, ЗначениеТЧ,МассивОшибок,ДопПараметры)
	Отказ = Ложь;
...
Процедура ТаблицаФормыСИменемСодержитСтроки(ИмяТЧ,ТабПарам,ИмяМакета = Неопределено) Экспорт
	...
	Отказ        = ПроизвестиСравнениеТаблицНаСервере(ИмяТЧ, ТабПарам, ЗначениеТЧ,МассивОшибок,ДопПараметры);
