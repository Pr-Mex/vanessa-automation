...
Функция ОШ_ЯЗаполняюРеквизитыОбъектаПоТаблице()
	...
	|	|'Имя'				|'Значение'						|
	|	|'ИмяРеквизита1'	|ИмяПеременной					|
	|	|'ИмяРеквизита2'	|ИмяПредопределенногоЗначения	|
	|	|'ИмяРеквизита3'	|'Значение реквизита'			|";
	Результат.ОписаниеШага = "Загрузка реквизитов в объект переменной. 
	|Реквизиты описываются таблицей, с обязательными колонками: Имя, Значение. 
	|В полях колонки ""Значение"" могут находятся:
	|1.	Имена переменных
	|2. Имена предопределенных значений
	|3. Значение примитивного типа (строка, число, дата)
	|4. Строка поиска значения (будет выполнен поиск как при вводе по строке и выбрано первое значение из списка).
	|Поля колонки ""Имя"" всегда строковый реквизит (имя реквизита, как оно задано в конфигурации). 
	|Допускаются поля, идентичные именам переменных.";
