...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯВыбираюФайлВК(ИмяФайла)",
		...
		|Разница с обычным шагом выбора файла — в способе: тот подставляет результат диалога программно, а этот честно набирает путь на клавиатуре в открытом диалоге. Из-за этого он и работает в браузере, где программно в диалог ОС не вмешаться.
		|Отсюда же требования: диалог уже должен быть открыт, окно клиента тестирования — активно, а в настройках базы разрешены синхронные вызовы.
		|Обратные слеши в пути **удваиваются**: `C:\\Temp\\Обработка.epf`.
		|### Пример. Открыть внешнюю обработку в веб-клиенте
		|```Gherkin
		|	И я нажимаю сочетание клавиш 'Ctrl+O'
		...
		|Из окна сообщений клиента тестирования этот текст не виден — для него есть <И я вывожу значение переменной 'ИмяПеременной'>.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯПоказываюУвеличениеФрагментаЭкрана(ТабПарам)",
		...
		|### Пример. Показать крупно итоговую сумму
		|```Gherkin
		|	И я показываю увеличение фрагмента экрана
		|		| 'left'   | '100' |
		|		| 'top'    | '100' |
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Отличие от обычного ввода текста в поле формы принципиальное: тот работает с полем через объектную модель, а этот стучит по клавиатуре в активное окно. Поэтому он годится там, где поля формы нет вовсе, — диалоги операционной системы, окно поиска конфигуратора, чужие приложения.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Заголовок можно задать маской: `*Блокнот*` найдёт окно с любым именем файла. Все переменные строковые, в том числе `ProcessId`, — это важно, потому что шаг активизации процесса ждёт именно строку.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Когда нужную область не посчитать заранее, координаты берут из поиска по картинке: <И я получаю координаты картинки ""ИмяКартинки""> кладёт их в переменные `left`, `top`, `width`, `height`, которые сюда и подставляются.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Скрыть сумму в кадре и подписать её
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Голый текст поверх экрана, без рамки и подложки — закадровый комментарий в видеоинструкции. Два параметра шага задают координаты левого верхнего угла надписи в пикселях.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Клик по вычисленным координатам
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Активизирует процесс по его PID. Если параметр ""PID"" передаётся через переменную, то есть нюанс с типом переменной — см. ниже.
