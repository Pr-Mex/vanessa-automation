...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		+ "Необходмо, чтобы в клиенте тестирования было подключено расширение VAExtension версии 1.01 и выше." + Символы.ПС 
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			+ "Необходмо, чтобы в клиенте тестирования было подключено расширение VAExtension 1.03 и выше." + Символы.ПС 
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			"Запускает мониторинг тестируемым приложением каталога с файлам, которые описывают вызов внешних событий" + Символы.ПС 
			+ "Необходмо, чтобы в клиенте тестирования было подключено расширение VAExtension 1.04 и выше.",
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			"Останавливает мониторинг тестируемым приложением каталога с файлам, которые описывают вызов внешних событий" + Символы.ПС 
			+ "Необходмо, чтобы в клиенте тестирования было подключено расширение VAExtension 1.04 и выше.",
			"Прочее.Расширение VAExtension.ВнешниеСобытия");			
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			+ "можно использовать тег ""\x_GS"" для передачи символа FNC1(29)" + Символы.ПС  
			+ " К примеру:" + Символы.ПС 
			+ "   И я эмулирую сканирование штрихкода БПО ""4670003110011"" через через файл события (Расширение)" + Символы.ПС 
			...
			+ "Использовать в тестируемом приложении VAExtension версии 1.04 и выше." + Символы.ПС 
			+ "В расширении требуется отключить безопасный режим и защиту от опасных действий." + Символы.ПС 
			+ "Используется эмуляция вызова 1С:Библиотека подключаемого оборудования",
			"Прочее.Расширение VAExtension.Оборудование"); 	
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			"Выполняет произвольный код на стороне клиента в тестируемом приложении." + Символы.ПС 
			+ "Необходмо, чтобы в клиенте тестирования было подключено расширение VAExtension 1.03 и выше." + Символы.ПС 
			+ "Код выполняется через вызов модуля приложения, таким образом он работает без смены активного окна" + Символы.ПС 
			+ "",
			"Прочее.Расширение VAExtension.Встроенный язык");
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			+ """""""",
			"Выполняет произвольный код на стороне сервера в тестируемом приложении." + Символы.ПС 
			+ "Необходмо, чтобы в клиенте тестирования было подключено расширение VAExtension 1.04 и выше." + Символы.ПС 
			+ "Код выполняется через вызов модуля приложения, таким образом он работает без смены активного окна" + Символы.ПС 
			+ "",
			"Прочее.Расширение VAExtension.Встроенный язык");
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			"Генерирует событие в файл в каталоге событий. Тестируемое приоложение эмулирует вызов внешнего события от компоненты" + Символы.ПС 
			+ "Необходмо, чтобы в клиенте тестирования было подключено расширение VAExtension 1.04 и выше." + Символы.ПС 
			+ "Код выполняется через вызов модуля приложения, таким образом он работает без смены активного окна" + Символы.ПС 
			+ "",
			"Прочее.Расширение VAExtension.ВнешниеСобытия");
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			+ "Если это было событие эмуляции внешнего события то результат будет содержать строку ""Событие было обработанно""" + Символы.ПС 
...
Функция ЯЭмулируюСканированиеШтрихкодаБПОЧерезФайлСобытияРасширение(Знач Штрихкод) Экспорт
	...
	Если НЕ Контекст.Свойство("_ЗапущенМониторингКаталогаСобытий") Тогда
		Возврат Неопределено;
	КонецЕсли;	
	
	Если Контекст._ЗапущенМониторингКаталогаСобытий Тогда
		...
		ФайлСобытия = Контекст._ПутьДоКаталогаСобытий + ПолучитьРазделительПути() + "Event_BK_"+Строка(УникальныйИдентификаторСобытия)+".json";
		...
		Контекст.Вставить("_ИмяФайлаПоследнегоСобытия", ФайлСобытия);
...
//И я запускаю мониторинг каталога ""C:\temp""для внешних событий(Расширение)
//@ЯЗапускаюМониторингКаталогаДляВнешнихСобытийРасширение(Парам)
Функция ЯЗапускаюМониторингКаталогаДляВнешнихСобытийРасширение(ПутьДоКаталога) Экспорт
	...
	Контекст.Вставить("_ПутьДоКаталогаСобытий", 			ПутьДоКаталога);	
	Контекст.Вставить("_ЗапущенМониторингКаталогаСобытий", 	Истина);
	Контекст.Вставить("_ИмяФайлаПоследнегоСобытия", 		"");
...
Функция ЯОстанавливаюМониторингКаталогаВнешнихСобытийРасширение() Экспорт
	...
	Контекст.Вставить("_ЗапущенМониторингКаталогаСобытий", Ложь);	
	Контекст.Вставить("_ПутьДоКаталогаСобытий", "");	
...
Функция ЯВыполняюКодВстроенногоЯзыкаЧерезФайлСобытияРасширениеСлужебный(Код, НаКлиенте, ПривилегированныйРежим = Ложь)
	...
	Если Контекст._ЗапущенМониторингКаталогаСобытий Тогда
		...
		УникальныйИдентификаторСобытия = Новый УникальныйИдентификатор;
		ФайлСобытия = Контекст._ПутьДоКаталогаСобытий + ПолучитьРазделительПути() + "Event_RC_"+Строка(УникальныйИдентификаторСобытия)+".json";
...
Функция ЯВызываюВнешнееСобытиеССобытиемСДаннымиЧерезФайлСобытияРасширение(Источник, Событие, Данные) Экспорт
	...
	Если Контекст._ЗапущенМониторингКаталогаСобытий Тогда
		...
		УникальныйИдентификаторСобытия = Новый УникальныйИдентификатор;
		ФайлСобытия = Контекст._ПутьДоКаталогаСобытий + ПолучитьРазделительПути() + "Event_OE_"+Строка(УникальныйИдентификаторСобытия)+".json";
...
Функция ЯОжидаюСекундРезультатОбработкиПоследнегоСобытияЧерезФайлИЗапоминаюРезультатВПеременнуюРасширение(КоличествоСекунд, ИмяПеременной) Экспорт
	...
	Если Контекст._ЗапущенМониторингКаталогаСобытий Тогда
		...
		Контекст.Вставить("_ИмяПеременнойРезультатаСобытия", ИмяПеременной);
...
Функция ЯЖдуРезультатПоследнегоСобытияЧерезФайлОбработчикОжидания()
	...
	ИмяФайлаРезультатСобытия = СтрЗаменить(Контекст._ИмяФайлаПоследнегоСобытия, "Event", "Result");
	ПроверкаФайла = Новый Файл(ИмяФайлаРезультатСобытия);
	...
	Если ПроверкаФайла.Существует() Тогда
		...
		Ванесса.СохранитьЗначениеПеременнойВКонтекст(Контекст._ИмяПеременнойРезультатаСобытия, ДанныеФайла);
