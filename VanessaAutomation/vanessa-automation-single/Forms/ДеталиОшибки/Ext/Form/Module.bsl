...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ДанныеОшибки") Тогда
		Элементы.ФормаАктуализироватьЗначение.Видимость = Истина;
...
Процедура АктуализироватьЗначение(Команда)
	...
	Если НомерСтрокиДерева > 0 Тогда
		...
		ПоискКонцаСтроки = "Технические данные:";
		...
		КонецСтроки = Найти(ТекстОшибки, ПоискКонцаСтроки);
		СколькоСимволовИскать = КонецСтроки - НачалоСтроки; 
		
		ЭталонноеЗначение = СокрЛП(Сред(ТекстОшибки, НачалоСтроки, СколькоСимволовИскать));		
		
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("НомерСтрокиДерева", НомерСтрокиДерева);
		СтруктураПараметров.Вставить("ПолученноеЗначение", ПолученноеЗначение);
