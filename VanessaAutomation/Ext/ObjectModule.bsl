...
Функция СтруктураОбщихНастроек(НастрокиВыгрузки = Ложь, НастройкиЗагрузки = Ложь) Экспорт 
	...
	Allure.Вставить("ПрикладыватьКОтчетуAllureВидеоТолькоНеудачныхТестов", Ложь);
...
Процедура ДобавитьФайлыКСценариюИлиШагуАллюр2(СтруктураВыгрузки, СписокФайлов, МассивXMLОтчетаAllure, ДопПараметрыАллюр2)
	...
	Для каждого СтруктураФайл Из СписокФайлов Цикл
		...
		// Не прикладываем видео если задана настройка на прикладывания только к провалившимся тестам.
		Если СтруктураФайл.ИмяФайла = "VideoRec.mp4" И СтруктураВыгрузки.status = "passed" Тогда
			Если ПрикладыватьКОтчетуAllureВидеоТолькоНеудачныхТестов Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
