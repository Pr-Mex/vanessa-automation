...
Процедура ДобавитьМетку2(СписокМеток, name, value, СписокПереопределяемый = Неопределено)
	...
	Поз = Найти(value, ":");
	Если Поз > 0 Тогда
		Метка.name  = НРег(Лев(value, Поз - 1));
		...
		ИначеЕсли (СтрСтроки.Тип = "Сценарий") ИЛИ (СтрСтроки.Тип = "Пример") Тогда
			...
			Иначе
				...
				Для каждого ТегДляОбработки Из МассивТеговДляОбработки Цикл
					...
					Если СтрНайти(Тег, "description:") > 0 Или СтрНайти(Тег, "описание:") > 0 Тогда
						СтруктураВыгрузки.description = СокрЛП(Сред(ТегИсходный, СтрНайти(ТегИсходный, ":") + 1));
						Продолжить;
					КонецЕсли;
					
					Если СтрНайти(Тег, "issue:") > 0 Или СтрНайти(Тег, "задача:") > 0 Тогда
						
						ОписаниеСсылки = Новый Структура;
						
						URL = СокрЛП(Сред(ТегИсходный, СтрНайти(ТегИсходный, ":") + 1));
					...
					Если СтрНайти(Тег, "link:") > 0 Или СтрНайти(Тег, "ссылка:") > 0 Тогда
						
						ОписаниеСсылки = Новый Структура;
						ОписаниеСсылки.Вставить("url", СокрЛП(Сред(ТегИсходный, СтрНайти(ТегИсходный, ":") + 1)));
					...
					ДобавитьМетку2(СписокМеток, "tag", ТегИсходный, СписокМетокПереопределяемый);
					Для каждого Строка Из ДанныеАллюрМеток Цикл 
						Если ПустаяСтрока(Строка.РегулярноеВыражение) Тогда
