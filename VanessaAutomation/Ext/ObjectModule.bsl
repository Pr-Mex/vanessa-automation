...
Функция СтруктураОбщихНастроек(НастрокиВыгрузки = Ложь, НастройкиЗагрузки = Ложь) Экспорт 
	...
	Настройки.Вставить("TakeBrowserScreenshots", Истина);
...
Процедура ДобавитьМетку2(СписокМеток, name, value, СписокПереопределяемый = Неопределено)
	...
	Если СтрНайти(value, "description:") > 0
			Или СтрНайти(value, "описание:") > 0 Тогда
		Возврат;
	КонецЕсли;
		...
		ИначеЕсли (СтрСтроки.Тип = "Сценарий") ИЛИ (СтрСтроки.Тип = "Пример") Тогда
			...
			Иначе
				...
				Для каждого Тег Из МассивТеговДляОбработки Цикл
					...
					Если СтрНайти(ДанныеТега, "description:") > 0 Тогда
						СтруктураВыгрузки.description = СокрЛП(СтрЗаменить(ДанныеТега, "description:", ""));
						Продолжить;
					КонецЕсли;
					
					Если СтрНайти(ДанныеТега, "описание:") > 0 Тогда
						СтруктураВыгрузки.description = СокрЛП(СтрЗаменить(ДанныеТега, "описание:", ""));
						Продолжить;
					КонецЕсли;
					
					Если СтрНайти(ДанныеТега, "issue:") > 0 Тогда
						
						ОписаниеСсылки = Новый Структура;
						
						URL = СокрЛП(СтрЗаменить(ДанныеТега, "issue:", ""));
						
						Если СтрНачинаетсяС(url, "(") Тогда
							КонецОписанияURL = СтрНайти(URL, ")");
							ОписаниеURL = Сред(URL, 2, КонецОписанияURL - 2);
							URL = СокрЛП(Сред(URL, КонецОписанияURL + 1));
					...
					Если СтрНайти(ДанныеТега, "link:") > 0 Тогда
						
						ОписаниеСсылки = Новый Структура;
						ОписаниеСсылки.Вставить("url", СокрЛП(СтрЗаменить(ДанныеТега, "link:", "")));
					...
					ДобавитьМетку2(СписокМеток, "tag", ДанныеТега, СписокМетокПереопределяемый);
					Для каждого Строка Из ДанныеАллюрМеток Цикл 
						Если ПустаяСтрока(Строка.РегулярноеВыражение) Тогда
						...
						Если СтрокаСоответствуетШаблону(ДанныеТега, Строка.РегулярноеВыражение) Тогда 
							ДобавитьМетку2(СписокМеток, Строка.ИмяМетки, Строка.Значение, СписокМетокПереопределяемый);
						КонецЕсли;
					КонецЦикла;
				КонецЦикла;
