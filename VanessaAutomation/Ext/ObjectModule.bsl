...
Функция СтруктураФайлаОшибкиСППР()
	...
	СтруктураОтчета.Вставить("ВерсияФормата","1.4");
	СтруктураОтчета.Вставить("УИ",Строка(Новый УникальныйИдентификатор));
	СтруктураОтчета.Вставить("ТипОшибки", "Ошибка сценария");
...
Процедура ОбойтиДеревоДляОтчетаСППР(Дерево, ДопПараметры)
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		Если      СтрСтроки.Тип = "Фича" Тогда
			...
			ОбойтиДеревоДляОтчетаСППР(СтрСтроки, ДопПараметры);
		...
		ИначеЕсли (СтрСтроки.Тип = "Сценарий") ИЛИ (СтрСтроки.Тип = "Пример") Тогда
			...
			Иначе
				...
				Если (СтрСтроки.Статус = "Failed") ИЛИ (СтрСтроки.Статус = "Pending") Тогда
					...
					СтруктураФайлаОшибкиСППР = СтруктураФайлаОшибкиСППР();
					...
					СтруктураФайлаОшибкиСППР.Вставить("ИсходнаяИнформация",ОписаниеОшибки);
					СтруктураФайлаОшибкиСППР.Вставить("ПорядокВоспроизведения",ПодробноеОписаниеОшибки);
					
					СистемнаяИнформация = Новый СистемнаяИнформация;
					
					СтруктураФайлаОшибкиСППР.Вставить("ВерсияПлатформы", СистемнаяИнформация.ВерсияПриложения);
					СтруктураФайлаОшибкиСППР.Вставить("КлиентОперативнаяПамять", СистемнаяИнформация.ОперативнаяПамять);
					СтруктураФайлаОшибкиСППР.Вставить("КлиентВерсияОС", СистемнаяИнформация.ВерсияОС);
					СтруктураФайлаОшибкиСППР.Вставить("КлиентПроцессор", СистемнаяИнформация.Процессор);
					СтруктураФайлаОшибкиСППР.Вставить("КлиентТипПлатформы", Строка(СистемнаяИнформация.ТипПлатформы));
					СтруктураФайлаОшибкиСППР.Вставить("КлиентИнформацияПрограммыПросмотра", СистемнаяИнформация.ИнформацияПрограммыПросмотра);
					СтруктураФайлаОшибкиСППР.Вставить("КлиентТекущаяДата", ТекущаяДата());
					
					СтруктураФайлаОшибкиСППР.Вставить("ВерсияПлатформы",ПодробноеОписаниеОшибки);
					...
					Если РезультатПрохожденияТестовСценария.Свойство("МассивФайлов") Тогда
						...
						КонецЦикла;	 
					КонецЕсли;	 
					СтруктураФайлаОшибкиСППР.Вставить("ИменаФайлов",ИменаФайлов);
					
					
					
					
					СтруктураФайлаОшибкиСППР.Вставить("ВерсияПлатформы", СистемнаяИнформация.ВерсияПриложения);
					СтруктураФайлаОшибкиСППР.Вставить("СерверОперативнаяПамять", СистемнаяИнформация.ОперативнаяПамять);
					СтруктураФайлаОшибкиСППР.Вставить("СерверВерсияОС", СистемнаяИнформация.ВерсияОС);
					СтруктураФайлаОшибкиСППР.Вставить("СерверПроцессор", СистемнаяИнформация.Процессор);
					СтруктураФайлаОшибкиСППР.Вставить("СерверТипПлатформы", Строка(СистемнаяИнформация.ТипПлатформы));
					СтруктураФайлаОшибкиСППР.Вставить("СерверИнформацияПрограммыПросмотра", СистемнаяИнформация.ИнформацияПрограммыПросмотра);
					
					СтруктураФайлаОшибкиСППР.Вставить("ВерсияКонфигурации", Метаданные.Версия);
					СтруктураФайлаОшибкиСППР.Вставить("ИмяКонфигурации", Метаданные.Имя);
					СтруктураФайлаОшибкиСППР.Вставить("ДатаОбнаружения", ТекущаяДатаСеанса());
					СтруктураФайлаОшибкиСППР.Вставить("ДостоверностьОбнаружения", "Высокая");
					
					ЗначениеТегаКодСценария = ЗначениеИзТегаСценария(СтрСтроки,"КодСценария=");
					Если ЗначениеТегаКодСценария <> Неопределено Тогда
						СтруктураФайлаОшибкиСППР.Вставить("КодСценария", ЗначениеТегаКодСценария);
					КонецЕсли;	 
					СтруктураФайлаОшибкиСППР.Вставить("ИмяСценария", СтрСтроки.Имя);
					...
					СериализаторXDTO.ЗаписатьXML(ЗаписьXML, СтруктураФайлаОшибкиСППР);
