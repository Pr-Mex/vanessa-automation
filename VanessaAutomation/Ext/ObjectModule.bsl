...
Функция СоздатьДанныеПоMакетам(ТестОбъект, ИменаMакетов) Экспорт
	Сериализатор = Плагин("СериализаторMXL");
	Возврат Сериализатор.СоздатьДанныеПоMакетам(ТестОбъект, ИменаMакетов);
КонецФункции
...
Функция ПолучитьСтруктуруКаталоговAllure(ПолныйПуть, Знач ОтносительныйПуть) 
	...
	Возврат СобранныеДанные;
КонецФункции

Процедура ДанныеПоИерархииОтчетаИзОтносительногоПути(СтрСтроки, Знач ОтносительныйПуть, epic, feature, story, suite = 0, ДляAllure2 = Ложь)
	
	ПолныйПуть = СтрСтроки.ПолныйПуть;
	Если ДляAllure2 И СтрСтроки.Тип = "Примеры" Тогда
		ПолныйПуть = СтрСтроки.Родитель.ПолныйПуть;
	КонецЕсли;	 
	СобранныеДанные = ПолучитьСтруктуруКаталоговAllure(ПолныйПуть, ОтносительныйПуть);
	Если СобранныеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;	
	
	Если ЗначениеЗаполнено(УровеньОтчета1) Тогда
		epic = УровеньОтчетаПоСобраннымДанным(УровеньОтчета1, СтрСтроки, СобранныеДанные, 1, ДляAllure2);
	КонецЕсли;	 
	Если ЗначениеЗаполнено(УровеньОтчета2) Тогда
		feature = УровеньОтчетаПоСобраннымДанным(УровеньОтчета2, СтрСтроки, СобранныеДанные, 2, ДляAllure2);
	КонецЕсли;	 
	Если ЗначениеЗаполнено(УровеньОтчета3) Тогда
		story = УровеньОтчетаПоСобраннымДанным(УровеньОтчета3, СтрСтроки, СобранныеДанные, 3, ДляAllure2);
	КонецЕсли;	 
	Если ДляAllure2 И ЗначениеЗаполнено(ГруппировкаTestSuites) Тогда
		suite = УровеньОтчетаПоСобраннымДанным(ГруппировкаTestSuites, СтрСтроки, СобранныеДанные, 1, ДляAllure2);
	КонецЕсли;	 
КонецПроцедуры 
...
Функция УровеньОтчетаПоСобраннымДанным(СтрокаКакОпределятьУровень, Знач СтрСтроки, СобранныеДанные, Уровень, ИскатьФичу = Ложь)
	...
	Для Каждого ТекЗначение Из МассивЗначений Цикл
		...
		ИначеЕсли ТекЗначение = "имяфичи" Или ТекЗначение = "featurename" Тогда
			Если ИскатьФичу Тогда
				СтрокаФичи = ВеткаФичи(СтрСтроки);
			Иначе
				СтрокаФичи = СтрСтроки;
			КонецЕсли;
		...
		ИначеЕсли Лев(ТекЗначение, 1) = "#" Тогда
			...
			Результат = "";
			Если НЕ (ТипЗнч(СтрокаФичи.ПроизвольныеЗначения) = Тип("Структура") 
					И СтрокаФичи.ПроизвольныеЗначения.Свойство("МассивКомментариев")) Тогда
				Продолжить;
			КонецЕсли;
	...
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		Если СтрСтроки.Тип = "Фича" Тогда
			...
			Если ЗначениеЗаполнено(УровеньОтчета1) ИЛИ ЗначениеЗаполнено(УровеньОтчета2) ИЛИ ЗначениеЗаполнено(УровеньОтчета3) Тогда
				...
				feature = "";
				story   = "";
				ДанныеПоИерархииОтчетаИзОтносительногоПути(СтрСтроки, КаталогОтносительноКоторогоНадоСтроитьИерархию, epic, feature, story);
			...
			Иначе	
					...
					Для Каждого КомментарийСтруктура Из СтрСтроки.ПроизвольныеЗначения.МассивКомментариев Цикл
						...
						Если Лев(НрегКомментарий, 12) = "report.epic=" Тогда
							...
							Если ЗначениеЗаполнено(ПромСтр) Тогда
								epic = ПромСтр;
							КонецЕсли;	 
						ИначеЕсли Лев(НрегКомментарий, 15) = "report.feature=" Тогда
							ПромСтр = Сред(Комментарий, 16);
							Если ЗначениеЗаполнено(ПромСтр) Тогда
								feature = ПромСтр;
							КонецЕсли;	 
						КонецЕсли;	 
						Если ЗначениеЗаполнено(epic) И ЗначениеЗаполнено(feature) Тогда
							Прервать;
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(epic) Тогда
					ДобавитьМетку(Фабрика, СписокМеток, "epic", epic);
				КонецЕсли;	 
				Если НЕ ЗначениеЗаполнено(feature) Тогда
...
Процедура ОбойтиДеревоДляОтчетаАллюр2(Дерево, ДеревоТестов, ДанныеАллюрМеток)
	...
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		Если (СтрСтроки.Тип = "Сценарий") ИЛИ (СтрСтроки.Тип = "Пример") Тогда
			Если (СтрСтроки.ДопТип = "Контекст") Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(УровеньОтчета1) ИЛИ ЗначениеЗаполнено(УровеньОтчета2) 
						ИЛИ ЗначениеЗаполнено(УровеньОтчета3) ИЛИ ЗначениеЗаполнено(ГруппировкаTestSuites) Тогда
					epic    = "";
					feature = "";
					story   = "";
					suite   = "";
					ДанныеПоИерархииОтчетаИзОтносительногоПути(СтрСтроки.Родитель, КаталогОтносительноКоторогоНадоСтроитьИерархию, epic, feature, story, suite, Истина);
...
Процедура ОбойтиДеревоДляОтчетаjUnit(Дерево, ЗаписьXML, ДопПараметры, ДанныеФичи = Неопределено)
	...
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		Если      СтрСтроки.Тип = "Фича" Тогда
			...
			ЗаписьXML.ЗаписатьАтрибут("time", XMLСтрока(ДанныеФичи.time / Миллисекунды));
			...
			Для Каждого Сценарий Из МассивСценариев Цикл
				...
				ЗаписьXML.ЗаписатьАтрибут("time", XMLСтрока(Сценарий.time / Миллисекунды));
		...
		ИначеЕсли (СтрСтроки.Тип = "Сценарий") ИЛИ (СтрСтроки.Тип = "Пример") Тогда
			...
			Иначе
				...
				Если СтрСтроки.Тип = "Пример" Тогда
					// для структуры сценария надо вести нумерацию
					Сценарий.Вставить("name", СтрСтроки.Родитель.Родитель.Имя + " №" + (СтрСтроки.Родитель.Строки.Индекс(СтрСтроки)));
					Если ТипЗнч(СтрСтроки.Родитель.ПроизвольныеЗначения) = Тип("Структура") 
							И СтрСтроки.Родитель.ПроизвольныеЗначения.Свойство("МассивТегов") Тогда
						МассивТегов = СтрСтроки.Родитель.ПроизвольныеЗначения.МассивТегов;
					КонецЕсли;	 
				Иначе	
					Сценарий.Вставить("name", СтрСтроки.Имя);
					Если ТипЗнч(СтрСтроки.ПроизвольныеЗначения) = Тип("Структура") 
							И СтрСтроки.ПроизвольныеЗначения.Свойство("МассивТегов") Тогда
...
Процедура AssertEqualValueTable(ExpectedValueTable, ActualValueTable, Message = "", Params = Неопределено) Экспорт
...
Процедура Проверить(ПроверяемоеЗначение, ДопСообщениеОшибки = "") Экспорт
	Если Не ПроверяемоеЗначение Тогда
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт1(
			"Переданный параметр (%1) не является Истиной, а хотели, чтобы являлся. %2", 
			ПроверяемоеЗначение, ДопСообщениеОшибки);
...
Процедура ПроверитьДату(ПроверямыйПериод, ПроверяемаяДата, ДопСообщениеОшибки = "") Экспорт
	Если ПроверяемаяДата < ПроверямыйПериод.ДатаНачала ИЛИ ПроверяемаяДата > ПроверямыйПериод.ДатаОкончания Тогда
		...
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт2(
			"Переданный параметр (%1) не входит в период (%2), а хотели, чтобы являлся. %3", 
			ПроверяемаяДата, представление, ДопСообщениеОшибки);
...
Процедура ПроверитьРавенствоДатСТочностью2Секунды(ПерваяДата, ВтораяДата, ДопСообщениеОшибки = "") Экспорт
	Если ВтораяДата < ПерваяДата - 2 ИЛИ ВтораяДата > ПерваяДата + 2 Тогда
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт2(
			...
			ПерваяДата, ВтораяДата, ДопСообщениеОшибки);
...
Процедура ПроверитьРавенство(ПервоеЗначение, ВтороеЗначение, ДопСообщениеОшибки = "") Экспорт
	Если ПервоеЗначение <> ВтороеЗначение Тогда
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт2(
			"Сравниваемые значения (%1; %2) не равны, а хотели, чтобы были равны. %3", 
			ПервоеЗначение, ВтороеЗначение, ДопСообщениеОшибки);
...
Процедура ПроверитьБольше(ЭталонноеЧисло, ПроверяемоеЧисло, ДопСообщениеОшибки = "") Экспорт
	Если ПроверяемоеЧисло <= ЭталонноеЧисло Тогда
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт2(
			...
			ПроверяемоеЧисло, ЭталонноеЧисло, ДопСообщениеОшибки);
...
//   ДопСообщениеОшибки - Строка - Описание ошибки, которое будет использовано в тексте исключения ассерта.
//
Процедура ПроверитьМеньше(ЭталонноеЧисло, ПроверяемоеЧисло, ДопСообщениеОшибки = "") Экспорт
	Если ПроверяемоеЧисло >= ЭталонноеЧисло Тогда
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт2(
			"Значение (%1) больше или равно, чем (%2), а ожидалось меньше. %3", 
			ПроверяемоеЧисло, ЭталонноеЧисло, ДопСообщениеОшибки);
		ОбработатьAssertОжидаемоеРеальноеЗначение(ЭталонноеЧисло, ПроверяемоеЧисло);
		ВызватьОшибкуПроверки(ТекстСообщения);
	КонецЕсли;
...
//   ДопСообщениеОшибки - Строка - Описание ошибки, которое будет использовано в тексте исключения ассерта.
//
Процедура ПроверитьМеньшеИлиРавно(ЭталонноеЧисло, ПроверяемоеЧисло, ДопСообщениеОшибки = "") Экспорт
	Если ПроверяемоеЧисло > ЭталонноеЧисло Тогда
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт2(
			"Значение (%1) больше, чем (%2), а ожидалось меньше или равно. %3", 
			ПроверяемоеЧисло, ЭталонноеЧисло, ДопСообщениеОшибки);
...
Процедура ПроверитьВыполнилось(Знач СтрокаАлгоритм, Знач ДопСообщениеОшибки = "") Экспорт
	...
	Исключение
		...
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт2(
			"Хотели, чтобы алгоритм (%1) выполнился, а он упал с ошибкой (%2). %3", 
			СтрокаАлгоритм, ПолученноеОписаниеОшибки, ДопСообщениеОшибки);
...
Процедура ПроверитьМетодВыполнился(Объект, Знач ИмяМетода, ПараметрыИлиДопСообщениеОшибки = Неопределено, Знач ДопСообщениеОшибки = "") Экспорт
	...
	Иначе
		...
		Если ПараметрыМетода <> Неопределено Тогда
			...
			Если ТипПараметра <> Тип("Массив") Тогда
				ТекстСообщения = ПолучитьФорматированныйТекстАссерт2(
					"Ожидали, что вторым параметром будет передан массив параметров для метода (%1), а получили другой объект с типом (%2). <%3>", 
					ИмяМетода, ТипПараметра, ДопСообщениеОшибки);
	...
	Исключение
		ПолученноеОписаниеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт2(
			"Хотели, чтобы код (%1) выполнился, а он упал с ошибкой (%2). %3", 
			СтрокаВыполнения, ПолученноеОписаниеОшибки, ДопСообщениеОшибки);
		ВызватьОшибкуПроверки(ТекстСообщения);
	КонецПопытки;
...
Процедура ПроверитьНеВыполнилось(Знач СтрокаАлгоритм, Знач ОжидаемоеОписаниеОшибки = "", Знач ДопСообщениеОшибки = "") Экспорт
	...
	Если НужноВызватьОшибку Тогда
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт1(
			"Хотели, чтобы алгоритм (%1) упал, а он выполнился. %2", 
			СтрокаАлгоритм, ДопСообщениеОшибки);
		ОбработатьAssertОжидаемоеРеальноеЗначение(Истина, Ложь);
...
Процедура ПроверитьМетодНеВыполнился(Объект, ИмяМетода, ПараметрыИлиОжидаемоеОписаниеОшибки, Знач ОжидаемоеОписаниеОшибкиИлиДопСообщениеОшибки = "", Знач ДопСообщениеОшибки = "") Экспорт
	...
	Иначе
		...
		Если ПараметрыМетода <> Неопределено Тогда
			...
			Если ТипПараметра <> Тип("Массив") Тогда
				ТекстСообщения = ПолучитьФорматированныйТекстАссерт2(
					"Ожидали, что третьим параметром будет передан массив параметров для метода (%1), а получили другой объект с типом (%2). %3", 
					ИмяМетода, ТипПараметра, ДопСообщениеОшибки);
				ВызватьИсключение ТекстСообщения;
			КонецЕсли;
		КонецЕсли;
	...
	ТекстСообщения = ПолучитьФорматированныйТекстАссерт1(
		"Хотели, чтобы код (%1) упал, а он выполнился. %2", 
		СтрокаВыполнения, ДопСообщениеОшибки);
	ВызватьОшибкуПроверки(ТекстСообщения);
...
Процедура ПроверитьНеЗаполненность(ПроверяемоеЗначение, ДопСообщениеОшибки = "") Экспорт
	ТекстСообщения = ПолучитьФорматированныйТекстАссерт1(
						"Значение (%1) заполнено, а ожидалась незаполненность. %2", 
						ПроверяемоеЗначение, ДопСообщениеОшибки);
	Попытка
		фЗаполнено = ЗначениеЗаполнено(ПроверяемоеЗначение);
	Исключение
...
Процедура ПроверитьТип(значение, ТипИлиИмяТипа, ДопСообщениеОшибки = "") Экспорт
	...
	Иначе
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт1(
			...
			ТипЗнч(ТипИлиИмяТипа), ДопСообщениеОшибки);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ПроверитьВхождение(строка, подстрокаПоиска, ДопСообщениеОшибки = "") Экспорт
	Если Найти(строка, подстрокаПоиска) = 0 Тогда
		...
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт2(
			...
			ПредставлениеСтроки, ПредставлениеПодСтроки, ДопСообщениеОшибки);
...
Процедура ПроверитьВхождениеВКоллекцию(Элемент, Коллекция, ДопСообщениеОшибки = "") Экспорт
	...
	Если Нашли = Неопределено Тогда
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт1(
			"Утверждение <ПроверитьВхождениеВКоллекцию> не умеет работать с типом коллекции (%1). %2", 
			ТипКоллекции, ДопСообщениеОшибки);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	Если Не Нашли Тогда
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт1(
			"Не нашли Элемент (%1) в коллекции, а хотели, чтобы он был в коллекции. %2", 
			Элемент, ДопСообщениеОшибки);
...
Процедура ПроверитьВалидностьЗапросаСКД(ТекстЗапроса, ДопСообщениеОшибки = "") Экспорт
	...
	Исключение
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт1(
			"%1. %2", 
			ОписаниеОшибки(), ДопСообщениеОшибки);
		ВызватьОшибкуПроверки(ТекстСообщения);
	КонецПопытки;
...
Процедура ПроверитьНаличиеОбъектаИБ(Ссылка, ДопСообщениеОшибки = "") Экспорт
	...
	Если лОбъект = Неопределено Тогда
		ТекстСообщения = ПолучитьФорматированныйТекстАссерт1(
			"Ожидали существование объекта в ИБ, а получили объект не найден (%1). %2", 
			Ссылка, ДопСообщениеОшибки);
		ВызватьОшибкуПроверки(ТекстСообщения);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьРавенствоФайлов(ПутьФайла1, ПутьФайла2, ДопСообщениеОшибки = "", 
									ПарамСравнениеФайлов = Неопределено, 
									ПоказатьРазличия = Ложь) Экспорт
...
Процедура ПроверитьРавенствоТабличныхДокументовТолькоПоЗначениям(ТабДок1, ТабДок2, 
	...
	Если ТипЗнч(ТабДок1) <> Тип("ТабличныйДокумент") Тогда
		ВызватьИсключение ПолучитьТекстСообщенияПользователю(
							"ПроверитьРавенствоТабличныхДокументовТолькоПоЗначениям: Первый параметр-таблица не является табличным документом");
	КонецЕсли;
	Если ТипЗнч(ТабДок2) <> Тип("ТабличныйДокумент") Тогда
		ВызватьИсключение ПолучитьТекстСообщенияПользователю(
							"ПроверитьРавенствоТабличныхДокументовТолькоПоЗначениям: Второй параметр-таблица не является табличным документом");
	КонецЕсли;
...
Функция ФорматЗначенияДляТекстаАссерта(Знач Значение)
	Возврат Формат(Значение, "ДФ='dd.MM.yyyy HH:mm:ss'; БЛ=ложь; БИ=истина;");
КонецФункции

Функция ПолучитьФорматированныйТекстАссерт1(Знач ТекстСообщения, Знач ПроверяемоеЗначение, Знач ДопСообщениеОшибки)
	ТекстСообщения = СтрЗаменить(ПолучитьТекстСообщенияПользователю(ТекстСообщения), "%1", ФорматЗначенияДляТекстаАссерта(ПроверяемоеЗначение));
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%2", ФорматДСО(ДопСообщениеОшибки));
	Возврат ТекстСообщения;
КонецФункции

Функция ПолучитьФорматированныйТекстАссерт2(Знач ТекстСообщения, Знач ПервоеЗначение, Знач ВтороеЗначение, Знач ДопСообщениеОшибки)
	ТекстСообщения = СтрЗаменить(ПолучитьТекстСообщенияПользователю(ТекстСообщения), "%1", ФорматЗначенияДляТекстаАссерта(ПервоеЗначение));
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%2", ФорматЗначенияДляТекстаАссерта(ВтороеЗначение));
...
Функция СравнитьЗначенияТаблиц(ТаблицаОжиданий, ТаблицаРезультатов, Различия, ДопПараметры)
	...
	Для Индекс = 0 По ГраницаСтрок Цикл
		...
		Для Каждого Колонка Из Колонки Цикл
			...
			Если НЕ СравнениеПоШаблону 
					И (ОжидаемоеЗначение = ЗначениеРезультата
						Или (Не ЗначениеЗаполнено(ОжидаемоеЗначение) И Не ЗначениеЗаполнено(ЗначениеРезультата))) // Пустые значения разных типов 1С-м не считаются равными :()
					ИЛИ СтрокаСоответствуетШаблону(ЗначениеРезультата, ОжидаемоеЗначение) Тогда 
				Продолжить;
			КонецЕсли;
...
Функция ПолучитьТаблицуЗначенийИзТабличногоДокумента(ТабличныйДокумент, УчитыватьТолькоВидимыеКолонки = Ложь, УчитыватьТолькоВидимыеСтроки = Ложь)
	...
	Если ТипТабличногоДокумента <> Тип("ТабличныйДокумент") И ТипТабличногоДокумента <> Тип("ПолеТабличногоДокумента") Тогда
		ВызватьИсключение ПолучитьТекстСообщенияПользователю(
							"ПолучитьТаблицуЗначенийИзТабличногоДокумента: Требуется тип ТабличныйДокумент или ПолеТабличногоДокумента");
	КонецЕсли;
	...
	Для НомерКолонки = 1 По НомерПоследнейКолонки Цикл
		...
		Если УчитыватьКолонку Тогда
			...
			ШиринаКолонки = Макс(ОбластьКолонки.ШиринаКолонки, 1);
...
Процедура ОбойтиДеревоДляВнутреннегоФормата(Дерево, ТабличныйДокумент, Mакет, Интервал, ПараметрыОтчетаУФ) 
	
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		Секция = Неопределено;
		Сценарий = Новый Структура("имя, name, start, stop, message, status");
		Если СтрСтроки.Тип = "Шаг" Тогда 
			Секция = Mакет.ПолучитьОбласть("Шаг");
		Иначе 
			Секция = Mакет.ПолучитьОбласть("Заголовок");
		КонецЕсли; 
		Если СтрСтроки.Тип = "Пример" Тогда
		...
		Если СтрСтроки.Статус = "Failed" И СтрСтроки.Тип <> "Сценарий" Тогда
			Секция = Mакет.ПолучитьОбласть("Ошибка");
			Секция.Параметры.Заполнить(Сценарий);
			ТабличныйДокумент.Вывести(Секция);		
		КонецЕсли; 
		...
		Если СтрСтроки.Строки.Количество() > 0 Тогда 
			...
			ОбойтиДеревоДляВнутреннегоФормата(СтрСтроки, ТабличныйДокумент, Mакет, Интервал + "   ", ПараметрыОтчетаУФ);
...
Функция ВывестиОтчетТестированияВоВнутреннемФормате(Дерево, ПараметрыОтчетаУФ) Экспорт
	...
	Mакет = ПолучитьМакет("ОтчетТестирования");
	
	Секция = Mакет.ПолучитьОбласть("Заголовок");
	
	Интервал = "";
	ОбойтиДеревоДляВнутреннегоФормата(Дерево, ТабДок, Mакет, Интервал + "   ", ПараметрыОтчетаУФ);
...
Процедура ДобавитьШагВМассивТестов(МассивТестов, Снипет, ИмяПроцедуры, 
								ПредставлениеТеста = Неопределено, ОписаниеШага = Неопределено, 
								ТипШагаДляОписания = Неопределено, ТипШагаВДереве = Неопределено) Экспорт
