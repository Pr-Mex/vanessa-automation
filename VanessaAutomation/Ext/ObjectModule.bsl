...
Процедура ЗаполнитьПараметрыДляAllureTestOps(СтруктураВыгрузки, СтрокаСценария, ДопПараметрыАллюр2)
	...
	// и также в зависимости от типа сценария формируем fullName. Для структуры сценария отсекаем номер примера в конце
	Если НРег(СтрокаСценария.Тип) = "пример" Тогда
	...
	// также формируем для name и historyId строки с параметрами, но в разном виде
	...
	Если СтруктураВыгрузки.Свойство("parameters") И СтруктураВыгрузки.parameters.Количество() > 0 Тогда
		...
		Для Каждого Значение Из МассивЗначенийПараметров Цикл			
			СтрокаПараметровДляИмени = СтрокаПараметровДляИмени + " | " + Строка(Значение);
			СтрокаПараметровДляИстории = СтрокаПараметровДляИстории + "_" + Строка(Значение);
		КонецЦикла;
...
Функция РазделитьИмяСценарияСтруктуры(ИмяСценария)
	...
	Результат = Новый Структура;
	Результат.Вставить("ИмяСценарияБезНомера");
	Результат.Вставить("НомерПримера");
	
	ЧастиИмени = РазложитьСтрокуВМассивПодстрок(ИмяСценария, "№", Ложь);
	ДлинаИмениСценария = СтрДлина(ИмяСценария);
	ДлинаПоследнейЧастиСНомером = СтрДлина(ЧастиИмени[ЧастиИмени.Количество() - 1]) + 1;
	
	Результат.ИмяСценарияБезНомера = СокрП(Лев(ИмяСценария, ДлинаИмениСценария - ДлинаПоследнейЧастиСНомером));
	Результат.НомерПримера = Прав(ИмяСценария,ДлинаПоследнейЧастиСНомером);
