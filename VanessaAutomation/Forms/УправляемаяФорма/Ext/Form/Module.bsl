...
&НаКлиенте
Функция ВидимостьЭлементаСУчетомРодителя(ТекущаяФормаЭлемента,ЭлементФормы) Экспорт
...
Процедура ЗагрузитьГлобальныеПеременные();
	...
	Если Не ЗначениеЗаполнено(Объект.КаталогПроекта) Тогда
		Объект.КаталогПроекта = ТекущийКаталогОбработки();
	КонецЕсли;
	...
	Для каждого СтрокаПользователя Из МассивНастроек Цикл
		Если ВРег(СтрокаПользователя["user"]) = ВРег(ТекЮзер) Тогда 
			...
			Для каждого ПеременнаяПользователя Из СтрокаНастроек Цикл
				//ОбъектКонтекстСохраняемый.Вставить(ПеременнаяПользователя.Ключ, ПеременнаяПользователя.Значение);
	...
	Если Не НастройкиЮзераНайдены Тогда
		Сообщить(СтрШаблон(НСтр("ru = 'Ошибка. Не найдены настройки пользователя %1 в файле %2'"), ТекЮзер, ПутьКФайлу));
		Возврат;
	КонецЕсли;
...
&НаСервере
Функция ТекущийКаталогОбработки()
	
	// Предполагаем, что каталог проекта равен  что-то вроде E:\repository\ripreal\mdm_main\tools\vanessa-behavior
	// и на нужно подняться на два уровня выше. Результат должен быть E:\repository\ripreal\mdm_main
	...
	КаталогПроекта = Лев(КаталогТулзов, СтрНайти(КаталогТулзов, "\", НаправлениеПоиска.СКонца) - 1);
...
Процедура ВывестиЗагруженныеНастройкиВЖР(знач НастройкиПользователя)
	...
	СтрокаНастроек = "";
	СтекСообщений = Новый Массив;
	
	Для каждого СтрокаФайла Из НастройкиПользователя Цикл
		СтекСообщений.Добавить(СтрокаФайла.Ключ + " " + СтрокаФайла.Значение)
	КонецЦикла;
	
	ЗаписьЖурналаРегистрации("VanessaBehavior.ЗагрузкаГлобальныхПеременных", 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru = 'Загружены глобальные переменные'") + Символы.ПС + СтрСоединить(СтекСообщений, Символы.ПС)
	);
