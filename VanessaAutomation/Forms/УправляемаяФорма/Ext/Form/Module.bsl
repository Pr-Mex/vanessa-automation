...
Процедура ИсследовательФормы(Команда)
	...
	ОткрытьФорму("ВнешняяОбработка.VanessaAutomation.Форма.ИсследовательФормы", ПараметрыФормы,ЭтаФорма);
КонецПроцедуры
...
Процедура ДобавитьИзвестныйШаг(Команда)
	...
	ОткрытьФорму("ВнешняяОбработка.VanessaAutomation.Форма.ВыборИзвестногоШага", ПараметрыФормы,ЭтаФорма);
КонецПроцедуры
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	СтрЗапуска = СокрЛП(ПараметрЗапуска);
	//СтрЗапуска = "GetStepsCache;VBParams=C:\Temp\111\111.json";
	Если СтрЗапуска = "" Тогда
		Возврат;
	КонецЕсли;	
...
Процедура ПодключитьБиблиотекуРаботыСВидео()
	...
	Если Объект.ВерсияПоставки = "single" Тогда
		ПараметрыФормы = Новый Структура;
		МодульРаботыСВидео = ПолучитьФорму("ВнешняяОбработка.VanessaAutomation.Форма.Video_РаботаСВидео", ПараметрыФормы);
	Иначе	
...
Процедура ОткрытьРедактированиеТаблицыGherkin()
	...
	ФормаДобавитьШаг = ПолучитьФорму("ВнешняяОбработка.VanessaAutomation.Форма.РедактированиеТаблицы", ПараметрыФормы);
...
Функция ПолучитьФайлыПеревода()
	Если Объект.ВерсияПоставки = "single" Тогда
		ПараметрыФормы = Новый Структура;
		ФормаОбработки = ПолучитьФорму("ВнешняяОбработка.VanessaAutomation.Форма.Локализация_Steps", ПараметрыФормы);
	Иначе	
...
Процедура ПоказатьТекстОшибки(ТекстОшибки,ПроизвольныеЗначения)
	...
	ОткрытьФорму("ВнешняяОбработка.VanessaAutomation.Форма.ДеталиОшибки", ПараметрыФормы,ЭтаФорма);
КонецПроцедуры 
...
Функция ТекстСообщенийПользователю()
	Если Объект.ВерсияПоставки = "single" Тогда
		ПараметрыФормы = Новый Структура;
		ФормаОбработки = ПолучитьФорму("ВнешняяОбработка.VanessaAutomation.Форма.Локализация_Messages", ПараметрыФормы);
