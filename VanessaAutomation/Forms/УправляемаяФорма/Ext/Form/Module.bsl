...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокКомандТекстовогоРедактораФичиПоУмолчанияю();
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		Если ЗнДелатьОтчетВФорматеjUnit = Истина Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputjUnit") Тогда
				...
				СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Использование устаревшего параметра ""КаталогOutputjUnit"". Новый параметр - ""КаталогВыгрузкиjUnit"""));					
			Иначе 
			// Конец Удалить	
...
&НаСервере
Процедура СписокКомандТекстовогоРедактораФичиПоУмолчанияю()
	
	СтрокаКомандыNotepadPP = """C:\Program Files (x86)\Notepad++\notepad++.exe"" %Feature% -n%LineNumber%";
	СтрокаКомандыVSCode = """C:\Program Files (x86)\Microsoft VS Code\code.exe"" -g %Feature%:%LineNumber%";
	
	Элементы.КомандаОткрытияФичиВТекстовомРедакторе.СписокВыбора.Добавить(СтрокаКомандыNotepadPP, "Notepad++");
	Элементы.КомандаОткрытияФичиВТекстовомРедакторе.СписокВыбора.Добавить(СтрокаКомандыVSCode, "VSCode");
