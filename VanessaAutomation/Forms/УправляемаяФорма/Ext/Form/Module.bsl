...
&НаКлиенте
Перем ИмяУправляющиегоФайлаSikuliXСевер;
...
//возврашет значение переменной ИмяУправляющиегоФайлаSikuliXСевер
&НаКлиенте
Функция ИмяУправляющиегоФайлаSikuliXСевер() Экспорт
	Возврат ИмяУправляющиегоФайлаSikuliXСевер; 
КонецФункции	 
...
Процедура НайтиФайлыКомандаСистемы(Путь, СписокКаталогов, СписокФайлов, ИскатьВПодкаталогах, ИскомоеИмяФайла = Неопределено) Экспорт
	Если НЕ ЕстьПоддержкаАсинхронныхВызовов Тогда
		...
		КонецЦикла;	
	ИначеЕсли ЕстьПоддержкаАсинхронныхВызовов И ВозможнаОптимизацияРаботыСФайлами Тогда
		НайтиФайлыСервер(Путь, СписокКаталогов, СписокФайлов, ИскатьВПодкаталогах, ИскомоеИмяФайла);
	Иначе
		//получение каталогов
		ИмяФайлаЛога = ПолучитьИмяВременногоФайла("txt");
...
&НаСервереБезКонтекста
Функция ПодключитьВнешнююОбработкуСервер(АдресХранилища,ИспользуетсяЗащитаОтОпасныхДействий,ДополнительныеПараметры = Неопределено)
	ИмяОбработки = Неопределено;
...
Функция ПодключитьКонтекстОбработкиКлиент(ОбъектСодержащийШаги, ВерсияФайла = Неопределено,ДопПараметры = Неопределено)
	...
	Если Лев(НРег(ОбъектСодержащийШаги),34) = "внешняяобработка.vanessaautomation" Тогда
...
// Файл - Тип: Файл
&НаКлиенте
Процедура ПодключитьСнипетыИзОбработкиКлиент(Файл, ВерсияФайла, БылиОшибки,СтруктураПараметров)
	...
	Если ИД = Неопределено Тогда
		БылиОшибки = Ложь;
		Если НЕ ПодключитьКонтекстОбработкиКлиент(Файл.ПолноеИмя, ВерсияФайла) Тогда
			Возврат;
		КонецЕсли;	 
	...
	Если ФормаОбработки = Неопределено Тогда
		ПодключитьКонтекстОбработкиКлиент(Файл.ПолноеИмя);
		ФормаОбработки = ТаблицаКонтекстовОбработок[ИД].Обработка;
	КонецЕсли;	 
...
Процедура ПолучитьУжеСуществующиеСнипетыИзОбработок(Знач КаталогФич,
	...
	Для Каждого ОбъектФайл Из МассивФайлов Цикл
		...
		ДопПараметры = Новый Структура;
		ПодключитьСнипетыИзОбработкиКлиент(Файл, ВерсияФайла, БылиОшибки,ДополнительныеПараметры);	
		Если МассивИменФайловКоторыеСовпалиСКеш <> Неопределено Тогда
			МассивИменФайловКоторыеСовпалиСКеш.Добавить(УниверсальноеПолноеИмя);
		КонецЕсли;	 
...
Функция СоздатьСтруктуруПараметровЗапуска()
	...
	Рез.Вставить("КоличествоПопытокВыполненияДействия", "3");
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	СтрЗапуска = СокрЛП(ПараметрЗапуска);
	//СтрЗапуска = "StartFeaturePlayer;VBParams=C:\Temp\111\VBParams8312UF_FastCheck.json";
	Если СтрЗапуска = "" Тогда
		Возврат;
	КонецЕсли;	
...
&НаКлиенте
Процедура УстановитьВидимостьКолонокДереваВСлужебномРежиме(ЕстьКонстантаВКонфигурацииПутьКVA)
...
Процедура УстановитьВидимостьСлужебныхЭлементов(ДопПараметры)
	...
	Элементы.ПроверкаРаботыVanessaBehaviorВРежимеTestClient.Видимость = ЕстьКонстантаВКонфигурацииПутьКVA;
	Элементы.ГруппаСлужебная.Видимость = Объект.ПроверкаРаботыVanessaBehaviorВРежимеTestClient И ЕстьКонстантаВКонфигурацииПутьКVA;
	Элементы.ГруппаСлужебная.Видимость = Объект.ПроверкаРаботыVanessaBehaviorВРежимеTestClient И ЕстьКонстантаВКонфигурацииПутьКVA;
	Элементы.ДеревоТестовСтатус.Видимость = Объект.ПроверкаРаботыVanessaBehaviorВРежимеTestClient И ЕстьКонстантаВКонфигурацииПутьКVA;
	УстановитьВидимостьКолонокДереваВСлужебномРежиме(ЕстьКонстантаВКонфигурацииПутьКVA);
...
Процедура ПодключитьБиблиотекуРаботыСВидео()
	...
	Иначе	
		...
		Если СокрЛП(Объект.КаталогИнструментов) = "" Тогда
			ЕстьКонстантаВКонфигурацииПутьКVA = ЕстьКонстантаВКонфигурацииСервер("ПутьКVanessaBehavior");
			Если ЕстьКонстантаВКонфигурацииПутьКVA Тогда
...
Функция ВывестиВСообщениеТекстОшибки(ДанныеОшибки)
	...
	Стр = Стр + "                ИмяМодуля: "      + ДанныеОшибки.ИмяМодуля + Символы.ПС;
	Стр = Стр + "                ИсходнаяСтрока: " + СокрЛП(ДанныеОшибки.ИсходнаяСтрока) + Символы.ПС;
	Стр = Стр + "                НомерСтроки: "    + ДанныеОшибки.НомерСтроки + Символы.ПС;
	Стр = Стр + "                Описание: "       + ДанныеОшибки.Описание;
	Если ДанныеОшибки.Причина <> Неопределено Тогда
		Стр = Стр + Символы.ПС + ПараметрыОшибкиИзПричиныОшибки(ДанныеОшибки.Причина);
	КонецЕсли;         
...
&НаКлиенте
Процедура УдалитьНедоступныеДляИнтерактивнойРаботыПользователюЭлементыИзМассиваЭлементовTestClient(МассивЭлементов)
	МассивИд = Новый Массив;
...
Процедура ОбработатьМассивЭлементовTestClient(КонтейнерЭлементов,МассивЭлементов,ИмяРеквизита,ДопПараметры)
	...
	Если МассивЭлементов.Количество() > 1 Тогда
		...
		Если ИсключитьНедоступныеДляИнтерактивнойРаботыЭлементы Тогда
			УдалитьНедоступныеДляИнтерактивнойРаботыПользователюЭлементыИзМассиваЭлементовTestClient(МассивЭлементов);
		КонецЕсли;	 
	КонецЕсли;
	...
	Пока XML.Прочитать() Цикл
		...
		Если XML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			ИначеЕсли ИмяУзла = "selectOption" Тогда
				Если МассивОбъектов[ИдВМассиве - 1] = "FormField" Тогда
					ИмяОбъекта = МассивСвойств[ИдВМассиве - 1]["title"];
					Если СокрЛП(МассивСвойств[ИдВМассиве - 1]["title"]) <> "" и (Не ЕстьДублиЗаголовковУПоля(МассивСтруктурФорм, ТекущаяФорма, ИмяОбъекта, "ПолеФормы")) Тогда
...
Функция ПолучитьФайлыПеревода()
	...
	Иначе	
		...
		Если СокрЛП(Объект.КаталогИнструментов) = "" Тогда
			...
			Если ЕстьКонстантаВКонфигурацииПутьКVA Тогда
				ПутьКVanessaBehavior     = ПолучитьЗначениеКонстантыСервер("ПутьКVanessaBehavior");
				ФайлПутьКVanessaBehavior = Новый Файл(ПутьКVanessaBehavior);
				ИмяФайла                 = ДополнитьСлешВПуть(ФайлПутьКVanessaBehavior.Путь) + "locales/Steps.epf";
			КонецЕсли;	 
		КонецЕсли;	 
	...
	ДанныеМакетовШагов = ФормаОбработки.ДанныеМакетовШагов(МассивЯзыков,Объект.ВерсияПоставки);
	...
	//Соответствие = Новый Соответствие;
	//Соответствие.Вставить("en",ФормаОбработки.ПолучитьМакетОбработки("en",Объект.ВерсияПоставки));
	//Соответствие.Вставить("ro",ФормаОбработки.ПолучитьМакетОбработки("ro",Объект.ВерсияПоставки));
	//Соответствие.Вставить("de",ФормаОбработки.ПолучитьМакетОбработки("de",Объект.ВерсияПоставки));
	//Соответствие.Вставить("lv",ФормаОбработки.ПолучитьМакетОбработки("lv",Объект.ВерсияПоставки));
	//Соответствие.Вставить("it",ФормаОбработки.ПолучитьМакетОбработки("it",Объект.ВерсияПоставки));
	//Соответствие.Вставить("pl",ФормаОбработки.ПолучитьМакетОбработки("pl",Объект.ВерсияПоставки));
	//Соответствие.Вставить("vi",ФормаОбработки.ПолучитьМакетОбработки("vi",Объект.ВерсияПоставки));
...
Функция ТекстСообщенийПользователю()
	...
	Если СокрЛП(Объект.КаталогИнструментов) = "" Тогда
		...
		Если ЕстьКонстантаВКонфигурацииПутьКVA Тогда
			ПутьКVanessaBehavior     = ПолучитьЗначениеКонстантыСервер("ПутьКVanessaBehavior");
			ФайлПутьКVanessaBehavior = Новый Файл(ПутьКVanessaBehavior);
			ИмяФайла                 = ДополнитьСлешВПуть(ФайлПутьКVanessaBehavior.Путь) + "locales/Messages.epf";
			
			Если Не ЗначениеЗаполнено(Объект.КаталогИнструментов) Тогда
				//ПутьКVanessaBehavior - это полный путь к epf с именем файла 
				Объект.КаталогИнструментов = КаталогИнструментовИзПутиКФреймворку(ПутьКVanessaBehavior);
			КонецЕсли;	 
		КонецЕсли;	 
	КонецЕсли;	 
...
&НаКлиенте
Процедура ЗапуститьSikuliXСевер(ДопПараметры)
...
Функция ВыполнитьSikuliСкриптЧерезSikuliXСервер(СтрокаКоманды, ЖдатьОкончания, ЗапускЧерезСкрипт,ОписаниеОшибки)
	Если SikuliXСеверЗапущен <> Истина Тогда
		...
		ЗапуститьSikuliXСевер(ДопПараметры);
		...
		ИмяУправляющиегоФайлаSikuliXСевер = ДопПараметры.ИмяУправляющиегоФайла;
...
&НаСервереБезКонтекста
Процедура НайтиФайлыСервер(Путь, СписокКаталогов, СписокФайлов, ИскатьВПодкаталогах,ИскомоеИмяФайла)
	Файлы = НайтиФайлы(Путь, "*", ИскатьВПодкаталогах);
