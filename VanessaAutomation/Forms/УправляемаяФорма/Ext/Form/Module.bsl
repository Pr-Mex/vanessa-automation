...
Функция ПолучитьОписаниеСтрокиИзТаблицы(ТабПарам,ИмяТаблицы = Неопределено,ДопПараметры = Неопределено) Экспорт
	...
	Для Ккк = 1 По КолКолонок Цикл
		...
		Ключ = Строка1["Кол"+Формат(Ккк, "ЧГ=")];
		Если Найти(Ключ,"\n") > 0 Тогда
			...
			КонецЕсли;	 
		КонецЕсли;	 
		Значение = Строка2["Кол"+Формат(Ккк, "ЧГ=")];
			...
			Для Ккк = (ИдСтроки-1) По Значение.Количество()-1 Цикл
				...
				Если МассивКлючей <> Неопределено Тогда
					Для Каждого Элем Из МассивКлючей Цикл
						ИдКолонки = ИдКолонки + 1;
						НовоеЗначение.Область(ИдСтроки,ИдКолонки,ИдСтроки,ИдКолонки).Текст = СтрокаЗначение["Кол" + Формат(ИдКолонки, "ЧГ=")];
					КонецЦикла;	
				КонецЕсли;	 
				...
				Иначе
					Если ТипЗнч(СтрокаЗначение) = Тип("Структура") Тогда
						Для СчетчикКолонок = 1 По СтрокаЗначение.Количество() Цикл
							НовоеЗначение.Область(ИдСтроки,СчетчикКолонок,ИдСтроки,СчетчикКолонок).Текст = СтрокаЗначение["Кол" + Формат(СчетчикКолонок, "ЧГ=")];
						КонецЦикла;	
					Иначе	
...
Процедура Sleep(КолСекунд) Экспорт
    ...
    Попытка
        Если ЭтоLinux Тогда
            Команда = "ping -i " + Формат(КолСекунд, "ЧГ=") + " 127.0.0.1 -c 2 > /dev/null";
            КомандаСистемы(Команда);
        Иначе
            Команда = "ping 127.0.0.1 -n " + Формат(КолСекунд + 1, "ЧГ="); // +1 нужен, т.к. в реальности происходит на один пинг меньше
            ВыполнитьКомандуОСБезПоказаЧерногоОкна(Команда);
        КонецЕсли;	    
    Исключение
...
Процедура ВычислитьPIDЭтогоСеансаСлужебный() Экспорт
	Попытка
        ...
        PIDЭтогоСеанса = Формат(oChildProcess.ParentProcessID, "ЧГ=");
...
Функция ПреобразоватьТабличныйДокументВМассивСтруктур(ТабДок) Экспорт
	...
	Для Ррр = 1 По ТабДок.ШиринаТаблицы Цикл
		СтрокаТаблицы.Вставить("Кол" + Формат(Ррр, "ЧГ="),ТабДок.Область(1,Ррр,1,Ррр).Текст);
	КонецЦикла;	
	...
	Для Ккк = 2 По ТабДок.ВысотаТаблицы Цикл
		СтрокаТаблицы = Новый Структура;
		Для Ррр = 1 По ТабДок.ШиринаТаблицы Цикл
			СтрокаТаблицы.Вставить("Кол" + Формат(Ррр, "ЧГ="),ТабДок.Область(Ккк,Ррр,Ккк,Ррр).Текст);
		КонецЦикла;	
		Массив.Добавить(СтрокаТаблицы);
	КонецЦикла;	
	...
	Если СтрокаШага.ШагСПараметрамиВТаблице = Истина Тогда
		...
		Для каждого СтрокиТаблицы Из ПодчиненныеСтроки Цикл
			...
			Для каждого Колонка Из СтрокиТаблицы.ПараметрыТаблицы Цикл
				...
				СтруктураПарамТаблица.Вставить("Кол" + Формат(НомерКолонки, "ЧГ="), Значение);
			КонецЦикла;
	...
	Исключение
		...
		Иначе
			...
			Если Объект.ДелатьЛогВыполненияСценариевВТекстовыйФайл Тогда
				...
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ТекШаг.Имя);
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%2",Формат(ТекШаг.ИдСтроки, "ЧГ="));
				ЗаписатьСтрокуВТекстовыйЛог(ТекстСообщения);
...
Функция ДобавитьНулей(Парам,КоличествоСимволов)
	Стр = СтрЗаменить(Формат(Парам, "ЧГ="),Символы.НПП,"");
...
Процедура ДобавитьВЛогИнструкцииHTMLНачалоСценария(СтруктураСценария)
	...
	ТД.ЗаписатьСтроку("ИдСценария=" + Формат(ТекИД_СценарияВМассиве, "ЧГ="));
...
Процедура СделатьСкриншотыОшибкиКаждогоОкна1СSikuliX()
	...
	ВыполнитьSikuliСкрипт(Объект.КаталогИнструментов + "\tools\Sikuli\ClickCoordinates.sikuli --args " + Формат(ТекКоординаты.x, "ЧГ=") + " " + Формат(ТекКоординаты.y, "ЧГ=") , -1, Истина,,ДопПараметры);
