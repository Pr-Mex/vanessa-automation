...
Функция Локализовать(Сообщение) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ТекстСообщенийПользователю) Тогда
		//TODO: Переписать название процедуры в объекте на Локализованно
...
Процедура ВыгрузитьНастройкиВФайлЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Настройки = ПолучитьОбщиеНастройки(Истина);
	...
	Для каждого СтрокаНастроек Из Настройки Цикл
		...
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("СписокЗначений") Тогда 
			Если СтрокаНастроек.Значение.Количество() > 0 Тогда 
				ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроек.Ключ);
		...
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("Структура") Тогда
			...
			Если СтрокаНастроек.Ключ = "ВыполнениеСценариев" 
				Или СтрокаНастроек.Ключ = "КлиентТестирования" Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроек.Ключ);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
					ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаСтруктуры.Ключ);
					ЗаписьJSON.ЗаписатьЗначение(СтрокаСтруктуры.Значение);			 
				КонецЦикла;
				ЗаписьJSON.ЗаписатьКонецОбъекта();
			КонецЕсли;
			// Отчет Allure
			Если СтрокаНастроек.Ключ = "ОтчетAllure" И Настройки.ДелатьОтчетВФорматеАллюр Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроек.Ключ);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
					ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаСтруктуры.Ключ);
			...
			КонецЕсли;
			// Отчет JUnit
			Если СтрокаНастроек.Ключ = "ОтчетJUnit" И Настройки.ДелатьОтчетВФорматеjUnit Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроек.Ключ);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаСтруктуры.Значение Цикл
					ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаСтруктуры.Ключ);
					ЗаписьJSON.ЗаписатьЗначение(СтрокаСтруктуры.Значение);			 
				КонецЦикла;
				ЗаписьJSON.ЗаписатьКонецОбъекта();
			КонецЕсли;
			// Отчет Cucumber
			Если СтрокаНастроек.Ключ = "ОтчетCucumber" И Настройки.ДелатьОтчетВФорматеCucumberJson Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроек.Ключ);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаСтруктуры.Значение Цикл
					ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаСтруктуры.Ключ);
			...
			КонецЕсли;
			// Инструкция HTML
			Если СтрокаНастроек.Ключ = "ИнструкцияHTML" И Настройки.СоздаватьИнструкциюHTML Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроек.Ключ);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаСтруктуры.Значение Цикл
					ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаСтруктуры.Ключ);
			...
			КонецЕсли;
			// Инструкция Markdown
			Если СтрокаНастроек.Ключ = "ИнструкцияMarkdown" И Настройки.СоздаватьИнструкциюMarkdown Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроек.Ключ);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаСтруктуры.Значение Цикл
					ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаСтруктуры.Ключ);
			...
			КонецЕсли;
			// Запись видео
			Если СтрокаНастроек.Ключ = "ЗаписьВидео" И Настройки.СоздаватьИнструкциюВидео Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства("ЗаписьВидео");
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
					ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаСтруктуры.Ключ);
			...
			КонецЕсли;
			// SikuliXServer
			Если СтрокаНастроек.Ключ = "SikuliXServer" И Настройки.ИспользоватьSikuliXСервер Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроек.Ключ);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
					ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаСтруктуры.Ключ);
...
Процедура ЗагрузитьНастройкиИзФайлаЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Для каждого СтрокаНастроек Из Настройки Цикл
		...
		Если СтрокаНастроек.Ключ = "ВерсияVA" Тогда		
			Версия = ПолучитьВерсиюОбработкиСервер();
			Если Версия <> СтрокаНастроек.Значение Тогда			
				СообщитьПользователю(Локализовать("Версия загружаемых настроек не соответсвует версии обработки."));	
			КонецЕсли;
			Продолжить;
		ИначеЕсли СтрокаНастроек.Ключ = "ЗапрашиватьПодтверждениеПриЗакрытии" Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ТипЗнч(СтрокаНастроек.Значение) = Тип("Строка")
			...
			Попытка
				Объект[СтрокаНастроек.Ключ]	= СтрокаНастроек.Значение;
			Исключение
				СтрокаОшибки = Локализовать("Параметр: %1 не загружен.");
				СтрокаОшибки = СтрЗаменить(СтрокаОшибки, "%1", СтрокаНастроек.Ключ);
				СообщитьПользователю(СтрокаОшибки);
			КонецПопытки;
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("Массив") Тогда 
			Попытка
				Объект[СтрокаНастроек.Ключ].Очистить();
			Исключение
				СтрокаОшибки = Локализовать("Параметр: %1 не загружен.");
				СтрокаОшибки = СтрЗаменить(СтрокаОшибки, "%1", СтрокаНастроек.Ключ);
				СообщитьПользователю(СтрокаОшибки);
			...
			Для каждого СтрокаМассива Из СтрокаНастроек.Значение Цикл
				Объект[СтрокаНастроек.Ключ].Добавить(СтрокаМассива);
			КонецЦикла;
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("Структура") Тогда
			Для Каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл 
				Если ТипЗнч(СтрокаСтруктуры.Значение) = Тип("Массив") Тогда
					// Данные меток Allure
					Если СтрокаСтруктуры.Ключ = "ДанныеАллюрМеток" Тогда
				...
				КонецЕсли;
				Попытка
					Объект[СтрокаСтруктуры.Ключ] = СтрокаСтруктуры.Значение;
				Исключение
					СтрокаОшибки = Локализовать("Параметр: %1 не загружен.");
					СтрокаОшибки = СтрЗаменить(СтрокаОшибки, "%1", СтрокаСтруктуры.Ключ);
					СообщитьПользователю(СтрокаОшибки);
				КонецПопытки;
			КонецЦикла;
		Иначе 
...
&НаСервереБезКонтекста
Процедура ДополнитьЗначенияИзСтруктуры(Приемник,Источник)
	Для Каждого КлючЗначение Из Источник Цикл
		Если ТипЗнч(КлючЗначение.Значение) = Тип("Структура") Тогда
			ДополнитьЗначенияИзСтруктуры(Приемник,КлючЗначение.Значение);
		Иначе
...
Процедура ВосстановитьНастройки(ЭтоLinux)
	...
	Если ТипЗнч(Настройки) = Тип("Структура") Тогда
		Для Каждого КлючЗначение Из Настройки Цикл
			Если ТипЗнч(КлючЗначение.Значение) = Тип("Структура") Тогда
				ДополнитьЗначенияИзСтруктуры(Настройки,КлючЗначение.Значение);
			КонецЕсли;	 
		КонецЦикла;	 
	КонецЕсли;	 
	...
	Если ТипЗнч(Настройки) = Тип("Структура") Тогда
		...
		Если Настройки.Свойство("КаталогOutputAllure") Тогда
			Настройки.Свойство("КаталогOutputAllure", Объект.КаталогВыгрузкиAllure);
		КонецЕсли;	 
		Если Настройки.Свойство("КаталогВыгрузкиAllure") Тогда
			Настройки.Свойство("КаталогВыгрузкиAllure", Объект.КаталогВыгрузкиAllure);
		КонецЕсли;	 
		...
		Если Настройки.Свойство("КаталогOutputjUnit") Тогда
			Настройки.Свойство("КаталогOutputjUnit", Объект.КаталогВыгрузкиJUnit);
		КонецЕсли;
		Если Настройки.Свойство("КаталогВыгрузкиJUnit") Тогда
		...
		Если Настройки.Свойство("КаталогOutputCucumberJson") Тогда
			Настройки.Свойство("КаталогOutputCucumberJson", Объект.КаталогВыгрузкиCucumberJson);
		КонецЕсли;	 
		Если Настройки.Свойство("КаталогВыгрузкиCucumberJson") Тогда
			Настройки.Свойство("КаталогВыгрузкиCucumberJson", Объект.КаталогВыгрузкиCucumberJson);
		КонецЕсли;	 
		...
		Если Настройки.Свойство("КаталогOutputИнструкцияHTML") Тогда
			Настройки.Свойство("КаталогOutputИнструкцияHTML", Объект.КаталогВыгрузкиИнструкцийHTML);
		КонецЕсли;	 
		Если Настройки.Свойство("КаталогВыгрузкиИнструкцийHTML") Тогда
			Настройки.Свойство("КаталогВыгрузкиИнструкцийHTML", Объект.КаталогВыгрузкиИнструкцийHTML);
		КонецЕсли;	 
		
		Настройки.Свойство("СоздаватьИнструкциюMarkdown", Объект.СоздаватьИнструкциюMarkdown);
		Если Настройки.Свойство("КаталогOutputИнструкцияMarkdown") Тогда
			Настройки.Свойство("КаталогOutputИнструкцияMarkdown", Объект.КаталогВыгрузкиИнструкцийMarkdown);
		КонецЕсли;	 
		Если Настройки.Свойство("КаталогВыгрузкиИнструкцийMarkdown") Тогда
			Настройки.Свойство("КаталогВыгрузкиИнструкцийMarkdown", Объект.КаталогВыгрузкиИнструкцийMarkdown);
		КонецЕсли;	 
		
		Настройки.Свойство("СоздаватьИнструкциюВидео", Объект.СоздаватьИнструкциюВидео);
		Если Настройки.Свойство("КаталогOutputИнструкцияВидео") Тогда
			Настройки.Свойство("КаталогOutputИнструкцияВидео", Объект.КаталогВыгрузкиИнструкцийВидео);
		КонецЕсли;	 
		Если Настройки.Свойство("КаталогВыгрузкиИнструкцийВидео") Тогда
			Настройки.Свойство("КаталогВыгрузкиИнструкцийВидео", Объект.КаталогВыгрузкиИнструкцийВидео);
		КонецЕсли;	 
		...
		Если Настройки.Свойство("КаталогOutputСкриншоты") Тогда
			Настройки.Свойство("КаталогOutputСкриншоты", Объект.КаталогВыгрузкиСкриншотов);
		КонецЕсли;	 
		Если Настройки.Свойство("КаталогВыгрузкиСкриншотов") Тогда
			Настройки.Свойство("КаталогВыгрузкиСкриншотов", Объект.КаталогВыгрузкиСкриншотов);
		КонецЕсли;	 
...
Процедура СохранитьНастройки()
	...
	Настройки = ПолучитьОбщиеНастройки();
...
Функция ПолучитьСтруктуруПараметров(Знач СтрокаПараметров)
	...
	Для каждого ЭлементМассива Из МассивПараметров Цикл
		...
		Если Позиция > 0 Тогда
			...
			Исключение
				ТекстСообщения = Локализовать("Не получилось прочитать значение из строки запуска: %1.");
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	//СтрокаПараметров = "StartFeaturePlayer;VBParams=C:\Commons\rep\vanessa-automation\tools\JSON\VBParams8314UF.json";
	...
	Если НастройкиОперСистемы <> Неопределено И КаталогПроекта = Неопределено Тогда
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		НастройкиVA = СтруктураПараметровЗапуска();
		...
		Иначе
			НастройкиVA = ПолучитьОбщиеНастройкиИзДвоичныхДанных(ИмяФайлаПараметров,НастройкиVA);
			Если НастройкиVA = Неопределено Тогда
...
//		ОсновныеНастройки = ПолучитьОбщиеНастройки(, Истина);
		...
		СтруктураJSON = ОбъединитьНастройкиСУчетомПриоритета(НастройкиVA, НастройкиОперСистемы, LangJSON);
		...
		Если Объект.ДелатьСкриншотПриВозникновенииОшибки Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputСкриншоты") Тогда
				...
				Если Не ПустаяСтрока(ЗнКаталогВыгрузкиСкриншотов) Тогда
					ТекстСообщения = Локализовать("Использование устаревшего параметра ""КаталогOutputСкриншоты"". Новый параметр - ""КаталогВыгрузкиСкриншотов"".");
					СообщитьПользователю(ТекстСообщения);
				КонецЕсли;
			КонецЕсли; 	
			// Конец Удалить
			Если НЕ ЗначениеЗаполнено(ЗнКаталогВыгрузкиСкриншотов) Тогда
		...
		Если Объект.СоздаватьИнструкциюHTML Тогда
			...
			ЗнКаталогВыгрузкиИнструкцийHTML = "";
			Если СтруктураJSON.Свойство("КаталогOutputИнструкцияHTML") Тогда
				...
				Если Не ПустаяСтрока(ЗнКаталогВыгрузкиИнструкцийHTML) Тогда
					ТекстСообщения = Локализовать("Использование устаревшего параметра ""КаталогOutputИнструкцияHTML"". Новый параметр - ""КаталогВыгрузкиИнструкцийHTML"".");
					СообщитьПользователю(ТекстСообщения);
				КонецЕсли;
			КонецЕсли; 	
			// Конец Устарела
			Если НЕ ЗначениеЗаполнено(ЗнКаталогВыгрузкиИнструкцийHTML) Тогда
		...
		Если Объект.СоздаватьИнструкциюMarkdown Тогда
			// Устарела.26 Обратная совместимость
			ЗнКаталогВыгрузкиИнструкцийMarkdown = "";
			Если СтруктураJSON.Свойство("КаталогOutputИнструкцияMarkdown") Тогда
				...
				Если Не ПустаяСтрока(ЗнКаталогВыгрузкиИнструкцийMarkdown) Тогда
					ТекстСообщения = Локализовать("Использование устаревшего параметра ""КаталогOutputИнструкцияMarkdown"". Новый параметр - ""КаталогВыгрузкиИнструкцийMarkdown"".");
					СообщитьПользователю(ТекстСообщения);
				КонецЕсли;
			КонецЕсли; 	
			// Конец Устарела
			Если НЕ ЗначениеЗаполнено(ЗнКаталогВыгрузкиИнструкцийMarkdown) Тогда
		...
		Если Объект.СоздаватьИнструкциюВидео Тогда
			// Устарела.26 Обратная совместимость
			ЗнКаталогВыгрузкиИнструкцийВидео = "";
			Если СтруктураJSON.Свойство("КаталогOutputИнструкцияВидео") Тогда
				...
				Если Не ПустаяСтрока(ЗнКаталогВыгрузкиИнструкцийВидео) Тогда
					ТекстСообщения = Локализовать("Использование устаревшего параметра ""КаталогOutputИнструкцияВидео"". Новый параметр - ""КаталогВыгрузкиИнструкцийВидео"".");
					СообщитьПользователю(ТекстСообщения);
				КонецЕсли;
			КонецЕсли; 	
			// Конец Устарела
			Если НЕ ЗначениеЗаполнено(ЗнКаталогВыгрузкиИнструкцийВидео) Тогда
		...
		Если ЗнДелатьОтчетВФорматеjUnit = Истина Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputjUnit") Тогда
				...
				Если Не ПустаяСтрока(ЗнКаталогВыгрузкиjUnit) Тогда
					ТекстСообщения = Локализовать("Использование устаревшего параметра ""КаталогOutputjUnit"". Новый параметр - ""КаталогВыгрузкиjUnit"".");
					СообщитьПользователю(ТекстСообщения);
					ОпределилиКаталогjUnit = Истина;
				КонецЕсли;					
			КонецЕсли; 
			// Конец Удалить	
			Если НЕ ЗначениеЗаполнено(ЗнКаталогВыгрузкиjUnit) Тогда
		...
		Если ЗнДелатьОтчетВФорматеCucumberJson = Истина Тогда
			...
			// Удалить. Оставлено для обратной совместимости
			ЗнКаталогВыгрузкиtCucumberJson = "";
			Если СтруктураJSON.Свойство("КаталогOutputCucumberJson") Тогда
				...
				Если не ПустаяСтрока(ЗнКаталогВыгрузкиtCucumberJson) Тогда
					ТекстСообщения = Локализовать("Использование устаревшего параметра ""КаталогOutputCucumberJson"". Новый параметр - ""КаталогВыгрузкиCucumberJson"".");
					СообщитьПользователю(ТекстСообщения);
					ОпределилиКаталогCucumberJson = Истина;
				КонецЕсли;
			КонецЕсли;
			// Конец Удалить	
			Если НЕ ЗначениеЗаполнено(ЗнКаталогВыгрузкиtCucumberJson) Тогда
		...
		Если СтруктураJSON.Свойство("КаталогOutputAllure") Тогда
			...
			Если не ПустаяСтрока(ЗнКаталогВыгрузкиAllure) Тогда
				ТекстСообщения = Локализовать("Использование устаревшего параметра ""КаталогOutputAllure"". Новый параметр - ""КаталогВыгрузкиAllure"".");
				СообщитьПользователю(ТекстСообщения);
				ОпределилиКаталогAllure = Истина;
			КонецЕсли;
		КонецЕсли;
		// Конец Удалить
		Если НЕ ЗначениеЗаполнено(ЗнКаталогВыгрузкиAllure) Тогда
		...
		Если СтруктураJSON.Свойство("КаталогOutputAllureБазовый") Тогда
			...
			Если Не ПустаяСтрока(ЗнКаталогВыгрузкиAllureБазовый) Тогда
				ТекстСообщения = Локализовать("Использование устаревшего параметра ""КаталогOutputAllureБазовый"". Новый параметр - ""КаталогВыгрузкиAllureБазовый"".");
				СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
		КонецЕсли;
		// Конец Удалить	
		
		Если НЕ ЗначениеЗаполнено(ЗнКаталогВыгрузкиAllureБазовый) Тогда
	...
	Для каждого СтрСтроки Из ДеревоСтроки Цикл
		...
		ИначеЕсли СтрСтроки.Тип = "Область" Тогда
			...
			Если (СтрСтроки.ДопТип = "Условие") или (СтрСтроки.ДопТип = "УсловиеИначеЕсли")  или (СтрСтроки.ДопТип = "Цикл")  Тогда
				...
				МожноВозвращатьИначе = Истина;
				Если ДопПараметры.МассивУсловий.Количество() > 1  Тогда
					МожноВозвращатьИначе = Ложь;
				КонецЕсли;	 
...
//
&НаКлиенте
Функция ПолучитьОбщиеНастройкиИзДвоичныхДанных(ИмяФайлаПараметров,НастройкиVA) Экспорт
	...
	Попытка
		...
		Для Каждого КлючЗначение Из Настройки Цикл
			НастройкиVA.Вставить(КлючЗначение.Ключ,КлючЗначение.Значение);
		КонецЦикла;	 
...
// Одноуровнеавая структура настроек с приоритетом настроек из системы
//
// Параметры:
