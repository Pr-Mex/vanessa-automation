...
Процедура ЗагрузитьНастройкиПоУмолчанию(Команда)
	...
	ПараметрыФайла = ПрочитатьСтруктуруИзJSONФайлаКлиент(ИмяФайла);
...
Процедура ВыполнитьСценарии(ТекСценарий = Неопределено, ДопПараметры = Неопределено) Экспорт
	...
	Если Объект.ДелатьОтчетВФорматеАллюр Тогда
		СоздатьКаталогЕслиЕгоНет(ПреобразоватьПутьСТочкамиКНормальномуПути(Объект.КаталогВыгрузкиAllure));
	КонецЕсли;	 
...
&НаКлиенте
Процедура СообщитьПользователю(Знач Сообщение, ТипСообщения = "") Экспорт
...
Функция ПолучитьЗначениеИзКонтекстаЕслиЭтоВозможно(Знач Значение, ВызыватьИсключение = Истина) Экспорт
	...
	ИначеЕсли (Лев(Значение,1) = "$") Тогда
		...
		ИначеЕсли (Лев(Значение,2) = "$$") и (Прав(Значение,2) = "$$") Тогда
			...
			ИмяПеременной = УбратьЗапрещенныеСимволыИзКлюча(ИмяПеременной);
...
Функция ТолькоПросмотрЭлементаСУчетомРодителя(ТекущаяФормаЭлемента,ЭлементФормы,ТекущееТолькоПросмотр) Экспорт
	...
	Для Каждого ТекГруппа Из ГруппыФормы Цикл
		ЭлементыГруппы = ТекГруппа.НайтиОбъекты();
		Сообщить(ТекГруппа.ТекстЗаголовка);
...
Функция ПанельРазделов() Экспорт
	...
	Исключение
		ОбработатьОшибкуПодключенияTestClient();
		ТекстСообщения = Локализовать("Клиент тестирования не подключен.");
...
Функция Локализовать(Сообщение) Экспорт
	...
	Результат = ТекстСообщенияПользователюВызовСервера(Сообщение, Объект.ТекстСообщенийПользователю);
	Возврат Результат;
...
//Возвращает массив снипетов, которые особенно создают видеовставку в автоинструкциях
&НаКлиенте
Функция ПолучитьМассивСнипетовПрерывающихЗаписьСценария() Экспорт
	Массив = Новый Массив;
	Массив.Добавить("обратитевнимание");
	Массив.Добавить("видеовставка");
	Массив.Добавить("note");
	Массив.Добавить("videoinsert");
...
&НаКлиенте
Функция Проверить(_Истина, ДопСообщениеОшибки = "") Экспорт
...
&НаКлиенте
Функция ПроверитьЛожь(_Ложь, ДопСообщениеОшибки = "") Экспорт
	Если _Ложь Тогда
...
&НаКлиенте
Функция ПроверитьДату(_Период,_Дата, ДопСообщениеОшибки = "") Экспорт
...
&НаКлиенте
Функция ПроверитьРавенствоДатСТочностью2Секунды(_Дата2,_Дата, ДопСообщениеОшибки = "") Экспорт
...
&НаКлиенте
Функция ПроверитьРавенство(_1, _2, ДопСообщениеОшибки = "") Экспорт
	Если _1 <> _2 Тогда
...
&НаКлиенте
Функция ПроверитьНеРавенство(_1, _2, ДопСообщениеОшибки = "") Экспорт
	Если _1 = _2 Тогда
...
&НаКлиенте
Функция ПроверитьБольше(ЭталонноеЧисло, ПроверяемоеЧисло, ДопСообщениеОшибки = "") Экспорт
...
&НаКлиенте
Функция ПроверитьБольшеИлиРавно(ЭталонноеЧисло,ПроверяемоеЧисло, ДопСообщениеОшибки = "") Экспорт
	Если ПроверяемоеЧисло < ЭталонноеЧисло Тогда
...
&НаКлиенте
Функция ПроверитьМеньше(ЭталонноеЧисло, ПроверяемоеЧисло, СообщениеОбОшибке = "") Экспорт
	Если ПроверяемоеЧисло >= ЭталонноеЧисло Тогда
...
&НаКлиенте
Функция ПроверитьМеньшеИлиРавно(ЭталонноеЧисло, ПроверяемоеЧисло, СообщениеОбОшибке = "") Экспорт
	Если ПроверяемоеЧисло > ЭталонноеЧисло Тогда
...
&НаКлиенте
Функция ПроверитьВыполнилось(Знач СтрокаАлгоритм, ПараметрыИлиДопСообщениеОшибки = Неопределено, Знач ДопСообщениеОшибки = "") Экспорт
...
&НаКлиенте
Функция ПроверитьНеВыполнилось(Знач СтрокаАлгоритм, ПараметрыИлиОжидаемоеОписаниеОшибки, Знач ОжидаемоеОписаниеОшибкиИлиДопСообщениеОшибки = "", Знач ДопСообщениеОшибки = "") Экспорт
...
&НаКлиенте
Функция ПроверитьТип(значение, ТипИлиИмяТипа, ДопСообщениеОшибки = "") Экспорт
...
Процедура ЗагрузитьГлобальныеПеременные()
	...
	Для каждого СтрокаПользователя Из МассивНастроек Цикл
		Если ВРег(СтрокаПользователя["user"]) = ВРег(ТекЮзер) Тогда 
			...
			ВывестиЗагруженныеНастройкиВЖР(СтрокаНастроек,Объект.ТекстСообщенийПользователю);
	...
	Если Не НастройкиЮзераНайдены Тогда
		ТекстСообщения = Локализовать("Ошибка. Не найдены настройки пользователя <%1> в файле <%2>.");
...
Функция ТекущийПользовательОС()
	...
	Иначе
		ТекстСообщения = Локализовать("Функция <%1> реализована только для Windows.");
...
&НаСервереБезКонтекста
Процедура ВывестиЗагруженныеНастройкиВЖР(знач НастройкиПользователя,ТекстСообщенийПользователю)
	...
	ЗаписьЖурналаРегистрации("VanessaBehavior.ЗагрузкаГлобальныхПеременных", 
		...
		ТекстСообщенияПользователюВызовСервера("Загружены глобальные переменные.",ТекстСообщенийПользователю) + НастройкиСтрокой);
...
// например СтрокаСоответствуетШаблону("Привет","*вет")
&НаСервереБезКонтекста
Функция СтрокаСоответствуетШаблонуСеврер(Знач ПроверяемаяСтрока, Знач Шаблон) Экспорт
	Шаблон = ПодготовитьШаблонКИспользованиюВРегулярке(Шаблон);
...
Функция ПоискВхожденияСтрокиПоШаблону(ЗначениеТЧ,ЗначениеСтроки)
	Для Каждого СтрокаЗначениеТЧ Из ЗначениеТЧ Цикл
		...
		Для Каждого Элем Из ЗначениеСтроки Цикл
			...
			Если НЕ СтрокаСоответствуетШаблонуСеврер(ЗначениеВСтрокеТаблицы,ЗначениеИскомойСтроки) Тогда
				СтрокаСовпала = Ложь;
				Прервать;
			КонецЕсли;	 
		КонецЦикла;	 
...
Процедура ЗаписатьЛогОшибки(СтрокаСценария,СтрокаШага,ТекстОшибки,ИнфоОбОшибке,ОписаниеОшибкиШага)
	...
	Если Объект.СобиратьДанныеОСостоянииАктивнойФормыПриОшибке Тогда
		...
		Если МакетСостояниеФормы <> Неопределено Тогда
...
Функция ПодключитьКонтекстОбработкиКлиент(ОбъектСодержащийШаги, ВерсияФайла = Неопределено,ДопПараметры = Неопределено)
	...
	Если ФормаОбработки <> Неопределено Тогда
		...
		Исключение
			...
			СообщитьПользователю(ТекстСообщения, "Важное");
...
Процедура ПолучитьУжеСуществующиеСценарииИзВстроенныхФич()
	Если ТекстВстроенныхСценариевСервер = Неопределено Тогда
		ТекстВстроенныхСценариевСервер = ПолучитьТекстВстроенныхСценариевСервер();
	КонецЕсли;	 
...
Процедура ЗаполнитьКаталогИнструментовЕслиОнПустой()
	...
	Если СокрЛП(КаталогИнструментов) = "" Тогда
		...
		Иначе	
			//попытаемся восстановить из настроек. Это нужно, когда Vanessa Automation вызвали программно, а затем сразу открыли интерактивно. В этом случае 1С подгружает версию из кеша от первой загрузки на сервере, и Объект().ИспользуемоеИмяФайла возвращает адрес в хранилище, а не имя исходного файла.
...
// Устарела.
&НаКлиенте 
Функция ПрочитатьСтруктуруИзJSONФайлаКлиент(ПутьКФайлуПараметров)
	
	//ДвоичныеДанные = Новый ДвоичныеДанные(ПутьКФайлуПараметров);
	//
	////ПараметрыУФ.Вставить("VBParamsДвоичныеДанные", ДвоичныеДанные);
	//
	//Результат = ПрочитатьСтруктуруИзJSONФайлаСервер(ПутьКФайлуПараметров, ДвоичныеДанные);
	//
	//Возврат Результат;
...
Процедура ВыполнитьСценарийАссинхронноТаймер()
	...
	Если НЕ Объект.РежимВыполненияОдногоШага
		И НЕ Объект.РежимВыполненияДанногоСценарияСТекущегоШага
		И НЕ Объект.РежимВыполненияДанногоСценарияСТекущегоШагаСПродолжением Тогда 
		// если надо выполнить ПередНачаломСценария и при этом сделать отладку по шагам - надо поставить брейкпоинт на первом шаге сценария
...
&НаКлиенте
Процедура ОбновитьСтатусыКнопокСценариев()
		...
		Если ЭлементФормы.Вид = ВидПоляФормы.ПолеТабличногоДокумента Тогда
			Попытка
				Если ЭлементФормы.Имя = "" Или ЭлементФормы.ТекущаяДоступность() Тогда
					// если табличный документ был открыт как отдельное окно как ТабДок.Показать(), тогда у него не работает ТекущаяДоступность() (проверено в 8.3.10)
...
Процедура ВыполнитьМетодШагЧерезВременныйФичаФайл(Стр)
	...
	Для каждого ТекШаг Из Шаги Цикл
		Если Не ЗначениеЗаполнено(ТекШаг.АдресСнипета) Тогда
			ТекстСообщения = Локализовать("Для шага <%1> не найден EPF файл.");
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ТекШаг.Имя);
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;	 
...
Процедура УстановитьУсловноеОформление()
	...
	Элементы.ЗаписьВидеоКэшФайловTTS.Видимость = НЕ Объект.ЗаписьВидеоКэшироватьФайлыTTS;
	
	Элементы.ЗаписьВидеоКэшВидеоВставок.Видимость = НЕ Объект.ЗаписьВидеоКэшироватьВидеоВставки;	 
...
&НаКлиенте
Функция УбратьЗапрещенныеСимволыИзКлюча(Знач Стр)
	Стр = СтрЗаменить(Стр,"""","");
	Стр = СтрЗаменить(Стр,Символы.ПС,"");
	Стр = СтрЗаменить(Стр,Символы.ВК,"");
	ЗапрещенныеСимволы = " <>.,\/|':;!№%?*()@#$%^&*+-`~↓↑";
...
Функция ОпределитьЯзыкЗапуска(ТекущийЯзык, ДопПараметры)
	...
	Если (Поз1 > 0) и (Поз2 > 0) Тогда
		Сообщить("Обнаружен больше чем 1 параметр L");
...
Процедура СохранитьНастройки()
	...
	Настройки = ТекущиеОбщиеНастройки();
...
//	СтрокаПараметров = "StartFeaturePlayer;VBParams=D:\vanessa-automation\tools\JSON\VBParams8314UF.json";
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		Если ИмяФайлаПараметров = Неопределено Тогда
			...
			НастройкиИзФайла = СтруктураПараметровЗапуска();
		Иначе
//			ДвоичныеДанные = Новый ДвоичныеДанные(ИмяФайлаПараметров);
//			НастройкиИзФайла = ПолучитьОбщиеНастройкиИзДвоичныхДанных(ДвоичныеДанные);

			Чтение = Новый ЧтениеJSON();
			Чтение.ОткрытьФайл(ИмяФайлаПараметров);
			Попытка
			...
			Если НастройкиИзФайла = Неопределено Тогда
				СообщитьПользователю(Локализовать("Ошибка чтения структуры JSON. Пропускаем чтение."));
				Возврат;
			КонецЕсли; 
		КонецЕсли;	 

		LangJSON = ПолучитьЗначениеПараметра(СтруктураПараметров, "Lang");
		
//		ОсновныеНастройки = ПолучитьОбщиеНастройки(, Истина);
		//ПараметрыЭталон = СтруктураПараметровЗапуска();
		
		СтруктураJSON = ОбъединитьНастройкиСУчетомПриоритета(НастройкиИзФайла, НастройкиОперСистемы, LangJSON);
		
		Если СтруктураJSON = Неопределено Тогда
			СообщитьПользователю(Локализовать("Ошибка чтения структуры JSON."));
			Возврат;
		КонецЕсли;
		
		ПромСтр = ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогФич"));
		Если ПромСтр = Неопределено Тогда
			СообщитьПользователю(Локализовать("В параметрах JSON не найден каталог Фич."));
			Возврат;
		КонецЕсли;	 
		...
		КаталогиБиблиотек = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогиБиблиотек");
		Если ТипЗнч(КаталогиБиблиотек) = Тип("Массив") Тогда
			Объект.КаталогиБиблиотек.Очистить();
			Для каждого ЭлементБиблиотеки Из КаталогиБиблиотек Цикл
				Объект.КаталогиБиблиотек.Добавить(ПреобразоватьПутьСТочкамиКНормальномуПути(ЭлементБиблиотеки));			
			КонецЦикла;			
		КонецЕсли;  
		...
		ЗнВыгрузитьPIDTestManager = ПолучитьЗначениеПараметра(СтруктураJSON, "ВыгрузитьPIDTestManager");
		Если ЗнВыгрузитьPIDTestManager = Истина Тогда
			ВычислитьPIDЭтогоСеанса("");
			ИмяФайлаPIDTestManager = ПолучитьЗначениеПараметра(СтруктураJSON, "ИмяФайлаPIDTestManager");
		...
		ЗнИмяФайлаВнешнихКомандФреймворка = ПолучитьЗначениеПараметра(СтруктураJSON, "ИмяФайлаВнешнихКомандФреймворка");
		...
		ЗнПриравниватьPendingКFailed = ПолучитьЗначениеПараметра(СтруктураJSON, "ПриравниватьPendingКFailed");
		Если ЗначениеЗаполнено(ЗнПриравниватьPendingКFailed) Тогда			
			Объект.ПриравниватьPendingКFailed = ЗнПриравниватьPendingКFailed;
		КонецЕсли;  
		
		ЗнКоличествоСекундПоискаОкна = ПолучитьЗначениеПараметра(СтруктураJSON, "КоличествоСекундПоискаОкна");
		...
		ЗнПаузаПриОткрытииОкна = ПолучитьЗначениеПараметра(СтруктураJSON, "ПаузаПриОткрытииОкна");
		Если ЗначениеЗаполнено(ЗнПаузаПриОткрытииОкна) Тогда			
			Попытка
				Объект.ПаузаПриОткрытииОкна = Число(ЗнПаузаПриОткрытииОкна);
			Исключение
			КонецПопытки;
		КонецЕсли;  
		
		ЗнКоличествоПопытокВыполненияДействия = ПолучитьЗначениеПараметра(СтруктураJSON, "КоличествоПопытокВыполненияДействия");
		...
		ЗнОстановкаПриВозникновенииОшибки = ПолучитьЗначениеПараметра(СтруктураJSON, "ОстановкаПриВозникновенииОшибки");
		...
		ЗнПоказыватьНомерСтрокиДереваПриВозникновенииОшибки = ПолучитьЗначениеПараметра(СтруктураJSON, "ПоказыватьНомерСтрокиДереваПриВозникновенииОшибки");
		...
		ЗнКаталогИнструментов = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогИнструментов");
		Если ЗначениеЗаполнено(ЗнКаталогИнструментов) Тогда			
			Объект.КаталогИнструментов = ЗнКаталогИнструментов;
		КонецЕсли;  
		
		ЗнКаталогПроекта = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогПроекта");
		Если ЗначениеЗаполнено(ЗнКаталогПроекта) Тогда			
			Объект.КаталогПроекта = ЗнКаталогПроекта;
		КонецЕсли;	 
	
		Объект.СписокТеговИсключение.Очистить();
		ЗначСписокТеговИсключение = ПолучитьЗначениеПараметра(СтруктураJSON, "СписокТеговИсключение");
		...
		ЗначСписокТеговОтбор = ПолучитьЗначениеПараметра(СтруктураJSON, "СписокТеговОтбор");
		Если ТипЗнч(ЗначСписокТеговОтбор) = Тип("Массив") Тогда
			Объект.СписокТеговОтбор.ЗагрузитьЗначения(ЗначСписокТеговОтбор);
		КонецЕсли;  
		
		ЗнДелатьСообщенияТранслитом = ПолучитьЗначениеПараметра(СтруктураJSON, "ДелатьСообщенияТранслитом");
		Если ЗнДелатьСообщенияТранслитом = Истина Тогда
			Объект.ДелатьСообщенияТранслитом = Истина;
		КонецЕсли;	 
		
		ЗнСоздаватьИнструкциюHTML = ПолучитьЗначениеПараметра(СтруктураJSON, "СоздаватьИнструкциюHTML");
		Если ЗнСоздаватьИнструкциюHTML = Истина Тогда
			Объект.СоздаватьИнструкциюHTML = Истина;
		КонецЕсли;	 
		
		ЗнСоздаватьИнструкциюMarkdown = ПолучитьЗначениеПараметра(СтруктураJSON, "СоздаватьИнструкциюMarkdown");
		Если ЗнСоздаватьИнструкциюMarkdown = Истина Тогда
			Объект.СоздаватьИнструкциюMarkdown = Истина;
		КонецЕсли;	 
		
		ЗнСоздаватьИнструкциюВидео = ПолучитьЗначениеПараметра(СтруктураJSON, "СоздаватьИнструкциюВидео");
		Если ЗнСоздаватьИнструкциюВидео = Истина Тогда
			Объект.СоздаватьИнструкциюВидео = Истина;
		КонецЕсли;	 
		
		ЗнДелатьЛогВыполненияСценариевВЖР = ПолучитьЗначениеПараметра(СтруктураJSON, "ДелатьЛогВыполненияСценариевВЖР");
		Если ЗнДелатьЛогВыполненияСценариевВЖР = Истина Тогда
			Объект.ДелатьЛогВыполненияСценариевВЖР = Истина;
		КонецЕсли;	  		
		
		ЗнДелатьЛогВыполненияСценариевВТекстовыйФайл = ПолучитьЗначениеПараметра(СтруктураJSON, "ДелатьЛогВыполненияСценариевВТекстовыйФайл");
		Если ЗнДелатьЛогВыполненияСценариевВТекстовыйФайл = Истина Тогда
			Объект.ДелатьЛогВыполненияСценариевВТекстовыйФайл = Истина;
			Объект.ИмяФайлаЛогВыполненияСценариев = ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначениеПараметра(СтруктураJSON, "ИмяФайлаЛогВыполненияСценариев"));
		КонецЕсли;	  		
		
		ЗнВыводитьВЛогВыполнениеШагов = ПолучитьЗначениеПараметра(СтруктураJSON, "ВыводитьВЛогВыполнениеШагов");
		Если ЗнВыводитьВЛогВыполнениеШагов = Истина Тогда
			Объект.ВыводитьВЛогВыполнениеШагов = Истина;
		КонецЕсли;	  		
		
		ЗнДелатьЛогОшибокВТекстовыйФайл = ПолучитьЗначениеПараметра(СтруктураJSON, "ДелатьЛогОшибокВТекстовыйФайл");
		Если ЗнДелатьЛогОшибокВТекстовыйФайл = Истина Тогда
			Объект.ДелатьЛогОшибокВТекстовыйФайл = Истина;
			Объект.ИмяКаталогаЛогОшибок = ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначениеПараметра(СтруктураJSON, "ИмяКаталогаЛогОшибок"));
		КонецЕсли;	  		
		
		ЗнСобиратьДанныеОСостоянииАктивнойФормыПриОшибке = ПолучитьЗначениеПараметра(СтруктураJSON, "СобиратьДанныеОСостоянииАктивнойФормыПриОшибке");
		Если ЗначениеЗаполнено(ЗнСобиратьДанныеОСостоянииАктивнойФормыПриОшибке) Тогда
			Объект.СобиратьДанныеОСостоянииАктивнойФормыПриОшибке = ЗнСобиратьДанныеОСостоянииАктивнойФормыПриОшибке;
		КонецЕсли;	 
		
		ЗнСобиратьДанныеОСостоянииВсехФормПриОшибке = ПолучитьЗначениеПараметра(СтруктураJSON, "СобиратьДанныеОСостоянииВсехФормПриОшибке");
		Если ЗначениеЗаполнено(ЗнСобиратьДанныеОСостоянииВсехФормПриОшибке) Тогда
			Объект.СобиратьДанныеОСостоянииВсехФормПриОшибке = ЗнСобиратьДанныеОСостоянииВсехФормПриОшибке;
		КонецЕсли;	 
		
		ЗнВерсияПлатформыДляГенерацииEPF = ПолучитьЗначениеПараметра(СтруктураJSON, "ВерсияПлатформыДляГенерацииEPF");
		...
		ЗнКлиентыТестирования = ПолучитьЗначениеПараметра(СтруктураJSON, "КлиентыТестирования");
		...
		ЗнАллюрМеток = ПолучитьЗначениеПараметра(СтруктураJSON, "ДанныеАллюрМеток");
		Если ЗначениеЗаполнено(ЗнАллюрМеток) Тогда
			Для Каждого Параметрыметок Из ЗнАллюрМеток Цикл
				ПрочитатьДанныеАллюрМеток(Параметрыметок, LangJSON);
			КонецЦикла;	
		КонецЕсли;
		
		ЗнДополнительныеПараметры = ПолучитьЗначениеПараметра(СтруктураJSON, "ДополнительныеПараметры");
		...
		ЗнЗапускатьТестКлиентВРежимеОтладки = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗапускатьТестКлиентВРежимеОтладки");
		Если ЗнЗапускатьТестКлиентВРежимеОтладки = Истина Тогда
			Объект.ЗапускатьТестКлиентВРежимеОтладки = Истина;
		КонецЕсли;
		
		ЗнДиапазонПортовTestclient = ПолучитьЗначениеПараметра(СтруктураJSON, "ДиапазонПортовTestclient");
		Если ЗначениеЗаполнено(ЗнДиапазонПортовTestclient) Тогда
			Объект.ДиапазонПортовTestclient = ЗнДиапазонПортовTestclient;
		КонецЕсли;	 
		
		ЗнЯзыкГенератораGherkin = ПолучитьЗначениеПараметра(СтруктураJSON, "ЯзыкГенератораGherkin");
		Если ЗначениеЗаполнено(ЗнЯзыкГенератораGherkin) Тогда
			Объект.ЯзыкГенератораGherkin = ЗнЯзыкГенератораGherkin;
		КонецЕсли;	 
		
		ЗнКомандаОткрытияФичиВТекстовомРедакторе = ПолучитьЗначениеПараметра(СтруктураJSON, "КомандаОткрытияФичиВТекстовомРедакторе");
		...
		ТаймаутЗапуска1С = Объект.ТаймаутЗапуска1С;
		Попытка
			ЗнТаймаутЗапуска1С = ПолучитьЗначениеПараметра(СтруктураJSON, "ТаймаутЗапуска1С");
			ТаймаутЗапуска1С = Число(ЗнТаймаутЗапуска1С);
		Исключение
			Если Объект.ТаймаутЗапуска1С = 0 Тогда
				ТаймаутЗапуска1С = 25;
				ТекстСообщения = Локализовать("Не удалось получить таймаут для запуска TestClient из значения %1. Используется значение по умолчанию: 25");
		...
		Попытка
			ЗнТаймаутДляАсинхронныхШагов = ПолучитьЗначениеПараметра(СтруктураJSON, "ТаймаутДляАсинхронныхШагов");
			Объект.ТаймаутДляАсинхронныхШагов = Число(ЗнТаймаутДляАсинхронныхШагов);
		Исключение
			ТекстСообщения = Локализовать("Не удалось получить таймаут для работы асинхронных шагов. Было передано значение: %1");
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ЗнТаймаутДляАсинхронныхШагов);
			СообщитьПользователю(ТекстСообщения);
			ЗаписатьЛогВЖРОшибка("VanessaAutomation.ЗапускВРежимеКоманднойСтроки", ТекстСообщения);
		КонецПопытки;
		
		ЗнБезопасноеВыполнениеШагов = ПолучитьЗначениеПараметра(СтруктураJSON, "БезопасноеВыполнениеШагов");
		...
		ЗнДелатьСкриншотПриВозникновенииОшибки = ПолучитьЗначениеПараметра(СтруктураJSON, "ДелатьСкриншотПриВозникновенииОшибки");
		...
		ЗнСниматьСкриншотКаждогоОкна1С = ПолучитьЗначениеПараметра(СтруктураJSON, "СниматьСкриншотКаждогоОкна1С");
		...
		ЗнКомандаСделатьСкриншот = ПолучитьЗначениеПараметра(СтруктураJSON, "КомандаСделатьСкриншот");
		...
		Если Объект.ДелатьСкриншотПриВозникновенииОшибки Тогда
			// Удалить. Обратная совместимость
			Если СтруктураJSON.Свойство("КаталогOutputСкриншоты") Тогда
				ЗнКаталогВыгрузкиСкриншотов = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогOutputСкриншоты");
				Если Не ПустаяСтрока(ЗнКаталогВыгрузкиСкриншотов) Тогда
					СообщитьУстаревшийПараметр("КаталогOutputСкриншоты", "КаталогВыгрузкиСкриншотов");
				КонецЕсли;
			КонецЕсли; 	
			// Конец Удалить
			Если СтруктураJSON.Свойство("КаталогВыгрузкиСкриншотов") Тогда
				ЗнКаталогВыгрузкиСкриншотов = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогВыгрузкиСкриншотов");
			КонецЕсли;
			Объект.КаталогвыгрузкиСкриншотов = ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиСкриншотов);
		КонецЕсли;	 
		//по скриншотам - окончание
		
		//по записи видео - начало
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоКомандаНачатьЗаписьВидео");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоКомандаНачатьЗаписьВидео = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоФайлКурсораМышки"));
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоФайлКурсораМышки = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоКаталогДляВременныхФайлов"));
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоКаталогДляВременныхФайлов = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоФайлВодянойЗнак"));
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоФайлВодянойЗнак = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоКомандаFfmpeg");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоКомандаFfmpeg = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоКомандаConvert");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоКомандаConvert = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоКаталогМузыки"));
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоКаталогМузыки = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоДелатьНаложениеTTS");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоДелатьНаложениеTTS = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоПутьКДвижкуTTS");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоПутьКДвижкуTTS = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоИмяTTS");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоИмяTTS = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоКэшироватьФайлыTTS");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоКэшироватьФайлыTTS = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоКэшФайловTTS");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоКэшФайловTTS = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоКэшироватьВидеоВставки");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоКэшироватьВидеоВставки = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоКэшВидеоВставок");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоКэшВидеоВставок = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоДобавлятьСубтитры");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоДобавлятьСубтитры = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоФормироватьИнструкциюТипВидео");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоФормироватьИнструкциюТипВидео = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоФормироватьИнструкциюТипHTML");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоФормироватьИнструкциюТипHTML = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоФормироватьИнструкциюТипMarkdown");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоФормироватьИнструкциюТипMarkdown = ЗначениеПараметра;
		КонецЕсли;	 
		
		Попытка
			Объект.ЗаписьВидеоПроцентУскорения = Число(ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоПроцентУскорения"));
		Исключение
			Объект.ЗаписьВидеоПроцентУскорения = 25;
		КонецПопытки;
		
		Попытка
			Объект.ЗаписьВидеоГромкостьМузыки = Число(ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоГромкостьМузыки"));
		Исключение
			Объект.ЗаписьВидеоГромкостьМузыки = 0.10;
		КонецПопытки;
		
		Попытка
			Объект.ЗаписьВидеоТипОзвучкиTTS = Число(ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоТипОзвучкиTTS"));
		Исключение
			Объект.ЗаписьВидеоТипОзвучкиTTS = 0;
		КонецПопытки;
		
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоYandexTTSЯзык");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоYandexTTSЯзык = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоYandexTTSГолос");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоYandexTTSГолос = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоYandexTTSЭмоция");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоYandexTTSЭмоция = ЗначениеПараметра;
		КонецЕсли;	 
		
		Попытка
			Объект.ЗаписьВидеоYandexTTSСкорость = Число(ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоYandexTTSСкорость"));
		Исключение
			Объект.ЗаписьВидеоYandexTTSСкорость = 1.1;
		КонецПопытки;
		
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоYandexTTSИдентификаторКаталога");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоYandexTTSИдентификаторКаталога = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоYandexTTSOauthToken");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоYandexTTSOauthToken = ЗначениеПараметра;
		КонецЕсли;	 
				
		ЗнЗаписьВидеоСловарьЗамен = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоСловарьЗамен");
		Если ЗнЗаписьВидеоСловарьЗамен <> Неопределено Тогда
			Объект.ЗаписьВидеоСловарьЗамен.Очистить();
			Объект.ЗаписьВидеоСловарьЗамен.Добавить(ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнЗаписьВидеоСловарьЗамен));
		КонецЕсли;	
		
		ЗнЗаписьВидеоСловарьЗаменСписок = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоСловарьЗаменСписок");
		...
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоЭкранШирина");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоЭкранШирина = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоЭкранВысота");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоЭкранВысота = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоКоличествоКадров");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоКоличествоКадров = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоПодсвечиватьАктивныеЭлементыФорм");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоПодсвечиватьАктивныеЭлементыФорм = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоПеремещатьКурсорМышкиКАктивномуЭлементуФормы");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоПеремещатьКурсорМышкиКАктивномуЭлементуФормы = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоГруппаШаговКакШаг");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоГруппаШаговКакШаг = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоОтключитьМасштабированиеШаговПоВремени");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоОтключитьМасштабированиеШаговПоВремени = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоПодсвечиватьНажатияМышки");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоПодсвечиватьНажатияМышки = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоОтключитьСлайдСЗаголовкомФичи");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоОтключитьСлайдСЗаголовкомФичи = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоОтключитьСлайдСценария");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоОтключитьСлайдСценария = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоЭмулироватьВводСКлавиатуры");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоЭмулироватьВводСКлавиатуры = ЗначениеПараметра;
		КонецЕсли;	 
		ЗначениеПараметра = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоСкрытьСлужебныеОкна");
		Если ЗначениеПараметра <> Неопределено Тогда
			Объект.ЗаписьВидеоСкрытьСлужебныеОкна = ЗначениеПараметра;
		КонецЕсли;	 
		
		Попытка
			Объект.ЗаписьВидеоСкоростьПроизношения                     = Число(ПолучитьЗначениеПараметра(СтруктураJSON, "ЗаписьВидеоСкоростьПроизношения"));
		Исключение
		КонецПопытки;
		// по записи видео - окончание
		
		Объект.ИмяТекущейСборки = ПолучитьЗначениеПараметра(СтруктураJSON, "ИмяСборки");
		ЗнЗакрытьTestClientПослеЗапускаСценариев =
		    ПолучитьЗначениеПараметра(СтруктураJSON, "ЗакрытьTestClientПослеЗапускаСценариев");
		Если ЗнЗакрытьTestClientПослеЗапускаСценариев = Истина Тогда
			Объект.ЗакрытьTestClientПослеЗапускаСценариев = Истина;
		КонецЕсли;	 
		
		Если Объект.СоздаватьИнструкциюHTML Тогда
			// Устарела.26 Обратная совместимость
			Если СтруктураJSON.Свойство("КаталогOutputИнструкцияHTML") Тогда
				ЗнКаталогВыгрузкиИнструкцийHTML = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогOutputИнструкцияHTML");
				Если Не ПустаяСтрока(ЗнКаталогВыгрузкиИнструкцийHTML) Тогда
					СообщитьУстаревшийПараметр("КаталогOutputИнструкцияHTML", "КаталогВыгрузкиИнструкцийHTML");	
				КонецЕсли;
			КонецЕсли; 	
			// Конец Устарела
			Если СтруктураJSON.Свойство("КаталогВыгрузкиИнструкцийHTML") Тогда
				ЗнКаталогВыгрузкиИнструкцийHTML = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогВыгрузкиИнструкцийHTML");
			КонецЕсли;
			Объект.КаталогВыгрузкиИнструкцийHTML = ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиИнструкцийHTML);
		КонецЕсли;	 
		
		Если Объект.СоздаватьИнструкциюMarkdown Тогда
			// Устарела.26 Обратная совместимость
			Если СтруктураJSON.Свойство("КаталогOutputИнструкцияMarkdown") Тогда
				ЗнКаталогВыгрузкиИнструкцийMarkdown = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогOutputИнструкцияMarkdown");
				Если Не ПустаяСтрока(ЗнКаталогВыгрузкиИнструкцийMarkdown) Тогда
					СообщитьУстаревшийПараметр("КаталогOutputИнструкцияMarkdown", "КаталогВыгрузкиИнструкцийMarkdown");	
				КонецЕсли;
			КонецЕсли; 	
			// Конец Устарела
			Если СтруктураJSON.Свойство("КаталогВыгрузкиИнструкцийMarkdown") Тогда
				ЗнКаталогВыгрузкиИнструкцийMarkdown = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогВыгрузкиИнструкцийMarkdown");
			КонецЕсли;
			Объект.КаталогВыгрузкиИнструкцийMarkdown = ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиИнструкцийMarkdown);
		КонецЕсли;	 
		
		Если Объект.СоздаватьИнструкциюВидео Тогда
			// Устарела.26 Обратная совместимость
			Если СтруктураJSON.Свойство("КаталогOutputИнструкцияВидео") Тогда
				ЗнКаталогВыгрузкиИнструкцийВидео = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогOutputИнструкцияВидео");
				Если Не ПустаяСтрока(ЗнКаталогВыгрузкиИнструкцийВидео) Тогда
					СообщитьУстаревшийПараметр("КаталогOutputИнструкцияВидео", "КаталогВыгрузкиИнструкцийВидео");
				КонецЕсли;
			КонецЕсли; 	
			// Конец Устарела
			Если СтруктураJSON.Свойство("КаталогВыгрузкиИнструкцийВидео") Тогда
				ЗнКаталогВыгрузкиИнструкцийВидео = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогВыгрузкиИнструкцийВидео");
			КонецЕсли;
			Объект.КаталогВыгрузкиИнструкцийВидео = ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиИнструкцийВидео);
		КонецЕсли;
		
		Если Объект.ЗапускатьТестКлиентВРежимеОтладки Тогда
			Объект.КлючиОтладки 	= ПолучитьЗначениеПараметра(СтруктураJSON, "КлючиОтладки");
			Объект.АдресОтладчика 	= ПолучитьЗначениеПараметра(СтруктураJSON, "АдресОтладчика");
		КонецЕсли;
				
		//для отчетов о прохождении сценариев
		ЗнКаталогОтносительноКоторогоНадоСтроитьИерархию = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогОтносительноКоторогоНадоСтроитьИерархию");
		Если ЗнКаталогОтносительноКоторогоНадоСтроитьИерархию <> Неопределено Тогда
		...
		ЗнУровеньОтчета1 = ПолучитьЗначениеПараметра(СтруктураJSON, "УровеньОтчета1");
		Если ЗнУровеньОтчета1 <> Неопределено Тогда
			Объект.УровеньОтчета1 = ЗнУровеньОтчета1;
		КонецЕсли;	 
		
		ЗнУровеньОтчета2 = ПолучитьЗначениеПараметра(СтруктураJSON, "УровеньОтчета2");
		Если ЗнУровеньОтчета2 <> Неопределено Тогда
			Объект.УровеньОтчета2 = ЗнУровеньОтчета2;
		КонецЕсли;	 
		
		ЗнУровеньОтчета3 = ПолучитьЗначениеПараметра(СтруктураJSON, "УровеньОтчета3");
		Если ЗнУровеньОтчета3 <> Неопределено Тогда
			Объект.УровеньОтчета3 = ЗнУровеньОтчета3;
		КонецЕсли;	 
		
		ЗнДобавлятьКИмениСценарияУсловияВыгрузки = ПолучитьЗначениеПараметра(СтруктураJSON, "ДобавлятьКИмениСценарияУсловияВыгрузки");
		Если ЗнДобавлятьКИмениСценарияУсловияВыгрузки = Истина Тогда
			Объект.ДобавлятьКИмениСценарияУсловияВыгрузки = Истина;
		КонецЕсли;	 
				
		ЗнВыгружатьСтатусВыполненияСценариевВФайл = ПолучитьЗначениеПараметра(СтруктураJSON, "ВыгружатьСтатусВыполненияСценариевВФайл");
		Если ЗнВыгружатьСтатусВыполненияСценариевВФайл = Истина Тогда
			Объект.ВыгружатьСтатусВыполненияСценариевВФайл = Истина;
		КонецЕсли;	 
		
		ЗнПутьКФайлуДляВыгрузкиСтатусаВыполненияСценариев = ПолучитьЗначениеПараметра(СтруктураJSON, "ПутьКФайлуДляВыгрузкиСтатусаВыполненияСценариев");
		...
		ЗнИспользоватьSikuliXСервер = ПолучитьЗначениеПараметра(СтруктураJSON, "ИспользоватьSikuliXСервер");
		Если ЗнИспользоватьSikuliXСервер = Истина Тогда
			Объект.ИспользоватьSikuliXСервер = Истина;
		КонецЕсли;	 
		
		ЗнКаталогиСкриптовSikuliX = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогиСкриптовSikuliX");
		Если ЗнКаталогиСкриптовSikuliX <> Неопределено Тогда
			Объект.КаталогиСкриптовSikuliX = ЗнКаталогиСкриптовSikuliX;
		КонецЕсли;	 
		
		ЗнИскатьЭлементыФормыПоИмени = ПолучитьЗначениеПараметра(СтруктураJSON, "ИскатьЭлементыФормыПоИмени");
		Если ЗнИскатьЭлементыФормыПоИмени <> Неопределено Тогда
			Объект.ИскатьЭлементыФормыПоИмени = ЗнИскатьЭлементыФормыПоИмени;
		КонецЕсли;	 
		
		ЗнДобавлятьПриНакликиванииМетаИнформацию = ПолучитьЗначениеПараметра(СтруктураJSON, "ДобавлятьПриНакликиванииМетаИнформацию");
		Если ЗнДобавлятьПриНакликиванииМетаИнформацию <> Неопределено Тогда
			Объект.ДобавлятьПриНакликиванииМетаИнформацию = ЗнДобавлятьПриНакликиванииМетаИнформацию;
		КонецЕсли;	 
		
		ЗнЗапускатьКлиентТестированияСМаксимизированнымОкном = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗапускатьКлиентТестированияСМаксимизированнымОкном");
		Если ЗнЗапускатьКлиентТестированияСМаксимизированнымОкном <> Неопределено Тогда
			Объект.ЗапускатьКлиентТестированияСМаксимизированнымОкном = ЗнЗапускатьКлиентТестированияСМаксимизированнымОкном;
		КонецЕсли;	 
		
		ЗнОбновлятьСтатистикуВДереве = ПолучитьЗначениеПараметра(СтруктураJSON, "ОбновлятьСтатистикуВДереве");
		Если ЗнОбновлятьСтатистикуВДереве <> Неопределено Тогда
			Объект.ОбновлятьСтатистикуВДереве = ЗнОбновлятьСтатистикуВДереве;
		КонецЕсли;	 
		
		ЗнОбновлятьДеревоПриНачалеВыполненияСценария = ПолучитьЗначениеПараметра(СтруктураJSON, "ОбновлятьДеревоПриНачалеВыполненияСценария");
		Если ЗнОбновлятьДеревоПриНачалеВыполненияСценария <> Неопределено Тогда
			Объект.ОбновлятьДеревоПриНачалеВыполненияСценария = ЗнОбновлятьДеревоПриНачалеВыполненияСценария;
		КонецЕсли;	 
		
		ЗначениеТегTreeВключенПоУмолчанию = ПолучитьЗначениеПараметра(СтруктураJSON, "ТегTreeВключенПоУмолчанию");
		Если ЗначениеТегTreeВключенПоУмолчанию <> Неопределено Тогда
			Объект.ТегTreeВключенПоУмолчанию = ЗначениеТегTreeВключенПоУмолчанию;
		КонецЕсли;
		
		//для отчета jUnit
		ЗнДелатьОтчетВФорматеjUnit = ПолучитьЗначениеПараметра(СтруктураJSON, "ДелатьОтчетВФорматеjUnit");
		Если ЗнДелатьОтчетВФорматеjUnit = Истина Тогда
			
			ОпределилиКаталогjUnit = Ложь;
			// Удалить. Оставлено для обратной совместимости
			Если СтруктураJSON.Свойство("КаталогOutputjUnit") Тогда
				ЗнКаталогВыгрузкиjUnit = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогOutputjUnit");
				Если Не ПустаяСтрока(ЗнКаталогВыгрузкиjUnit) Тогда
					СообщитьУстаревшийПараметр("КаталогOutputjUnit", "КаталогВыгрузкиjUnit");
				КонецЕсли;					
			КонецЕсли; 
			// Конец Удалить	
			Если СтруктураJSON.Свойство("КаталогВыгрузкиjUnit") Тогда
				ЗнКаталогВыгрузкиjUnit = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогВыгрузкиjUnit");
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ЗнКаталогВыгрузкиjUnit) Тогда
				Файл = Новый Файл(ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиjUnit));
				Если Не ФайлСуществуетКомандаСистемы(Файл.ПолноеИмя, "ЗнКаталогВыгрузкиjUnit") Тогда
					СоздатьКаталогКомандаСистемы(Файл.ПолноеИмя);
					ФайлПроверкаСуществования = Новый Файл(ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиjUnit));
					Если НЕ ФайлСуществуетКомандаСистемы(ФайлПроверкаСуществования.ПолноеИмя) Тогда
						ТекстСообщения = Локализовать("Не смог создать каталог для отчета jUnit: %1");
						ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ЗнКаталогВыгрузкиjUnit);
						СообщитьПользователю(ТекстСообщения);
						ЗаписатьЛогВЖРОшибка("VanessaAutomation.ЗапускВРежимеКоманднойСтроки", ТекстСообщения);
						Возврат;
					КонецЕсли;	 
				...
				КонецЕсли;	 
				Объект.КаталогВыгрузкиJUnit = ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиjUnit);
				Объект.ДелатьОтчетВФорматеjUnit = Истина;
				ОпределилиКаталогjUnit = Истина;
			КонецЕсли;	 
			
			Если НЕ ОпределилиКаталогjUnit Тогда
				ТекстСообщения = Локализовать("В параметрах запуска JSON не определен каталог отчета jUnit.");
				СообщитьПользователю(ТекстСообщения);
				Возврат;
			КонецЕсли;	 
		КонецЕсли;
		...
		ЗнДелатьОтчетВФорматеCucumberJson = ПолучитьЗначениеПараметра(СтруктураJSON, "ДелатьОтчетВФорматеCucumberJson");
		Если ЗнДелатьОтчетВФорматеCucumberJson = Истина Тогда
			
			ОпределилиКаталогCucumberJson = Ложь;
			
			// Удалить. Оставлено для обратной совместимости
			Если СтруктураJSON.Свойство("КаталогOutputCucumberJson") Тогда
				ЗнКаталогВыгрузкиtCucumberJson = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогOutputCucumberJson");
				Если Не ПустаяСтрока(ЗнКаталогВыгрузкиtCucumberJson) Тогда
					СообщитьУстаревшийПараметр("КаталогOutputCucumberJson", "КаталогВыгрузкиCucumberJson");
				КонецЕсли;
			КонецЕсли;
			// Конец Удалить	
			Если СтруктураJSON.Свойство("КаталогВыгрузкиCucumberJson") Тогда
				ЗнКаталогВыгрузкиtCucumberJson = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогВыгрузкиCucumberJson");
			КонецЕсли;
		
			Если ЗначениеЗаполнено(ЗнКаталогВыгрузкиtCucumberJson) Тогда
				Файл = Новый Файл(ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиtCucumberJson));
				Если Не ФайлСуществуетКомандаСистемы(Файл.ПолноеИмя, "ЗнКаталогВыгрузкиCucumberJson") Тогда
					СоздатьКаталогКомандаСистемы(Файл.ПолноеИмя);
					ФайлПроверкаСуществования = Новый Файл(ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиtCucumberJson));
					Если НЕ ФайлСуществуетКомандаСистемы(ФайлПроверкаСуществования.ПолноеИмя) Тогда
						ТекстСообщения = Локализовать("Не смог создать каталог для отчета CucumberJson: %1");
						ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ЗнКаталогВыгрузкиtCucumberJson); 
						СообщитьПользователю(ТекстСообщения);
						ЗаписатьЛогВЖРОшибка("VanessaAutomation.ЗапускВРежимеКоманднойСтроки", ТекстСообщения);
						Возврат;
					КонецЕсли;	 
				...
				КонецЕсли;	 
				Объект.КаталогВыгрузкиCucumberJson = ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиtCucumberJson);
			...
			Если НЕ ОпределилиКаталогCucumberJson Тогда
				СообщитьПользователю(Локализовать("В параметрах запуска JSON не определен отчета CucumberJson."));
				Возврат;
			КонецЕсли;	 
		КонецЕсли;
				
		//для отчета Allure
		ЗнДелатьОтчетВФорматеАллюр = ПолучитьЗначениеПараметра(СтруктураJSON, "ДелатьОтчетВФорматеАллюр");
		...
		// Удалить. Оставлено для обратной совместимости
		Если СтруктураJSON.Свойство("КаталогOutputAllure") Тогда
			ЗнКаталогВыгрузкиAllure = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогOutputAllure");
			Если не ПустаяСтрока(ЗнКаталогВыгрузкиAllure) Тогда
				СообщитьУстаревшийПараметр("КаталогOutputAllure", "КаталогВыгрузкиAllure");
			КонецЕсли;
		КонецЕсли;
		// Конец Удалить
		Если СтруктураJSON.Свойство("КаталогВыгрузкиAllure") Тогда
			ЗнКаталогВыгрузкиAllure = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогВыгрузкиAllure");
		КонецЕсли;
	
		Если ЗначениеЗаполнено(ЗнКаталогВыгрузкиAllure) Тогда
			УдалитьКаталогКомандаСистемы(ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиAllure));
			Файл = Новый Файл(ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиAllure));
			Если Не ФайлСуществуетКомандаСистемы(Файл.ПолноеИмя, "КаталогВыгрузкиAllure") Тогда
				СоздатьКаталогКомандаСистемы(Файл.ПолноеИмя);
			КонецЕсли;	 
			Объект.КаталогВыгрузкиAllure = ЗнКаталогВыгрузкиAllure;
			ОпределилиКаталогAllure = Истина;
		КонецЕсли;	 
		
		// Удалить. Оставлено для обратной совместимости
		Если СтруктураJSON.Свойство("КаталогOutputAllureБазовый") Тогда
			ЗнКаталогВыгрузкиAllureБазовый = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогOutputAllureБазовый");
			Если Не ПустаяСтрока(ЗнКаталогВыгрузкиAllureБазовый) Тогда
				СообщитьУстаревшийПараметр("КаталогOutputAllureБазовый", "КаталогВыгрузкиAllureБазовый");
			КонецЕсли;
		КонецЕсли;
		// Конец Удалить	
		Если СтруктураJSON.Свойство("КаталогВыгрузкиAllureБазовый") Тогда
			ЗнКаталогВыгрузкиAllureБазовый = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогВыгрузкиAllureБазовый");
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ЗнКаталогВыгрузкиAllureБазовый) Тогда
			...
			Если Не ФайлСуществуетКомандаСистемы(Файл.ПолноеИмя, "ЗнКаталогВыгрузкиAllureБазовый1") Тогда		
				СоздатьКаталогКомандаСистемы(Файл.ПолноеИмя);
			КонецЕсли;	 
			
			Если ПолучитьЗначениеПараметра(СтруктураJSON, "СоздаватьПодкаталогВКаталогеAllureДляЭтойСборки") = Истина Тогда
				ЗнКаталогВыгрузкиAllureБазовый = ДополнитьСлешВПуть(ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогВыгрузкиAllureБазовый))
					+ Лев(ПолучитьЗначениеПараметра(СтруктураJSON, "ИмяСборки"),40);
			...
			Если НЕ ФайлСуществуетКомандаСистемы(ФайлПроверкаСуществования.ПолноеИмя, "ЗнКаталогВыгрузкиAllureБазовый2") Тогда
				...
				Если НЕ ФайлСуществуетКомандаСистемы(ФайлПроверкаСуществования.ПолноеИмя, "ЗнКаталогВыгрузкиAllureБазовый3") Тогда
		...
		Если Объект.ДелатьОтчетВФорматеАллюр Тогда
			Если НЕ ОпределилиКаталогAllure Тогда
				СообщитьПользователю(Локализовать("В параметрах запуска JSON не определен каталог отчета Allure."));
				Возврат;
			КонецЕсли;	 
		КонецЕсли;	 
		
		ПриоритетНастроекПользователя = ПолучитьЗначениеПараметра(СтруктураJSON, "ПриоритетНастроекПользователя");
		
		ВыполнятьШагиАссинхронноЗн = ПолучитьЗначениеПараметра(СтруктураJSON, "ВыполнятьШагиАссинхронно");
		...
		ЗнКаталогИсполняемогоФайла = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогИсполняемогоФайла");
		...
		ВыполнитьСценарии = ПолучитьЗначениеПараметра(СтруктураJSON, "ВыполнитьСценарии");
		ВыполнитьСценарииСТекущегоШага = ПолучитьЗначениеПараметра(СтруктураJSON, "ВыполнитьСценарииСТекущегоШага");
		Если ВыполнитьСценарии = Истина Тогда
			ЗнЗавершитьРаботуСистемы = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗавершитьРаботуСистемы");
		...
		ИначеЕсли ВыполнитьСценарииСТекущегоШага = Истина Тогда	
			НадоВыполнитьСценарийСоСтрокиПереданнойИзвне = Истина;
			НомерСтрокиСценарияПереданныйИзвне           = Число(ПолучитьЗначениеПараметра(СтруктураJSON, "НомерСтрокиДляЗапускаСТекущегоШагаСценария"));
			ЗагрузитьФичи();
			ПодключитьОбработчикОжидания("ЗапуститьСценарийСоСтрокиПрисланнойИзвне",0.1,Истина);
		Иначе	
			ЗагрузитьСценарии = ПолучитьЗначениеПараметра(СтруктураJSON, "ЗагрузитьСценарии");
	...
	ИначеЕсли ПолучитьЗначениеПараметра(СтруктураПараметров, "GetStepsCache") = Истина Тогда
		...
		Иначе 
			НастройкиИзФайла = ПрочитатьСтруктуруИзJSONФайлаКлиент(ИмяФайлаПараметров);
			Если НастройкиИзФайла = Неопределено Тогда
...
Процедура ВосстановитьНастройки(ЭтоLinux)
	...
	Если ТипЗнч(Настройки) = Тип("Структура") Тогда
		...
		ЗагрузитьНастройкиНаСревере(Настройки);
...
Процедура ЗагрузитьНастройкиИзФайлаЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	ЗагрузитьНастройкиНаСревере(НастройкиИзФайла);
...
&НаСервере
Процедура ЗагрузитьНастройкиНаСревере(Настройки)

		ВерсияОбработки = "";
		ТекущийЯзык = "ru";

		Если Настройки.Свойство("Version") Тогда
			ВерсияОбработки = Настройки.Version;
		КонецЕсли;
...
//	Версия = ПолучитьВерсиюОбработкиСервер(Истина);
//	Если Версия <> ВерсияОбработки Тогда
//		ТекстСообщения = ЛокализоватьСервер("Версия загружаемых настроек не соответсвует версии обработки.");
//		СообщитьПользователю(ТекстСообщения);
//	КонецЕсли;
	...
	Для каждого СтрокаНастроек Из Настройки Цикл
		...
		Если НаДругомЯзыке Тогда
			СтрокаСИменемКлюча = ТаблицаИменКлючей.Найти(СтрокаНастроек.Ключ, "ИмяEn");
			Если СтрокаСИменемКлюча <> Неопределено Тогда
				ИмяКлюча = СтрокаСИменемКлюча.ИмяRu;
			КонецЕсли;
		КонецЕсли;
		
		Если ТипЗнч(СтрокаНастроек.Значение) = Тип("Строка")
			Или ТипЗнч(СтрокаНастроек.Значение) = Тип("Булево") Тогда
			
			Если ИмяКлюча = "ИмяСборки" Тогда
				Объект.ИмяТекущейСборки = СтрокаНастроек.Значение;
				Продолжить;
			...
			ИначеЕсли ИмяКлюча = "КаталогOutputAllureБазовый" Тогда
				Объект.КаталогВыгрузкиAllure = СтрокаНастроек.Значение;
				Продолжить;
			ИначеЕсли ИмяКлюча = "КаталогOutputjUnit" Тогда
				Объект.КаталогВыгрузкиJUnit = СтрокаНастроек.Значение;
				Продолжить;
			ИначеЕсли ИмяКлюча = "КаталогOutputCucumberJson" Тогда
				Объект.КаталогВыгрузкиCucumberJson = СтрокаНастроек.Значение;
				Продолжить;
			ИначеЕсли ИмяКлюча = "КаталогOutputСкриншоты" Тогда
				Объект.КаталогВыгрузкиСкриншотов = СтрокаНастроек.Значение;
				Продолжить;				
			КонецЕсли;
			
			Попытка
				Объект[ИмяКлюча] = СтрокаНастроек.Значение;
			Исключение
				СтрокаОшибки = ЛокализоватьСервер("Параметр: %1 не загружен.");
				СтрокаОшибки = СтрЗаменить(СтрокаОшибки, "%1", ИмяКлюча);
				Сообщить(СтрокаОшибки);
			КонецПопытки;
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("Массив") Тогда 
			Попытка
			...
			Исключение
				...
				Продолжить;
			КонецПопытки;
			Для каждого СтрокаМассива Из СтрокаНастроек.Значение Цикл
		...
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("Структура") Тогда
			Для Каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
				...
				Если НаДругомЯзыке Тогда
					СтрокаСИменемКлюча = ТаблицаИменКлючей.Найти(ИмяКлючаСтруктуры, "ИмяEn");
					Если СтрокаСИменемКлюча <> Неопределено Тогда
						ИмяКлючаСтруктуры = СтрокаСИменемКлюча.ИмяRu;
					КонецЕсли;
				КонецЕсли;
				
				Если ТипЗнч(СтрокаСтруктуры.Значение) = Тип("Массив") Тогда
				// Данные меток Allure
					Если ИмяКлючаСтруктуры = "ДанныеАллюрМеток" Тогда
						ДанныеАллюрМеток.Очистить();
						Для Каждого ЭлементМассив Из СтрокаСтруктуры.Значение Цикл
				...
				КонецЕсли;
				Попытка
					Объект[ИмяКлючаСтруктуры] = СтрокаСтруктуры.Значение;
				Исключение
					СтрокаОшибки = ЛокализоватьСервер("Параметр: %1 не загружен.");
					СтрокаОшибки = СтрЗаменить(СтрокаОшибки, "%1", ИмяКлючаСтруктуры);
					Сообщить(СтрокаОшибки);
				КонецПопытки;
			КонецЦикла;
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("СписокЗначений") Тогда
		Попытка
			Объект[ИмяКлюча].ЗагрузитьЗначения(СтрокаНастроек.Значение.ВыгрузитьЗначения());
		Исключение
		КонецПопытки;
		Иначе 
			Продолжить;
...
//	СтруктураНастроек = СтруктураЗагрузкиОбщихНастроек();
		
//	ЧтениеТекста = Новый ЧтениеТекста(ПолноеИмяФайла);
//	СтрокаЗаголовок = ЧтениеТекста.ПрочитатьСтроку();
//	Если Лев(СтрокаЗаголовок, 3) = "###" Тогда
//		НовыйФорматФайла = Истина;
//	Иначе
//		НовыйФорматФайла = Ложь;
//	КонецЕсли;
//	
//	Если НовыйФорматФайла Тогда
//		МассивПодстрок = РазложитьСтрокуВМассивПодстрокКлиент(СтрокаЗаголовок, " ");
//
//		// Версия обработки
//		ИндексСтроки = Неопределено;
//		Для Индекс = 0 По МассивПодстрок.Количество() - 1 Цикл
//			РезультатПоиска = СтрНайти(МассивПодстрок[Индекс], "version");
...
//			ВерсияОбработки = МассивПодстрок.Получить(ИндексСтроки);
//			ВерсияОбработки = СтрЗаменить(ВерсияОбработки, "version=", "");
//			ВерсияОбработки = СтрЗаменить(ВерсияОбработки, """", "");
//
//			СтруктураНастроек.ВерсияОбработки = ВерсияОбработки;
//		КонецЕсли;
//
//		// Текущий язык
////		ИндексСтроки = МассивПодстрок.Найти("lang");
//		ИндексСтроки = Неопределено;
//		Для Индекс = 0 По МассивПодстрок.Количество() - 1 Цикл
//			РезультатПоиска = СтрНайти(МассивПодстрок[Индекс], "lang");
//			Если РезультатПоиска > 0 Тогда
//				ИндексСтроки = Индекс;
//				Прервать;
//			КонецЕсли;
...
//		
//		Если ИндексСтроки <> Неопределено Тогда
//			ТекущийЯзык = МассивПодстрок.Получить(ИндексСтроки);
//			ТекущийЯзык = СтрЗаменить(ТекущийЯзык, "lang=", "");
//			ТекущийЯзык = СтрЗаменить(ТекущийЯзык, """", "");
//
//			СтруктураНастроек.ТекущийЯзык = НРег(ТекущийЯзык);
//		КонецЕсли;
//
//		// Настройки
//		СтрокаНастроек = "";
//		СтрокаНастройки = ЧтениеТекста.ПрочитатьСтроку();
//		Пока СтрокаНастройки <> Неопределено Цикл
//			СтрокаНастроек = СтрокаНастроек + СтрокаНастройки;
//			СтрокаНастройки = ЧтениеТекста.ПрочитатьСтроку();
//		КонецЦикла;
//
...
//			ЧтениеJSON.Закрыть();
//			
//			СтруктураНастроек.Настройки = НастройкиJSON;
//		Исключение
//			ЧтениеJSON.Закрыть();		
...
//		КонецПопытки;
//		
//		СтруктураНастроек.НовыйФорматИзФайла = Истина;
//
...
// Возвращает структуру общих настроек переданных из файла настроек VAParams
// прочитанных в виде двоичных данных с клиента.
//
// Параметры:
...
&НаСервере
Функция ПолучитьОбщиеНастройки(НастройкиВыгрузки = Ложь, НастройкиЗагрузки = Ложь)
	
	Обработка = РеквизитФормыВЗначение("Объект");
	ОбщиеНастройки = Обработка.СтруктураОбщихНастроек(НастройкиВыгрузки, НастройкиЗагрузки);
	...
	Если НастройкиВыгрузки Тогда
		...
		Если ДанныеАллюрМеток.Количество() > 0 Тогда
			...
			Для Каждого СтрокаКоллекции Из ДанныеАллюрМеток Цикл
				...
				СтрокаМассива.Вставить("РегулярноеВыражение", СтрокаКоллекции.РегулярноеВыражение);
				СтрокаМассива.Вставить("ИмяМетки", СтрокаКоллекции.ИмяМетки);
				СтрокаМассива.Вставить("Значение", СтрокаКоллекции.Значение);
				МассивМеток.Добавить(СтрокаМассива);
			КонецЦикла;
...
// Одноуровнеавая структура настроек с приоритетом настроек из операционной системы
//
// Параметры:
//  СтруктураНастроек	 - Структура - Базовые настройки заполненные значениями по умолчанию
//  ПараметрыПриоритет	 - Структура - Какие-то параметры среды
//  ОбщиеНастройки		 - Структура - Настройки прочитанные из файла с параметрами .json
//  LangJSON			 - Строка - 
// 
// Возвращаемое значение:
//  Структура - 
...
&НаСервереБезКонтекста
Функция ИменаКлючейОбщихНастроек()
	
	ТаблицаИмен = Новый ТаблицаЗначений;
	ТаблицаИмен.Колонки.Добавить("ИмяRU");
	ТаблицаИмен.Колонки.Добавить("ИмяEn");

	ТаблицаИменНоваяСтрока("StartFeaturePlayer", "startfeatureplayer", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("АдресОтладчика", "testclientdebugaddress", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("БезопасноеВыполнениеШагов", "safeexecutionofsteps", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ВерсияVA", "VersionVA", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ВерсияПлатформыДляГенерацииEPF", "platformversiongenerateepf", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ВыводитьВЛогВыполнениеШагов", "logstepstotext", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ВыгружатьСтатусВыполненияСценариевВФайл", "createlogs", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ВыгрузитьPIDTestManager", "returnpidoftestmanager", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ВыполнениеСценариев", "RunningScripts", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ВыполнитьСценарии", "vanessarun", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ВыполнитьСценарииСТекущегоШага", "vanessarunfromthisstep", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ВыполнятьШагиАссинхронно", "makestepsasync", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДанныеАллюрМеток", "DataAllureMarks", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДелатьЛогВыполненияСценариевВЖР", "logtogr", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДелатьЛогВыполненияСценариевВТекстовыйФайл", "logtotext", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДелатьЛогОшибокВТекстовыйФайл", "logerrorstotext", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДелатьОтчетВоВнутреннемФормате", "simplereport", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДелатьОтчетВФорматеCucumberJson", "cucumbercreatereport", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДелатьОтчетВФорматеjUnit", "junitcreatereport", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДелатьОтчетВФорматеАллюр", "allurecreatereport", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДелатьСкриншотПриВозникновенииОшибки", "onerrorscreenshot", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДелатьСообщенияТранслитом", "translatemassage", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДиапазонПортовTestclient", "testclientports", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДобавлятьКИмениСценарияУсловияВыгрузки", "addtofeaturefilter", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДобавлятьПриНакликиванииМетаИнформацию", "addmetainformationclicking", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ДополнительныеПараметры", "dopparams", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗавершитьРаботуСистемы", "vanessashutdown", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗагрузитьСценарии", "loadscenarios", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗагрузкаФичПриОткрытии", "DownloadFeaturesOpen", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗакрытьTestClientПослеЗапускаСценариев", "closetestclient", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидео", "VideoRecording", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоYandexTTSOauthToken", "yandexttsoauthtoken", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоYandexTTSГолос", "yandexttsvoice", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоYandexTTSИдентификаторКаталога", "yandexttsfolderId", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоYandexTTSСкорость", "yandexttsspeed", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоYandexTTSЭмоция", "yandexttsemotion", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоYandexTTSЯзык", "yandexttslanguage", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоГромкостьМузыки", "musicvolume", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоГруппаШаговКакШаг", "groupofstepsasastep", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоДелатьНаложениеTTS", "dotts", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоДобавлятьСубтитры", "addsubtitles", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоИмяTTS", "ttsvoice", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоКаталогДляВременныхФайлов", "tempfolderforvideo", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоКаталогМузыки", "musicpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоКоличествоКадров", "fps", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоКомандаConvert", "convertcomand", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоКомандаFfmpeg", "ffmpegcomand", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоКомандаНачатьЗаписьВидео", "comandrecvideo", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоКэшВидеоВставок", "videocachepath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоКэшироватьВидеоВставки", "cachevideo", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоКэшироватьФайлыTTS", "cachettsfiles", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоКэшФайловTTS", "ttsfilescache", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоОтключитьМасштабированиеШаговПоВремени", "disablescalingofsteps", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоОтключитьСлайдСЗаголовкомФичи", "turnofffeaturetitleslide", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоОтключитьСлайдСценария", "turnoffscenarioslide", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоПеремещатьКурсорМышкиКАктивномуЭлементуФормы", "movemouse", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоПодсвечиватьАктивныеЭлементыФорм", "showframe", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоПодсвечиватьНажатияМышки", "highlightmouseclicks", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоПроцентУскорения", "accelerationpercentage", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоПутьКДвижкуTTS", "audioenginetts", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоСкоростьПроизношения", "speedofspeech", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоСкрытьСлужебныеОкна", "hideservicewindows", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоСловарьЗамен", "changewordslib", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоСловарьЗаменСписок", "changewordsliblist", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоТипОзвучкиTTS", "ttstype", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоФайлВодянойЗнак", "watermarkfile", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоФайлКурсораМышки", "mousecurfile", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоФормироватьИнструкциюТипHTML", "instructiontypehtml", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоФормироватьИнструкциюТипMarkdown", "instructiontypemarkdown", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоФормироватьИнструкциюТипВидео", "instructiontypevideo", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоЭкранВысота", "screenheight", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоЭкранШирина", "screenwidth", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗаписьВидеоЭмулироватьВводСКлавиатуры", "emulatekeyboardinput", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗапускатьКлиентТестированияСМаксимизированнымОкном", "runtestclientwithmaximizedwindow", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЗапускатьТестКлиентВРежимеОтладки", "testclientdebug", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ИмяКаталогаЛогОшибок", "texterrorslogname", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ИмяСборки", "buildname", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ИмяТекущейСборки", "NameCurrentBuild", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ИмяФайлаPIDTestManager", "filenamepidtestmanager", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ИмяФайлаВнешнихКомандФреймворка", "fileofcommand", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ИмяФайлаЛогВыполненияСценариев", "textlogname", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ИнструкцияHTML", "HTMLInstruction", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ИнструкцияMarkdown", "MarkdownInstruction", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ИнтервалВыполненияШагаЗаданныйПользователем", "SpacingStepSpecifiedUser", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ИскатьЭлементыФормыПоИмени", "searchformelementsbyname", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ИспользоватьSikuliXСервер", "usesikulixserver", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогВыгрузкиAllure", "allurepath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогВыгрузкиAllureБазовый", "allurepathbase", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогВыгрузкиCucumberJson", "cucumberreportpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогВыгрузкиJUnit", "junitpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогВыгрузкиИнструкцийHTML", "htmlpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогВыгрузкиИнструкцийMarkdown", "markdownpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогВыгрузкиИнструкцийВидео", "videopath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогВыгрузкиСкриншотов", "outputscreenshot", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогиБиблиотек", "librarycatalogs", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогИнструментов", "instrpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогиСкриптовSikuliX", "sikulixscriptpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогИсполняемогоФайла", "pathtoplatform", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогОтносительноКоторогоНадоСтроитьИерархию", "directorytobuildhierarchy", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогПроекта", "projectpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогФич", "featurepath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КлиентТестирования", "TestClient", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КлиентыТестирования", "testclienttable", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КлючиОтладки", "testclientdebugkeys", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КоличествоПопытокВыполненияДействия", "numberofattemptstoperformanaction", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КоличествоСекундПоискаОкна", "timetofindwindow", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КомандаОткрытияФичиВТекстовомРедакторе", "commandtoopenfeatureintexteditor", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КомандаСделатьСкриншот", "commandscreenshot", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("НомерСтрокиДляЗапускаСТекущегоШагаСценария", "stepnumber", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ОбновлятьДеревоПриНачалеВыполненияСценария", "updatetreewhenscenariostarts", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ОбновлятьСтатистикуВДереве", "updatestatistics", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ОстановкаПриВозникновенииОшибки", "stoponerror", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ОтчетAllure", "AllureReport", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ОтчетJUnit", "JUnitReport", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ОтчетCucumber", "CucumberReport", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ПаузаПриОткрытииОкна", "pauseonwindowopening", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ПоказыватьНомерСтрокиДереваПриВозникновенииОшибки", "showrownumberonerror", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ПриоритетНастроекПользователя", "priorityofusersettings", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ПриравниватьPendingКFailed", "pendingequalfailed", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ПутьКФайлуДляВыгрузкиСтатусаВыполненияСценариев", "logpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("СниматьСкриншотКаждогоОкна1С", "onerrorscreenshoteverywindow", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("СоздаватьИнструкциюHTML", "htmlcreate", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("СоздаватьИнструкциюMarkdown", "markdowncreate", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("СоздаватьИнструкциюВидео", "videcreate", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("СоздаватьПодкаталогВКаталогеAllureДляЭтойСборки", "createsubfolderforbuild", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("СписокТеговИсключение", "ignoretags", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("СписокТеговОтбор", "filtertags", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ТаймаутДляАсинхронныхШагов", "timeoutforasynchronoussteps", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ТаймаутЗапуска1С", "testclienttimeout", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ТегTreeВключенПоУмолчанию", "thetreetagisenabledbydefault", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("УровеньОтчета1", "reportlevel1", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("УровеньОтчета2", "reportlevel2", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("УровеньОтчета3", "reportlevel3", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("ЯзыкГенератораGherkin", "gherkinlanguage", ТаблицаИмен);

	// Удалить. Обратная совместимость
	ТаблицаИменНоваяСтрока("КаталогOutputAllure", "allurepath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогOutputAllureБазовый", "allurepathbase", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогOutputCucumberJson", "cucumberreportpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогOutputjUnit", "junitpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогOutputИнструкцияHTML", "htmlpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогOutputИнструкцияMarkdown", "markdownpath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогOutputИнструкцияВидео", "videopath", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("КаталогOutputСкриншоты", "outputscreenshot", ТаблицаИмен);
...
&НаСервереБезКонтекста
Процедура ТаблицаИменНоваяСтрока(ИмяRu, ИмяEn, Таб)

	НоваяСтрока = Таб.Добавить();
	НоваяСтрока.ИмяRu = ИмяRu;
	НоваяСтрока.ИмяEn = ИмяEn;
