...
Процедура ЗагрузитьНастройкиИзФайла(Команда)
	...
	Диалог.Заголовок = Локализовать("Выберите файл настроек VA");
...
Функция ОбъединитьПараметрыСУчетомПриоритета(ПараметрыЭталон, ПараметрыПриоритет, Параметры, LangJSON = "Ru")
	...
	Для каждого ключЗначение из ПараметрыЭталон Цикл 
		...
		СтруктураПараметров.Вставить(ключЗначение.Ключ, ?(ключЗначениеЗначение = Неопределено, ключЗначение.Значение, ключЗначениеЗначение));
	КонецЦикла;
...
//	СтрокаПараметров = "StartFeaturePlayer;VBParams=D:\vanessa-automation\tools\JSON\VBParams8314UF.json";
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		Если ИмяФайлаПараметров = Неопределено Тогда
			СообщитьПользователю(Локализовать("Не найден путь к файлу JSON. Параметр: VBParams."));
...
//
&НаСервере
Функция ПолучитьОбщиеНастройкиИзДвоичныхДанных(Данные) Экспорт
