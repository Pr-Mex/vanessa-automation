...
Процедура ЯзыкГенератораGherkinПриИзменении(Элемент)
	...
	ТекущийЯзык = Элемент.ВыделенныйТекст;
...
Функция ОпределитьЯзыкЗапуска(ТекущийЯзык, ДопПараметры)
	  ...
	  Если ЧислоВхожденийЯзыковогоПараметра > 1 Тогда
		Сообщить("Обнаружен больше чем 1 параметр L");
		Возврат ДопПараметры;  
	ИначеЕсли ЧислоВхожденийЯзыковогоПараметра = 1 Тогда
		ПозицияЯзыковогоПараметра = СтрНайти(ДопПараметры,"/L ");
		ЯзыкСтрока = Сред(ДопПараметры,ПозицияЯзыковогоПараметра+3, ПозицияЯзыковогоПараметра+4);
