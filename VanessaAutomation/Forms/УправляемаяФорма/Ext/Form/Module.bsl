...
Процедура ОткрытьФичаФайл(Команда)
	...
	Если ИмяФайлаФичи = Неопределено Тогда
		Если СтрокаФичи.Тип <> "Фича" Тогда
			...
			Если СтрокаФичи = Неопределено Тогда
				СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Строка с фича-файлом не найдена."));
				Возврат;
			КонецЕсли;	
...
Процедура НачатьЗаписьДействийПользователяКнопка(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		СообщитьПользователю("Не найден TestClient для подключения.");
		Возврат;
	КонецЕсли;	 
...
Процедура СнятьПометкуВыполнятьСценарийСЭтогоШага(Команда)
	...
	СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Пометка снята."));
...
Процедура ДанныеКлиентовТестированияПослеУдаления(Элемент)
	...
	НайденныеСтроки = ДанныеКлиентовТестирования.НайтиСтроки(ПараметрыОтбора);
	Если НайденныеСтроки.Количество() = 0 Тогда
		СообщитьПользователю("Данные по основному подключению Testclient перезаполнены.");
		ЗаполнитьДанныеТекущейИнфобазы();
	КонецЕсли;
...
Процедура ПродолжитьВыполнениеШагов(ШагУпал = Ложь, ОписаниеОшибки = "") Экспорт
	...
	Если ТекИД_ШагаВМассиве >= 0 Тогда
		Если (МассивСценариевДляВыполнения[ТекИД_СценарияВМассиве].Шаги.Количество()-1) >= ТекИД_ШагаВМассиве Тогда
			...
			ИдСледующегоШага = Пол��читьИдСледующегоШага(ТекШаг);
		КонецЕсли;	 
	КонецЕсли;	 
...
Функция ПолучитьДанныеКлиентаТестирования(ИмяTestClient) Экспорт
	...
	ТекстСообщения = ПолучитьТекстСообщенияПользователю("Не существует клиента тестирования <%1>.");
...
Процедура НачатьЗаписьДействийПользователя(ОдинКлиентТестирования = Истина) Экспорт 
	...
	ИначеЕсли Элементы.кнПриостановитьЗаписьДействийПользователя.Видимость = Истина Тогда
		...
		Попытка
			...
			СообщитьПользователю("Запись действий пользователя приостановлена.");
		Исключение
			ТекстСообщения = ПолучитьТекстСообщенияПользователю("НачатьЗаписьДействийПользователя:%1");
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ОписаниеОшибки()); 
			СообщитьПользователю(ТекстСообщения);
	...
	ИначеЕсли Элементы.кнПродолжитьЗаписьДействийПользователя.Видимость = Истина Тогда
		...
		Попытка
			...
			СообщитьПользователю("Запись действий пользователя продолжена.");
		Исключение
			ТекстСообщения = ПолучитьТекстСообщенияПользователю("НачатьЗаписьДействийПользователя:%1");
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ОписаниеОшибки()); 
			СообщитьПользователю(ТекстСообщения);
...
Процедура ЗавершитьЗаписьДействийПользователя() Экспорт
	...
	Попытка
		...
		СообщитьПользователю("Запись действий пользователя закончена.");
	Исключение
...
&НаСервере
Функция ПолучитьИДДляОбщегоСохраненияНастроек() Экспорт
...
Процедура Sleep(КолСекунд) Экспорт
    ...
    Попытка
        Если ЭтоLinux Тогда
            Команда = "ping -i " + XMLСтрока(КолСекунд) + " 127.0.0.1 -c 2 > /dev/null";
            КомандаСистемы(Команда);
        Иначе
            Команда = "ping 127.0.0.1 -n " + XMLСтрока(КолСекунд + 1); // +1 нужен, т.к. в реальности происходит на один пинг меньше
            ВыполнитьКомандуОСБезПоказаЧерногоОкна(Команда);
        КонецЕсли;	    
    Исключение
...
Процедура СоздатьШаблоныОбработок() Экспорт
	...
	Если Не ЗначениеЗаполнено(Объект.ВерсияПлатформыДляГенерацииEPF) Тогда
		СообщитьПользователю("Не указан каталог с нужной версией платформы 8.3.10 или выше.");
		Возврат;
	КонецЕсли;	 
...
Функция ПолучитьЗначениеПеременнойИзКонтекста(Знач ИмяПеременной) Экспорт
	Если Найти(ИмяПеременной,"$") = 0 Тогда
		ИмяПеременной = УбратьЗапрещенныеСимволыИзКлюча(ИмяПеременной);
		Если Не ОбъектКонтекст.Свойство(ИмяПеременной) Тогда
			ТекстСообщения = ПолучитьТекстСообщенияПользователю("Переменная <%1> не найдена.");
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ИмяПеременной);
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;	 
...
Функция ВремяИзмененияФайлаКомандаСистемы(ИмяФайла) Экспорт
	...
	Иначе
		Если ЭтоLinux Тогда
			ВызватьИсключение ПолучитьТекстСообщенияПользоват��лю("TODO: Сделать для Linux.");
		КонецЕсли;	 
...
Процедура ЗагрузитьГлобальныеПеременные()
	...
	Если Не НастройкиЮзераНайдены Тогда
		ТекстСообщения = ПолучитьТекстСообщенияПользователю("Ошибка. Не найдены настройки пользователя <%1> в файле <%2>.");
...
Функция ТекущийПользовательОС()
	...
	Иначе
		ТекстСообщения = ПолучитьТекстСообщенияПользователю("Функция <%1> реализована только для Windows.");
...
Процедура ВывестиЗагруженныеНастройкиВЖР(знач НастройкиПользователя,ТекстСообщенийПользователю)
	...
	ЗаписьЖурналаРегистрации("VanessaBehavior.ЗагрузкаГлобальныхПеременных", 
		...
		ПолучитьТекстСообщенияПользователюСервер("Загружены глобальные переменные.",ТекстСообщенийПользователю) + НастройкиСтрокой);
...
Процедура ВосстановитьНастройки(ЭтоLinux)
	...
	Если ТипЗнч(Настройки) = Тип("Структура") Тогда
		...
		Настройки.Свойство("ДобавлятьКИмениСценарияУсловияВыгрузки", Объект.ДобавлятьКИмениСценарияУсловияВыгрузки);
		Настройки.Свойство("ИмяТекущейСборки", Объект.ИмяТекущейСборки);
		
		Настройки.Свойство("ДелатьОтчетВФорматеАллюр", Объект.ДелатьОтчетВФорматеАллюр);
		Настройки.Свойство("КаталогOutputAllure", Объект.КаталогВыгрузкиAllure);
		...
		Настройки.Свойство("КаталогВыгрузкиJUnit", Объект.КаталогВыгрузкиJUnit);
		
		Настройки.Свойство("ДелатьОтчетВФорматеCucumberJson", Объект.ДелатьОтчетВФорматеCucumberJson);
		Настройки.Свойство("КаталогOutputCucumberJson", Объект.КаталогВыгрузкиCucumberJson);
		...
		Настройки.Свойство("КаталогOutputИнструкцияHTML", Объект.КаталогВыгрузкиИнструкцийHTML);
		
		Настройки.Свойство("СоздаватьИнструкциюMarkdown", Объект.СоздаватьИнструкциюMarkdown);
		Настройки.Свойство("КаталогOutputИнструкцияMarkdown", Объект.КаталогВыгрузкиИнструкцийMarkdown);
		
		Настройки.Свойство("СоздаватьИнструкциюВидео", Объект.СоздаватьИнструкциюВидео);
		Настройки.Свойство("КаталогOutputИнструкцияВидео", Объект.КаталогВыгрузкиИнструкцийВидео);
		...
		тзДанныеАллюрМеток=Неопределено;
		Если  Настройки.Свойство("ДанныеАллюрМеток") ТОгда
		Настройки.Свойство("ДанныеАллюрМеток",					тзДанныеАллюрМеток);
		КонецЕсли;
		Если ДанныеАллюрМеток <> Неопределено и ТипЗНч(тзДанныеАллюрМеток) = Тип("ТаблицаЗначений") Тогда
			тзнДанныеАллюрМеток = РеквизитФормыВЗначение("ДанныеАллюрМеток");
			Для каждого Строка из тзДанныеАллюрМеток Цикл
				новаяСтрока = ДанныеАллюрМеток.Добавить();
				ЗаполнитьЗначенияСвойств( новаяСтрока, Строка);
			КонецЦикла;
		   //ЗначениеВДанныеФормы(тзнДанныеАллюрМеток, "ДанныеАллюрМеток");
		КонецЕсли;		
		...
		Настройки.Свойство("КаталогOutputСкриншоты", Объект.КаталогвыгрузкиСкриншотов);
		...
		Настройки.Свойство("ЗаписьВидеоТипОзвучкиTTS", Объект.ЗаписьВидеоТипОзвучкиTTS);
		Настройки.Свойство("ЗаписьВидеоYandexTTSЯзык", Объект.ЗаписьВидеоYandexTTSЯзык);
		Настройки.Свойство("ЗаписьВидеоYandexTTSГолос", Объект.ЗаписьВидеоYandexTTSГолос);
		Настройки.Свойство("ЗаписьВидеоYandexTTSЭмоция", Объект.ЗаписьВидеоYandexTTSЭмоция);
		Настройки.Свойство("ЗаписьВидеоYandexTTSСкорость", Объект.ЗаписьВидеоYandexTTSСкорость);
		Настройки.Свойство("ЗаписьВидеоYandexTTSИдентификаторКаталога", Объект.ЗаписьВидеоYandexTTSИдентификаторКаталога);
		Настройки.Свойство("ЗаписьВидеоYandexTTSOauthToken", Объект.ЗаписьВидеоYandexTTSOauthToken);
		...
		Настройки.Свойство("ЗаписьВидеоСкоростьПроизношения", Объект.ЗаписьВидеоСкоростьПроизношения);
		Настройки.Свойство("ЗаписьВидеоКэшироватьФайлыTTS", Объект.ЗаписьВидеоКэшироватьФайлыTTS);
		Настройки.Свойство("ЗаписьВидеоКэшФайловTTS", Объект.ЗаписьВидеоКэшФайловTTS);
		Настройки.Свойство("ЗаписьВидеоКэшироватьВидеоВставки", Объект.ЗаписьВидеоКэшироватьВидеоВставки);
		Настройки.Свойство("ЗаписьВидеоКэшВидеоВставок", Объект.ЗаписьВидеоКэшВидеоВставок);
		Настройки.Свойство("ЗаписьВидеоДобавлятьСубтитры", Объект.ЗаписьВидеоДобавлятьСубтитры);
		Настройки.Свойство("ЗаписьВидеоПроцентУскорения", Объект.ЗаписьВидеоПроцентУскорения);
		Настройки.Свойство("ЗаписьВидеоФормироватьИнструкциюТипВидео", Объект.ЗаписьВидеоФормироватьИнструкциюТипВидео);
		Настройки.Свойство("ЗаписьВидеоФормироватьИнструкциюТипHTML", Объект.ЗаписьВидеоФормироватьИнструкциюТипHTML);
		Настройки.Свойство("ЗаписьВидеоФормироватьИнструкциюТипMarkdown", Объект.ЗаписьВидеоФормироватьИнструкциюТипMarkdown);
		...
		Настройки.Свойство("ДобавлятьПриНакликиванииМетаИнформацию", Объект.ДобавлятьПриНакликиванииМетаИнформацию);
		...
		Если Настройки.Свойство("ОбновлятьСтатистикуВДереве") Тогда
			Настройки.Свойство("ОбновлятьСтатистикуВДереве", Объект.ОбновлятьСтатистикуВДереве);
		КонецЕсли;	 
		Если Настройки.Свойство("ОбновлятьДеревоПриНачалеВыполненияСценария") Тогда
			Настройки.Свойство("ОбновлятьДеревоПриНачалеВыполненияСценария", Объект.ОбновлятьДеревоПриНачалеВыполненияСценария);
		КонецЕсли;	 
		Настройки.Свойство("ПаузаПриОткрытииОкна", Объект.ПаузаПриОткрытииОкна);
...
Процедура СохранитьНастройки()
	...
	Настройки.Вставить("ДобавлятьКИмениСценарияУсловияВыгрузки", Объект.ДобавлятьКИмениСценарияУсловияВыгрузки);
	Настройки.Вставить("ИмяТекущейСборки", Объект.ИмяТекущейСборки);
	
	Настройки.Вставить("ДелатьОтчетВФорматеАллюр", Объект.ДелатьОтчетВФорматеАллюр);
	Настройки.Вставить("КаталогOutputAllure", Объект.КаталогВыгрузкиAllure);
	...
	Настройки.Вставить("КаталогВыгрузкиJUnit", Объект.КаталогВыгрузкиJUnit);
	
	Настройки.Вставить("ДелатьОтчетВФорматеCucumberJson", Объект.ДелатьОтчетВФорматеCucumberJson);
	Настройки.Вставить("КаталогВыгрузкиCucumberJson", Объект.КаталогВыгрузкиCucumberJson);
	...
	Настройки.Вставить("КаталогВыгрузкиИнструкцийHTML", Объект.КаталогВыгрузкиИнструкцийHTML);
	
	Настройки.Вставить("СоздаватьИнструкциюMarkdown", Объект.СоздаватьИнструкциюMarkdown);
	Настройки.Вставить("КаталогOutputИнструкцияMarkdown", Объект.КаталогВыгрузкиИнструкцийMarkdown);
	
	Настройки.Вставить("СоздаватьИнструкциюВидео", Объект.СоздаватьИнструкциюВидео);
	Настройки.Вставить("КаталогOutputИнструкцияВидео", Объект.КаталогВыгрузкиИнструкцийВидео);
	...
	Настройки.Вставить("КаталогOutputСкриншоты", Объект.КаталогвыгрузкиСкриншотов);
	...
	Настройки.Вставить("ЗаписьВидеоТипОзвучкиTTS", Объект.ЗаписьВидеоТипОзвучкиTTS);
	Настройки.Вставить("ЗаписьВидеоYandexTTSЯзык", Объект.ЗаписьВидеоYandexTTSЯзык);
	Настройки.Вставить("ЗаписьВидеоYandexTTSГолос", Объект.ЗаписьВидеоYandexTTSГолос);
	Настройки.Вставить("ЗаписьВидеоYandexTTSЭмоция", Объект.ЗаписьВидеоYandexTTSЭмоция);
	Настройки.Вставить("ЗаписьВидеоYandexTTSСкорость", Объект.ЗаписьВидеоYandexTTSСкорость);
	Настройки.Вставить("ЗаписьВидеоYandexTTSИдентификаторКаталога", Объект.ЗаписьВидеоYandexTTSИдентификаторКаталога);
	Настройки.Вставить("ЗаписьВидеоYandexTTSOauthToken", Объект.ЗаписьВидеоYandexTTSOauthToken);
	...
	Настройки.Вставить("ЗаписьВидеоСкоростьПроизношения", Объект.ЗаписьВидеоСкоростьПроизношения);
	Настройки.Вставить("ЗаписьВидеоКэшироватьФайлыTTS", Объект.ЗаписьВидеоКэшироватьФайлыTTS);
	Настройки.Вставить("ЗаписьВидеоКэшФайловTTS", Объект.ЗаписьВидеоКэшФайловTTS);
	Настройки.Вставить("ЗаписьВидеоКэшироватьВидеоВставки", Объект.ЗаписьВидеоКэшироватьВидеоВставки);
	Настройки.Вставить("ЗаписьВидеоКэшВидеоВставок", Объект.ЗаписьВидеоКэшВидеоВставок);
	Настройки.Вставить("ЗаписьВидеоДобавлятьСубтитры", Объект.ЗаписьВидеоДобавлятьСубтитры);
	Настройки.Вставить("ЗаписьВидеоПроцентУскорения", Объект.ЗаписьВидеоПроцентУскорения);
	Настройки.Вставить("ЗаписьВидеоФормироватьИнструкциюТипВидео", Объект.ЗаписьВидеоФормироватьИнструкциюТипВидео);
	Настройки.Вставить("ЗаписьВидеоФормироватьИнструкциюТипHTML", Объект.ЗаписьВидеоФормироватьИнструкциюТипHTML);
	Настройки.Вставить("ЗаписьВидеоФормироватьИнструкциюТипMarkdown", Объект.ЗаписьВидеоФормироватьИнструкциюТипMarkdown);
	...
	Настройки.Вставить("ВерсияVA", ПолучитьВерсиюОбработкиСервер());
	...
	Настройки.Вставить("ДобавлятьПриНакликиванииМетаИнформацию", Объект.ДобавлятьПриНакликиванииМетаИнформацию);
	...
	Настройки.Вставить("ОбновлятьСтатистикуВДереве", Объект.ОбновлятьСтатистикуВДереве);
	Настройки.Вставить("ОбновлятьДеревоПриНачалеВыполненияСценария", Объект.ОбновлятьДеревоПриНачалеВыполненияСценария);
	Настройки.Вставить("ПаузаПриОткрытииОкна", Объект.ПаузаПриОткрытииОкна);
	Настройки.Вставить("ТегTreeВключенПоУмолчанию", Объект.ТегTreeВключенПоУмолчанию);
	Настройки.Вставить("ДанныеАллюрМеток", РеквизитФормыВЗначение("ДанныеАллюрМеток"));
...
Функция ПараметрыДляСохраненияНастроек()
	...
	Структура.Вставить("ИДДляОбщег��СохраненияНастроек",ИДДляОбщегоСохраненияНастроек);
...
Процедура ЗаполнитьДеревоФичПродолжение()
	...
	ПоказатьОповещениеПользователя(ПолучитьТекстСообщенияПользователю("Фичи загружены"));
...
Функция ПолучитьСтруктуруПараметров(Стр)
	...
	Для каждого Элем Из Массив Цикл
		...
		Если Поз > 0 Тогда
			...
			Исключение
				СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Не смог получить значение из строки запуска") + ": " + Ключ);
			КонецПопытки;
		Иначе
			Если НЕ ПустаяСтрока(Элем) Тогда 
				...
				Исключение
					СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Не смог получить значение из строки запуска") + ": " + Элем);
				КонецПопытки;
			КонецЕсли;
		КонецЕсли;	 
	КонецЦикла;
...
Функция СтруктураПараметровЗапуска()
	...
	ПараметрыЗапуска.Вставить("ЗаписьВидеоТипОзвучкиTTS", "");
...
Функция СоздатьСоответствиеСАнглийскимЯзыком()
	СтруктураКоммандЗапуска = КоллекцияКомандЗапуска();
	...
	Для Каждого Команда Из СтруктураКоммандЗапуска Цикл
		Если Команда.Value = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		СоответствиеКоммандЗапуска.Вставить(КPоманда.Значение.Р,Команда.Значение.А);
	КонецЦикла;
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	// Для эмуляции запуска в пакетном режиме укажите ниже путь к файлу параметров в формате json
	 //СтрокаЗапуска = "StartFeaturePlayer;VBParams=D:\vanessa-automation\tools\JSON\VBParams8314UF_FastCheck.json";
	Если ПустаяСтрока(СтрокаЗапуска) Тогда
		Возврат;
	КонецЕсли;	
	...
	КаталогПроекта = ?(КаталогПроекта = Неопределено, ПолучитьЗначениеПараметра(ПеременныеСреды, "workspaceRoot"), КаталогПроекта);
	...
	StartFeaturePlayer = ПолучитьЗначениеПараметра(СтруктураПараметров, "StartFeaturePlayer") = Истина 
						Или ПолучитьЗначениеПараметра(ПеременныеСреды, "StartFeaturePlayer") = Истина;
						
	Если StartFeaturePlayer = Истина Тогда
		Объект.ЗапускИзКоманднойСтроки = Истина;
		Объект.СохранятьКонтекстыПередВыполнениемШагов = Ложь;
		
		VBParams = ПолучитьЗначениеПараметра(СтруктураПараметров, "VBParams");
		Если VBParams = Неопределено Тогда
			СообщитьПользователю("Не найден путь к файлу JSON. Параметр: VBParams.");
			ПараметрыФайла = СтруктураПараметровЗапуска();
		Иначе 
			ПараметрыФайла = ПрочитатьСтруктуруИзJSONФайлаКлиент(VBParams);
			Если ПараметрыФайла = Неопределено Тогда
				СообщитьПользователю("Ошибка чтения структуры JSON пропускаем чтение");
			КонецЕсли; 
		КонецЕсли;	 

		LangJSON = ПолучитьЗначениеПараметра(СтруктураПараметров, "Lang");
		
		СтруктураJSON = ОбъединитьПараметрыСУчетомПриоритета(ПараметрыЭталон, ПеременныеСреды, ПараметрыФайла, LangJSON);
		
		Если СтруктураJSON = Неопределено Тогда
			СообщитьПользователю("Ошибка чтения структуры JSON.");
			Возврат;
		КонецЕсли;
		
		ПромСтр = ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогФич"));
		Если ПромСтр = Неопределено Тогда
			СообщитьПользователю("В параметрах JSON не найден каталог Фич.");
			Возврат;
		КонецЕсли;	 
		...
		ЗначКаталогиБиблиотек = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогиБиблиотек");
		...
		Если Объект.ДелатьСкриншотПриВозникновенииОшибки Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputСкриншоты") Тогда
				...
				ТекстСообщения = ПолучитьТекстСообщенияПользователю("Использование устаревшего параметра ""КаталогOutputСкриншоты"". Новый параметр - ""КаталогВыгрузкиСкриншотов""");
				СообщитьПользователю(ТекстСообщения);
				ЗаписатьЛогВЖРОшибка("VanessaAutomation.ЗапускВРежимеКоманднойСтроки", ТекстСообщения);
			Иначе 	
			// Конец Удалить
		...
		Если Объект.СоздаватьИнструкциюHTML Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputИнструкцияHTML") Тогда
				...
				СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Использование устаревшего параметра ""КаталогOutputИнструкцияHTML"". Новый параметр - ""КаталогВыгрузкиИнструкцийHTML"""));
			Иначе 	
			// Конец Удалить
		...
		Если Объект.СоздаватьИнструкциюMarkdown Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputИнструкцияMarkdown") Тогда
				...
				СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Использование устаревшего параметра ""КаталогOutputИнструкцияMarkdown"". Новый параметр - ""КаталогВыгрузкиИнструкцийMarkdown"""));
			Иначе 	
			// Конец Удалить
		...
		Если Объект.СоздаватьИнструкциюВидео Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputИнструкцияВидео") Тогда
				ЗнКаталогВыгрузкиИнструкцийMarkdown = ПолучитьЗначениеПараметра(СтруктураJSON, "КаталогOutputИнструкцияВидео");
				СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Использование устаревшего параметра ""КаталогOutputИнструкцияВидео"". Новый параметр - ""КаталогВыгрузкиИнструкцийВидео"""));
			Иначе 	
			// Конец Удалить
		...
		Объект.ТегTreeВключенПоУмолчанию = ПолучитьЗначениеПараметра(СтруктураJSON, "ТегTreeВключенПоУмолчанию");
		...
		Если ЗнДелатьОтчетВФорматеjUnit = Истина Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputjUnit") Тогда
				...
				СообщитьП��льзователю(ПолучитьТекстСообщенияПользователю("Использование устаревшего параметра ""КаталогOutputjUnit"". Новый параметр - ""КаталогВыгрузкиjUnit"""));					
			Иначе 
			// Конец Удалить	
			...
			Если НЕ ОпределилиКаталогjUnit Тогда
				СообщитьПользователю("В параметрах запуска JSON не определен каталог отчета jUnit.");
				Возврат;
			КонецЕсли;	 
		КонецЕсли;
		...
		Если ЗнДелатьОтчетВФорматеCucumberJson = Истина Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputCucumberJson") Тогда
				...
				СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Использование устаревшего параметра ""КаталогOutputCucumberJson"". Новый параметр - ""КаталогВыгрузкиCucumberJson"""));
			Иначе
			// Конец Удалить	
			...
			Если НЕ ОпределилиКаталогCucumberJson Тогда
				СообщитьПользователю("В параметрах запуска JSON не определен отчета CucumberJson.");
				Возврат;
			КонецЕсли;	 
		КонецЕсли;
		...
		Если СтруктураJSON.Свойство("КаталогOutputAllure") Тогда
			...
			СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Использование устаревшего параметра ""КаталогOutputAllure"". Новый параметр - ""КаталогВыгрузкиAllure"""));
		Иначе
		// Конец Удалить
		...
		Если СтруктураJSON.Свойство("КаталогOutputAllureБазовый") Тогда
			...
			СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Использование устаревшего параметра ""КаталогOutputAllureБазовый"". Новый параметр - ""КаталогВыгрузкиAllureБазовый"""));
		Иначе
		// Конец Удалить	
		...
		Если Объект.ДелатьОтчетВФорматеАллюр Тогда
			Если НЕ ОпределилиКаталогAllure Тогда
				СообщитьПользователю("В параметрах запуска JSON не определен каталог отчета Allure.");
				Возврат;
			КонецЕсли;	 
		КонецЕсли;	 
...
Функция НайтиСледующуюИсполнимуюСтрокуДерева(СтрокаДерева)
	...
	Если СтрокаДерева.СтрокаПараметровШагаВВидеТаблицы = Истина Тогда
		...
		Возврат НайтиСледующуюИсполнимуюСтрокуДерева(СтрокаДерева.Родитель.Строки[ИД+1]);
	КонецЕсли;	 
...
Процедура ВызватьМетод(Обработка, Знач ИмяПроцедуры, МассивАргументов)
	...
	Если МассивАргументов <> Неопределено Тогда
		...
		Для Ккк = 0 По КолАргументов - 1 Цикл
			...
			Команда = Команда + "ЗначенияПараметров[" + XMLСтрока(Ккк) + "]";
	...
	Если Объект.DebugLog Тогда
		...
		Если МассивАргументов <> Неопределено Тогда
			Для Ккк = 0 По КолАргументов - 1 Цикл
				...
				"ЗначенияПараметров[" + XMLСтрока(Ккк) + "]",
				"""" + Строка(МассивАргументов[Ккк]) + """");
			КонецЦикла;	
		КонецЕсли;	 
		Отладка(ПолучитьТекстСообщенияПользователю(ТекстСообщения) + ": " + КомандаОтладка);
	КонецЕсли;	 
...
Функция СформироватьОписаниеОшибки(Знач Стр, ИмяФайла, ИмяПроцедуры,ИнфоОбОшибке)
	...
	Стр = Стр + Символы.ПС 
          ...
          + Символы.ПС + "        " + ПолучитьТекстСообщенияПользователю("ЗаголовокАктивногоОкна") +": " + ЗаголовокАктивногоОкна
          + Символы.ПС + "        " + ПолучитьТекстСообщенияПользователю("ИмяАктивнойФормы") +": " + ИмяАктивнойФормы
          ...
          + Символы.ПС + "        " + ПолучитьТекстСообщенияПользователю("ИмяРаннера") + ": " + ИмяКомпьютера() 
          + Символы.ПС + "        " + ПолучитьТекстСообщенияПользователю("ТипКлиентаТестирования") + ": " + ТипАктивногоКлиентаТестирования() 
          + Символы.ПС + "        " + ПолучитьТекстСообщенияПользователю("СтрокаСоединения") + ": " + СтрокаСоединенияАктивногоКлиентаТестирования() 
          + Символы.ПС + "        " + ПолучитьТекстСообщенияПользователю("ВерсияVA") + ": " + ПолучитьВерсиюОбработкиКлиент() 
...
Функция ПроверитьНаличиеИнструментаДляСозданияСкриншотов()
	...
	Если Объект.СоздаватьИнструкциюHTML Тогда
		Если СокрЛП(Объект.КаталогВыгрузкиИнструкцийHTML) = "" Тогда
			Сообщить(ПолучитьТекстСообщенияПользователю("КаталогВыгрузкиИнструкцийHTML не задан."));
			Возврат Ложь;
		КонецЕсли;  
	КонецЕсли;	 
	Если Объект.СоздаватьИнструкциюMarkdown Тогда
		Если СокрЛП(Объект.КаталогВыгрузкиИнструкцийMarkdown) = "" Тогда
			Сообщить(ПолучитьТекстСообщенияПользователю("КаталогВыгрузкиИнструкцийMarkdown не задан."));
			Возврат Ложь;
		КонецЕсли;  
	КонецЕсли;	 
	Если Объект.ДелатьСкриншотПриВозникновенииОшибки Тогда
		Если СокрЛП(Объект.КаталогвыгрузкиСкриншотов) = "" Тогда
			Сообщить(ПолучитьТекстСообщенияПользователю("КаталогВыгрузкиСкриншотов не задан."));
			Возврат Ложь;
		КонецЕсли;  
	КонецЕсли;	 
	Если ЭтоLinux Тогда 
...
Процедура СделатьСообщениеВсеСценарииОбработаны()
	Если СтатусЗапускаСценариев Тогда
		СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Выполнение сценариев закончено. Ошибок не было."));
	Иначе	
		СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Выполнение сценариев закончено. БЫЛИ ОШИБКИ."));
	КонецЕсли;	 
КонецПроцедуры
...
Процедура ПерейтиКВыполнениюСледующегоСценария()
	...
	Если ПрекратитьВыполнениеВсехСценариев Тогда
		...
		Если Объект.ЗапускИзКоманднойСтроки Тогда
			ВыполнинтьСценарийАссинхронноТаймер();
		КонецЕсли;	 
		Возврат;
	КонецЕсли;	 
...
&НаКлиенте
Процедура ПерейтиКВыполнениюСледующегоШага(ВыполнитьШагЧерезТаймер = Неопределено, НадоОбновитьСтатус��ага = Ложь, ИдСледующегоШага = Неопределено)
...
Процедура ВыполнинтьСценарийАссинхронноТаймер()
	...
	РезультатПрохожденияТестовСценария.Вс��авить("ИДСтроки", ИДСтроки);
	...
	Если НЕ Объект.РежимВыполненияОдногоШага Тогда
		...
		СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Работаю по сценарию") + ": " + МассивСценариевДляВыполнения[ТекИД_СценарияВМассиве].Имя);
	КонецЕсли;	 
...
Процедура ОбновитьСтатусыКнопокСценариев()
	...
	Если Объект.ИдетВыполнениеСценариев Тогда
		Элементы.ВыполнитьСценарии.Заголовок = "Выполняется...";
	Иначе                                       
		Элементы.ВыполнитьСценарии.Заголовок = "";
	КонецЕсли;
	
	Если Объект.ОстановитьВыполнениеШагов Тогда
	    Элементы.ОстановитьСценарии.Заголовок = "Останавливается...";
	Иначе                                       
...
&НаКлиенте
��роцедура УдалитьИсходникиEPF(ИмяФайлаEpf)
	...
	Пока XML.Прочитать() Цикл
		...
		Если XML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			ИначеЕсли ИмяУзла = "goOneLevelUp" Тогда
				Если ТекущаяТЧ <> "" Тогда
					ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице  """ + ТекущаяТЧ + """ я перехожу на один уров��нь вверх");
				КонецЕсли;	 
...
Функция ОткрытьНовыйСеансTestClientИлиПодключитьУжеСуществующийДляНесколькихКлиентов(ДанныеЗапускаКлиентаТестирования,СтрокаОшибки=Неопределено,ДополнительныеПараметры = Неопределено)
	...
	Исключение
		...
		СообщитьПользователю(ПолучитьТекстСообщенияПользователю("TestClient НЕ подключен."));
	    Возврат Ложь;
	КонецПопытки;
...
Функция МакетTestClientПоЯчейкеКакМассив(ЭлементФормы,ДопПараметры = Неопределено)
	...
	Пока Истина Цикл
		...
		Пока Истина Цикл
			...
			Если НЕ Версия8313Или��ыше И ШиринаТаблицы = Неопределено Тогда
...
Функция ПолучитьСостояниеФормыTestClient(ДопПараметры)
	...
	Попытка
		...
		Если ТекущаяФорма = Неопределено Тогда
			СообщитьПользователю(ПолучитьТекстСообщенияПользователю("Активная форма не найдена"));
			Возврат Неопределено;
		КонецЕсли;	 
...
Функция ПолучитьМассивПараметровИзСписокАргументов(СписокАргументов)
	...
	Для Каждого ЭлементМассива Из СписокАргументов Цикл
		...
		Если Элем.Тип = "Строка" Тогда
			МассивПараметров.Добавить(ПолучитьЗначениеИзКонтекстаЕслиЭтоВозможно(Элем.Значение,Ложь));
...
Функция ПолучитьМассивКоординатОкон1С()
	...
	Если Рез <> 0 Тогда
		СообщитьПользователю("Не смог выполнить команду по получению координат на экране всех окон 1С в панели задач.");
		Возврат Неопределено;
	КонецЕсли;	 
...
Процедура СделатьСкриншотыОшибки()
	Если Объект.СниматьСкриншотКаждогоОкна1С Тогда
		...
		Если МассивКоординатОкон1С.Количество() = 0 Тогда
			СообщитьПользователю("Не найдено ни одного окна 1С. Делаю обычный скриншот.");
			СделатьСкриншотОшибкиТекущегоОкна();
			ПерейтиКВыполнениюСледующегоСценария();
			Возврат;
		КонецЕсли;	 
		...
		СообщитьПользователю("Делаю общий скриншот.");
		СделатьСкриншотОшибкиТекущегоОкна();
...
Процедура НастройкаДоступностиВидимостиЭлементов()
	...
	Если НЕ Объект.ЗаписьВидеоКэшироватьФайлыTTS Тогда
		Элементы.ЗаписьВидеоКэшФайловTTS.Видимость = Ложь;
	КонецЕсли;	 
	
	Если НЕ Объект.ЗаписьВидеоКэшироватьВидеоВставки Тогда
		Элементы.ЗаписьВидеоКэшВидеоВставок.Видимость = Ложь;
	КонецЕсли;	 
...
Процедура ИнициализацияПеременныхМодуляФормы()
	...
	ЗаполнитьИмена��лужебныхПеременныхКонтекста();
...
Процедура ОбновитьЗначенияВТаблоПеременных()
	...
	СтрокаТаблоПеременных.Имя = ПолучитьТекстСообщенияПользователю("Контекст");
	...
	СтрокаТаблоПеременных = СтрокиТаблоПеременных.Добавить();	
	СтрокаТаблоПеременных.Имя = ПолучитьТекстСообщенияПользователю("Контекст глобальный");
	СтрокаТаблоПеременных.СлужебнаяСтрока = Истина;
