...
//				СообщитьПользователю(ТекстСообщения);	
		...
		Если ТипЗнч(СтрокаНастроек.Значение) = Тип("Строка")
			...
			Если СтрокаНастроек.Ключ = "ИмяСборки" Тогда
				Объект.ИмяТекущейСборки = СтрокаНастроек.Значение;
				Продолжить;
			ИначеЕсли СтрокаНастроек.Ключ = "ВерсияПлатформы" Тогда
				Продолжить;
			ИначеЕсли СтрокаНастроек.Ключ = "ВыводитьСообщенияВФайл" Тогда
				Продолжить;
			ИначеЕсли СтрокаНастроек.Ключ = "КаталогOutputAllureБазовый" Тогда
				Объект.КаталогВыгрузкиAllure = СтрокаНастроек.Значение;
				Продолжить;
			ИначеЕсли СтрокаНастроек.Ключ = "КаталогOutputjUnit" Тогда
				Объект.КаталогВыгрузкиJUnit = СтрокаНастроек.Значение;
				Продолжить;
			ИначеЕсли СтрокаНастроек.Ключ = "КаталогOutputCucumberJson" Тогда
				Объект.КаталогВыгрузкиCucumberJson = СтрокаНастроек.Значение;
				Продолжить;
			ИначеЕсли СтрокаНастроек.Ключ = "КаталогOutputСкриншоты" Тогда
...
//					СообщитьПользователю(СтрокаОшибки);
		...
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("СписокЗначений") Тогда
		Попытка
			Объект[СтрокаНастроек.Ключ].ЗагрузитьЗначения(СтрокаНастроек.Значение.ВыгрузитьЗначения());
		Исключение
		КонецПопытки;
		Иначе 
			Продолжить;
...
// Не использовать пока режим совметимости меньше 8.3.9
