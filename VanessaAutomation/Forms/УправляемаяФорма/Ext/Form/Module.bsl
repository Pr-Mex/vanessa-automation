...
Процедура ПерейтиКВыполнениюСледующегоШага(ВыполнитьШагЧерезТаймер = Неопределено, НадоОбновитьСтатусШага = Ложь, ИдСледующегоШага = Неопределено)
	...
	Если (Шаги.Количество() - 1) < ТекИД_ШагаВМассиве ИЛИ ТекИД_ШагаВМассиве < 0 Тогда
...
Процедура НастройкаДоступностиВидимостиЭлементов()
	...
	Если Объект.СоздаватьИнструкциюВидео Тогда
		...
		Элементы.КаталогВыгрузкиИнструкцийВидео.Видимость = Истина;
		Элементы.ГруппаТипВидеоИнструкций.Видимость = Истина;
	Иначе
		Элементы.НастройкиВидеоИнструкций.Видимость     = Ложь;
		Элементы.КаталогВыгрузкиИнструкцийВидео.Видимость = Ложь;
	...
	Если Объект.ДелатьСкриншотПриВозникновенииОшибки Тогда
		Элементы.КаталогВыгрузкиСкриншотов.Видимость = Истина;
	...
	Иначе
		Элементы.КаталогВыгрузкиСкриншотов.Видимость = Ложь;
...
Процедура ВывестиСтатистикуНаСервере()
	...
	Если НеРеализованыхСпипетовВсего > 0 Тогда
		СтрокаНеРеализованыхСпипетовВсего = Вычислить("Новый ФорматированнаяСтрока(""("" + Формат(НеРеализованыхСпипетовВсего, ""ЧГ=; ЧН=0"") + "")"", Новый Шрифт(, , Истина), Новый Цвет(255, 92, 0))"); // Жирный, оранжевый
	Иначе
