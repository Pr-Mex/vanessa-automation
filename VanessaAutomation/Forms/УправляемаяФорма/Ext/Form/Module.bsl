...
Процедура ОткрытьФормуГенератораСценариев(Команда)
	...
	ОткрытьФорму(ПолучитьИмяОбработкиVA() + ".Форма.ГенераторТестов", , ЭтотОбъект);
...
Функция СформироватьОписаниеОшибки(Знач Стр, ИмяФайла, ИмяПроцедуры,ИнфоОбОшибке)
	...
	Если ЗначениеЗаполнено(ТекстОшибкиTestClient) Тогда
		Стр = Стр + Символы.ПС + "        ДеталиОшибки:" + Символы.ПС + ТекстОшибкиTestClient;
	Иначе
...
//	КонецЕсли;
	...
	Для каждого СтрокаНастроек Из Настройки Цикл
		...
		Если ИмяКлюча = "version" Или ИмяКлюча = "versionva"
				Или ИмяКлюча = "версияva" Или ИмяКлюча = "lang" Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ТипЗнч(СтрокаНастроек.Значение) = Тип("Строка")
			...
			ИначеЕсли ИмяКлюча = "выводитьсообщенияВФайл" Тогда
				Продолжить;
			ИначеЕсли ИмяКлюча = "каталогutputallureБазовый" Тогда
		...
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("Структура") Тогда
			Для Каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
				...
				ИмяКлючаСтруктуры = НРег(СтрокаСтруктуры.Ключ);
...
Функция ИменаКлючейОбщихНастроек()
	...
	ТаблицаИменНоваяСтрока("СобиратьДанныеОСостоянииАктивнойФормыПриОшибке", "CollectDataStateActiveFormError", ТаблицаИмен);
	ТаблицаИменНоваяСтрока("СобиратьДанныеОСостоянииВсехФормПриОшибке", "CollectDataStatusAllFormsOfError", ТаблицаИмен);
