	...
	Для каждого СтрокаНастроек Из Настройки Цикл
		...
		Если СтрокаНастроек.Ключ = "ВерсияVA" Тогда		
			...
			Если Версия <> СтрокаНастроек.Значение Тогда			
				ПоказатьОповещениеПользователя(Локализовать("Версия загружаемых настроек не соответсвует версии обработки"));	
			КонецЕсли;
			Продолжить;
...
&НаСервере
Функция ПолучитьОбщиеНастройки(СохранениеНастроек = Ложь)
	
	Обработка = РеквизитФормыВЗначение("Объект");
	ОбщиеНастройки = Обработка.СтруктураОбщихНастроек();
