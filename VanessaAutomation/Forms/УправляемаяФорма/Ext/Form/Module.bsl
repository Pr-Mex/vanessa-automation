...
Процедура ВыгрузитьНастройкиВФайлЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	ИмяФайла = "VAParams.json";
	ПолноеИмяФайла = Результат[0] + "\"+ ИмяФайла;
	...
	Для каждого СтрокаНастроек Из Настройки Цикл
		...
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("Структура") Тогда
			// Allure
			Если СтрокаНастроек.Ключ = "Allure" И Настройки.ДелатьОтчетВФорматеАллюр Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроек.Ключ);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
			...
			КонецЕсли;
			// JUnit
			Если СтрокаНастроек.Ключ = "JUnit" И Настройки.ДелатьОтчетВФорматеjUnit Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроек.Ключ);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаНастроекВидео Из СтрокаНастроек.Значение Цикл
					ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроекВидео.Ключ);
			...
			КонецЕсли;
			// Cucumber
			Если СтрокаНастроек.Ключ = "Cucumber" И Настройки.ДелатьОтчетВФорматеCucumberJson Тогда
				ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроек.Ключ);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаНастроекВидео Из СтрокаНастроек.Значение Цикл
					ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроекВидео.Ключ);
...
Функция ПолучитьОбщиеНастройки(СохранениеНастроек = Ложь)
	...
	ЗаполнитьЗначенияСвойств(ОбщиеНастройки, Объект);
	ЗаполнитьЗначенияСвойств(ОбщиеНастройки.Allure, Объект);
	ЗаполнитьЗначенияСвойств(ОбщиеНастройки.JUnit, Объект);
	ЗаполнитьЗначенияСвойств(ОбщиеНастройки.Cucumber, Объект);
	ЗаполнитьЗначенияСвойств(ОбщиеНастройки.ЗаписьВидео, Объект);
