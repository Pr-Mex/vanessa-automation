...
// Файл - Тип: Файл
&НаКлиенте
Процедура ПодключитьСнипетыИзОбработкиКлиент(Файл, ВерсияФайла, БылиОшибки, ДанныеДляСнипетов, АдресХранилища)
	ДопПараметры = Новый Структура;
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	СтрЗапуска = СокрЛП(ПараметрЗапуска);
	//СтрЗапуска = "StartFeaturePlayer;VBParams=C:\temp\111\VBParams8312UF_FastCheck.json";
	Если СтрЗапуска = "" Тогда
		Возврат;
	КонецЕсли;	
...
Процедура ПодключитьОбработкиКлиент(ПомещенныеФайлы,ДополнительныеПараметры)
	...
	Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
		...
		ПодключитьСнипетыИзОбработкиКлиент(Файл, ВерсияФайла, БылиОшибки, ДанныеДляСнипетов, ПомещенныйФайл.Хранение);
	КонецЦикла;	 
...
&НаКлиенте
Процедура УдалитьНедоступныеДляИнтерактивнойРаботыПользователюЭлементыИзМассиваЭлементовTestClient(МассивЭлементов,ДопПараметры)
	МассивИд = Новый Массив;
	...
	Для Каждого ПроверяемыйЭлемент Из МассивЭлементов Цикл
		...
		Попытка
			...
			ИначеЕсли НЕ ПроверяемыйЭлемент.ТекущаяДоступность() Тогда
				МассивИд.Добавить(Ид);
			ИначеЕсли ПроверяемыйЭлемент.ТекущееТолькоПросмотр() Тогда
				Если ПроверяемыйЭлемент.Вид <> ВидДекорацииФормы.Картинка Тогда
					//Если картинка является гиперссылкой, то по ней можно кликать в режиме ТолькоПросмотр.
					МассивИд.Добавить(Ид);
				КонецЕсли;	 
			КонецЕсли;	 
		Исключение
		...
		Если XML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Если ИмяУзла = "click" Тогда
				...
				ИначеЕсли (МассивОбъектов[ИдВМассиве - 1] = "FormField") или (МассивОбъектов[ИдВМассиве - 1] = "FormDecoration") Тогда	
					...
					Если СокрЛП(ИмяОбъекта) <> "" и (Не ЕстьДублиЗаголовковУПоля(МассивСтруктурФорм, ТекущаяФорма, ИмяОбъекта, "Кнопка")) Тогда
