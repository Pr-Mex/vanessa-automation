	...
	Для каждого СтрокаНастроек Из Настройки Цикл
		...
		ИначеЕсли ТипЗнч(СтрокаНастроек.Значение) = Тип("Структура") Тогда
			...
			Если СтрокаНастроек.Ключ = "ВыполнениеСценариев" 
				...
				ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроек.Ключ);
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
				Для каждого СтрокаНастроекВидео Из СтрокаНастроек.Значение Цикл
					ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроекВидео.Ключ);
					ЗаписьJSON.ЗаписатьЗначение(СтрокаНастроекВидео.Значение);			 
				КонецЦикла;
			...
			Если СтрокаНастроек.Ключ = "ЗаписьВидео" И Настройки.СоздаватьИнструкциюВидео Тогда
				...
				Для каждого СтрокаНастроекВидео Из СтрокаНастроек.Значение Цикл
					ЗаписьJSON.ЗаписатьИмяСвойства(СтрокаНастроекВидео.Ключ);
					Если ТипЗнч(СтрокаНастроекВидео.Значение) = Тип("СписокЗначений") Тогда 
						ЗаписьJSON.ЗаписатьНачалоМассива();
						Для каждого СтрокаСписка Из СтрокаНастроекВидео.Значение Цикл
							ЗаписьJSON.ЗаписатьЗначение(СтрокаСписка.Значение);
						КонецЦикла;
						ЗаписьJSON.ЗаписатьКонецМассива();
					Иначе
...
Функция ПолучитьОбщиеНастройки(НастройкиВыгрузки = Ложь, НастройкиЗагрузки = Ложь)
	...
	Если НастройкиВыгрузки Тогда
		//ОбщиеНастройки.ДанныеАллюрМеток = "";	
	КонецЕсли;
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	//СтрокаПараметров = "StartFeaturePlayer;VBParams=D:\vanessa-automation\VAParams.json";
	...
	КаталогПроекта = ?(КаталогПроекта = Неопределено, ПолучитьЗначениеПараметра(НастройкиОперСистемы, "workspaceRoot"), КаталогПроекта);
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		СтруктураJSON = ОбъединитьНастройкиСУчетомПриоритета(ОсновныеНастройки, НастройкиОперСистемы, НастройкиИзФайла, LangJSON);
...
// TODO @Fenixnow: Я не понимаю этого монстра
//
// Параметры:
...
//
&НаКлиенте
Функция ОбъединитьНастройкиСУчетомПриоритета(ОсновныеНастройки, НастройкиОперСистемы, НастройкиИзФайла, LangJSON = "Ru")
	...
	Если НастройкиИзФайла.Свойство("ПриоритетНастроекПользователя") Тогда
		ПриоритетНастроекПользователя = НастройкиИзФайла.ПриоритетНастроекПользователя;
	КонецЕсли;	 
	...
	// В плоскую структуру записываем данные из основной структуры, и сверху при этом накладываем данные из файла.
	СтруктураПараметров = Новый Структура;
	
	Для каждого СтрокаНастроек Из ОсновныеНастройки Цикл 
		...
		Если ТипЗнч(СтрокаНастроек.Значение) = Тип("Структура") Тогда
			...
			Для каждого СтрокаСтруктуры Из СтрокаНастроек.Значение Цикл
				...
				Если Значение = Неопределено Тогда 			
//					НастройкиИзФайла.Свойство(СтрокаСтруктуры.Ключ, Значение);
					Значение = НайтиВСтруктуреРекурсивно(НастройкиИзФайла, СтрокаСтруктуры.Ключ);
					Если Значение = Неопределено И LangJSON = "En" Тогда
						...
						Если Не АнглийскийАналог = Неопределено Тогда
//							НастройкиИзФайла.Свойство(АнглийскийАналог, Значение);
							Значение = НайтиВСтруктуреРекурсивно(НастройкиИзФайла, АнглийскийАналог);
						КонецЕсли;
					КонецЕсли;			
				КонецЕсли;
		...
		Если Значение = Неопределено Тогда 						
//			НастройкиИзФайла.Свойство(СтрокаНастроек.Ключ, Значение);
			Значение = НайтиВСтруктуреРекурсивно(НастройкиИзФайла, СтрокаНастроек.Ключ);
			Если Значение = Неопределено И LangJSON = "En" Тогда
				СоответствиеСАнглийскимЯзыком = СоздатьСоответствиеСАнглийскимЯзыком();
