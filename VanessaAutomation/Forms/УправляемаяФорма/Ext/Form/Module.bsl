...
Процедура ОткрытьФичаФайл(Команда)
	...
	Если ЗначениеЗаполнено(Объект.КомандаОткрытияФичиВТекстовомРедакторе) Тогда
		...
		КомандаДляЗапуска = СтрЗаменить(КомандаДляЗапуска,"%LineNumber%", Формат(НомерСтрокиВФиче, "ЧГ="));
	КонецЕсли;	 
...
Функция НайтиФормыТекущегоОкнаTestClient(ДопПараметры = Неопределено) Экспорт
	...
	Если ОкноПриложения = Неопределено Тогда
		ТекстСообщения = Локализовать("Не получилось определить активное окно TestClient.");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Функция ПолучитьОписаниеСтрокиИзТаблицы(ТабПарам,ИмяТаблицы = Неопределено,ДопПараметры = Неопределено) Экспорт
	...
	Для Ккк = 1 По КолКолонок Цикл
		...
		Если Найти(Ключ,"\n") > 0 Тогда
			...
			КонецЕсли;	 
		КонецЕсли;	 
		Значение = Строка2["Кол"+Строка(Ккк)];
			...
			Для Ккк = (ИдСтроки-1) По Значение.Количество()-1 Цикл
				...
				Если МассивКлючей <> Неопределено Тогда
					Для Каждого Элем Из МассивКлючей Цикл
						ИдКолонки = ИдКолонки + 1;
						НовоеЗначение.Область(ИдСтроки,ИдКолонки,ИдСтроки,ИдКолонки).Текст = СтрокаЗначение["Кол" + Строка(ИдКолонки)];
					КонецЦикла;	
				КонецЕсли;	 
				...
				Иначе
					Если ТипЗнч(СтрокаЗначение) = Тип("Структура") Тогда
						Для СчетчикКолонок = 1 По СтрокаЗначение.Количество() Цикл
							НовоеЗначение.Область(ИдСтроки,СчетчикКолонок,ИдСтроки,СчетчикКолонок).Текст = СтрокаЗначение["Кол" + Строка(СчетчикКолонок)];
						КонецЦикла;	
					Иначе	
...
Процедура Sleep(КолСекунд) Экспорт
    ...
    Попытка
        ...
        Иначе
            Команда = "ping 127.0.0.1 -n " + Строка(КолСекунд + 1); // +1 нужен, т.к. в реальности происходит на один пинг меньше
            ВыполнитьКомандуОСБезПоказаЧерногоОкна(Команда);
        КонецЕсли;	    
    Исключение
...
Процедура ВычислитьPIDЭтогоСеансаСлужебный() Экспорт
	Попытка
        ...
        oChildProcess = oService.Get("Win32_Process.Handle=" + Строка(oApp.ProcessID));
        
        PIDЭтогоСеанса = Строка(oChildProcess.ParentProcessID);
...
Функция ПреобразоватьТабличныйДокументВМассивСтруктур(ТабДок) Экспорт
	...
	Для Ррр = 1 По ТабДок.ШиринаТаблицы Цикл
		СтрокаТаблицы.Вставить("Кол" + Строка(Ррр),ТабДок.Область(1,Ррр,1,Ррр).Текст);
	КонецЦикла;	
	...
	Для Ккк = 2 По ТабДок.ВысотаТаблицы Цикл
		СтрокаТаблицы = Новый Структура;
		Для Ррр = 1 По ТабДок.ШиринаТаблицы Цикл
			СтрокаТаблицы.Вставить("Кол" + Строка(Ррр),ТабДок.Область(Ккк,Ррр,Ккк,Ррр).Текст);
		КонецЦикла;	
		Массив.Добавить(СтрокаТаблицы);
	КонецЦикла;	
...
Функция ПолучитьТаблицуGherkinИзМакета(Макет) Экспорт
	...
	Для Ккк = 1 По Макет.ВысотаТаблицы Цикл
		...
		Для Ррр = 1 По Макет.ШиринаТаблицы Цикл
			СтрокаТаблицы.Вставить("Кол" + Формат(Ррр, "ЧГ="),Макет.Область(Ккк,Ррр,Ккк,Ррр).Текст);
		КонецЦикла;	
	...
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		...
		Для каждого Элем Из Объект.КаталогиБиблиотек Цикл
			...
			Если НЕ ПолучитьЗначениеПоРанееСобраннойИнформации(ДополнительныеПараметры, "КаталогиБиблиотек" + НПП, "Существует") Тогда
				...
				СообщениеПользователю.Поле = СтрЗаменить(СообщениеПользователю.Поле, "%1", Формат(НПП, "ЧГ="));
...
Функция ПрочитатьПеременныеСреды() 
	...
	Попытка
        ...
        КонецЕсли;
    Исключение
        ТекстСообщения = Локализовать("ПрочитатьПеременныеСреды:%1");
        ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ОписаниеОшибки());
        СообщитьПользователю(ТекстСообщения);
    КонецПопытки;
...
Функция ОбъединитьПараметрыСУчетомПриоритета(ПараметрыЭталон, ПараметрыПриоритет, Параметры, LangJSON = "Ru")
	...
	Если Параметры.Свойство("ПриоритетНастроекПользователя") Тогда
		ПриоритетНастроекПользователя = Параметры.ПриоритетНастроекПользователя;
		//если ПриоритетНастроекПользователя = Истина, тогда если в файле josn настройка не указана, то будет взята настройка пользователя,
	...
	Для каждого ключЗначение из ПараметрыЭталон Цикл 
		...
		Если ключЗначениеЗначение = Неопределено и ПриоритетНастроекПользователя = Истина Тогда
			Если Объект.Свойство(ключЗначение.Ключ) Тогда
				ТипЗначения = ТипЗнч(Объект[ключЗначение.Ключ]);
				Если ТипЗначения = Тип("Булево")
				Или  ТипЗначения = Тип("Строка")
				Или  ТипЗначения = Тип("Число")
				Тогда
					ключЗначениеЗначение = Объект[ключЗначение.Ключ];
				КонецЕсли;	 
			КонецЕсли;	 
		КонецЕсли;	 
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		Если Объект.СоздаватьИнструкциюHTML Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputИнструкцияHTML") Тогда
				...
				СообщитьПользователю(Локализовать("Использование устаревшего параметра ""КаталогOutputИнструкцияHTML"". Новый параметр - ""КаталогВыгрузкиИнструкцийHTML"""));
			Иначе 	
			// Конец Удалить
		...
		Если Объект.СоздаватьИнструкциюMarkdown Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputИнструкцияMarkdown") Тогда
				...
				СообщитьПользователю(Локализовать("Использование устаревшего параметра ""КаталогOutputИнструкцияMarkdown"". Новый параметр - ""КаталогВыгрузкиИнструкцийMarkdown"""));
			Иначе 	
			// Конец Удалить
		...
		Если Объект.СоздаватьИнструкциюВидео Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputИнструкцияВидео") Тогда
				...
				СообщитьПользователю(Локализовать("Использование устаревшего параметра ""КаталогOutputИнструкцияВидео"". Новый параметр - ""КаталогВыгрузкиИнструкцийВидео"""));
			Иначе 	
			// Конец Удалить
		...
		Если ЗнДелатьОтчетВФорматеjUnit = Истина Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputjUnit") Тогда
				...
				СообщитьПользователю(Локализовать("Использование устаревшего параметра ""КаталогOutputjUnit"". Новый параметр - ""КаталогВыгрузкиjUnit"""));					
			Иначе 
			// Конец Удалить	
		...
		Если ЗнДелатьОтчетВФорматеCucumberJson = Истина Тогда
			...
			Если СтруктураJSON.Свойство("КаталогOutputCucumberJson") Тогда
				...
				СообщитьПользователю(Локализовать("Использование устаревшего параметра ""КаталогOutputCucumberJson"". Новый параметр - ""КаталогВыгрузкиCucumberJson"""));
			Иначе
			// Конец Удалить	
		...
		Если СтруктураJSON.Свойство("КаталогOutputAllure") Тогда
			...
			СообщитьПользователю(Локализовать("Использование устаревшего параметра ""КаталогOutputAllure"". Новый параметр - ""КаталогВыгрузкиAllure"""));
		Иначе
		// Конец Удалить
		...
		Если СтруктураJSON.Свойство("КаталогOutputAllureБазовый") Тогда
			...
			СообщитьПользователю(Локализовать("Использование устаревшего параметра ""КаталогOutputAllureБазовый"". Новый параметр - ""КаталогВыгрузкиAllureБазовый"""));
		Иначе
		// Конец Удалить	
	...
	ИначеЕсли ПолучитьЗначениеПараметра(СтруктураПараметров, "GetStepsCache") = Истина Тогда
		VBParams = ПолучитьЗначениеПараметра(СтруктураПараметров, "VBParams");
		Если VBParams = Неопределено Тогда
			ВызватьИсключение Локализовать("Не найден путь к файлу JSON. Параметр: VBParams."); 
		Иначе 
			ПараметрыФайла = ПрочитатьСтруктуруИзJSONФайлаКлиент(VBParams);
			Если ПараметрыФайла = Неопределено Тогда
...
Процедура ВызватьМетод(Обработка, Знач ИмяПроцедуры, МассивАргументов)
	...
	Если МассивАргументов <> Неопределено Тогда
		...
		Для Ккк = 0 По КолАргументов - 1 Цикл
			...
			Команда = Команда + "ЗначенияПараметров[" + Формат(Ккк, "ЧГ=") + "]";
	...
	Если Объект.DebugLog Тогда
		...
		Если МассивАргументов <> Неопределено Тогда
			Для Ккк = 0 По КолАргументов - 1 Цикл
				...
				"ЗначенияПараметров[" + Формат(Ккк, "ЧГ=") + "]",
...
Процедура ДобавитьВЛогИнструкцииHTMLВыполненияШага(СтруткураШага = Неопределено)
	...
	ТД.ЗаписатьСтроку("ИДШага=" + Формат(ТекИД_ШагаВМассиве, "ЧГ="));
...
Процедура СнятьСкриншотШагаЕслиЭтоНеобходимо(Шаг,РезультатПрохожденияТестовСценария)
	...
	Если НЕ ФайлСуществуетКомандаСистемы(Объект.КаталогOutputСкриншоты) Тогда
		ТекстСообщения = Локализовать("Не найден каталог: %1");
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",Объект.КаталогOutputСкриншоты);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;	 
	...
	Если СтрокаШага.ШагСПараметрамиВТаблице = Истина Тогда
		...
		Для каждого СтрокиТаблицы Из ПодчиненныеСтроки Цикл
			...
			Для каждого Колонка Из СтрокиТаблицы.ПараметрыТаблицы Цикл
				...
				СтруктураПарамТаблица.Вставить("Кол" + Строка(НомерКолонки), Значение);
			КонецЦикла;
	...
	Исключение
		...
		Иначе
			...
			Если Объект.ДелатьЛогВыполненияСценариевВТекстовыйФайл Тогда
				...
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ТекШаг.Имя);
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%2",Строка(ТекШаг.ИдСтроки));
				ЗаписатьСтрокуВТекстовыйЛог(ТекстСообщения);
...
Процедура ДобавитьВЛогИнструкцииHTMLНачалоСценария(СтруктураСценария)
	...
	ТД.ЗаписатьСтроку("ИдСценария=" + Строка(ТекИД_СценарияВМассиве));
...
Функция СформироватьАдресЯчейки(НомерСтроки, НомерКолонки)
	Возврат "R" + Формат(НомерСтроки, "ЧГ=") + "C" + Формат(НомерКолонки, "ЧГ="); 
КонецФункции	
			...
			Если СтрокаДерева.ШагСПараметрамиВТаблице = Истина Тогда
				...
				Для каждого СтрокаТаблицы Из ПодчиненныеСтроки Цикл
					...
					Для каждого Колонка Из СтрокаТаблицы.ПараметрыТаблицы Цикл
						...
						СтрокаПарамТаблица.Вставить("Кол" + Формат(НомерКолонки, "ЧГ="), Значение);
					КонецЦикла;
...
Процедура СделатьСкриншотыОшибкиКаждогоОкна1СSikuliX()
	...
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", Формат(ТекКоординаты.x, "ЧГ=")); 
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%2", Формат(ТекКоординаты.y, "ЧГ=")); 
	СообщитьПользователю(ТекстСообщения);
	
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("СлужебныйВызов",Истина);
	ВыполнитьSikuliСкрипт(Объект.КаталогИнструментов + "\tools\Sikuli\ClickCoordinates.sikuli --args " + Строка(ТекКоординаты.x) + " " + Строка(ТекКоординаты.y) , -1, Истина,,ДопПараметры);
