...
Функция ЭтоПанельРазделов(Стр) Экспорт
	Если НРег(Стр) = НРег("Панель разделов") Тогда
		Возврат Истина;
	ИначеЕсли НРег(Стр) = НРег("Section panel") Тогда
		Возврат Истина;
	КонецЕсли;	 
...
Функция ЭтоПанельОткрытых(Стр) Экспорт
	Если НРег(Стр) = НРег("Панель открытых") Тогда
		Возврат Истина;
	ИначеЕсли НРег(Стр) = НРег("Open items panel") Тогда
		Возврат Истина;
	КонецЕсли;	 
...
Функция ЭтоМенюФункций(Стр) Экспорт
	Если НРег(Стр) = НРег("Меню функций") Тогда
		Возврат Истина;
	ИначеЕсли НРег(Стр) = НРег("Functions menu") Тогда
		Возврат Истина;
...
Процедура ДобавитьСнипетыСервер(МассивСнипетовИзОбработки, ИмяФайла, ВерсияФайла, БылиОшибки,СтруктураПараметров)
	...
	Для каждого Снипет Из МассивСнипетовИзОбработки Цикл
		ДобавитьСнипет(Снипет, ИмяФайла, ВерсияФайла, БылиОшибки, ТзнТаблицаИзвестныхStepDefinition,СтруктураПараметров);
	КонецЦикла;
...
Функция ПолучитьФайлыПеревода()
	...
	ИмяФайла = ДополнитьСлешВПуть(Объект.КаталогИнструментов) + "locales/Locales.epf";
	...
	Если СокрЛП(Объект.КаталогИнструментов) = "" Тогда
		...
		Если ЕстьКонстантаВКонфигурации Тогда
			ПутьКVanessaBehavior     = ПолучитьЗначениеКонстантыСервер("ПутьКVanessaBehavior");
			ФайлПутьКVanessaBehavior = Новый Файл(ПутьКVanessaBehavior);
			ИмяФайла                 = ДополнитьСлешВПуть(ФайлПутьКVanessaBehavior.Путь) + "locales/Locales.epf";
		КонецЕсли;	 
	КонецЕсли;	 
	...
	Соответствие.Вставить("en",ФормаОбработки.ПолучитьМакетОбработки("en"));
	Соответствие.Вставить("ro",ФормаОбработки.ПолучитьМакетОбработки("ro"));
	Соответствие.Вставить("de",ФормаОбработки.ПолучитьМакетОбработки("de"));
	Соответствие.Вставить("lv",ФормаОбработки.ПолучитьМакетОбработки("lv"));
	Соответствие.Вставить("it",ФормаОбработки.ПолучитьМакетОбработки("it"));
	Соответствие.Вставить("pl",ФормаОбработки.ПолучитьМакетОбработки("pl"));
	Соответствие.Вставить("vi",ФормаОбработки.ПолучитьМакетОбработки("vi"));
...
Функция ТекстСообщенийПользователю()
	...
	Возврат ФормаОбработки.ДанныеПереводаСообщений();
КонецФункции	 
