...
Процедура ЗапуститьSikuliXСевер(ВанессаФорма,ДопПараметры) Экспорт
	...
	Для Каждого ТекКаталогСкриптовДляОбработки Из МассивКаталоговДляОбработки Цикл
		...
		ОставитьТолькоФайлыСРасширением(СписокКаталогов,".sikuli");
		
		//Файлы = НайтиФайлы(ТекКаталогСкриптовДляОбработки,"*.sikuli",Истина);
		Для Каждого ФайлКаталогСкрипта Из СписокКаталогов Цикл
			Если НРег(ФайлКаталогСкрипта.Значение.ПолноеИмя) = НРег(КаталогНовогоСкрипта) Тогда
				Продолжить;
			КонецЕсли;	 
...
Функция ДанныеСкрипта(КаталогСкрипта,ОбщаяСекцияИмпорт)
	...
	ОставитьТолькоФайлыСРасширением(СписокФайлов,".png");
	...
	СписокФайлов    = Новый СписокЗначений;
	Ванесса.НайтиФайлыКомандаСистемы(КаталогСкрипта, СписокКаталогов, СписокФайлов, Ложь);
	ОставитьТолькоФайлыСРасширением(СписокФайлов,".py");
...
&НаКлиенте
Процедура ОставитьТолькоФайлыСРасширением(СписокФайлов,Расширение)
