...
Процедура ЗапуститьSikuliXСевер(ВанессаФорма,ДопПараметры) Экспорт
	...
	Ванесса.ЗаписатьЛогВЖРИнформация("ЗапуститьSikuliXСевер.ПутьКСкрипту", ИмяФайлаСкрипта);
	...
	Ванесса.ЗаписатьЛогВЖРИнформация("ЗапуститьSikuliXСевер.СтрокаКоманды", СтрокаКоманды);
	Рез = Ванесса.ВыполнитьSikuliСкрипт(СтрокаКоманды, 0, ,ОписаниеОшибки,ДопПараметрыКоманды);
	
	Если НЕ ПустаяСтрока(ОписаниеОшибки) Тогда
		Ванесса.ЗаписатьЛогВЖРИнформация("ЗапуститьSikuliXСевер.ОписаниеОшибки", ОписаниеОшибки);
	КонецЕсли;	 
	...
	Если Рез = 0 Тогда
		...
		Ванесса.ЗаписатьЛогВЖРИнформация("ЗапуститьSikuliXСевер.ТаймаутЗапуска", ТекстСообщения);
		...
		Для СчетчикСекунд = 1 По КоличествоСекундОжиданияОтвета Цикл
			Если Ванесса.ФайлСуществуетКомандаСистемы(ИмяФайлаОтвета) Тогда
				Попытка
					...
					Ванесса.ЗаписатьЛогВЖРИнформация("ЗапуститьSikuliXСевер.Ответ", Ответ);
				Исключение
					Ванесса.sleep(1);
					Продолжить;
				КонецПопытки;
				...
				Если Ответ = "sikulix server started" Тогда
					...
					Ванесса.ЗаписатьЛогВЖРИнформация("ЗапуститьSikuliXСевер.Запуск", Ванесса.ПолучитьТекстСообщенияПользователю("SikuliX сервер успешно запущен."));
					Прервать;
				КонецЕсли;	 
			КонецЕсли;	 
	...
	Иначе
		...
		Ванесса.ЗаписатьЛогВЖРИнформация("ЗапуститьSikuliXСевер.ОшибкаЗапуска", ТекстСообщения);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;	 
...
Процедура ОстановитьSikuliXСервер() Экспорт
	...
	Ванесса.ЗаписатьЛогВЖРИнформация("ЗапуститьSikuliXСевер.ОстановкаSikuliXСервера", "");
КонецПроцедуры 
