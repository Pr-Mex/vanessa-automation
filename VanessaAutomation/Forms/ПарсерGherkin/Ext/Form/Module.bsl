...
Функция СкопироватьМассивТегов(Источник)
	...
	Для Каждого Элем Из Источник Цикл
		Тег = Новый Структура;
		Тег.Вставить("Тег", НРег(Элем));
		Тег.Вставить("НомерСтроки", 0);
		Тег.Вставить("ТегИсходный", Элем);
...
Функция СкопироватьМассивКомментариев(Источник)
	...
	Для Каждого Элем Из Источник Цикл
		...
		Данные.Вставить("Комментарий", Элем);
...
Процедура ДобавитьФичу(ЭлементыРодителя, ДанныеФичи, ФайлФичи, ИдСтроки, КешСнипетовНаКлиенте, КаталогиФичаФайлов, СтруктураПараметров)
	...
	Если ДанныеФичи.feature.Свойство("description") Тогда
		СтрокаФичи.ПроизвольныеЗначения.Вставить("МассивКомментариев", СкопироватьМассивТегов(ДанныеФичи.feature.description));
	КонецЕсли;	 
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	Если Источник = "AddIn.GherkinParser" Тогда
		Если Событие = "PARSING_FINISHED" Тогда
			СтруктураПараметров = Новый Структура;
			СтруктураПараметров.Вставить("КаталогФич", Ванесса.Объект.КаталогФич);
			ПолученРезультатЧтения(Данные, Новый Массив, СтруктураПараметров);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция КаталогФичиВДереве(ИмяФайла, ЭлементыРодителя, КаталогиФичаФайлов, ОсновнойКаталогФич, ИмяКаталога, ИдСтроки)
	...
	Если ПутьКФиче = ОсновнойКаталогФич Тогда
		ЭлементКаталогФичи = КаталогиФичаФайлов[ПутьКФиче];
		Если ЭлементКаталогФичи = Неопределено Тогда
