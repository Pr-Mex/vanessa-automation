...
Процедура КонструкцияШагаПрочиеПриИзменении(Элемент)
	...
	Иначе
		...
		Если Найти(ТекстШаблона, "%4") = 0 Тогда
			КонструкцияШагаПрочиеТекст = Ванесса._СтрШаблон(ТекстШаблона, ТочкаX,ТочкаY);
		Иначе
			КонструкцияШагаПрочиеТекст = Ванесса._СтрШаблон(ТекстШаблона, ТочкаX,ТочкаY,ОбластьWidth,ОбластьHeight);
		КонецЕсли;
...
Процедура КонструкцияШагаВБуфер(Команда)   
	...
	Если Не ПустаяСтрока(КонструкцияШага) Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопроса", ЭтаФорма, Новый Структура());
