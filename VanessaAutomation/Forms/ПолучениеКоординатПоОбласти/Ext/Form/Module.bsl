...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если Источник = "AddIn.WindowsControl" И Событие = "MOUSEHOOK" Тогда
		...
		ИначеЕсли ДанныеСобытия.event = "LBUTTONUP" Тогда
			...
			Ванесса.ВнешняяКомпонентаДляСкриншотов.НачатьВызовПрекратитьМониторингСобытий(Новый ОписаниеОповещения());
			ФигураПрямоугольник();
...
Процедура МониторингСобытийНачать(Команда)
	Ванесса.ВнешняяКомпонентаДляСкриншотов.НачатьВызовНачатьМониторингСобытий(Новый ОписаниеОповещения());
КонецПроцедуры
...
Функция ПолучитьНастройкиРисования(ИспользуемКнопки = Ложь)
	...
	ВремСтруктура.Вставить("fontSize", 12);
