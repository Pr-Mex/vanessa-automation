...
Процедура СкачатьКешОзвучки(Команда)
	...
	Если СписокКаталогов.Количество() > 0 ИЛИ СписокФайлов.Количество() > 0 Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеОтветаНаВопросОбОчисткеКаталога", ЭтаФорма, ДопПараметры);
...
Функция НайденКешОзвучки()
	...
	Если НЕ Ванесса.ВКешЕстьВсеMp3ДляДаннойФичи(ДанныеКешОзвучкиИнтерактивнойСправки, ПроверяемыйФичаФайл) Тогда
		Возврат Ложь;
	КонецЕсли;	 
...
Функция КаталогИнтерактивнойСправки()
	...
	Если НЕ ЗначениеЗаполнено(ИмяКаталога) ИЛИ НЕ Ванесса.ФайлСуществуетКомандаСистемы(ИмяКаталога) Тогда
		ИмяКаталога = Ванесса.ДополнитьСлешВПуть(Ванесса.КаталогИнструментов) + "HelpTTS";
...
&НаКлиенте
Процедура ПослеОтветаНаВопросОбОчисткеКаталога(Результат, ДопПараметры) Экспорт
