...
Процедура ВосстановитьНастройки()
	...
	Если ТипЗнч(Настройки) = Тип("Структура") Тогда
		...
		Настройки.Свойство("КаталогOutputAllure", КаталогВыгрузкиAllure);
		Настройки.Свойство("КаталогOutputjUnit", КаталогВыгрузкиJUnit);
		Настройки.Свойство("КаталогOutputCucumberJson", КаталогВыгрузкиCucumberJson);
		...
		Если Настройки.Свойство("ВыполнятьШагиАсинхронно") Тогда
			Настройки.Свойство("ВыполнятьШагиАсинхронно", ВыполнятьШагиАсинхронно);
		КонецЕсли;	 
...
Процедура СохранитьНастройки()
	...
	Настройки.Вставить("КаталогOutputAllure", КаталогВыгрузкиAllure);
	Настройки.Вставить("КаталогOutputjUnit", КаталогВыгрузкиJUnit);
	Настройки.Вставить("КаталогOutputCucumberJson", КаталогВыгрузкиCucumberJson);
	...
	Настройки.Вставить("ВыполнятьШагиАсинхронно", ВыполнятьШагиАсинхронно);
...
Функция ПолучитьСтруктуруПараметров(Стр)
	...
	Для каждого Элем Из Массив Цикл
		...
		Если Поз > 0 Тогда
			...
			Исключение
				Сообщить("Не вышло получить значение из строки запуска: " + Ключ);
			КонецПопытки;
		Иначе	
			Попытка
				Результат.Вставить(Элем,Истина);
			Исключение
				Сообщить("Не вышло получить значение из строки запуска: " + Элем);
			КонецПопытки;
		КонецЕсли;	 
	КонецЦикла;
...
Функция ПрочитатьОбщиеНастройкиИзФайла(ПолноеИмяФайла)
	...
	Исключение
		ЧтениеJSON.Закрыть();
		ТекстСообщения = "Не удалось прочитать файл настроек JSON.";
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	//СтрЗапуска = "StartFeaturePlayer;VBParams=C:\Commons\rep\vanessa-automation\tools\JSON\VBParams82OF.json";
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		СтрутктураJSON = ПрочитатьОбщиеНастройкиИзФайла(VBParams);
		...
		Если ЗначениеЗаполнено(ЗнКаталогОтносительноКоторогоНадоСтроитьИерархию) Тогда
			КаталогОтносительноКоторогоНадоСтроитьИерархию = ЗнКаталогОтносительноКоторогоНадоСтроитьИерархию;
		КонецЕсли;	 
		...
		Если ЗначениеЗаполнено(ЗнУровеньОтчета1) Тогда
			УровеньОтчета1 = ЗнУровеньОтчета1;
		КонецЕсли;	 
		...
		Если ЗначениеЗаполнено(ЗнУровеньОтчета2) Тогда
			УровеньОтчета2 = ЗнУровеньОтчета2;
		КонецЕсли;	 
		...
		Если ЗначениеЗаполнено(ЗнУровеньОтчета3) Тогда
			УровеньОтчета3 = ЗнУровеньОтчета3;
		КонецЕсли;	 
		...
		Если ЗнДелатьОтчетВФорматеАллюр = "Истина" Тогда
			...
			Если ЗнКаталогOutputAllure <> Неопределено Тогда
				...
				Если НЕ Файл.Существует() Тогда
					...
					Возврат;
				КонецЕсли;	 
				КаталогВыгрузкиAllure      = ЗнКаталогOutputAllure;
			...
			Если ЗнКаталогOutputAllureБазовый <> Неопределено Тогда
				...
				КаталогВыгрузкиAllure      = ЗнКаталогOutputAllureБазовый;
				ДелатьОтчетВФорматеАллюр = Истина;
				ОпределилиКаталогAllure  = Истина;
			КонецЕсли;	 
		...
		Если ЗнДелатьОтчетВФорматеjUnit = "Истина" Тогда
			...
			Если ЗнКаталогOutputjUnit <> Неопределено Тогда
				...
				КонецЕсли;	 
				КаталогВыгрузкиJUnit       = ЗнКаталогOutputjUnit;
		...
		Если ЗнДелатьОтчетВФорматеCucumberJson = "Истина" Тогда
			...
			Если ЗнКаталогOutputCucumberJson <> Неопределено Тогда
				...
				КонецЕсли;	 
				КаталогВыгрузкиCucumberJson       = ЗнКаталогOutputCucumberJson;
		...
		Если ВыполнитьСценарии = "Истина" Тогда
			...
			Если ЗавершитьРаботуСистемы = "Истина" Тогда
				ЗавершитьРаботуСистемыПослеВыполненияВсехСценариев = Истина;
			КонецЕсли;	 
...
Процедура ПолучитьУжеСуществующиеСнипетыИзОбработокОФ(Знач КаталогФич,ТаблицаКонтекстовОбработок,ТаблицаИзвестныхStepDefinition,ТаблицаВерсийEPF,КонтекстVanessaBehavoir)
...
Процедура СделатьGenerateEpf(СтруктураПараметров)
	...
	ПолучитьУжеСуществующиеСнипетыИзОбработокОФ(КаталогФич,ТаблицаКонтекстовОбработок,ТаблицаИзвестныхStepDefinition,ТаблицаВерсийEPF,КонтекстVanessaBehavoir);
	Для каждого Элем Из КаталогиБиблиотек Цикл
		ПолучитьУжеСуществующиеСнипетыИзОбработокОФ(Элем.Значение,ТаблицаКонтекстовОбработок,ТаблицаИзвестныхStepDefinition,ТаблицаВерсийEPF,КонтекстVanessaBehavoir);
	КонецЦикла;
	...
	Для каждого ИмяФайлаФичи Из МассивФич Цикл
		...
		Снипеты = СнипетыПолучитьСнипетыПоШагамОФ(Шаги,ДвДанныеvbFeatureReader);
		ЗаполнитьПроцедуруПолучитьСписокТестовОФ(СтруктураОписанияEpf.ТелоМодуля,Снипеты,ДвДанныеvbFeatureReader,ИмяФайлаФичи,СоздаватьЗакоментированныйКодДляПереиспользуемыхПроцедур);
		Для каждого ЭлемСнипет Из Снипеты Цикл
			ДобавитьStepDefinitionВТекстМодуляОФ(СтруктураОписанияEpf,ЭлемСнипет.StepDefinition,ЭлемСнипет.Шаг,ШагСтрокДляМодуля,ЭлемСнипет.АдресСнипета,ГенерироватьУФ,ДвДанныеvbFeatureReader,СоздаватьЗакоментированныйКодДляПереиспользуемыхПроцедур);
		КонецЦикла;
...
Функция СнипетыПолучитьСнипетыПоШагамОФ(Шаги,ДвДанныеvbFeatureReader)
	FeatureReader = ПолучитьИнструментПарсерФич();
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("КешСнипетов",Новый Соответствие);
	СтруктураПараметров.Вставить("СтрокаПробелов",СтрокаПробелов);
	Возврат FeatureReader.СнипетыПолучитьСнипетыПоШагам(Шаги,СтруктураПараметров);
КонецФункции

Процедура ЗаполнитьПроцедуруПолучитьСписокТестовОФ(ТелоМодуля,Снипеты,ДвДанныеvbFeatureReader,ИмяФайлаФичи,СоздаватьЗакоментированныйКодДляПереиспользуемыхПроцедур)
	FeatureReader = ПолучитьИнструментПарсерФич();
	FeatureReader.ЗаполнитьПроцедуруПолучитьСписокТестов(ТелоМодуля,Снипеты,ЭтоУФОбычнойФормы,ИмяФайлаФичи,СоздаватьЗакоментированныйКодДляПереиспользуемыхПроцедур);
КонецПроцедуры

Процедура ДобавитьStepDefinitionВТекстМодуляОФ(СтруктураОписанияEpf,StepDefinition,Шаг,ШагСтрокДляМодуля,АдресСнипета,ГенерироватьУФ,ДвДанныеvbFeatureReader,СоздаватьЗакоментированныйКодДляПереиспользуемыхПроцедур)
	FeatureReader = ПолучитьИнструментПарсерФич();
	FeatureReader.ДобавитьStepDefinitionВТекстМодуля(СтруктураОписанияEpf,StepDefinition,Шаг,ШагСтрокДляМодуля,АдресСнипета,СоздаватьЗакоментированныйКодДляПереиспользуемыхПроцедур,ГенерироватьУФ,ЭтоУФОбычнойФормы);
КонецПроцедуры
...
Функция СоздатьСтруктураОписанияEpf(ОтносительныйКаталогФичи,ИмяТекущейФичи,GenerateEpf,ДвДанныеvbFeatureReader,ДополнительныеПараметры)
	...
	Если НЕ СтруктураОписанияEpf.ФайлEpfПересоздавался Тогда
		ДобавитьНачальноеЗаполнениеВМодульОФ(СтруктураОписанияEpf.ТелоМодуля,ШагСтрокДляМодуля,ГенерироватьУФ,ДвДанныеvbFeatureReader,СоздаватьОбластиПриГенерацииКода);
	КонецЕсли;
	
	Возврат СтруктураОписанияEpf;
КонецФункции

Процедура ДобавитьНачальноеЗаполнениеВМодульОФ(ТелоМодуля,ШагСтрокДляМодуля,ГенерироватьУФ,ДвДанныеvbFeatureReader,ЗнСоздаватьОбластиПриГенерацииКода)
...
Процедура ДобавитьНачальноеЗаполнениеВМодульСервер(ТелоМодуля,ШагСтрокДляМодуля,ГенерироватьУФ,ДвДанныеvbFeatureReader,знСоздаватьОбластиПриГенерацииКода)
	FeatureReader = ПолучитьИнструментПарсерФич();
	FeatureReader.ДобавитьНачальноеЗаполнениеВМодуль(ТелоМодуля,ШагСтрокДляМодуля,ГенерироватьУФ,ЭтоУФОбычнойФормы,знСоздаватьОбластиПриГенерацииКода);
КонецПроцедуры
...
Процедура ПерейтиКВыполнениюСледующегоШага(НадоОбновитьСтатусШага = Ложь,ИдСледующегоШага = Неопределено)
	...
	Если (ТекИД_СценарияВМассиве >= 0) и (ТекИД_ШагаВМассиве >= 0) Тогда
		...
		Если ТекШаг.ШагСчитаетсяУпавшим Тогда
			...
			ПодключитьОбработчикОжидания("ВыполнинтьСценарийАсинхронноТаймер",0.1,Истина);
	...
	Если (НЕ ВыполнятьШагиАсинхронно) или ТранзакцияАктивна() Тогда //если транзакция активна, то нельзя использовать таймер
...
Процедура ВыполнитьШаг()
	...
	Если СокрЛП(АдресСнипета) = "" Тогда
		...
		ТекстСообщения = "Не реализован. Пустой адрес снипета у шага: %1";
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ТекШаг.Имя);
...
Процедура ВыгрузитьСтатусВыполненияСценариевВФайл(Статус,ПутьКФайлу)
	...
	ТекстСообщения = "Записал файл статуса сборки: %1";
...
Процедура ВыполнинтьСценарийАсинхронноТаймер()
	...
	Если (МассивСценариевДляВыполнения.Количество()-1) < ТекИД_СценарияВМассиве Тогда
		...
		Если ДелатьОтчетВФорматеjUnit Тогда
			...
			Если ДобавлятьКИмениСценарияУсловияВыгрузки Тогда
				...
				ИмяФайлаXML           = КаталогВыгрузкиJUnit + "\" + ИмяСборки  + "-junit.xml";
			Иначе	
				ИмяФайлаXML           = КаталогВыгрузкиJUnit + "\junit.xml";
			КонецЕсли;	 
			
			Файл = Новый Файл(КаталогВыгрузкиJUnit);
			Если Не Файл.Существует() Тогда
				СоздатьКаталог(КаталогВыгрузкиJUnit);
			КонецЕсли;	 
		...
		Если ДелатьОтчетВФорматеCucumberJson Тогда
			...
			Если ДобавлятьКИмениСценарияУсловияВыгрузки Тогда
				ИмяСборки             = ИмяТекущейСборки;
				УбратьВсеЗапрещенныеСимволыИзИмениФайла(ИмяСборки);
				ИмяФайлаJson           = КаталогВыгрузкиCucumberJson + "\" + ИмяСборки + "-cucumber.json";
			Иначе	
				ИмяФайлаJson           = КаталогВыгрузкиCucumberJson + "\cucumber.json";
			КонецЕсли;	 
			
			Файл = Новый Файл(КаталогВыгрузкиCucumberJson);
			Если Не Файл.Существует() Тогда
				СоздатьКаталог(КаталогВыгрузкиCucumberJson);
			КонецЕсли;	 
		...
		Если ЗавершитьРаботуСистемыПослеВыполненияВсехСценариев Тогда
	...
	Сообщить(" ");
	ТекстСообщения = "Работаю по сценарию: %1";
	ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",МассивСценариевДляВыполнения[ТекИД_СценарияВМассиве].Имя);
	СделатьСообщение(ТекстСообщения);
...
Процедура ВыполнитьСценарии(ТекСценарий = Неопределено) Экспорт
	...
	Объект.ЗначениеТегаStartStopRecordVideo = "";  //+++ ЦВ_ПР 25.12.2025 Инициализация для Анализа тегов StartRecordVideo и StopRecordVideo 
...
Процедура СделатьПараметрыКорректными()
	...
	КонецЕсли; 
	Если Прав(КаталогИнструментов,1) = "\" Тогда
		КаталогВыгрузкиAllure = Лев(КаталогВыгрузкиAllure,СтрДлина(КаталогВыгрузкиAllure)-1);
	КонецЕсли; 
	Если Прав(КаталогИнструментов,1) = "\" Тогда
		КаталогВыгрузкиJUnit = Лев(КаталогВыгрузкиJUnit,СтрДлина(КаталогВыгрузкиJUnit)-1);
	КонецЕсли; 
	Если Прав(КаталогИнструментов,1) = "\" Тогда
		КаталогВыгрузкиCucumberJson = Лев(КаталогВыгрузкиCucumberJson,СтрДлина(КаталогВыгрузкиCucumberJson)-1);
	КонецЕсли; 
КонецПроцедуры
...
Процедура ЗагрузитьФичи() Экспорт
	...
	Если Не ПутьКФичам.Существует() Тогда
		ТекстСообщения = "Путь к фичам <%1> не существует.";
	...
	СтруктураПараметров.Вставить("КешСнипетов",Новый Соответствие);
	СтруктураПараметров.Вставить("СтрокаПробелов",СтрокаПробелов);
	СтруктураПараметров.Вставить("ВозможнаОптимизацияРаботыСФайлами",Ложь);
	...
	Исключение
		ТекстСообщения = "Ошибка при построении дерева. %1";
	...
	//+++ ЦВ_ПР 25.12.2025 // Сброс кэша деревьев (только на клиенте)
    Попытка
        // Проверяем, доступна ли переменная мКэшПоискаДерева
        Если ТипЗнч(мКэшПоискаДерева) <> Тип("Неопределено") Тогда
            мКэшПоискаДерева = Неопределено;
        КонецЕсли;
    Исключение
        // Переменная не доступна (скорее всего, на сервере). Ничего не делаем
    КонецПопытки;
	//---  ЦВ_ПР 25.12.2025
