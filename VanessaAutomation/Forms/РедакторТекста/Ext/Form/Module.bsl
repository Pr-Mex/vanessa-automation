...
Процедура ПоказатьСтраницуWelcome(НедавноЗагруженныеФичи) Экспорт
	...
	Если НЕ Ванесса.Объект.ИспользоватьКомпонентуVanessaExt Тогда
		МассивСоветы.Добавить(Новый Структура("event,href,gliph,name", "WELCOME_LAUNCH", "ВключитьИспользованиеВнешнейКомпоненты()", "debug-stackframe-dot", Ванесса.Локализовать("Рекомендуется включить использование компоненты VanessaExt")));
	КонецЕсли;	 
...
Функция ЗначениеПеременной(ПроверяемоеИмяПеременной)
	Если ТипЗнч(ПроверяемоеИмяПеременной) <> Тип("Строка") Тогда
		Возврат Неопределено;
	КонецЕсли;	 
