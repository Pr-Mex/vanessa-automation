...
Функция СохранитьТекстИзVanessaEditorВФайлИВыполнитьОбработчик(ДопПараметры = Неопределено) Экспорт
	...
	КонецЕсли;	 
	ДопПараметры.Вставить("ВыполнитьСценарии", Истина);
...
Процедура РаскраситьСтрокиРедактора()
	РаскраситьСтрокиРедактораРекурсивно(Ванесса.Объект.ДеревоТестов);
КонецПроцедуры 
...
&НаКлиенте
Процедура РаскраситьСтрокиРедактораРекурсивно(СтрокаДерева)
	Для Каждого ТекСтрока Из СтрокаДерева.ПолучитьЭлементы() Цикл
		Если ТекСтрока.Тип = "ШагСценарий" Тогда
		...
		РаскраситьСтрокиРедактораРекурсивно(ТекСтрока);
	КонецЦикла;	 
КонецПроцедуры 
...
Процедура ОбновитьВставкиКодаVanessaEditorРекурсивно(СтрокаДерева, ТекУровень, СтрокиПодсценариевВиджетов, СтрокиНеявныхОбластей)
	Для Каждого ТекСтрока Из СтрокаДерева.ПолучитьЭлементы() Цикл
		Если ТекСтрока.Тип = "ШагСценарий" Тогда
			Ванесса.VanessaEditor.setLineStyle(ТекСтрока.НомерСтрокиВФиче, 0, Ложь, Ложь, Истина);
...
&НаКлиенте
Функция ЗначениеПоляТекущегоПоляVanessaEditor(ЭлементФормы)
	ЗначениеПоля = "";
