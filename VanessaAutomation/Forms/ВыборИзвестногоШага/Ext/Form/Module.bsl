...
Процедура ПриОткрытии(Отказ)
	...
	ИспользоватьMarkdownViewer = Ванесса.МодульОбщегоНазначенияVA().ВерсияПриложенияБольшеИлиРавнаЧемЗаданнаяКлиент(СистемнаяИнформация.ВерсияПриложения, "8.3.14.0");
...
Процедура ДеревоШаговИмяФайлаПолеОткрытие(Элемент, СтандартнаяОбработка)
	...
	СвойстваФайла = Ванесса.МодульОбщегоНазначенияVA().РазложитьПутьКФайлу(ТекДанные.ИмяФайла);
...
Процедура ВыполнитьПоиск()
	...
	ДопПараметры.Вставить("ТекстСообщенийПользователю", Ванесса.ДанныеПереводаСообщенийПользователю());
	ДопПараметры.Вставить("ДанныеДляВставкиГиперссылокВОписаниеШагов",Ванесса.ДанныеДляВставкиГиперссылокВОписаниеШагов());
	ДопПараметры.Вставить("ЭтоРежимОбучения", Ложь);
	ЗаполнитьДеревоИзвестныхШаговНаСервере(ДвДанныеvbFeatureReader, ДопПараметры);
...
// Выгрузка идет из ДЕРЕВА шагов, а не из ТаблицаИзвестныхStepDefinition:
// при ЯзыкГенератораGherkin <> "ru" перевод применяется только к дереву
// (см. ЗаполнитьДеревоИзвестныхШаговНаСервере), таблица формы остается
// русской. Выгрузка из таблицы отдавала MCP-клиентам русские имена шагов
// при любом языке генератора, а сверка таблицы с переведенным деревом по
// русскому имени молча теряла все переведенные шаги.
&НаСервере
Функция ПодготовитьТаблицуДляВыгрузкиШагов()
	ДеревоШаговСервер = РеквизитФормыВЗначение("ДеревоШагов");

	Массив = Новый Массив;
	ДобавитьШагиДереваДляВыгрузки(ДеревоШаговСервер.Строки, Массив);

	Возврат Массив;
КонецФункции

// Рекурсивно собирает листья дерева шагов (группы пропускаются — у них
// не заполнено ПредставлениеТеста) в массив структур для выгрузки в JSON.
&НаСервере
Процедура ДобавитьШагиДереваДляВыгрузки(СтрокиДерева, Массив)
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		Если СтрокаДерева.Строки.Количество() > 0 Тогда
			ДобавитьШагиДереваДляВыгрузки(СтрокаДерева.Строки, Массив);
			Продолжить;
		КонецЕсли;

		Если НЕ ЗначениеЗаполнено(СтрокаДерева.ПредставлениеТеста) Тогда
			Продолжить;
		КонецЕсли;

		ПолныйТипШага = "";
		Если СтрокаДерева.Родитель <> Неопределено Тогда
			ПолныйТипШага = СтрокаДерева.Родитель.ПолныйТипШага;
		КонецЕсли;

		Структура = Новый Структура;
		Структура.Вставить("ИмяШага", СтрокаДерева.ПредставлениеТеста);
		Структура.Вставить("ОписаниеШага", СтрокаДерева.ОписаниеШага);
		Структура.Вставить("ПолныйТипШага", ПолныйТипШага);
...
Процедура УстановитьАдресMarkdownViewer()
	...
	ПолноеИмяФайла = Ванесса._СтрШаблон("%1%4%2_%3.html", КаталогВременныхФайлов(), ИмяФайла, Версия, ПолучитьРазделительПути());
