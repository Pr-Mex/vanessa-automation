...
// Возвращает описание текущего элемента формы TestClient.
// Используется внешними модулями для анализа интерактивной формы.
//
&НаКлиенте
...
// Инициализирует форму исследователя. Принимает владельца (обычно Ванесса).
//
&НаКлиенте
...
// Завершает чтение формы в режиме MCP и закрывает форму с результатом.
//
&НаКлиенте
...
// Перестраивает дерево формы и возвращает результат чтения формы в формате MCP.
// Если ФорматЧтенияФормы указан, он применяется только на время построения дерева;
// после возврата значения MCPФорматЧтенияФормы восстанавливается в исходное.
//
&НаКлиенте
Функция MCPПолучитьРезультатЧтенияФормы(ДопПараметры) Экспорт
	
	MCPФорматЧтенияФормы = Неопределено;
	Если ДопПараметры.Свойство("ФорматЧтенияФормы") Тогда
		MCPФорматЧтенияФормы = ДопПараметры.ФорматЧтенияФормы;
	КонецЕсли;
	...
	Возврат MCPСформироватьРезультатЧтенияФормы();
...
КонецПроцедуры
  ...
  #Область ВзаимодействиеСТестКлиентом
...
КонецФункции
  ...
  #Область ФормированиеТекстаШагов
...
Функция MCPСформироватьРезультатЧтенияФормы()
	...
	ДанныеДерева = Ванесса._СтрСоединить(Строки, Символы.ПС);
