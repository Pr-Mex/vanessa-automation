...
Процедура СформироватьФайлыСценариевТестирования()
	...
	ТекстСообщения = _СтрШаблон(ТекстСообщения, (ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяНачала) / 1000);
...
Процедура ЗаполнитьИсключенияАсинхВызовФормированиеФайловТестов(ТекСтрока = 0)
	...
	ОписаниеОповещ = Новый ОписаниеОповещения("ПолучитьИсключенияОбработкаЗавершения", ЭтаФорма, ДополнительныеПараметры, "ОбработкаОшибки", ЭтаФорма);
...
Функция ПолучитьМассивСтрокИзФайлаИсключений(НайденныеФайлы)
	...
	Для Каждого НайденныйФайл Из НайденныеФайлы Цикл
		...
		Пока Строка <> Неопределено Цикл
			Если НЕ ПустаяСтрока(Строка) И Найти(Строка, "//") <> 1 Тогда
...
Процедура ОбработкаОшибки(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения = _СтрШаблон(ТекстСообщения, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	СообщитьПользователю(ТекстСообщения);
...
Функция ПолучитьФайлИзменныхОбъектов()
	...
	Если КодВозврата <> 0 Тогда
		...
		|Строка запуска: %2");
		ТекстСообщения = _СтрШаблон(ТекстСообщения, КодВозврата, КомандаЗапуска);
		СообщитьПользователю(ТекстСообщения);
		Попытка
			...
			Если ЗначениеЗаполнено(ТекстФайла) Тогда
				...
				ТекстСообщения = _СтрШаблон(ТекстСообщения, ТекстФайла);
				СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
...
Процедура ВыполнитьНастройкиСценариевИзФайлаНаСервере(Знач ИмяФайла)	
	...
	МассивCSV = _СтрРазделить(ШапкаCSV, ИспользуемыйРазделитель);
	...
	Для НомерСтроки = 2 По ФайлCSV.КоличествоСтрок() Цикл
		...
		МассивCSV = _СтрРазделить(СтрокаCSV, ИспользуемыйРазделитель);
...
Процедура ДобавлениеМетаданныхГруппы(Знач ТипМетаданных)
	...
	Для Каждого ОбъектМетаданных Из ДобавляемыеМетаданные Цикл
		...
		Если ПроверитьВМассивеИсключений(МассивИсключений, Имя, "") Тогда
			ТекстСообщения = _СтрШаблон(НСтр("ru = 'Исключен по файлу исключений %1.%2'; en = 'Excluded by exception file %1.%2'"), ТипОбъектаМетаданных, Имя);
			СообщитьПользователю(ТекстСообщения);
			Продолжить;
		КонецЕсли;
...
Процедура ДеревоМетаданныхВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ФормаСпискаДляОткрытия = _СтрШаблон("%1.%2.%3", 
...
Функция СформироватьТекстПоОбъектуМетаданныхИзДерева(ТекЭлемент, СтруктураПараметров)
	...
	ПолноеИмяОбъектаКонфигурации = _СтрШаблон("%1.%2", Родитель.Данные, ТекЭлемент.Данные); // Пример: Справочник.Номенклатура
	...
	Для СчТип = 1 По СтруктураПараметров.СоответствиеНастроек.Количество() Цикл
		...
		Если ТекЭлемент[ТипТеста] Тогда
			...
			КонецЕсли;
			ТекстКоличествоСценариев = ?(КоличествоСценариевВнутри > 1, _СтрШаблон(" (%1)", КоличествоСценариевВнутри) , "");
			
			ЗаголовокКодаСценария = _СтрШаблон("%1 - %2%3", ПолноеИмяОбъектаКонфигурации, ТипТеста, ТекстКоличествоСценариев);
...
Функция ОпределитьПутьКФичаФайлу(СписокФичаФайлов, ВидОбъектаМетаданных, ТипТеста)
	...
	СтрокаПоиска = _СтрШаблон("_%1_%2", ВидОбъектаМетаданных, ТипТеста);
	Для каждого ТекФичаФайл Из СписокФичаФайлов Цикл
	
		Если _СтрНайти(ТекФичаФайл.Представление, СтрокаПоиска) > 0 Тогда
...
Процедура ОбновитьФичаФайлыКлиентЗавершить(СоздаватьНовыеФичаФайлы = Ложь, СценарииБезФичаФайлов = Неопределено)
	...
	Для каждого ТекСтрока Из ТаблицаКодаСценариев1С Цикл
		...
		Если Не ЗначениеЗаполнено(ПутьЗаписи) Тогда
			ТекстСообщения = _СтрШаблон(НСтр("ru = 'Для %1 не заполнен путь к feature-файлу.'; en = 'The feature file path is not filled in for %1.'"), ТекСтрока.ЗаголовокКодаСценария);
			СообщитьПользователю(Локализовать(ТекстСообщения));
			Продолжить;
		КонецЕсли;	
		...
		Если Не Файл.Существует() Тогда
			ТекстСообщения = _СтрШаблон(НСтр("ru = 'Для %1 файл по указанному пути не существует: %2'; en = 'For %1, the file at the specified path does not exist: %2'"), ТекСтрока.ЗаголовокКодаСценария, ПутьЗаписи);
			СообщитьПользователю(Локализовать(ТекстСообщения));
			Продолжить;
		КонецЕсли;
		...
		Если СценарийУжеЕстьВФайле(ТекстВсегоФичаФайла, МассивЗаголовков) Тогда
			ТекстСообщения = _СтрШаблон(НСтр("ru = 'Отклонено обновление. Для %1 уже существует такой сценарий в feature-файле: %2'; en = 'Update rejected. A scenario for %1 already exists in the feature file: %2'"), ТекСтрока.ЗаголовокКодаСценария, ТекСтрока.ИмяФичаФайла);
			СообщитьПользователю(Локализовать(ТекстСообщения));
			Продолжить;
		КонецЕсли;		
...
Функция СоздатьНовыеФичаФайлыПоСценариям(СценарииБезФичаФайлов)
	...
	Для каждого ТекСтрока Из СценарииБезФичаФайлов Цикл
		...
		Попытка
			...
			ТекстСообщения = _СтрШаблон(НСтр("ru = 'Создан новый feature-файл: %1'; en = 'New feature file created: %1'"), ТекСтрока.ИмяФичаФайла);
			СообщитьПользователю(Локализовать(ТекстСообщения));
...
Функция МассивЗаголовковСценариевИзСтроки(ТекстСценария)
	...
	Для Счетчик = 1 По СтрЧислоСтрок(ТекстСценария) Цикл
		...
		Если _СтрНайти(ТекСтрока, СтрокаПоиска) > 0 Тогда
...
Функция СценарийУжеЕстьВФайле(ТекстВсегоФичаФайла, Заголовки)
	...
	Для каждого ТекЗаголовок Из Заголовки Цикл
		...
		Если _СтрНайти(ТекстВсегоФичаФайла, ТекЗаголовок) > 0 Тогда
...
Функция ПутьКФичаФайлу(СтруктураПараметров)
	...
	ИмяФичаФайла = _СтрШаблон("%1%2_%3_%4.feature", СтруктураПараметров.СчОбъектов, СтруктураПараметров.СчТип, 
	...
	ПутьЗаписи = _СтрШаблон("%1%2", КаталогВыходныхФайловНормализованный, ИмяФичаФайла); // Пример: C:\temp\КаталогДымовыеТесты\121_Справочники_ФормаСписка.feature
...
Процедура ПодготовкаДанныхКлиент()
	...
	Для Каждого ЭлементКласс Из ЭлементыДерева Цикл
		...
		Для Каждого ЭлементКласса Из ЭлементыКласса Цикл
			...
			ИмяИсточникаДляПоиска = _СтрШаблон("%1.%2", ТипМетаданныхНаАнгл, ЭлементКласса.Данные); // Например будет:  Catalog.Организации
...
Функция РавенОбъектуИзПодготовкаДанных(ТипыМетаданныхНаАнгл, ЭлементДерева, ПолноеИмяСравниваемогоНаАнгл)
	...
	ИмяИсточникаДляСравнения = _СтрШаблон("%1.%2", ТипМетаданныхНаАнгл, ЭлементДерева.Данные); // Например будет:  Catalog.Организации
...
Функция СформироватьТекстФайлаПоОбъектамМетаданных(Параметры)
	...
	Для Каждого ОбъектМетаданных Из Метаданные[Параметры.ВидОбъектаМетаданных] Цикл
		...
		Если НЕ ПустаяСтрока(Параметры.ТекстСравнения) Тогда
			Если _СтрНайти(Параметры.ТекстСравнения, Параметры.ВидОбъектаЕЧ + "." + Имя) = 0 Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
	...
	#Область Печать
		...
		Для Каждого КомандаПечати Из КомандыПечати Цикл
			...
			Если Параметры.ЯзыкШагов = "ru" Тогда
				...
				ТекстШагов = _СтрШаблон(ТекстШагов, ИмяГруппы, ИмяКоманды1, ИмяКоманды2, ПредставлениеОбъекта);
...
// Локальные серверно-доступные полифиллы для режима совместимости 8.2 (урок 44/45).
// Перем Ванесса — &НаКлиенте, на сервере недоступна, поэтому вызовы Ванесса._Стр* в серверных
// методах не компилировались. Полифиллы объявлены &НаКлиентеНаСервереБезКонтекста (доступны везде).
#Область ЛокальныеПолифиллыСтрок8_2

// Обёртка для эмуляции СтрШаблон (8.2: встроенный недоступен).
&НаКлиентеНаСервереБезКонтекста
Функция _СтрШаблон(Знач Стр, Парам1 = Неопределено, Парам2 = Неопределено, Парам3 = Неопределено, Парам4 = Неопределено, Парам5 = Неопределено, Парам6 = Неопределено, Парам7 = Неопределено, Парам8 = Неопределено, Парам9 = Неопределено)
...
// Обёртка для эмуляции СтрНайти (8.2: встроенный недоступен).
&НаКлиентеНаСервереБезКонтекста
Функция _СтрНайти(Стр, Подстрока)
	Возврат Найти(Стр, Подстрока);
КонецФункции

// Обёртка для эмуляции СтрРазделить (8.2: встроенный недоступен).
&НаКлиентеНаСервереБезКонтекста
Функция _СтрРазделить(Знач Строка, Знач Разделитель = ",", Знач ПропускатьПустыеСтроки = Неопределено)
