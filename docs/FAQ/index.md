...
14. Появляется ошибка, в которой есть текст: "Неизвестный идентификатор формы". Это означает, что, скорее всего, в одном сеансе открывались разные версии Vanessa Automation. Нужно перезапустить текущий сеанс 1С или перезапустить сервер 1С.
...
33. Как правильно добавить новую настройку Vanessa Automaion?
	...
	Далее его надо прописать в:
	    ...
	    форма ОбщегоНазначенияVA - СтруктураПараметровЗапуска()
