...
## 17. Получить состояние объекта (Проведен, ПометкаУдаления, Код, Дата, Номер)
Некоторые свойства обхектов, такие как ПометкаУдаления, Проведен могут не выведены на форму. Чтобы их получить из формы открытого объекта Справочника или Документа можно использовать шаг
```Gherkin
...
## 19. Открыть внешнюю обработку или отчет
Для этого надо использовать шаг
```Gherkin
