...
* Добавить проверку что это не каталог для для поля с файлом лога #1551
...
* Рефакторинг дерева известых шагов #1614
...
* Не нажимается кнопка в диалоге типа ПоказатьВопрос при включенной настройке Использовать UI Automation #1534
* Падение парсера из компоненты #1542
* Не реализовано выполнение шага И я проверяю или создаю для справочника "ИмяОбъекта" объекты с обмен данными загрузка истина: #1517
* Ошибка определения пути при использовании workspaceRoot в параметрах #1483
* Прерывание раскраски строк после переключения выполняемого сценария #1460
* Ошибка создания файла при обработке видео если присутствует условие в сценарии #1431
* UI Automation падает при нажатии кнопки переключения режима просмотра (список / иерархический список) #1323
* При включенном флаге "Различать broken и failed по ключевому слову Тогда" в Allure не всегда прикрепляется вся доп. информация #1479
* Ошибка сравнения области макета с эталонным макетом #1556
* Ошибка после прохождения тестов при заполненном пути в настройке Каталог allure отчета (несколько сборок) #1544
* Добавление известного шага в окно Сценарий поведения #1576
* При "Создавать Markdown инструкцию" и относительном пути каталога #1581
* Метод объекта не обнаружен (КаталогУроков) #1624
* Проблема с вычислением выражения из области Переменные #1635
* Опечатки в предварительном релизе 1.2.038.32 #1642
