window.VanessaMouse = {
    ...
    showClick: (options) => {
        VanessaVisualEffect.fixNumericOptions(0, options);
        let props = {
            color: "red",
            duration: 1000,
            left: VanessaMouse.x,
        ...
        let w1 = {}; w1[props.width] = 0;
        let w2 = {}; w2[props.width / 2] = 0;
        new mojs.Shape({
            parent: parent,
            left: props.left,
            top: props.top,
...
window.setInterval(function() {
	document.querySelectorAll('.gridBody').forEach(function(grid) {
		if(grid.hasAttribute("data-vanessa-listener-attached"))
			return;
		
		grid.addEventListener('mousemove', VanessaMouse.handler, false);
		grid.addEventListener('mouseenter', VanessaMouse.handler, false);
		grid.setAttribute("data-vanessa-listener-attached", true);
	});
}, 200);

document.addEventListener('mousemove', VanessaMouse.handler, false);
document.addEventListener('mouseenter', VanessaMouse.handler, false);
