window.VanessaVisualEffect = {
    show: (text, timeout = 3000, options = undefined) => {
        ...
        let xprops = { color: "white", count: 10, backgroundColor: '#013cff', fontSize: '200%', duration: 3000 };
        Object.assign(xprops, options);
        if (options && options.effect) {
            let effect = VanessaVisualEffect[options.effect];
            if (effect) return effect(text, timeout, xprops);
        }
        fillScreen(text, timeout, xprops);
    },
    ...
    fillScreen: (text, options = undefined) => {
        ...
        let xprops = ['color', 'fontSize', 'backgroundColor'];
    ...
    2: (text, timeout, options) => {
        ...
        let textNode = undefined;
        let showText = () => textNode = VanessaVisualEffect.fillScreen(text, 0, options);
