...
Процедура ДобавитьСубтитрыКВидео(ПараметрыВидео, ИмяФайлаШага, Шаг, ДлительностьMP3)
	Если НЕ ПараметрыВидео.ЗаписьВидеоДобавлятьСубтитры Тогда
		Возврат;
	КонецЕсли;	 
	//+++ ЦВ_ПР 25.12.2025 Если нет "смысловых титров", то пропустим их
	Если Шаг.Свойство("ТекстИнструкция") = Неопределено ИЛИ Шаг.Имя = Шаг.ТекстИнструкция Тогда
		Возврат;
	КонецЕсли;	 	
	//--- ЦВ_ПР 25.12.2025 конец вставки
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			Если ЭтоВложенныйШагВнутриОбъединеннойГруппы(Шаг) Тогда
				Продолжить;
			КонецЕсли;	 
			//+++ ЦВ_ПР 25.12.2025 Уточнения в приоритетах анализа Директива [autodoc.mute] перед шагом должна отключать озвучку
			Текст = "";
			ПолучитьТекстШагаИзТекстаФичиЕслиОнТамЗадан(ПараметрыВидео, Текст, Шаг, Сценарий);
			Если Текст = "<НЕ ОЗВУЧИВАТЬ>" Тогда 
				Текст = "*";	        
				Шаг.Вставить("ТекстДиктораВШаге", Текст); 
			ИначеЕсли Шаг.Свойство("ТекстДиктораВШаге") и ЗначениеЗаполнено(Шаг.ТекстДиктораВШаге) Тогда
				Текст = Шаг.ТекстДиктораВШаге;
			ИначеЕсли Не ЗначениеЗаполнено(Текст) Тогда				Если Шаг.Свойство("ТекстИнструкция") И ЗначениеЗаполнено(Шаг.ТекстИнструкция) Тогда
					Текст = Шаг.ТекстИнструкция;
				Иначе	
					//+++ ЦВ_ПР 25.12.2025 Уточнение. Нужно учесть тег StartRecordVideo, который включает принудительно запись Видео и нежелательную озвучку названия шага в тексте отключим
					Если (Шаг.Свойство("ДирективныйЗапускЗаписиВидео") и Шаг.ДирективныйЗапускЗаписиВидео) И   
						(Шаг.Свойство("ТекстИнструкция") и Шаг.ТекстИнструкция =Шаг.Имя) Тогда
						Текст =""; 
					Иначе
						Текст = Шаг.Имя;
					КонецЕсли;	     
			...
			КонецЕсли;
			//--- ЦВ_ПР 25.12.2025 конец анализа текста диктора
			Шаг.Вставить("ТекстИнструкция", Текст);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры 
...
Процедура ЗавершитьЗаписьИнструкцииВидео(ПараметрыВидео)
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			// +++ ЦВ_ПР 25.12.2025 Логика фильтрации шагов:
			// В новом алгоритме для работы в Циклах (и с ИндексШагаИсходный) ШагиУточ уже содержат 
			// только шаги, которые выполнялись и нужны для инструкции.
			// В старом алгоритме нужна дополнительная фильтрация.
			Если Не Шаг.Свойство("ИндексШагаИсходный") Тогда
			    // Старый алгоритм: фильтруем по ШагиКоторыеВыполнялись
			    Если ШагиКоторыеВыполнялись[Шаг.ИдСтроки] = Неопределено Тогда
			        Продолжить; // Шаг не выполнялся
			    КонецЕсли;
			КонецЕсли;
			// --- ЦВ_ПР 25.12.2025		
			...
			//+++ ЦВ_ПР 25.12.2025 С уточнением для пустого или без имени. Файлы в \TTS\ заберем как было сохранено в Шаг.ИмяАудиоФайла при записи 
			...
			иначе	
				ИмяMp3Шага = "";
			КонецЕсли;	
			Если НЕ ЗначениеЗаполнено(ИмяMp3Шага) и ПараметрыВидео.ЗаписьВидеоДелатьНаложениеTTS тогда
					Продолжить;
			КонецЕсли;	       
			//--- ЦВ_ПР 25.12.2025
...
Процедура ЗавершитьЗаписьИнструкцииHTML(ПараметрыВидео)
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			// +++ ЦВ_ПР 25.12.2025 Логика фильтрации шагов:
			// В новом алгоритме (с ИндексШагаИсходный) ШагиУточ уже содержат 
			// только шаги, которые выполнялись и нужны для инструкции.
			// В старом алгоритме нужна дополнительная фильтрация.
			Если Не Шаг.Свойство("ИндексШагаИсходный") Тогда
			    // Старый алгоритм: фильтруем по ШагиКоторыеВыполнялись
...
Процедура СоздатьФайлыTTS(ПараметрыВидео) Экспорт
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		НомерШага = -1;
		Для каждого Шаг Из Шаги Цикл
			//+++ ЦВ_ПР 25.12.2025 Счетчики перенесены в начало цикла, так как Продолжить, тоже должны считаться  
			НомерШага = НомерШага + 1;
			ТекИДШага = ТекИДШага + 1;   
			//--- ЦВ_ПР 25.12.2025 конец вставки 
			Если НЕ Ванесса.ИдетВыполнениеСценариев() Тогда
				Возврат;
			КонецЕсли;	
			//+++ ЦВ_ПР 25.12.2025 Используем часть общего кода с целью записи в Видео.
			НужноВключитьВВидео = Ложь;
			//Ванесса.ДобавитьСвойстваВыполняемогоШагаДляВидеоИнструкций ("ДляОзвучки", Шаг, Шаг,НужноВключитьВВидео); 
			НужноВключитьВHTML =  Ложь;

			Если НЕ Ванесса.АктуальноДляАвтоинструкций (Шаг,НужноВключитьВHTML,НужноВключитьВВидео) Тогда
				Продолжить;
			КонецЕсли;	 

			Если НЕ НужноВключитьВВидео Тогда
				Продолжить;
			КонецЕсли;	 
			//--- ЦВ_ПР 25.12.2025 конец вставки
			Если НЕ Шаг.Свойство("ТекстИнструкция") Тогда
