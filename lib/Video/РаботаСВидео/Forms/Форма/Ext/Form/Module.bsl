...
Функция СБЕР_ШифроватьПодключение(ПараметрыВидео)
	...
	ДвоичныеДанныеСтроки = ПолучитьДвоичныеДанныеИзСтроки(ИсходнаяСтрока);
...
Функция ПолучитьMP3ИзТекстаСбер(ПараметрыВидео, ТекстДляMp3, ИмяФайла, ПараметрыГолоса)
	...
	СтрокаЗапроса = СтрШаблон("rest/v1/text:synthesize?format=%1&voice=%2"
								, "opus"
...
Процедура СоздатьФайлыTTS(ПараметрыВидео) Экспорт
	...
	Если ПараметрыВидео.ЗаписьВидеоТипОзвучкиTTS = 0 И СокрЛП(ПараметрыВидео.ЗаписьВидеоИмяTTS) = "" Тогда
