...
Процедура СоздатьOutro(ПараметрыВидео)
	...
	СтрокаФичи = Текст.Прочитать();
	Текст.Закрыть();
	Если Найти(СтрокаФичи,"#[autodoc.ignore.finalslide]") > 0 Или ПараметрыВидео.ЗаписьВидеоОтключитьСлайдФинал Тогда
...
&НаКлиенте
Процедура СобратьВидеоВставку(ПараметрыВидео,ИмяФайлаШага,Знач ТекстВставки,НомерСценария,НомерШага,ПараметрыГолоса)
		...
		Для каждого Шаг Из Шаги Цикл
			...
			Если ЭтоШагПервыйВВеткеДерева(ПараметрыВидео,Шаг,МассивВставок,Сценарий) Тогда
				...
				Для Каждого ТекстВставкиВеткиДерева Из МассивВставок Цикл
					...
					СобратьВидеоВставку(ПараметрыВидео,ИмяФайлаШагаВетки,ТекстВставкиВеткиДерева,НомерСценария,НомерШага,ПараметрыГолоса);
			...
			Если Ванесса.ЭтоПрерывающйВидеоШаг(Шаг) Тогда
				СобратьВидеоВставку(ПараметрыВидео,ИмяФайлаШага,Шаг.ЗначенияПараметров[0].Значение.Значение,НомерСценария,НомерШага,ПараметрыГолоса);
			Иначе
