...
Функция ПолучитьMP3ИзТекстаAmazonPoly(ПараметрыВидео,Текст,ИмяФайлаШага)
	...
	Если Ответ.StatusCode = 200 Тогда
		...
		Иначе 
			ВызватьИсключение СтрШаблон("Не удалось получить данные. Код результата: <%1>. Ответ: <%2>.", Ответ.КодСостояния, Ответ.ПолучитьТелоКакСтроку("UTF-8"));
		КонецЕсли;
...
Функция ТестГолоса(ПараметрыВидео) Экспорт
	...
	ИмяФайла = ПолучитьФайлMP3ИзТекста(ПараметрыВидео,ПараметрыВидео.ТекстДляТестаГолоса,"test");
