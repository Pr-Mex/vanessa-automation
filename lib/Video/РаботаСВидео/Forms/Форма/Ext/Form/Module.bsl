...
&НаКлиенте
Процедура СобратьВидеоВставкуИзТекста(ПараметрыВидео,ИмяФайлаШага,Знач ТекстВставки,НомерСценария,НомерШага)
		...
		Для каждого Шаг Из Шаги Цикл
			...
			Если ЭтоШагПервыйВВеткеДерева(ПараметрыВидео,Шаг,МассивВставок,Сценарий) Тогда
				...
				Для Каждого ТекстВставкиВеткиДерева Из МассивВставок Цикл
					...
					СобратьВидеоВставкуИзТекста(ПараметрыВидео,ИмяФайлаШагаВетки,ТекстВставкиВеткиДерева,НомерСценария,НомерШага);
			...
			Если Ванесса.ЭтоШагВидеовставкиИзТекста(Шаг) Тогда
				СобратьВидеоВставкуИзТекста(ПараметрыВидео,ИмяФайлаШага,Шаг.ЗначенияПараметров[0].Значение.Значение,НомерСценария,НомерШага);
...
Процедура ЗавершитьЗаписьИнструкцииHTML(ПараметрыВидео)
	...
	СнипетыСлайдИзКартинки           = Ванесса.ПолучитьМассивСнипетовСлайдИзкартинки();
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			ИЛИ  СнипетыСлайдИзКартинки.Найти(СнипетНрег) <> Неопределено
