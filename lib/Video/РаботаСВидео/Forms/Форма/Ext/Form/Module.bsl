...
Процедура СоздатьIntro(ПараметрыВидео)
	...
	Текст.Закрыть();
	Если Найти(СтрокаФичи,"#[autodoc.ignore.featureslide]") > 0 или ПараметрыВидео.ЗаписьВидеоОтключитьСлайдСЗаголовкомФичи Или ПараметрыВидео.ЗаписьВидеоЗаменитьСлайдСЗаголовкомФичи Тогда
...
Процедура СоздатьOutro(ПараметрыВидео)
	...
	СтрокаФичи = Текст.Прочитать();
	Текст.Закрыть();
	Если Найти(СтрокаФичи,"#[autodoc.ignore.finalslide]") > 0 Или ПараметрыВидео.ЗаписьВидеоОтключитьСлайдФинал Или ПараметрыВидео.ЗаписьВидеоЗаменитьСлайдФинал Тогда
...
Процедура СобратьФинальноеВидеоИзЧастей(ПараметрыВидео,МассивЧастей)
	...
	Если ПараметрыВидео.ФайлМузыки <> Неопределено Тогда
		...
		Если ПараметрыВидео.ЗаписьВидеоЗаменитьСлайдСЗаголовкомФичи И ПараметрыВидео.ЗаписьВидеоДобавитьМузыкуКСлайдуСЗаголовкомФичи Тогда
			
			ИмяФайлаСобранноеВидео = ДобавитьСлайдСЗаголовкомФичиКСобранномуВидео(ПараметрыВидео, ГдеИскать, ИмяФайлаСобранноеВидео, СобранноеВидео4)
	...
	Если ПараметрыВидео.ЗаписьВидеоЗаменитьСлайдСЗаголовкомФичи И (Не ПараметрыВидео.ЗаписьВидеоДобавитьМузыкуКСлайдуСЗаголовкомФичи Или ПараметрыВидео.ФайлМузыки = Неопределено) Тогда
		
		ИмяФайлаСобранноеВидео = ДобавитьСлайдСЗаголовкомФичиКСобранномуВидео(ПараметрыВидео, ГдеИскать, ИмяФайлаСобранноеВидео, СобранноеВидео5)
		
	КонецЕсли;
	
	Если ПараметрыВидео.ЗаписьВидеоЗаменитьСлайдФинал И (Не ПараметрыВидео.ЗаписьВидеоДобавитьМузыкуКСлайдуФинал Или ПараметрыВидео.ФайлМузыки = Неопределено) Тогда
...
&НаКлиенте
Функция ДобавитьСлайдСЗаголовкомФичиКСобранномуВидео(ПараметрыВидео, ГдеИскать, ИмяФайлаСВидео, СобранноеВидео)
	
	Если Ванесса.ФайлСуществуетКомандаСистемы(ПараметрыВидео.ЗаписьВидеоПутьКСлайдуСЗаголовкомФичи) Тогда
		
		ФайлIntro = Новый Файл(ПараметрыВидео.ЗаписьВидеоПутьКСлайдуСЗаголовкомФичи);
		СобранноеВидеоIntro = ГдеИскать + "\intro.mp4";
		
		Если ФайлIntro.Расширение = "mp4" Тогда
			
			Ванесса.КопироватьФайлКомандаСистемы(ПараметрыВидео.ЗаписьВидеоПутьКСлайдуСЗаголовкомФичи, СобранноеВидеоIntro);
			
		Иначе
			ПолноеИмяФайлаIntro = ГдеИскать + "\"+ФайлIntro.Имя;
			Ванесса.КопироватьФайлКомандаСистемы(ПараметрыВидео.ЗаписьВидеоПутьКСлайдуСЗаголовкомФичи, ПолноеИмяФайлаIntro);
			//конверт в mp4
			Состояние("Сжатие видео...");
			ИмяВременногоФайла = ПолучитьИмяВременногоФайла("cmd");
			ЗТ = Новый ЗаписьТекста(ИмяВременногоФайла,"UTF-8",,Истина);
