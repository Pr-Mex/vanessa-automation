...
Функция ОпределитьДлительностьФайла(ПараметрыВидео,ИмяИсходногоФайла)
	...
	Если Зн = Неопределено Тогда
		ТекстСообщения = "Не смог определить длительность у файла: %1";
	...
	Зн = Лев(Зн,8);
	МассивСтрок = ПараметрыВидео.Ванесса.РазложитьСтрокуВМассивПодстрокКлиент(Зн,":");
	КолСек = 0;
...
Функция ОбъединитьФайлыMP3(ПараметрыВидео,КаталогРаботы,Файл1,Файл2,Файл3)
	...
	Если Не ПараметрыВидео.Ванесса.ФайлСуществуетКомандаСистемы(ФайлВременныйMp3.ПолноеИмя) Тогда
		ТекстСообщения = "Команда по объединению mp3 не выполнена. Файл1=%1, Файл2=%2, Файл3=%3";
...
Процедура СоздатьIntro(ПараметрыВидео)
	...
	Если Найти(СтрокаФичи,"#[autodoc.ignore.featureslide]") > 0 Тогда
...
Функция ПолучитьСписокФайловКаталога(ПараметрыВидео,Каталог)
	...
	Если Не ПараметрыВидео.Ванесса.ФайлСуществуетКомандаСистемы(ИмяЛога) Тогда
		ТекстСообщения = "Команда по получению списка файлов каталога не выполнена. Каталог = %1";
...
Функция ПолучитьMP3МузыкиНужнойДлительности(ПараметрыВидео,ДлительностьВидео)
	...
	Если ДлительностьMP3 = 0 Тогда
		ТекстСообщения = "Длительность MP3 = 0. Файл=%1";
...
Процедура СобратьФинальноеВидеоИзЧастей(ПараметрыВидео,МассивЧастей)
	...
	Если ПараметрыВидео.Ванесса.ФайлСуществуетКомандаСистемы(ДанныеФайла.ПолныйПуть) Тогда
	...
	Если Не ПараметрыВидео.Ванесса.Объект.РежимСамотестирования  //чтобы не блокировался каталог
	   И Не ПараметрыВидео.Ванесса.ЭтоЗапускВРежимеКоманднойСтроки //чтобы не открыть каталог в режиме запуска из командной строки
	   Тогда 
		Если ПараметрыВидео.Ванесса.ЕстьПоддержкаАсинхронныхВызовов Тогда
		...
		Для каждого Шаг Из Шаги Цикл
			...
			Если Шаг.Свойство("ИгнорироватьШагВАвтоинструкции") Тогда
				Если Шаг.ИгнорироватьШагВАвтоинструкции Тогда
					ПараметрыВидео.Ванесса.УдалитьФайлыКомандаСистемы(ПараметрыВидео.ЗаписьВидеоКаталогДляВременныхФайлов + "\" + ИмяФайлаШага);
					Продолжить;
				КонецЕсли;	 
			КонецЕсли;	 
...
Процедура ОбработатьСтрокуПоСловарю(ПараметрыВидео,ИсходнаяСтрока)
	...
	Для Каждого ЭлементСловоряЗамен Из ПараметрыВидео.ЗаписьВидеоСловарьЗамен Цикл
		Если НЕ ПараметрыВидео.Ванесса.ФайлСуществуетКомандаСистемы(ЭлементСловоряЗамен.Значение) Тогда
			ТекстСообщения = "Не найден файл словаря замен. <%1>";
		...
		Пока Истина Цикл
			...
			Массив = ПараметрыВидео.Ванесса.РазложитьСтрокуВМассивПодстрокКлиент(Стр,"|");
...
Функция ПолучитьВременныйФайлШаг(ПараметрыВидео,Знач Текст)
	...
	//Сообщить("Текст=" + Символы.ПС + Текст);
...
Процедура ПолучитьТекстШагаИзТекстаФичиЕслиОнТамЗадан(ПараметрыВидео,Текст,Шаг,Сценарий)
	ПараметрыВидео.Ванесса.ПолучитьТекстШагаИзТекстаФичиЕслиОнТамЗадан(Текст,Шаг,Сценарий);
КонецПроцедуры
...
Процедура Инициализация(ПараметрыВидео) Экспорт
	...
	Если СокрЛП(Каталог) = "" Тогда
		ТекстСообщения = "Каталог ЗаписьВидеоКаталогДляВременныхФайлов <%1> не указан.";
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",Каталог);
		ВызватьИсключение Ванесса.ПолучитьТекстСообщенияПользователю(ТекстСообщения);
	КонецЕсли;	 
	...
	Если НЕ ПараметрыВидео.Ванесса.ФайлСуществуетКомандаСистемы(Каталог) Тогда
		...
		Если НЕ ПараметрыВидео.Ванесса.ФайлСуществуетКомандаСистемы(Каталог) Тогда
			ТекстСообщения = "Каталог ЗаписьВидеоКаталогДляВременныхФайлов <%1> не существует.";
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",Каталог);
			Сообщить(Ванесса.ПолучитьТекстСообщенияПользователю(ТекстСообщения));
			ТекстСообщения = "Каталог ЗаписьВидеоКаталогДляВременныхФайлов <%1> не существует.";
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",Каталог);
...
Процедура ОстановитьЗаписьВидеоОдногоШага(ПараметрыВидео) Экспорт
	...
	Если Рез <> 0 Тогда
		ТекстСообщения = "Команда ОстановитьЗаписьВидео вернула код возврата = %1";
...
Процедура ПроверитьЧтоЗаписьВидеоБудетРаботать(ПараметрыВидео) Экспорт
	...
	Если ПараметрыВидео.ЗаписьВидеоПодсвечиватьАктивныеЭлементыФорм Тогда
		...
		Если Не ПараметрыВидео.Ванесса.ФайлСуществуетКомандаСистемы(ИмяФайла) Тогда
			ТекстСообщения = "Файл <%1> не найден.";
