...
Процедура СобратьВидеоВставку(ПараметрыВидео,ИмяФайлаШага,Знач ТекстВставки,НомерСценария,НомерШага)
	...
	Если ПараметрыВидео.ЗаписьВидеоКэшироватьВидеоВставки Тогда
		Если ПараметрыВидео.ДанныеКэшВидеоВставки[ТекстВставки] <> Неопределено Тогда
			КаталогГдеЛежитКэшВидеоВставок = КаталогГдеЛежитКэшВидеоВставок(ПараметрыВидео);
...
Процедура ЗавершитьЗаписьВидео(ПараметрыВидео) Экспорт
	Если ПараметрыВидео.ЗаписьВидеоКэшироватьФайлыTTS Тогда
		...
		КаталогГдеЛежитКэшMp3 = КаталогГдеЛежитКэшMp3(ПараметрыВидео);
	КонецЕсли;	 
	Если ПараметрыВидео.ЗаписьВидеоКэшироватьВидеоВставки Тогда
...
&НаКлиенте
Процедура ВыгрузитьКешВJosn(ИмяФайла,Кэш)
...
Процедура ВыгрузитьКэшMp3(ПараметрыВидео)
	...
	ВыгрузитьКешВJosn(ИмяФайла,ПараметрыВидео.ДанныеКэшMp3);
КонецПроцедуры 
...
Процедура ВыгрузитьКэшВидеоВставок(ПараметрыВидео)
	...
	ВыгрузитьКешВJosn(ИмяФайла,ПараметрыВидео.ДанныеКэшВидеоВставки);
КонецПроцедуры 
...
&НаКлиенте
Функция КаталогГдеЛежитКэшMp3(ПараметрыВидео)
	КатлогДанногоГолоса = ПараметрыВидео.ЗаписьВидеоКэшФайловTTS + "\" + ПараметрыВидео.ЗаписьВидеоИмяTTS;
	Если НЕ Ванесса.ФайлСуществуетКомандаСистемы(КатлогДанногоГолоса) Тогда
		Ванесса.СоздатьКаталогКомандаСистемы(КатлогДанногоГолоса);
	КонецЕсли;	 
	
	ИмяКаталогПараметров = "S" + XMLСтрока(ПараметрыВидео.ЗаписьВидеоСкоростьПроизношения);
	Если НЕ Ванесса.ФайлСуществуетКомандаСистемы(КатлогДанногоГолоса + "\" + ИмяКаталогПараметров) Тогда
		Ванесса.СоздатьКаталогКомандаСистемы(КатлогДанногоГолоса + "\" + ИмяКаталогПараметров);
	КонецЕсли;	 
	
	Возврат КатлогДанногоГолоса + "\" + ИмяКаталогПараметров; 
КонецФункции	 

&НаКлиенте
Функция КаталогГдеЛежитКэшВидеоВставок(ПараметрыВидео)
	ИмяГолоса = "_";
	Если ПараметрыВидео.ЗаписьВидеоДелатьНаложениеTTS Тогда
		ИмяГолоса = ПараметрыВидео.ЗаписьВидеоИмяTTS;
	КонецЕсли;	 
	КатлогДанногоГолоса = ПараметрыВидео.ЗаписьВидеоКэшВидеоВставок + "\" + ИмяГолоса;
	Если НЕ Ванесса.ФайлСуществуетКомандаСистемы(КатлогДанногоГолоса) Тогда
		Ванесса.СоздатьКаталогКомандаСистемы(КатлогДанногоГолоса);
	КонецЕсли;	 
