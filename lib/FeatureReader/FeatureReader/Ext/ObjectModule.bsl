...
Процедура СкопироватьСтрокуВДереваПодчиненныеЭлементы(КудаКопировать,ОткудаКопировать,ПараметрыКопирования,СтруктураПараметров)
	МассивПараметровКопированияПодчиненныхЭлементов = Новый Массив;
	
	// ОткудаКопировать.ДопТип равен "СтруктураСценария" только в главной строке структуры сценария, в подчиненных строках он пустой.
	// Поэтому устанавливаем признак, по коротому можно будет узнать что находимся внутри структуры сценария. 
	// А после выхода обратно на уровень главной строки сцернария, очищаем признак.
	Если ОткудаКопировать.ДопТип = "СтруктураСценария" Тогда
		ПараметрыКопирования.Вставить("КопированиеЭлементовСтруктурыСценария", Истина);
	КонецЕсли; 
	...
	Для каждого СтрОткудаКопировать Из ОткудаКопировать.Строки Цикл
		...
		Если ПараметрыКопирования.Свойство("КопированиеЭлементовСтруктурыСценария") Тогда
			СкопироватьСтрокуВДереваПодчиненныеЭлементы(	
				СтрокаКуда,	
				СтрОткудаКопировать,	
				ПараметрыКопирования,	
				СтруктураПараметров);
		Иначе
			ПарамерыКопированияПодчиненных = Новый Структура;	
			ПарамерыКопированияПодчиненных.Вставить("СтрокаКуда", 			СтрокаКуда);	
			ПарамерыКопированияПодчиненных.Вставить("СтрОткудаКопировать", 	СтрОткудаКопировать);	
			ПарамерыКопированияПодчиненных.Вставить("ПараметрыКопирования", ПараметрыКопирования);	
			ПарамерыКопированияПодчиненных.Вставить("СтруктураПараметров", 	СтруктураПараметров);	
			МассивПараметровКопированияПодчиненныхЭлементов.Добавить(ПарамерыКопированияПодчиненных);
		КонецЕсли; 	
	КонецЦикла;
	
	Если ОткудаКопировать.ДопТип = "СтруктураСценария"
	   И ПараметрыКопирования.Свойство("КопированиеЭлементовСтруктурыСценария") Тогда
		ПараметрыКопирования.Удалить("КопированиеЭлементовСтруктурыСценария");	
	КонецЕсли; 	
	...
	Для каждого ЭлементМассива Из МассивПараметровКопированияПодчиненныхЭлементов Цикл	
		СкопироватьСтрокуВДереваПодчиненныеЭлементы(	
			ЭлементМассива.СтрокаКуда,	
			ЭлементМассива.СтрОткудаКопировать,	
			ЭлементМассива.ПараметрыКопирования,	
			ЭлементМассива.СтруктураПараметров);	
	КонецЦикла;	
КонецПроцедуры
