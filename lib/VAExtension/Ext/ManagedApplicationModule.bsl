		...
		Попытка
			Если ДанныеJSON.ВидСобытия = "СканерШтрихкода" Тогда
				...
				Если МодульБСПМенеджерОборудованияКлиент <> Неопределено Тогда
					...
					// Проверка модуля на этой строке выдаёт ошибку, но данная процедура присутствует в расширяемом модуле.
					ОбработкаВнешнегоСобытия(Источник, "Штрихкод", Штрихкод);
			...
			ИначеЕсли ДанныеJSON.ВидСобытия = "ВнешнееСобытие" Тогда
				...
				ОбработкаВнешнегоСобытия(ДанныеJSON.Источник, ДанныеJSON.Событие, ДанныеJSON.Данные);
				
				Результат = "Событие было обработанно";
