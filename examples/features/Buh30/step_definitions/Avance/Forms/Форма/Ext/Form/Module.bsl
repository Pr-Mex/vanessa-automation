...
Процедура УстановитьСоединениеССеансомTESTCLIENT()
	...
	ТекстСообщения = "ТипЗнч ГлавноеОкноТестКлиента= %1";
	ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ТипЗнч(ГлавноеОкноТестКлиента));
...
Процедура ЯЗапускаюСеанс1ССКлючомTestClient() Экспорт
	Если КонтекстСохраняемый.Свойство("ТестируемоеПриложение") Тогда
		Если КонтекстСохраняемый.ТестируемоеПриложение <> Неопределено Тогда
			...
			ТекстСообщения = "ТипЗнч КонтекстСохраняемый.ТестируемоеПриложение = %1";
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ТипЗнч(КонтекстСохраняемый.ТестируемоеПриложение));
			Сообщить(ТекстСообщения);
			Попытка
	...
	Если НЕ ФайлПроверкаСуществования.Существует() Тогда
		ТекстСообщения = "Не нашел путь к 1cv8c.exe: %1";
	...
	ТекстСообщения = "Строка запуска=%1";
