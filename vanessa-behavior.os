...
Процедура ЗапуститьСерверДляКукумбера()
	...
	Пока Истина Цикл
		...
		Попытка
			...
			//Сообщить(ЧтоВернем);
			Если Соединение.Активно тогда
				Соединение.ОтправитьСтроку(ЧтоВернем, "utf-8"); //Хитрый ход для передачи управления в Cucumber - добавлять в конце ответа пустую строку
				sleep(100);
			КонецЕсли;
...
ИначеЕсли  АргументыКоманднойСтроки.Количество() = 1 Тогда
	...
	КаталогПроекта         = ПолучитьЗначениеПараметраЗапуска(МассивПараметрыЗапуска,"ProjectPath");
	СтрокаПодключенияКБазе = ПолучитьЗначениеПараметраЗапуска(МассивПараметрыЗапуска,"1cConnectString");
	GenerateEpf            = ЕстьПараметр(МассивПараметрыЗапуска,"GenerateEpf");
	TestRun                = ЕстьПараметр(МассивПараметрыЗапуска,"TestRun");
	DebugLog               = ЕстьПараметр(МассивПараметрыЗапуска,"DebugLog");
	//СтрокаПодключенияКБазе = АргументыКоманднойСтроки[1];
