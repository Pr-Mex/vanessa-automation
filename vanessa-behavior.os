	...
	Пока Истина Цикл
		...
		Попытка
			Если Соединение.Активно тогда
				Стр = НакопитьСтроку(Соединение);//Соединение.ПрочитатьСтроку("utf-8");
				Если DebugLog Тогда
			...
			Исключение
				Сообщить("Соединение с Cucumber разорвано");
				Прервать;
			КонецПопытки;
...
Функция НакопитьСтроку(Знач Соединение)
	...
	Пока Истина Цикл
		...
		// Если СтрДлина(Порция) = 0 Тогда
			// Сообщить("Получили пустую строку");
			// Возврат Порция;
		// КонецЕсли;
		Если DebugLog Тогда
