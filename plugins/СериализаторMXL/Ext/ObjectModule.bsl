...
Функция СоздатьДанныеПоДокументуМакета(ТабличныйДокументИлиМассивТабличныхДокументов, РежимыЗагрузкиИлиИмяКолонкиЗамещения = Неопределено, ИмяКолонкиЗамещения = Неопределено, ВозвращатьНаборЗаписей = Ложь) Экспорт
	...
	Для каждого Элем Из Данные Цикл
		...
		Если ВозвращатьНаборЗаписей И (Метаданные.РегистрыБухгалтерии.Содержит(Элем.Значение.Метаданные())
	...
	Попытка
		...
		Иначе
			...
			Если ЗначениеПоУмолчанию = Неопределено И ТипЗначения.Типы().Количество() > 1 И НЕ УжеВычислилиЗначение И стрПоле.Значение <> "00000000-0000-0000-0000-000000000000" Тогда
				...
				Если НЕ ЗначениеЗаполнено(Значение) Тогда
					ВызватьИсключение СтрШаблон("Не нашли реквизит %1 по типам %2", стрПоле.Значение, СтрСоединить(ТипЗначения.Типы(), ","));
				КонецЕсли;				
			КонецЕсли;
