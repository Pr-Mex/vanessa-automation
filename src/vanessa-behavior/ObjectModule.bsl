...
Процедура ОбойтиДеревоДляОтчетаxUnit(Дерево, ЗаписьXML, ДанныеФичи = Неопределено)
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		Если      СтрСтроки.Фича = Истина Тогда
			...
			ОбойтиДеревоДляОтчетаxUnit(СтрСтроки,ЗаписьXML,ДанныеФичи);
			...
			ТекущаяФича =  СтрСтроки.Имя;
			
			ЗаписьXML.ЗаписатьНачалоЭлемента("testsuite");
			Если ДобавлятьКИмениСценарияУловияВыгрузки Тогда
				ЗаписьXML.ЗаписатьАтрибут("name", ПрефиксИмениСценария  + "/" + СтрСтроки.Имя);
			Иначе	
			...
			//Типtestsuites      = Фабрика.Тип("jUnit", "testsuites");
			//ТекущаяФича        = Фабрика.Создать(Типtestsuites);
			//ТекущаяФича.name   = СтрСтроки.Имя;
			//
			//Типtestsuite       = Фабрика.Тип("jUnit", "testsuite");
			//СписокСценариев    = Фабрика.Создать(Типtestsuite);
			//
			...
			//
			//
			//Типlabels   = Фабрика.Тип("jUnit", "labels");
			//СписокМеток = Фабрика.Создать(Типlabels);
			//
			//ДобавитьМетку(Фабрика,СписокМеток,"feature",СтрСтроки.Имя);
			//ДобавитьМетку(Фабрика,СписокМеток,"framework","vanessa-behavior");
...
Процедура СформироватьОтчетxUnit(СтруктураОФ = Неопределено) Экспорт
	...
	ОбойтиДеревоДляОтчетаxUnit(ДеревоТестов, ЗаписьXML);
