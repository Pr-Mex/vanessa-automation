...
Функция Определить_classname(ДопПараметры)
	...
	Если ДобавлятьКИмениСценарияУловияВыгрузки Тогда
		Если ДопПараметры.ТекУровеньКаталога >= 2 Тогда
			classname = СокрЛП(ПолучитьПрефиксИмениСценария()) + "." + ДопПараметры.УровниКаталогов[0];
		КонецЕсли;	 
	Иначе	
		Если ДопПараметры.ТекУровеньКаталога = 2 Тогда
			classname = ДопПараметры.УровниКаталогов[0];
		ИначеЕсли ДопПараметры.ТекУровеньКаталога >= 3 Тогда
			classname = ДопПараметры.УровниКаталогов[0] + "." + ДопПараметры.УровниКаталогов[1];
		КонецЕсли;	 
	КонецЕсли;	 
