...
Процедура ОбойтиДеревоДляОтчетаCucumberJson(Дерево, ЗаписьJson, ДопПараметры,ДанныеФичи = Неопределено)
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		Если      СтрСтроки.Фича = Истина Тогда
			...
			ЗаписьJson.ЗаписатьНачалоЭлемента("testsuite");
			Если ДобавлятьКИмениСценарияУловияВыгрузки Тогда
				ЗаписьJson.ЗаписатьАтрибут("name", СтрСтроки.Имя);
			Иначе	
				ЗаписьJson.ЗаписатьАтрибут("name", СтрСтроки.Имя);
			КонецЕсли;	 
			...
			ЗаписьJson.ЗаписатьАтрибут("errors", XMLСтрока(ДанныеФичи.errors));
			ЗаписьJson.ЗаписатьАтрибут("skipped", XMLСтрока(ДанныеФичи.skipped));
			ЗаписьJson.ЗаписатьАтрибут("tests", XMLСтрока(ДанныеФичи.tests));
			ЗаписьJson.ЗаписатьАтрибут("failures", XMLСтрока(ДанныеФичи.failures));
			ЗаписьJson.ЗаписатьАтрибут("time", XMLСтрока(ДанныеФичи.time/1000));
			
			
			Для Каждого Сценарий Из МассивСценариев Цикл
				ЗаписьJson.ЗаписатьНачалоЭлемента("testcase");
				
				Если ДобавлятьКИмениСценарияУловияВыгрузки Тогда
					ЗаписьJson.ЗаписатьАтрибут("name", ПрефиксИмениСценария + Сценарий.name);
				Иначе	
					ЗаписьJson.ЗаписатьАтрибут("name", Сценарий.name);
				КонецЕсли;	 
...
Процедура СформироватьОтчетCucumberJson(СтруктураОФ) Экспорт
	...
	ЗаписьJson.ОткрытьФайл(ВременныйJson,,Ложь,Новый ПараметрыЗаписиJSON(,Символы.Таб));
