...
Процедура ЗаполнитьДеревоИзвестныхШаговНаСервере(ДвДанныеvbFeatureReader)
	...
	Если ЗначениеЗаполнено(ФильтрДереваШагов) Тогда
		...
		Построитель = Новый ПостроительЗапроса;
		ТабРезультат2 = РеквизитФормыВЗначение("ТаблицаИзвестныхStepDefinition");
		Построитель.ИсточникДанных = Новый ОписаниеИсточникаДанных(ТабРезультат2);
		тОтбор = Построитель.Отбор.Добавить("ОписаниеШага");
		тОтбор.ВидСравнения = ВидСравнения.Содержит;
		тОтбор.Значение = ФильтрДереваШагов;
		тОтбор.Использование = Истина;
		...
		ТабРезультат2 = Построитель.Результат.Выгрузить();
