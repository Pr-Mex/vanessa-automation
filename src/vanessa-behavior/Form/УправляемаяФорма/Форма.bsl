...
Функция СоздатьКоллекциюКомандЗапуска()
	...
	Рез.Вставить("ВерсияПлатформыДляГенерацииEPF", 						Новый Структура("Р, А", "ДополнительныеПараметры", "dopparams"));	
	Возврат Рез;
КонецФункции
...
Функция ОпределитьИдСледующейСтроки(СтрокаДерева,СтрокиРодителя)
	...
	Если (СтрокиРодителя.Количество() - 1) >= ИдСледующейСтроки Тогда
		Возврат СтрокиРодителя[ИдСледующейСтроки];
	КонецЕсли;	 
...
&НаКлиенте
Функция НайтиТЧПоИмени(ИмяТЧ, НужнаяФорма = Неопределено, ИскатьПоИмени = Истина) Экспорт
	Получилось = Ложь;
