...
//#Область Сообщения пользователю
...
//#Область Обработка событий ДеревоТестов
...
&НаКлиенте
Процедура ЗаписатьЛогОшибки(СтрокаСценария,СтрокаШага,ТекстОшибки,ИнфоОбОшибке)
...
Функция ВывестиВСообщениеТекстОшибки(ТекстОшибки)
	...
	Стр = Стр + "ИсходнаяСтрока=" + ТекстОшибки.ИсходнаяСтрока + Символы.ПС;
	Стр = Стр + "НомерСтроки="    + ТекстОшибки.НомерСтроки + Символы.ПС;
	Стр = Стр + "Описание="       + ТекстОшибки.Описание + Символы.ПС;
	Стр = Стр + "Причина="        + ТекстОшибки.Причина;
	
	Возврат Стр;
КонецФункции	 

&НаКлиенте
Функция СформироватьОписаниеОшибки(Знач Стр, ИмяФайла, ИмяПроцедуры,ИнфоОбОшибке) Экспорт
	СисИнфо            = Новый СистемнаяИнформация; 
	...
	Попытка
		...
		Если ИнфоОбОшибке <> Неопределено Тогда
			ТекстОшибкиTestClient = "," + Символы.ПС + ВывестиВСообщениеТекстОшибки(ИнфоОбОшибке);
		КонецЕсли;	 
	Исключение
	КонецПопытки;
...
Процедура ВыполнитьШаг()
	...
	Исключение
		...
		Иначе
			...
			ИнфоОбОшибке = Неопределено;
			СтрОшибка = СформироватьОписаниеОшибки(ОписаниеОшибки(), АдресСнипета, ИмяПроцедуры,ИнфоОбОшибке);
			...
			Если Объект.ДелатьЛогОшибокВТекстовыйФайл Тогда
				ЗаписатьЛогОшибки(СтрокаСценария,СтрокаШага,СтрОшибка,ИнфоОбОшибке);
			КонецЕсли;	 
...
&НаКлиенте
Процедура ЗапомнитьАктивноеОкно(АктивноеОкно) Экспорт
...
Функция ПолучитьАктивноеОкноИзТестовоеПриложение(ДопПараметры = Неопределено) Экспорт
	...
	ЗапомнитьАктивноеОкно(АктивноеОкно);
