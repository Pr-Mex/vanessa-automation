...
Процедура СоздатьФайлMarkdownИнструкции()
	...
	Для каждого Сценарий Из МассивСценариев Цикл
		...
		КонецЕсли;	 
		Страница.ЗаписатьСтроку("#### <p align=""center"">Сценарий: " + Сценарий.ИмяСценария + "</p>");
...
Процедура ЗапуститьСеансTestClient(ДопПараметр = "")
	...
	Если ЭтоЗапускВРежимеКоманднойСтроки Тогда
		СтрокаЗапуска = " /TESTCLIENT /CTemp " + ДопПараметр;//добавил /CTemp, чтобы убрать передачу параметра в TestClient, с которым был запущен TestManager
	КонецЕсли;	 
