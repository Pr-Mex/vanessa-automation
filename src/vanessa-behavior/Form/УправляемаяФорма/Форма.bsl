...
Процедура ПерейтиКВыполнениюСледующегоСценария()
	...
	Если Объект.РежимВыполненияОдногоШага Тогда
		Если ТекИД_СценарияВМассиве >= 1 Тогда
			Объект.ИдетВыполнениеСценариев = Ложь;
			ОбновитьСтатусКнопкиВыполнитьСценарии();
			Возврат;
		КонецЕсли;	 
...
Процедура ВыполнинтьСценарийАссинхронноТаймер()
	...
	БылаОшибка = Ложь;
	ОписаниеОшибки = "";
	Если НЕ Объект.РежимВыполненияОдногоШага Тогда //если надо выполнить ПередНачаломСценария и при этом сделать отладку по шагам - надо поставить брейкпоинт на первом шаге сценария
...
&НаКлиенте
Процедура БрейпоинтУбратьВсе(Команда)
