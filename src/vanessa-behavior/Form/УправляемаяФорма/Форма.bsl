...
Процедура ДобавитьСнипетыВДеревоШаговНаСервере()
	...
	Если ЗначениеЗаполнено(ФильтрДереваШагов) Тогда
		...
		Построитель.ИсточникДанных = Новый ОписаниеИсточникаДанных(ТзнТаблицаИзвестныхStepDefinition);
	...
	тДерево = РеквизитФормыВЗначение("ДеревоИзвестныхШагов");
	тДерево.Строки.Очистить();
	
	Для каждого СтрСнипет Из ТабРезультат Цикл
		...
		ДобавитьСнипетВДеревоШаговНаСервере(тДерево,Снипет, СтрСнипет.ИмяФайла)
	КонецЦикла;
	
	ЗначениеВРеквизитФормы(тДерево, "ДеревоИзвестныхШагов");
...
Процедура ДобавитьСнипетВДеревоШаговНаСервере(тДерево, Снипет, ИмяФайла)
	...
	тСнипет = тДерево.Строки.Добавить();
	тСнипет.Наименование = Снипет.ПредставлениеТеста;
	тСнипет.Значение     = ИмяФайла;
	
	тИмяПроцедуры = тСнипет.Строки.Добавить();
	тИмяПроцедуры.Наименование = "Имя процедуры";
	тИмяПроцедуры.Значение     = Снипет.ИмяПроцедуры;
		
	тИмяСнипета = тСнипет.Строки.Добавить();
	...
	Если МассивПром.Количество() > 0 Тогда
		тПараметры = тСнипет.Строки.Добавить();
		тПараметры.Наименование = "Параметры";
		//тПараметры.Значение     = ТипЗнч(Снипет.Параметр);		
	...
	Если ЗначениеЗаполнено(Снипет.Транзакция) Тогда
		тТранзакция = тСнипет.Строки.Добавить();
...
Функция ПолучитьСтрокуЗапуска(ДанныеЗапускаКлиентаТестирования)
	...
	" ENTERPRISE " +
	?(ДанныеЗапускаКлиентаТестирования.Серверная, "/S ", "/F ") + ОбернутьВКавычки(ДанныеЗапускаКлиентаТестирования.ПутьКИнфобазе) +
	" /TESTCLIENT -TPort " + ОбернутьВКавычки(ДанныеЗапускаКлиентаТестирования.Порт);
