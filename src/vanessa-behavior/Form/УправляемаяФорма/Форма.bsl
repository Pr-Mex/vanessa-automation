...
Процедура ОбработатьУправляющуюСтрокуИнструкции(СлужебнаяСтрока,Параметры)
	...
	Если Параметры.Свойство("ОбработатьГруппыШагов") Тогда
		Если Параметры.ОбработатьГруппыШагов Тогда
			ДопПараметры = Новый Структура;
			...
			//ЭлементыГруппы = Параметры.СтрокаДереваГруппа.ПолучитьЭлементы();
			//Для Каждого ЭлементГруппы Из ЭлементыГруппы Цикл
			//	ИДСтроки = ЭлементГруппы.ИДСтроки;
			//КонецЦикла;	
		КонецЕсли;	 
	КонецЕсли;	 
...
Процедура НайтиГруппыШаговКоторыеНадоОбрабатыватьКакОдинШаг(МассивТекстФичи,Шаг,Сценарий)
	...
	Если Индекс = 0 Тогда
		...
		СлужебнаяСтрока = НайтиСлужебнуюСтрокуВФичеДляЭтогоШага(МассивТекстФичи,"[autodoc.groupsteps]",РодительШага.НомерСтрокиВФиче);
		Если НЕ ЗначениеЗаполнено(СлужебнаяСтрока) Тогда
			Возврат;
		КонецЕсли;	 
		...
		ПараметрыОбработки = Новый Структура;
		ПараметрыОбработки.Вставить("ВычислитьВыражения",Истина);
		ПараметрыОбработки.Вставить("ОбработатьГруппыШагов",Истина);
		ПараметрыОбработки.Вставить("Шаг",Шаг);
