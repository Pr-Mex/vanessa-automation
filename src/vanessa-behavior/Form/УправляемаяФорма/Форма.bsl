...
Процедура ЗаполнитьСтрокиДереваФичамиСервер(Дерево, FeatureReader, ТзнТаблицаИзвестныхStepDefinition, СтруктураПараметров)
	...
	Для каждого Строка Из Дерево.Строки Цикл
		Если Строка.Тип = "Фича" Тогда
			...
			СтруктураПараметров.Вставить("ТекущийФичаФайл",Строка.ПолныйПуть);
...
Процедура ЗагрузитьФичиПродолжение(ДополнительныеПараметры = Неопределено)
	...
	СтруктураПараметров.Вставить("ТекущийФичаФайл", Неопределено);
	...
	Исключение
		...
		Сообщить("Ошибка при построении дерева. <" + СтруктураПараметров.ТекущийФичаФайл + ">");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("КаталогИнструментов") Тогда
		ОбъектНаСервере.КаталогИнструментов = Параметры.КаталогИнструментов;
	КонецЕсли;	
	Если Параметры.Свойство("КаталогПроекта") Тогда
		ОбъектНаСервере.КаталогПроекта = Параметры.КаталогПроекта;
	КонецЕсли;	
...
&НаКлиенте
Функция НайтиРеквизитОткрытойФормыПоЗаголовку(ИмяРеквизита, ИскатьПоИмени = Ложь,ВызыватьИсключение = Истина, ТипПоля = Неопределено) Экспорт
	Нашли = Ложь;
	...
	Пока XML.Прочитать() Цикл
		...
		Если XML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			ИначеЕсли ИмяУзла = "setCurrentArea" Тогда
				Если МассивОбъектов[ИдВМассиве - 1] = "FormField" Тогда
					ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " у табличного документа с именем """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я перехожу к ячейке """ + МассивСвойств[ИдВМассиве]["area"] + """");
				КонецЕсли;	 
...
Функция ПолучитьТабДокИзTestClient(ЭлементФормы) Экспорт
	...
	Если НЕ ТекущийTestclientЭтоWebКлиент И ВерсияПриложенияБольшеИлиРавнаЧемЗаданная(СистемнаяИнформация.ВерсияПриложения, "8.3.8.0") Тогда
		...
		ТабДок = ПреобразоватьМакетВМассивСогласноПараметровСчитывания(ФайлМакета);
		УбратьПустыеСтрокиИКолонкиСКраёвМакета(ТабДок);
		Возврат ТабДок;
	Иначе	
...
&НаКлиенте
Функция ТабличныйДокументTestClientПоЯчейкам(ЭлементФормы)
	ТабДок = Новый ТабличныйДокумент;
	СчетчикСтрок = 0;
	Для Стр = СчитываниеМакетаНачСтр По СчитываниеМакетаНачСтр + СчитываниеМакетаКолСтр - 1 Цикл
		СчетчикСтрок = СчетчикСтрок + 1;
...
//параметры чтения макета задаются через шаг: Тогда Я задаю системные параметры считывания макета НачСтр 1 НачКол 1 КолСтр 5 КолКол 3
&НаКлиенте
Функция ПолучитьТабличныйДокументTestClient(ЭлементФормы) Экспорт
	СистемнаяИнформация = Новый СистемнаяИнформация;
	Если НЕ ТекущийTestclientЭтоWebКлиент И ВерсияПриложенияБольшеИлиРавнаЧемЗаданная(СистемнаяИнформация.ВерсияПриложения, "8.3.8.0") Тогда
		//значит можно сохранить макет в файл и пробросить в TestManager
		ИмяВременногоФайла = ПолучитьИмяВременногоФайла("mxl");
		...
		ФайлМакета = Новый ДвоичныеДанные(ИмяВременногоФайла);
		ТабДок = ОставитьЧастьМакетаСогласноПараметровЧтенияМакета(ФайлМакета);
