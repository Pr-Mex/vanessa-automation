...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ПодключитьУжеЗапущенныйTestClient() Тогда
		...
		Если СвободныйПорт <> Объект.ПортЗапускаТестКлиента Тогда
			СделатьСообщение("Порт " + Объект.ПортЗапускаТестКлиента + " занят, порт подключение изменен на " + СвободныйПорт);
...
Функция ПолучитьМассивЗанятыхПортов()
	...
	Если НЕ ЭтоLinux Тогда
		...
		Текст.Закрыть();
	Иначе
		// в каком формате вывод, что читать??
		СтрокаЗапуска = "ss -tuwan4 2>/dev/null | awk '{print $5}' | grep ':' | cut -d "":"" -f 2 | sort | uniq";
...
Функция СоздатьТестовоеПриложение()
	...
	ПараметрыОбъекта.Добавить(Объект.ПортЗапускаТестКлиента);
	//ТестовоеПриложение = Новый (ТипЗначения,ПараметрыОбъекта);  // не работает с несколькими параметрами
	ТестовоеПриложение = Новый ТестируемоеПриложение("localhost", Объект.ПортЗапускаТестКлиента);
