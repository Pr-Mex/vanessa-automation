...
Функция ВремяИзмененияФайлаКомандаСистемы(ИмяФайла)
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		...
		День    = Число(Сред(ТекстЛога,1,2));
		Месяц   = Число(Сред(ТекстЛога,4,2));
		Год     = Число(Сред(ТекстЛога,7,4));
		Часы    = Число(Сред(ТекстЛога,12,2));
		Минуты  = Число(Сред(ТекстЛога,15,2));
		Секунды = Число(Сред(ТекстЛога,18,2));
