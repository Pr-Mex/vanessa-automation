	...
	Если НЕ Объект.ЗапускИзКоманднойСтроки Тогда
		Если СокрЛП(Объект.КаталогФич) <> "" Тогда
			...
			ИначеЕсли Объект.ЗагрузкаФичПриОткрытии = "Задавать вопрос" Тогда
				Оповещение = Новый ОписаниеОповещения("ПослеОтветаНаВопросНадоЛиЗагружатьФичи",ЭтотОбъект);
				ПоказатьВопрос(Оповещение, "Загрузить фичи? <" + СокрЛП(Объект.КаталогФич) + ">", РежимДиалогаВопрос.ОКОтмена,0,КодВозвратаДиалога.ОК,"Демонстрируем немодальные окна.");
			КонецЕсли;	 
		КонецЕсли;	 
	КонецЕсли;	 
КонецПроцедуры
