...
Процедура ЗаписатьЛогВТекстПередВыполнениемСценария(ИмяСценария)
	ЗаписатьСтрокуВТекстовыйЛог("ПередВыполнениемСценария: " + ИмяСценария); 
КонецПроцедуры
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		Если ЗнДелатьЛогВыполненияСценариевВТекстовыйФайл = Истина Тогда
			...
			Объект.ИмяФайлаЛогВыполненияСценариев             = ПолучитьЗначениеПереданногоПараметра(СтрутктураJSON,"ИмяФайлаЛогВыполненияСценариев");
		КонецЕсли;	  		
...
Процедура ВыполнитьШаг()
	...
	Попытка
		...
		Если Объект.ДелатьЛогВыполненияСценариевВТекстовыйФайл Тогда
			ЗаписатьЛогВТекстПередВыполнениемШага(ТекШаг.Имя);
		КонецЕсли;	 
...
Процедура ВыполнитьСценарии(ТекСценарий = Неопределено) Экспорт
	...
	ИначеЕсли (ТекСценарий = Неопределено) и (Объект.РежимВыполненияДанногоСценарияСТекущегоШага) Тогда
		ИдСценарияПоСтрокеШага = ОпределитьИдСценарияПоСтрокеШага(ТекущаяСтрокаДереваИдСтроки);
		ТекСценарий = Новый Массив;
		ТекСценарий.Добавить(ИдСценарияПоСтрокеШага);
...
Функция СоздатьТестовоеПриложение(ИмяКомпьютера = "localhost", Порт = 1538)
	...
	Если Объект.ДелатьЛогВыполненияСценариевВТекстовыйФайл Тогда
		ЗаписатьЛогВТекстИнформация("СозданиеОбъектаТестируемоеПриложение","ИмяКомпьютера=" + ИмяКомпьютера + "; порт=" + Формат(Порт, "ЧГ="));
	КонецЕсли;	 
...
Функция ОткрытьНовыйСеансTestClientИлиПодключитьУжеСуществующийДляНесколькихКлиентов(ДанныеЗапускаКлиентаТестирования)
	...
	Если ПодключитьУжеЗапущенныйTestClient(ДанныеЗапускаКлиентаТестирования.ПортЗапускаТестКлиента) Тогда
		...
		КонецЕсли;	 
		Если Объект.ДелатьЛогВыполненияСценариевВТекстовыйФайл Тогда
			ЗаписатьЛогВТекстИнформация("НайденПодключенныйTestClient","порт=" + Формат(ДанныеЗапускаКлиентаТестирования.ПортЗапускаТестКлиента, "ЧГ="));
		КонецЕсли;	 
		Возврат Истина;
	КонецЕсли;
