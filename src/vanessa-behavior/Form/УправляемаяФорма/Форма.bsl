	...
	Если СокрЛП(АдресСнипета) = "" Тогда
		...
		Если БылаОшибка Тогда
			...
			Если Объект.ОстановкаПриВозникновенииОшибки Тогда
				СделатьСообщение("Установлен флег остановки при возникновении первой ошибки. Выполнение сценариев будет прервано.");
...
Процедура НачатьЗаписьДействийПользлователяВВыбранномКлиентеТестирования(Команда)
	ТекущиеДанные = Элементы.ДанныеКлиентовТестирования.ТекущиеДанные;
	Если ТекущиеДанные.Подключен Тогда
...
Процедура КлиентТестированияТекущейИнфобазы(Команда)
	...
	ЗаполнитьДанныеТекущйИнфобазы(ДанныеТекущейИнфобазы);
	
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		ОписаниеОповещения = Вычислить("Новый ОписаниеОповещения(""КлиентТестированияТекущейИнфобазыЗавершение"", ЭтотОбъект, ДанныеТекущейИнфобазы)");
		Выполнить("НачатьПоискФайлов(ОписаниеОповещения, ""C:\"", ""1cv8c.exe"", Истина)");

	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура КлиентТестированияТекущейИнфобазыЗавершение(НайденныеФайлы, ДанныеТекущейИнфобазы) Экспорт
	СистемнаяИнформация = Новый СистемнаяИнформация;
	
	ВерсияПриложения = СистемнаяИнформация.ВерсияПриложения;
	Для Каждого Файл Из НайденныеФайлы Цикл
		ПолноеИмяФайла = Файл.ПолноеИмя;
		Если НайтиВСтроке(ПолноеИмяФайла, ВерсияПриложения) > 0 Тогда
			ДанныеТекущейИнфобазы.Вставить("ПутьКПлатформе", ПолноеИмяФайла);
...
&НаКлиентеНаСервереБезКонтекста
Функция НайтиВСтроке(Знач СтрокаПоиска, Знач ИскомаяСтрока)
	Результат = 0;
	
	Если Ложь
		Или ПустаяСтрока(ИскомаяСтрока)
		Или ПустаяСтрока(СтрокаПоиска)
		Тогда
	...
	ДлинаТекста = СтрДлина(ИскомаяСтрока);
	
	КоличествоПроверяемыхСимволово = ДлинаСтрокиПоиска - ДлинаТекста + 1;
	
	Для Счетчик = 1 По КоличествоПроверяемыхСимволово Цикл
		ТекущийФрагмент = Сред(СтрокаПоиска, Счетчик, ДлинаТекста);
		Если ТекущийФрагмент = ИскомаяСтрока Тогда
			Результат = Счетчик;
...
&НаКлиенте
Функция ПолучитьКлиентаТестирования(ИмяКлиентаТестирования) Экспорт
...
&НаСервере
Процедура ЗаполнитьДанныеТекущйИнфобазы(ДанныеТекущейИнфобазы)
	ДанныеТекущейИнфобазы.Вставить("Имя", "Этот клиент");
	
	СтрокаСоединения =  СтрокаСоединенияИнформационнойБазы();

	ЭтоСерверная = НайтиВСтроке(ВРег(СтрокаСоединения), "FILE=") = 0;
	ДанныеТекущейИнфобазы.Вставить("Серверная", ЭтоСерверная);
	
	ПутьКИнфобазе = "";
	Если ЭтоСерверная Тогда
		ПутьКИнфобазе = СтрЗаменить(СтрокаСоединения, """", "");
	Иначе
		ПутьКИнфобазе = СтрЗаменить(СтрЗаменить(СтрокаСоединения,  "File=""", ""), """;", "");
	КонецЕсли;
	
	ДанныеТекущейИнфобазы.Вставить("ПутьКИнфобазе", ПутьКИнфобазе);
	
	ДанныеТекущейИнфобазы.Вставить("Порт", "1538");
КонецПроцедуры
...
Функция УдалосьПодключитьсяККлиентуТестирования(КлиентТестирования, ДанныеЗапускаКлиентаТестирования)
	...
	ТестКлиент = Вычислить("Новый ТестируемоеПриложение(""localhost"", ДанныеЗапускаКлиентаТестирования.Порт)");
...
Процедура ЗапуститьКлиентаТестирования(ДанныеЗапускаКлиентаТестирования)
	СтрокаЗапуска = ПолучитьСтрокуЗапуска(ДанныеЗапускаКлиентаТестирования);
...
&НаКлиенте
Функция ПолучитьСтрокуЗапуска(ДанныеЗапускаКлиентаТестирования)
	Результат = "";
	
	ПутьКОбработке = "";
	Если Не ПустаяСтрока(ДанныеЗапускаКлиентаТестирования.ПутьКЗапускаемойОбработке) Тогда
		ПутьКОбработке = "/EXECUTE " + ОбернутьВКавычки(ДанныеЗапускаКлиентаТестирования.ПутьКЗапускаемойОбработке);
		
	КонецЕсли;

	Пользователь = "";
	Если Не ПустаяСтрока(ДанныеЗапускаКлиентаТестирования.Пользователь) Тогда
		Пользователь = "/N " + ОбернутьВКавычки(ДанныеЗапускаКлиентаТестирования.Пользователь);
		
	КонецЕсли;
	
	Пароль = "";
	Если Не ПустаяСтрока(ДанныеЗапускаКлиентаТестирования.Пароль) Тогда
		Пароль = "/P " + ОбернутьВКавычки(ДанныеЗапускаКлиентаТестирования.Пароль);
		
	КонецЕсли;

	Результат = "" +
	ОбернутьВКавычки(ДанныеЗапускаКлиентаТестирования.ПутьКПлатформе) +
	" ENTERPRISE " +
	?(ДанныеЗапускаКлиентаТестирования.Серверная, "/S ", "/F ") + ОбернутьВКавычки(ДанныеЗапускаКлиентаТестирования.ПутьКИнфобазе) + " " + Пользователь + " " + Пароль +
	" /TESTCLIENT -TPort " + ОбернутьВКавычки(ДанныеЗапускаКлиентаТестирования.Порт) + ПутьКОбработке;
