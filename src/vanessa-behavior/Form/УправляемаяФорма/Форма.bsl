...
&НаСервереБезКонтекста
Процедура СохранитьНедавноЗагруженныеФичи(НедавноЗагруженныеФичи)
...
Процедура ДобавитьФичуВСписокНедавноЗагруженныхФич(Знач Путь)
	...
	СохранитьНедавноЗагруженныеФичи(НедавноЗагруженныеФичи);
	СкопироватьНедавноЗагруженныеФичиВИнтерфейс(НедавноЗагруженныеФичи);
...
Процедура ЗаполнитьКонтекстноеМеню()
	...
	//Пункт1=Элементы.Добавить("ДеревоИзвестныхШаговКонтекстноеМенюПунктСвернутьДеревоИзвестныхШагов", Тип("КнопкаФормы"), Элементы.ГруппаКнопокНедавноЗагруженныеФичи);
	//Пункт1.Заголовок  = НСтр("ru='Свернуть до шагов';en='Switch to steps'");
	//Пункт1.ИмяКоманды = "СвернутьДеревоИзвестныхШагов";
	
	
	//Пункт1=Элементы.Добавить("ДеревоИзвестныхШаговКонтекстноеМенюПунктСвернутьДеревоИзвестныхШагов", Тип("КнопкаФормы"), Элементы.ДеревоИзвестныхШагов.КонтекстноеМеню);
	//Пункт1.Заголовок  = НСтр("ru='Свернуть до шагов';en='Switch to steps'");
	//Пункт1.ИмяКоманды = "СвернутьДеревоИзвестныхШагов";
	//
	//Пункт2=Элементы.Добавить("ДеревоИзвестныхШаговКонтекстноеМенюПунктСвернутьДеревоИзвестныхФич", Тип("КнопкаФормы"), Элементы.ДеревоИзвестныхШагов.КонтекстноеМеню);
	//Пункт2.Заголовок  = НСтр("ru='Свернуть до фич';en='Switch to features'");
	//Пункт2.ИмяКоманды = "СвернутьДеревоИзвестныхФич";	
	
	
	//меню создаётся в стандартных настройках
	...
	//Пункт1.Заголовок=НСтр("ru='Выполнить выделенный сценарий';en='Invoke only selected scenario'");
...
Функция СоздатьКоллекциюКомандЗапуска()
	...
	Рез.Вставить("ДелатьОтчетВФорматеjUnit", Новый Структура("Р,А", "ДелатьОтчетВФорматеjUnit","xunitcreatereport"));	 
		...
		Если ЗнДелатьОтчетВФорматеjUnit = Истина Тогда
			...
			Если ЗначениеЗаполнено(ЗнКаталогOutputxUnit) Тогда
				...
				Если Не ФайлСуществуетКомандаСистемы(Файл.ПолноеИмя, "ЗнКаталогOutputxUnit") Тогда
					СоздатьКаталогКомандаСистемы(ЗнКаталогOutputxUnit);
					ФайлПроверкаСуществования = Новый Файл(ЗнКаталогOutputxUnit);
					Если НЕ ФайлСуществуетКомандаСистемы(ФайлПроверкаСуществования.ПолноеИмя) Тогда
						Стр = "Не смог создать каталог для отчета xUnit: " + ЗнКаталогOutputxUnit;
	...
	Если (МассивСценариевДляВыполнения.Количество()-1) < ТекИД_СценарияВМассиве Тогда
		...
		Если Объект.ДелатьОтчетВФорматеjUnit Тогда
			...
			Если Объект.ДобавлятьКИмениСценарияУловияВыгрузки Тогда
				...
				ИмяФайлаXML           = ДополнитьСлешВПуть(ПреобразоватьПутьСТочкамиКНормальномуПути(Объект.КаталогOutputxUnit)) + "/" + ИмяСборки + "-junit.xml";
			Иначе	
				ИмяФайлаXML           = ДополнитьСлешВПуть(ПреобразоватьПутьСТочкамиКНормальномуПути(Объект.КаталогOutputxUnit)) + "/junit.xml";
			КонецЕсли;	 
...
Процедура ВыполнитьСценарии(ТекСценарий = Неопределено) Экспорт
	...
	Если Объект.ДелатьОтчетВФорматеjUnit Тогда
		СоздатьКаталогЕслиЕгоНет(Объект.КаталогOutputxUnit);
	КонецЕсли;	 
...
&НаКлиенте
Процедура ПреобразоватьИсходныйXML(Команда)
...
Процедура ЗапуститьСеансTestClient(ДопПараметр = "")
	...
	//ДанныеЗапускаКлиентаТестирования.Вставить("Имя",            "Этот клиент");
	ДанныеЗапускаКлиентаТестирования.Вставить("ПутьКПлатформе", СтрокаЗапуска1с);
	ДанныеЗапускаКлиентаТестирования.Вставить("ПутьКИнфобазе",  КаталогБазы);
	ДанныеЗапускаКлиентаТестирования.Вставить("Порт",           Объект.ПортЗапускаТестКлиента);
	
	ЗапуститьКлиентаТестирования(ДанныеЗапускаКлиентаТестирования, ДопПараметр);
...
Процедура ПодключитьВыбранногоКлиентаТестирования(Команда)
	...
	КлиентТестирования = ПолучитьКлиентаТестирования(ТекущиеДанные.Имя);
	...
	Иначе		
		...
		ЗаполнитьКонтекстСохраняемыйДляТестовоеПриложение(КлиентТестирования);
	КонецЕсли;
...
Процедура ОтключитьВыбранногоКлиентаТестирования(Команда)
	//Сообщить("Функционал отключения полностью не реализован");
	
	ОбъектКонтекстСохраняемый.Вставить("ТестовоеПриложение", "");
	ОбъектКонтекстСохраняемый.Вставить("ГлавноеОкноТестируемого", "");
	
	СброситьФлагПодключенияКлиентаТестирования(Элементы.ДанныеКлиентовТестирования.ТекущиеДанные.Имя);
КонецПроцедуры
...
Функция ПолучитьКлиентаТестирования(ИмяКлиентаТестирования, ДопПараметр = "") Экспорт
	ИскомыйКлиент = Неопределено;

	ДанныеЗапускаКлиентаТестирования = НайтиДанныеЗапускаКлиентаТестирования(ИмяКлиентаТестирования);
	 
	Если Не УдалосьПодключитьсяККлиентуТестирования(ИскомыйКлиент, ДанныеЗапускаКлиентаТестирования) Тогда
		ДопПараметр = ДопПараметр + ДанныеЗапускаКлиентаТестирования.ДопПараметры;
		ЗапуститьКлиентаТестирования(ДанныеЗапускаКлиентаТестирования, ДопПараметр);
	...
	Если НайденныеСтроки.Количество() > 0 Тогда
		...
		Если ИскомыйКлиент = Неопределено Тогда
	...
	Возврат ИскомыйКлиент;
	
КонецФункции

&НаКлиенте
Процедура СброситьФлагПодключенияКлиентаТестирования(ТекущееПодключение)
	Отбор = Новый Структура;
	Отбор.Вставить("Подключен", Истина);
	Отбор.Вставить("Имя", ТекущееПодключение);
...
Процедура ЗаполнитьДанныеТекущейИнфобазы()
	...
	Если Найти(ВРег(КаталогБазы),ВРег("File=")) = 0 Тогда
		ДанныеТекущейИнфобазы.Вставить("Серверная", Истина);
	КонецЕсли;
	...
	ДанныеТекущейИнфобазы.Вставить("Порт", Формат(СвободныйПорт, "ЧГ="));	
...
&НаКлиенте
Функция УдалосьПодключитьсяККлиентуТестирования(КлиентТестирования, ДанныеЗапускаКлиентаТестирования)
	Результат = Ложь;

	ТестКлиент = Вычислить("Новый ТестируемоеПриложение(""localhost"", Формат(ДанныеЗапускаКлиентаТестирования.Порт,""ЧГ=""))");
...
Процедура ЗапуститьКлиентаТестирования(ДанныеЗапускаКлиентаТестирования, ДопПараметр = "")
	...
	СвободныйПорт = ПроверитьПортНаЗанятость(ДанныеЗапускаКлиентаТестирования.Порт);
	Если СвободныйПорт <> ДанныеЗапускаКлиентаТестирования.Порт Тогда
		СделатьСообщение("Порт " + ДанныеЗапускаКлиентаТестирования.Порт + " занят, порт подключения изменен на " + СвободныйПорт);
		ДанныеЗапускаКлиентаТестирования.Порт = СвободныйПорт;
		Отбор = Новый Структура;
		...
		Если НайденныеКлиенты.Количество() = 1 Тогда
			НайденныеКлиенты[0].Порт = ДанныеЗапускаКлиентаТестирования.Порт;
		КонецЕсли;
	КонецЕсли;
	
	СтрокаЗапуска = СтрокаЗапуска + "-TPort " + Формат(ДанныеЗапускаКлиентаТестирования.Порт,"ЧГ=") + " ";	
...
&НаКлиенте
Процедура ПодготовитьСценарийКВыполнению(Команда)
	...
	СоздатьКаталог(КаталогВременногоFeature);
	
	ИмяВременногоFeature = КаталогВременногоFeature + Разделитель + ВременныйФайл.Имя;
	...
	УдалитьФайлы(ИмяВременногоFeature);
