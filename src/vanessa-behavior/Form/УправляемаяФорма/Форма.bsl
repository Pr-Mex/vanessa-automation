...
Процедура ВыполнитьШаг()
	...
	Исключение
		...
		Иначе
			...
			Если Объект.ДелатьЛогВыполненияСценариевВТекстовыйФайл Тогда
				ЗаписатьСтрокуВТекстовыйЛог("Шаг (" + ТекШаг.Имя + ") не выполнен. Строка №" + XMLСтрока(ТекШаг.НомерСтрокиВФиче));
...
Функция НайтиФормыТекущегоОкнаTestClient(ДопПараметры = Неопределено) Экспорт
	...
	Если ДопПараметры <> Неопределено Тогда
		ДопПараметры.Вставить("ТекущееОкно",ОкноПриложения);
	КонецЕсли;	 
	Возврат МассивФорм;
КонецФункции	
	...
	Для каждого ЭлемСтало Из Стало.МассивЭлементов Цикл
		...
		Для каждого Свойство Из ЭлемСтало Цикл
			...
			Если Ключ = "ПредставлениеДанных" Тогда
				...
				Если Было = Неопределено Тогда
					Изменение = "элемент формы с именем """ + ЭлемСтало.Имя + """ стал равен " + ПолучитьКорректноеПредставлениеСтроковогоЗначенияПоля("""" + ПредставлениеДанных + """");
					...
					Если ЭлемСтало.Свойство("ТекстРедактирования") Тогда
						Если ЭлемСтало.ТекстРедактирования <> ПредставлениеДанных Тогда
							Изменение = "у элемента формы с именем """ + ЭлемСтало.Имя + """ текст редактирования стал равен " + ПолучитьКорректноеПредставлениеСтроковогоЗначенияПоля("""" + ЭлемСтало.ТекстРедактирования + """");
