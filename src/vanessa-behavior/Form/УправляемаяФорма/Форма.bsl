...
Функция СоздатьСтруктуруПараметровЗапуска()
	...
	Рез.Вставить("КаталогOutputjUnitБазовый", "");
...
Функция СоздатьКоллекциюКомандЗапуска()
	...
	Рез.Вставить("КаталогOutputjUnit", Новый Структура("Р,А", "КаталогOutputjUnit","xunitpath"));	
	Рез.Вставить("КаталогOutputjUnitБазовый", Новый Структура("Р,А", "КаталогOutputjUnitБазовый","xunitpathbase"));
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		Если ЗнДелатьОтчетВФорматеjUnit = Истина Тогда
			...
			ЗнКаталогOutputjUnitБазовый = ПолучитьЗначениеПереданногоПараметра(СтрутктураJSON,"КаталогOutputjUnitБазовый");
			Если ЗначениеЗаполнено(ЗнКаталогOutputjUnitБазовый) Тогда
				Файл = Новый Файл(ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогOutputjUnitБазовый));
				Если Не ФайлСуществуетКомандаСистемы(Файл.ПолноеИмя, "ЗнКаталогOutputjUnitБазовый") Тогда
					СоздатьКаталогКомандаСистемы(ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогOutputjUnitБазовый));
				КонецЕсли;	 
				
				ЗнКаталогOutputjUnitБазовый = ПреобразоватьПутьСТочкамиКНормальномуПути(ДополнитьСлешВПуть(ЗнКаталогOutputjUnitБазовый) + ПолучитьЗначениеПереданногоПараметра(СтрутктураJSON,"ИмяСборки"));
				ОчиститьСодержимоеКаталога(ЗнКаталогOutputjUnitБазовый);
				ФайлПроверкаСуществования = Новый Файл(ЗнКаталогOutputjUnitБазовый);
				Если НЕ ФайлСуществуетКомандаСистемы(ФайлПроверкаСуществования.ПолноеИмя, "ЗнКаталогOutputjUnitБазовый") Тогда
					СоздатьКаталогКомандаСистемы(ЗнКаталогOutputjUnitБазовый);
					
					ФайлПроверкаСуществования = Новый Файл(ЗнКаталогOutputjUnitБазовый);
					Если НЕ ФайлСуществуетКомандаСистемы(ФайлПроверкаСуществования.ПолноеИмя) Тогда
						СделатьСообщение("Не смог создать каталог для отчета jUnit: " + ЗнКаталогOutputjUnitБазовый);
						Возврат;
					КонецЕсли;	 
				КонецЕсли;	 
				
				Объект.КаталогOutputjUnit       = ЗнКаталогOutputjUnitБазовый;
				Объект.ДелатьОтчетВФорматеjUnit = Истина;
				ОпределилиКаталогjUnit          = Истина;
			КонецЕсли;	 
...
Процедура ВыполнинтьСценарийАссинхронноТаймер()
	...
	Если (МассивСценариевДляВыполнения.Количество()-1) < ТекИД_СценарияВМассиве Тогда
		...
		Если Объект.ДелатьОтчетВФорматеjUnit Тогда
			...
			//Если СтруктураОФ.Свойство("МассивXMLОтчетаjUnit") Тогда 
			//	МассивXMLОтчетUnit = СтруктураОФ.МассивXMLОтчетаjUnit;
			//	Для каждого Элем Из МассивXMLОтчетUnit Цикл
			//		ИмяФайлаXML           = ДополнитьСлешВПуть(ПреобразоватьПутьСТочкамиКНормальномуПути(Объект.КаталогOutputjUnit)) + Элем.РеальноеИмяФайла;
			//		ФайлXMLДвоичныеДанные = Элем.ФайлXMLДвоичныеДанные;
			//		ФайлXMLДвоичныеДанные.Записать(ИмяФайлаXML);
			//		
			//		СделатьСообщение("Файл отчета jUnit-report (" + ИмяФайлаXML + ") записан.");
...
Функция ИдентификаторКонтекстаДанныхТекущегоТеста()
	...
	Возврат "jUnitFor1C_ДанныеТекущегоТеста";
...
Процедура ЗапуститьКлиентаТестирования(ДанныеЗапускаКлиентаТестирования, ДопПараметр = "")
	...
	Если WshShell = Неопределено Тогда
		WshShell	= Новый COMОбъект("WScript.Shell");
	КонецЕсли;	 
	Рез			= WshShell.Run(СтрокаЗапуска,1,Ложь);
	WshShell	= Неопределено;	
