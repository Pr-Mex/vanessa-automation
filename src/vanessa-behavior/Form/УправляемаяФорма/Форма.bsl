		...
		Пока Истина Цикл
			...
			Если НашлиТегExportScenarios Тогда
				Если Лев(СтрНРег,9) = "сценарий:" Тогда
					...
					ТаблицаУжеСуществующихСценариев.Добавить(ПолучитьСтруктуруДляСценария((ИмяСценария),ИмяФайла,ТаблицаШагов,Файл.ПолноеИмя));
					Продолжить;
...
&НаКлиенте
Функция ПолучитьСтруктуруДляСценария(ИмяСценария,ИмяФайла,ТаблицаШагов,ПолноеИмя)
...
Процедура ВыполнитьШаг()
	...
	Если СтрокаШага.ШагСПараметрамиВТаблице = Истина Тогда
		...
		Для каждого СтрокиТаблицы Из ПодчиненныеСтроки Цикл
			...
			Если НЕ СтрокаПарамТаблица.Свойство("_КоличествоКолонок") Тогда
				СтрокаПарамТаблица.Вставить("_КоличествоКолонок",СтрокиТаблицы.ПараметрыТаблицы.Количество());
			КонецЕсли;	 
