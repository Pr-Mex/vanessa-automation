...
Процедура ВосстановитьНастройки(ЭтоLinux)
	...
	Если ТипЗнч(Настройки) = Тип("Структура") Тогда
		...
		Настройки.Свойство("ЗаписьВидеоПоказыватьКурсорМыши", Объект.ЗаписьВидеоПоказыватьКурсорМыши);
...
Процедура СохранитьНастройки()
	...
	Настройки.Вставить("ЗаписьВидеоПоказыватьКурсорМыши", Объект.ЗаписьВидеоПоказыватьКурсорМыши);
...
Функция СоздатьСтруктуруПараметровЗапуска()
	...
	Рез.Вставить("ЗаписьВидеоПоказыватьКурсорМыши", Ложь);
...
Функция СоздатьКоллекциюКомандЗапуска()
	...
	Рез.Вставить("ЗаписьВидеоПоказыватьКурсорМыши", Новый Структура("Р,А", "ЗаписьВидеоПоказыватьКурсорМыши","showmousecur"));
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		Объект.ЗаписьВидеоПоказыватьКурсорМыши                         = ПолучитьЗначениеПереданногоПараметра(СтрутктураJSON,"ЗаписьВидеоПоказыватьКурсорМыши");
...
Функция ПолучитьПараметрыЗаписиВидео()
	...
	ПараметрыВидео.Вставить("ЗаписьВидеоПоказыватьКурсорМыши",Объект.ЗаписьВидеоПоказыватьКурсорМыши);
	...
	ИнформацияЭкранаКлиента = ПолучитьИнформациюЭкрановКлиента()[0];
...
Процедура ПодсветитьАктивноеОкно(Длительность,ЖдатьЗавершения = Истина) Экспорт
	...
	ПутьКПриложению = Объект.КаталогИнструментов + "\tools\VideoTools\FrameShow\FrameShow.exe";
...
Процедура ПереместитьКурсорМышиКАктивномуОкну(Скорость,ЖдатьЗавершения = Истина) Экспорт
	...
	ПутьКПриложению = Объект.КаталогИнструментов + "\tools\VideoTools\MouseMove\MouseMove.exe";
	Если Не ФайлСуществуетКомандаСистемы(ПутьКПриложению) Тогда
		Возврат;
	КонецЕсли;	 
