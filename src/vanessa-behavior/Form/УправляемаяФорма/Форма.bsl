...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		ЗнТаймаутЗапуска1С = ПолучитьЗначениеПереданногоПараметра(СтрутктураJSON, "ДиапазонПортовTestclient");
		Если ЗначениеЗаполнено(ЗнТаймаутЗапуска1С) Тогда
			Объект.ДиапазонПортовTestclient = ЗнТаймаутЗапуска1С;
		КонецЕсли;	 
...
Функция СформироватьОписаниеОшибки(Знач Стр, ИмяФайла, ИмяПроцедуры) Экспорт
	...
	Стр = Стр + Символы.ПС + "ИмяФайла=" + ИмяФайла + ", ИмяПроцедуры=" + ИмяПроцедуры + ", версия VB=" + ПолучитьВерсиюОбработкиКлиент() + ", версия платформы=" + ВерсияПриложения + ", режим совместимости=" + РежимСовместимости;
	Возврат Стр;
КонецФункции
	...
	Для каждого ЭлемСтало Из Стало.МассивЭлементов Цикл
		...
		Для каждого Свойство Из ЭлемСтало Цикл
			...
			ИначеЕсли Ключ = "ТекущаяДоступность" Тогда
				Значение = "доступен";
				Если ЭлемСтало[Ключ] = Строка(Ложь) Тогда
