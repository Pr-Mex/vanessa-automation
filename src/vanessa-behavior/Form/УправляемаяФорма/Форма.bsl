...
&НаКлиенте
Процедура ПодключтьСнипетыИзОбработкиКлиент(Файл,ВерсияФайла,БылиОшибки,ДобавлятьСнипеты)
	Попытка
...
Процедура ПолучитьУжеСуществующиеСнипетыИзОбработок(Знач КаталогФич,ДополнительныеПараметры = Неопределено,МассивВерсийФайловИзТаблицаИзвестныхStepDefinition = Неопределено,МассивИменФайловИзТаблицаИзвестныхStepDefinition = Неопределено,МассивИменФайловКоторыеСовпалиСКеш = Неопределено)
	...
	Для Каждого ОбъектФайл Из МассивФайлов Цикл
		...
		ПодключтьСнипетыИзОбработкиКлиент(Файл,ВерсияФайла,БылиОшибки,Истина);	
...
Процедура ВыполнитьШаг()
	...
	Если ИД = Неопределено Тогда
		...
		ПодключтьСнипетыИзОбработкиКлиент(Файл,Неопределено,БылиОшибки,Ложь);	
		ИД = МассивИменКонтекстовОбработок.Найти(АдресСнипета);
	КонецЕсли;	 
	...
	Для каждого ЭлемСтало Из Стало.МассивЭлементов Цикл
		...
		Для каждого Свойство Из ЭлемСтало Цикл
			...
			ИначеЕсли Ключ = "ЗначениеТЧ" Тогда
				...
				Если ЭлемБыло[Ключ] = Неопределено Тогда
