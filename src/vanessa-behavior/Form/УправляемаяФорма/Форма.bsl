...
Процедура ДеревоТестовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТекДанные.ТекстОшибки) Тогда
		...
		ПоказатьТекстОшибки(ТекДанные.ТекстОшибки);
	КонецЕсли;	 
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	Версия8311ИлиВыше   = ВерсияПриложенияБольшеИлиРавнаЧемЗаданная(СистемнаяИнформация.ВерсияПриложения, "8.3.11.2867");
...
Процедура ВыполнитьСценарии(ТекСценарий = Неопределено, ДопПараметры = Неопределено) Экспорт
	...
	Если Объект.СоздаватьИнструкциюВидео Тогда
		Если Не ЗначениеЗаполнено(Объект.КаталогOutputИнструкцияВидео) Тогда
			ВызватьИсключение "Не указан каталог для формирования видео инструкции.";
		КонецЕсли;	 
	...
	Если Объект.ДелатьЛогОшибокВТекстовыйФайл Тогда
		Если ФайлСуществуетКомандаСистемы(Объект.ИмяКаталогаЛогОшибок) Тогда
			ОчиститьСодержимоеКаталога(Объект.ИмяКаталогаЛогОшибок);
		КонецЕсли;	 
	КонецЕсли;	 
...
&НаКлиенте
Процедура ОбработатьСменуАктивногоОкна() Экспорт
...
Функция ПолучитьКорректноеЗначениеДляРаботыСПолем(Знач Значение) Экспорт
	Если Лев(Значение,2) = "$$" и  Прав(Значение,2) = "$$" Тогда
		...
		Если СохраненноеЗначение = Неопределено Тогда
			ВызватьИсключение "Не смог найти сохранненное значение по переменой <" + ИмяПеременной + ">";
		КонецЕсли;	 
...
Функция НайтиТЧПоИмени(ИмяТЧ, НужнаяФорма = Неопределено, ИскатьПоИмени = Истина,ВызыватьИсключение = Истина) Экспорт
	...
	Если ТЧ = Неопределено и ВызыватьИсключение Тогда
		Стр = "Таблица формы с именем <" + ИмяТЧ + "> не найдена.";
		Сообщить(Стр);
		ВызватьИсключение Стр;
	КонецЕсли;	 
...
Функция ПолучитьАктивноеОкноИзТестовоеПриложение(ДопПараметры = Неопределено) Экспорт
	Если Не ОбъектКонтекстСохраняемый.Свойство("ТестовоеПриложение") Тогда
		ВызватьИсключение ПолучитьТекстСообщенияПользователю("$TestClientНЕПодключен$");
	КонецЕсли;	 
...
Функция НайтиОкноПредупреждения() Экспорт
	...
	ОкнаПриложения = ОбъектКонтекстСохраняемый.ТестовоеПриложение.НайтиОбъекты(Тип("ТестируемоеОкноКлиентскогоПриложения"));
	Для Каждого ОкноПриложения Из ОкнаПриложения Цикл
		Если (ОкноПриложения.Заголовок = "1С:Предприятие")
			или (ОкноПриложения.Заголовок = "1C:Enterprise")
			или (ОкноПриложения.Заголовок = "Выбор типа данных")
			или (ОкноПриложения.Заголовок = "Список значений")
			Тогда
	...
	Если НЕ ТекущийTestclientЭтоWebКлиент И ВерсияПриложенияБольшеИлиРавнаЧемЗаданная(СистемнаяИнформация.ВерсияПриложения, "8.3.8.0") Тогда
		...
		Пока Истина Цикл
			...
			Если КоличествоПопытокПолученияДвоичныхДанных > 10 Тогда
				ВызватьИсключение "Не смог получить данные файла <" + ИмяВременногоФайла + ">";
			КонецЕсли;	 
...
Процедура ЗапомнитьСостояниеФормыTestClient() Экспорт
	...
	Элементы.ПолучитьИзмененияФормыGherkin.Заголовок = "Получить изменения формы";
КонецПроцедуры

&НаКлиенте
Процедура ЗабытьСостояниеФормыTestClient() Экспорт
	СохраненноеСостояниеФормыTestClient = Неопределено;
	Элементы.ПолучитьИзмененияФормыGherkin.Заголовок = "Получить состояние всей формы";
КонецПроцедуры
			...
			ИначеЕсли (Лев(Значение,2) = "$$") и (Прав(Значение,2) = "$$") Тогда
				...
				Если НЕ ЕстьТакаяПеременная Тогда
					...
					Если Значение = Неопределено Тогда
						ВызватьИсключение "Не смог по переменной <" + ИмяПеременной + "> найти сохраненное значение.";
					КонецЕсли;	 
				Иначе	
...
Функция ТекстСообщенийПользователю()
	...
	ФормаСообщений = ПолучитьФорму("ВнешняяОбработка.VanessaBehavior.Форма.ПереводСообщений", ,ЭтаФорма);
	Для Каждого ЭлементФормы Из ФормаСообщений.Элементы Цикл
		Если ТипЗнч(ЭлементФормы) = Тип("ПолеФормы") Тогда
			ЗначенияПереводов.Вставить("$" + ЭлементФормы.Имя + "$",ЭлементФормы.Заголовок)
		КонецЕсли;	 
	КонецЦикла;	 
...
Процедура ЗаписатьЛогОшибки(СтрокаСценария,СтрокаШага,ТекстОшибки,ИнфоОбОшибке,ОписаниеОшибкиШага)
	...
	ВтораяЧастьФайла = СтрЗаменить("Ошибка_" + ТекущаяУниверсальнаяДатаВМиллисекундах(),Символы.НПП,"") + ".json";
...
&НаКлиентеНаСервереБезКонтекста
Функция СравнитьПутиФайлов(Путь1,Путь2)
	Стр1 = НРег(СтрЗаменить(Путь1,"/","\"));
	Стр2 = НРег(СтрЗаменить(Путь2,"/","\"));
	
	Возврат Стр1 = Стр2;
КонецФункции	

&НаКлиентеНаСервереБезКонтекста
Функция ПреобразоватьПутьКФайлуКСтандарту(Путь)
...
Процедура ЗагрузитьФичиПродолжение(ДополнительныеПараметры = Неопределено)
	...
	Исключение
		...
		ТекстСообщения = "Ошибка при построении дерева из файла <" + ТекущийЗагружаемыйФичаФайл + ">" + Символы.ПС + ОписаниеОшибки();
	...
	СделатьСообщение("$ФичиЗагружены$");
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		Исключение
			ТаймаутЗапуска1С = 25;
			Стр = "Не удалось получить таймаут для запуска TestClient из значения " + ЗнТаймаутЗапуска1С + ". Используется значение по умолчанию: 25";
...
Процедура ПодключитьБиблиотекуРаботыСВидео()
	...
	Если НЕ ФайлСуществуетКомандаСистемы(ИмяФайла) Тогда
		СделатьСообщение("Файл " + ИмяФайла + " не существует!");
		Возврат;
	КонецЕсли;	
...
Функция ПолучитьТекстПредупреждения()
	Попытка
		ОкноПредупреждения = ОбъектКонтекстСохраняемый.ТестовоеПриложение.НайтиОбъект(Тип("ТестируемоеОкноКлиентскогоПриложения"),"1С:Предприятие");
	Исключение
		Возврат Неопределено;
	КонецПопытки;
...
Функция СформироватьОписаниеОшибки(Знач Стр, ИмяФайла, ИмяПроцедуры,ИнфоОбОшибке)
	СисИнфо            = Новый СистемнаяИнформация; 
	ВерсияПриложения   = СисИнфо.ВерсияПриложения;
	РежимСовместимости = УзнатьРежимСовместимостиКлиент(ПолучитьТекстСообщенияПользователю("$НеИспользовать$"));
	ТекстИДРаннера     = "";
	...
	Стр = Стр + Символы.ПС 
	      + Символы.ПС              + ПолучитьТекстСообщенияПользователю("$ТехническиеДанные$") + ":"
...
Процедура ВыполнитьШаг()
	...
	Если СокрЛП(АдресСнипета) = "" Тогда
		...
		Если Объект.ПриравниватьPendingКFailed Тогда
			...
			Если Объект.ДелатьЛогОшибокВТекстовыйФайл Тогда
				ЗаписатьЛогОшибки(СтрокаСценария,СтрокаШага,"Шаг сценария не реализован.",Неопределено,ОписаниеОшибки);
			КонецЕсли;	 
		КонецЕсли;	 
	...
	Если ОбработкаТеста = Неопределено Тогда
		...
		ОбработкаТеста = ТаблицаКонтекстовОбработок[ИД].Обработка;
		Если ОбработкаТеста = Неопределено Тогда
			ВызватьИсключение "Не смог подключить обработку <" + АдресСнипета + ">";
		КонецЕсли;	 
	КонецЕсли;	 
	...
	Исключение
		...
		Если Найти(ОписаниеОшибкиСтр, "}: Не реализовано.") > 0 Тогда //тогда это Pending
			ОписаниеОшибкиШага = "Шаг (" + ТекШаг.Имя + ") не реализован.";
			...
			Если Объект.ПриравниватьPendingКFailed Тогда
				Если Объект.ДелатьЛогОшибокВТекстовыйФайл Тогда
					ЗаписатьЛогОшибки(СтрокаСценария,СтрокаШага,"Шаг сценария не реализован.",Неопределено,ОписаниеОшибкиШага);
				КонецЕсли;	 
			КонецЕсли;	 
		Иначе
			//значит возникла ошибка
			ОписаниеОшибкиШага = ПолучитьТекстСообщенияПользователю("$Шаг$") + " (" + ТекШаг.Имя + ") " + ПолучитьТекстСообщенияПользователю("$невыполнен$") +".";
			СделатьСообщение(ОписаниеОшибкиШага);
			ИнфоОбОшибке = Неопределено;
			...
			Если Объект.ДелатьЛогВыполненияСценариевВТекстовыйФайл Тогда
				ЗаписатьСтрокуВТекстовыйЛог("Шаг (" + ТекШаг.Имя + ") не выполнен. Строка дерева №" + XMLСтрока(ТекШаг.ИдСтроки));
...
Процедура СделатьСообщениеВсеСценарииОбработаны()
	Если СтатусЗапускаСценариев Тогда
		СделатьСообщение(ПолучитьТекстСообщенияПользователю("$ВыполнениеСценариевЗаконченоОшибокНеБыло$"));
	Иначе	
		СделатьСообщение(ПолучитьТекстСообщенияПользователю("$ВыполнениеCценариевPаконченоБЫЛИОШИБКИ$"));
	КонецЕсли;	 
КонецПроцедуры
...
Процедура ПерейтиКВыполнениюСледующегоШага(ВыполнитьШагЧерезТаймер = Неопределено, НадоОбновитьСтатусШага = Ложь, ИдСледующегоШага = Неопределено)
	...
	Если (ТекИД_СценарияВМассиве <> Неопределено) и (ТекИД_СценарияВМассиве >= 0) и (ТекИД_ШагаВМассиве >= 0) и (ТекИД_СценарияВМассиве <= (МассивСценариевДляВыполнения.Количество() - 1) ) Тогда
		...
		Если ТекШаг.ШагСчитаетсяУпавшим Тогда
			...
			ОписаниеОошибкиШага = "Шаг (" + ТекШаг.Имя + ") не выполнен.";
			СделатьСообщение(ОписаниеОошибкиШага);
...
Процедура ВыгрузитьСтатусВыполненияСценариевВФайл(Статус, ПутьКФайлу)
	...
	Если Статус Тогда
		Стр = "0";
	Иначе	
		Если ВозниклаОшибкаПодключенияTestClient Тогда
			Стр = "2";
		Иначе
			Стр = "1";
		КонецЕсли;	 
...
Процедура ВыполнинтьСценарийАссинхронноТаймер()
	...
	Если НЕ Объект.РежимВыполненияОдногоШага Тогда
		Сообщить(" ");
		СделатьСообщение(ПолучитьТекстСообщенияПользователю("$РаботаюПоСценарию$") + ": " + МассивСценариевДляВыполнения[ТекИД_СценарияВМассиве].Имя);
	КонецЕсли;	 
	...
	Пока XML.Прочитать() Цикл
		...
		Если XML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			ИначеЕсли ИмяУзла = "beginEditingCurrentArea" Тогда
				...
				ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в табличном документе """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я делаю двойной клик на текущей ячейке");
			...
			ИначеЕсли ИмяУзла = "setCurrentArea" Тогда
				Если МассивОбъектов[ИдВМассиве - 1] = "FormField" Тогда
					...
					Если ТекстРезультат.КоличествоСтрок() > 0 Тогда
						ПредыдущаяСтрока = ТекстРезультат.ПолучитьСтроку(ТекстРезультат.КоличествоСтрок());
					КонецЕсли;	 
					ТекущаяСтрока = ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в табличном документе """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я перехожу к ячейке """ + МассивСвойств[ИдВМассиве]["area"] + """";
					...
					Иначе	
						...
						Если ТекущаяТЧ <> "" Тогда	
							Если СокрЛП(ЗаголовокПоля) <> "" Тогда
								ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + ТекущаяТЧ + """ в поле """ + МассивСвойств[ИдВМассиве - 1]["title"] + """ я ввожу текст " + ПолучитьКорректноеПредставлениеСтроковогоЗначенияПоля("'" + МассивСвойств[ИдВМассиве]["text"] + "'"));
							Иначе	
								ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице """ + ТекущаяТЧ + """ в поле с именем """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я ввожу текст " + ПолучитьКорректноеПредставлениеСтроковогоЗначенияПоля("'" + МассивСвойств[ИдВМассиве]["text"] + "'"));
							КонецЕсли;	 
						Иначе	
							Если (СокрЛП(ЗаголовокПоля) <> "") и (Не ЕстьДублиЗаголовковУПоля(МассивСтруктурФорм, ТекущаяФорма, ЗаголовокПоля, "ПолеФормы")) Тогда
								ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в поле """ + МассивСвойств[ИдВМассиве - 1]["title"] + """ я ввожу текст " + ПолучитьКорректноеПредставлениеСтроковогоЗначенияПоля("'" + МассивСвойств[ИдВМассиве]["text"] + "'"));
							Иначе	
								ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в поле с именем """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я ввожу текст " + ПолучитьКорректноеПредставлениеСтроковогоЗначенияПоля("'" + МассивСвойств[ИдВМассиве]["text"] + "'"));
							КонецЕсли;	 
						КонецЕсли; 
					КонецЕсли;	 
				КонецЕсли;	 
...
Функция ОткрытьНовыйСеансTestClientИлиПодключитьУжеСуществующийДляНесколькихКлиентов(ДанныеЗапускаКлиентаТестирования,СтрокаОшибки=Неопределено)
	...
	Попытка
		ТестовоеПриложение = СоздатьТестовоеПриложение(ДанныеЗапускаКлиентаТестирования);
	Исключение
		СтрокаОшибки = ПолучитьТекстСообщенияПользователю("$СеансДолженБытьЗапущенСКлючомTESTMANAGER$");
		СделатьСообщение(СтрокаОшибки);
		СделатьСообщение(ПолучитьТекстСообщенияПользователю("$TestClientНЕПодключен$"));
	    Возврат Ложь;
	КонецПопытки;
...
Функция ПолучитьСтрокуЗапускаDesktopПриложение(ДанныеЗапускаКлиентаТестирования)
	...
	Если СвободныйПорт <> ДанныеЗапускаКлиентаТестирования.ПортЗапускаТестКлиента Тогда
		ТекстСообщения = ПолучитьТекстСообщенияПользователю("$ПортПортЗапускаТестКлиентаЗанятИлиНаходитсяВнеУказанногоДиапазонаПпортПодключенияИзмененНаСвободныйПорт$");
...
Процедура ПодключитьФайлОбработкиНаСервере(Знач АдресХранилища, Знач ИдентификаторОбработки, ЭтоОтчет = Ложь)
	ПодключитьВнешнююОбработкуСервер(АдресХранилища,ЕстьЗащитаОтОпасныхДействий);
КонецПроцедуры
...
Функция НайтиТЧВДаннойФорме(ИмяТЧ,НужнаяФорма,ИскатьПоИмени)
	Если НужнаяФорма = Неопределено Тогда
		ВызватьИсключение "Не смогли найти таблицу <" + ИмяТЧ + "> в активном окне.";
	КонецЕсли;	 
	Если ИмяТЧ = "" Тогда
...
&НаКлиенте
Процедура ПоказатьТекстОшибки(ТекстОшибки)
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ТекстОшибки",ТекстОшибки);
	ОткрытьФорму("ВнешняяОбработка.VanessaBehavior.Форма.ТекстОшибки", ПараметрыФормы,ЭтаФорма);
