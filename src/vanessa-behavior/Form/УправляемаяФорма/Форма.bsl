		...
		ИначеЕсли СтрСтроки.ШагСценарий = Истина Тогда
			Если СтрСтроки.ЭтоScenarioOutline = Истина Тогда //это вложенная структура сценария
				...
				Если СтрокаПримеров = Неопределено Тогда
					ВызватьИсключение "Не найдена строка примеров во вложенной структуре сценария.";
				КонецЕсли;	 
...
Функция РаспаковатьEPF(СтруктураОписанияEpf)
	...
	Попытка
		...
		ЛогРаспаковки = ПолучитьИмяВременногоФайла("txt");
		СтрокаРазборкиEpf = СтрокаЗапуска + " " + ДополнитьСлешВПуть(КаталогИнструментов) + "vendor/precommit1c/v8files-extractor.os  --decompile """ +  ВременноеИмяФайла + """ """ + ИмяКаталогаДляИсходников + """ > """ + ЛогРаспаковки + """ 2>&1";
		...
		Если Объект.DebugLog Тогда
			Отладка("Лог команды распаковки EPF:");
			Отладка(ПолучитьТекстовыйФайлОднойСтрокой(ЛогРаспаковки));
		КонецЕсли;	 
		
		УдалитьФайлыКомандаСистемы(ЛогРаспаковки);
...
Процедура СоздатьФайлыОбработок(БылиОшибки,СтруктураОписанияEpf,ДополнительныеПараметры = Неопределено)
	...
	УдалитьФайлыКомандаСистемы(ФайлЛога);
...
Процедура ЗагрузитьВременнуюФичуСервер(СтруктураПараметров)
	...
	Шаги = Новый Массив;
	ЭтоСтруктураСценария = Ложь;
	ПолучитьШагиИзДерева(Дерево.Строки,Шаги,ЭтоСтруктураСценария);
	
	СтруктураПараметров.Вставить("Шаги",Шаги);	
КонецПроцедуры

&НаСервере
Процедура ПолучитьШагиИзДерева(ДеревоСтроки,Шаги,ЭтоСтруктураСценария)
	Для каждого СтрокаДерева Из ДеревоСтроки Цикл
		Если СтрокаДерева.Шаг = Истина Тогда
			
			Если ЭтоСтруктураСценария Тогда
				Если ДеревоСтроки.Родитель.Пример <> Истина Тогда
					Продолжить;
				КонецЕсли;	 
			КонецЕсли;	 
		...
		Иначе	
			Если СтрокаДерева.ЭтоScenarioOutline = Истина Тогда
				ЭтоСтруктураСценария = Истина;
			КонецЕсли;	 
			
			ПолучитьШагиИзДерева(СтрокаДерева.Строки,Шаги,ЭтоСтруктураСценария);
		КонецЕсли;	 
	КонецЦикла;
КонецПроцедуры	
