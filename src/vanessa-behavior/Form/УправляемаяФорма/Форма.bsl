...
&НаСервере
Процедура ОбработатьСлежебныеЗначенияВСтруктуреШага(СтруктураШага,СтрСтроки,ДеревоСтроки,Шаги)
	Если ДеревоСтроки.Индекс(СтрСтроки) = (ДеревоСтроки.Количество()-1) Тогда
		//надо проверить для последнего шага тип родителя
		Если ДеревоСтроки.Родитель.ДопТип = "Цикл" Тогда
			
			ИдВМассиве = Шаги.Количество()-1;
			
			Пока Истина Цикл
				Если Шаги[ИдВМассиве].СтрокаШага = ДеревоСтроки.Родитель.ИдСтроки Тогда
					Прервать;
				КонецЕсли;	 
...
Процедура ДобавитьШагиДляВыполненияСценария(Шаги, ДеревоСтроки, СтрокаДерева, СтрокаСценария, ЭтоШагКонтекста)
	Для каждого СтрСтроки Из ДеревоСтроки Цикл
		Если СтрСтроки.Тип = "Шаг" Тогда
			...
			ОбработатьСлежебныеЗначенияВСтруктуреШага(СтруктураШага,СтрСтроки,ДеревоСтроки,Шаги);
		...
		ИначеЕсли СтрСтроки.Тип = "Область" Тогда
			Если (СтрСтроки.ДопТип = "Условие") или (СтрСтроки.ДопТип = "Цикл")  Тогда
				...
				ИдСледующейСтроки = ДеревоСтроки.Индекс(СтрСтроки) + 1;
				Если (ДеревоСтроки.Количество() - 1) >= ИдСледующейСтроки Тогда
					СледующаяСтрока = ДеревоСтроки[ИдСледующейСтроки];
	...
	Пока XML.Прочитать() Цикл
		...
		Если XML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			ИначеЕсли ИмяУзла = "deleteRow" Тогда
				ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице с именем """ + МассивСвойств[ИдВМассиве - 1]["name"] + """ я удаляю текущую строку");
