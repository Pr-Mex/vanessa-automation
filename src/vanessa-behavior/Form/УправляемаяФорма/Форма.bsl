...
&НаКлиенте
Процедура УдалитьСтрокиИз_ТаблицаИзвестныхStepDefinition_ДляКоторыхНетФайловИСтрокиИзмененныхФайлов(ДанныеПоВерсиямОбработок,ДополнительныеПараметры)
...
Процедура ЗаполнитьДеревоФич(СтруктураПараметров)
	...
	УдалитьСтрокиИз_ТаблицаИзвестныхStepDefinition_ДляКоторыхНетФайловИСтрокиИзмененныхФайлов(ДанныеПоВерсиямОбработок,ДополнительныеПараметры);
	
	
	
	ПолучитьУжеСуществующиеСнипетыИзОбработок(Путь, ДополнительныеПараметры,ДанныеПоВерсиямОбработок.МассивВерсийФайловИзТаблицаИзвестныхStepDefinition,ДанныеПоВерсиямОбработок.МассивИменФайловИзТаблицаИзвестныхStepDefinition,МассивИменФайловКоторыеСовпалиСКеш);
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	СтрЗапуска = СокрЛП(ПараметрЗапуска);
	//СтрЗапуска = "StartFeaturePlayer;VBParams=C:\Commons\rep\vanessa-behavior\tools\JSON\VBParams839UF.json";
	Если СтрЗапуска = "" Тогда
		Возврат;
	КонецЕсли;	
	...
	Иначе
		...
		Для каждого СтрокаДанныеКлиентовТестирования Из ДанныеКлиентовТестирования Цикл
			Если СтрокаДанныеКлиентовТестирования.Имя = "Этот клиент" Тогда
				ПредыдущийПорт =  СтрокаДанныеКлиентовТестирования.ПортЗапускаТестКлиента;
				ДанныеКлиентовТестирования.Удалить(СтрокаДанныеКлиентовТестирования);
			КонецЕсли;
		КонецЦикла;
		ЗаполнитьДанныеТекущейИнфобазы(ПредыдущийПорт);
	КонецЕсли;
	...
	Пока XML.Прочитать() Цикл
		...
		Если XML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			ИначеЕсли ИмяУзла = "selectAllRows" Тогда
				ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " в таблице с именем """ + МассивСвойств[ИдВМассиве-1]["name"] + """ я выделяю все строки");
		...
		Если XML.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда 
			...
			Если (ИмяУзла = "gotoRow") или (ИмяУзла = "expand") или (ИмяУзла = "collapse") Тогда
				...
				Если ТаблицаСвойствСтроки.Количество() > 0 Тогда
					...
					ИначеЕсли (ИмяУзла = "expand") Тогда
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В форме """ + ТекущаяФорма + """ в таблице """ + МассивСвойств[ИдВМассиве-1]["name"] +  """ я разворачиваю строку:");
					ИначеЕсли (ИмяУзла = "collapse") Тогда
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В форме """ + ТекущаяФорма + """ в таблице """ + МассивСвойств[ИдВМассиве-1]["name"] +  """ я сворачиваю строку:");
					КонецЕсли;	 
...
Функция ПолучитьМассивКоординатОкон1С()
	ИмяФайлаЛога = ПолучитьИмяВременногоФайла("txt");
	Рез = ВыполнитьSikuliСкрипт(Объект.КаталогИнструментов + "\tools\Sikuli\GetAll1CWindowsCoordinates.sikuli > """ + ИмяФайлаЛога + """",-1,Истина);
	Если Рез <> 0 Тогда
...
&НаКлиенте
Процедура ЗаполнитьДанныеТекущейИнфобазы(ПредыдущийПорт = Неопределено)
	...
	Если ПредыдущийПорт <> Неопределено Тогда
		СвободныйПорт = ПредыдущийПорт;
	КонецЕсли;	 
...
Процедура ДанныеКлиентовТестированияПередНачаломИзменения(Элемент, Отказ)
	...
	Если ТекущиеДанные.Имя = "Этот клиент" И Элементы.ДанныеКлиентовТестирования.ТекущийЭлемент.Имя <> "ДанныеКлиентовТестированияДопПараметры" Тогда
...
Процедура ПоказатьСкрытьВсеКолонки(Команда)
	...
	Если Не Элементы.ДеревоТестов.ПодчиненныеЭлементы.ДеревоТестовСтрокаРеальнойПроцедуры.Видимость Или
		 ...
		 Не Элементы.ДеревоТестов.ПодчиненныеЭлементы.ДеревоТестовПолныйПуть.Видимость
...
Процедура ОткрытьРедактированиеТаблицыGherkin()
	...
	ФорматироватьСтрокиТаблицыПараметровШага(МассивСтрокДляРедактирования);
...
Процедура ЗаменитьТаблицуGherkinПослеРедактирования(МассивСтрокТаблицы)
	...
	ФорматироватьСтрокиТаблицыПараметровШага(МассивСтрокТаблицы);
...
Процедура ВывестиСтатистикуНаСервере()
	...
	КонецЕсли;
	ОбщаяСтатистикаПоФиче = "" + ВсегоФич + " / " + ВсегоСценариев + " / " + ВсегоШаговОбщее;
	ОбщаяСтатистикаПоФиче = Вычислить("Новый ФорматированнаяСтрока(ОбщаяСтатистикаПоФиче, Новый Шрифт(,,Истина))");
	Элементы.Статистика.Заголовок = Вычислить("Новый ФорматированнаяСтрока(ОбщаяСтатистикаПоФиче, ОбщаяСтрока)");
	
	Элементы.Статистика.Подсказка = "Всего фич: " + ВсегоФич + Символы.ПС +
									"Всего сценариев: " + ВсегоСценариев + Символы.ПС +
									"Всего шагов: " + ВсегоШаговОбщее + 
									?(ВсегоШаговSuccess+ВсегоШаговPending+ВсегоШаговFailed > 0, 
										Символы.ПС + "Результат выполнения шагов (Success, Failed, Pending): " + 
										ВсегоШаговSuccess + " / " + ВсегоШаговFailed + " / " + ВсегоШаговPending + Символы.ПС + 
										"Пропущено шагов: " + ВсегоШагов, ""
									 );
...
Процедура ЗагрузитьПлагины()
	...
	КаталогПлагинов = ПолучитьПутьКПлагинам();
	ОбработчикПоискаКлиентскихПлагинов = Новый ОписаниеОповещения("ОбработчикПоискаКлиентскихПлагинов", ЭтаФорма);
	НачатьПоискФайлов(ОбработчикПоискаКлиентскихПлагинов, КаталогПлагинов, "*.epf", Ложь);
...
Процедура ОбработчикПоискаКлиентскихПлагинов(Знач НайденныеФайлы, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПомещаемыеФайлы.Количество() Тогда
		...
		НачатьПомещениеФайлов(Новый ОписаниеОповещения("ОбработчикПоискаКлиентскихПлагиновЗавершение", ЭтаФорма, ПараметрыЗавершения),
