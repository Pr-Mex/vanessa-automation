...
&НаКлиенте
Процедура ЗакончитьЗаписьВидеоОдногоСценария(НадоПрерватьСценарийДляВставки = Ложь)
	...
	ВыполнитьКомандуОбработатьСценарийВидео(ТекИД_СценарияВМассиве,НадоПрерватьСценарийДляВставки);
...
&НаКлиенте
Процедура ВыполнитьКомандуОбработатьСценарийВидео(ИД_СценарияВМассиве,НадоПрерватьСценарийДляВставки)
	Если БылаОшибкаСкриптовСборкиВидео Тогда
		Возврат;
	КонецЕсли;	 
	...
	ПараметрыФайла = ПрочитатьСтруктуруИзJSONФайлаСервер(ФайлПараметровСценария);
	ИмяЧастиВидео = ПараметрыФайла["ИмяЧастиВидео"];
	ИмяЧастиСубтитры = ПараметрыФайла["ИмяЧастиСубтитры"];
	МассивЧастейВидео.Добавить(Новый Структура("ИмяЧастиВидео,ИмяЧастиСубтитры,КоличествоШаговВДаннойЧастиВидео",ИмяЧастиВидео,ИмяЧастиСубтитры,КоличествоШаговВДаннойЧастиВидео));
		...
		Если НЕ ТекШаг.ЭтоШагКонтекста Тогда
			...
			Если Объект.ИдетЗаписьВидео Тогда
				...
				Если ЭтоПрерывающйВидеоШаг(ТекШаг) Тогда
					...
					ЗакончитьЗаписьВидеоОдногоСценария(Истина);
					НачатьЗаписьВидео();
				Иначе	
