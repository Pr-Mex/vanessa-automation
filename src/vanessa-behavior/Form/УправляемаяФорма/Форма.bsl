...
Процедура ПодготовитьСозданиеИнструкцииHTML()
	...
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		ДопПараметры = Новый Структура;
		ОписаниеОповещения = Вычислить("Новый ОписаниеОповещения(""ОбработатьЗаписьФайла"", ЭтаФорма, ДопПараметры)");
		Выполнить("ТД.НачатьЗапись(ОписаниеОповещения, ИмяФайлаЛогаИнструкцииHTML,КодировкаТекста.UTF8)");
	Иначе 
		ТД.Записать(ИмяФайлаЛогаИнструкцииHTML,КодировкаТекста.UTF8);
	КонецЕсли;
...
Процедура ПодготовитьСозданиеИнструкцииMarkdown()
	...
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		ДопПараметры = Новый Структура;
		ОписаниеОповещения = Вычислить("Новый ОписаниеОповещения(""ОбработатьЗаписьФайла"", ЭтаФорма, ДопПараметры)");
		Выполнить("ТД.НачатьЗапись(ОписаниеОповещения, ИмяФайлаЛогаИнструкцииMarkdown,КодировкаТекста.UTF8)");
	Иначе 
		ТД.Записать(ИмяФайлаЛогаИнструкцииMarkdown,КодировкаТекста.UTF8);
	КонецЕсли;
