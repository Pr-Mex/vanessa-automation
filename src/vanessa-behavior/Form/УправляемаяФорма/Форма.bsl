...
Процедура ВосстановитьНастройки(ЭтоLinux)
	...
	Если ТипЗнч(Настройки) = Тип("Структура") Тогда
		...
		Иначе
			...
			Иначе
				Объект.КомандаСделатьСкриншот = """C:\Program Files (x86)\IrfanView\i_view32.exe"" /capture=2 /convert=";
			КонецЕсли;	 
		КонецЕсли;	 
...
Процедура ЗаполнитьКонтекстноеМеню()
	...
	Пункт3.Заголовок="Выполнять сценарий с этого шага";
	...
	Пункт4.Заголовок="Выполнять сценарии с начала";
...
Функция ПроверитьНаличиеИнструментаДляСозданияСкриншотов()
	Если НЕ Объект.СоздаватьИнструкциюHTML и НЕ Объект.ДелатьСкриншотПриВозникновенииОшибки Тогда
		Возврат Истина;
	КонецЕсли;  
...
Процедура ВыполнитьШаг()
	...
	//ЗапуститьПриложение("C:\Commons\Rep\vanessa-ci\MakeVideoInstr\mousemover\untitled11.exe",,Истина);
...
&НаКлиенте
Процедура ПреобразоватьСубтитрыВФорматSRT(ЛогФайл,ФайлSRT)
	...
	Для Ккк = 0 По МассивСубтитров.Количество()-1 Цикл
		...
		СтрокаДлительностиТитра = ПолучитьСтркуДлительностиТитра(НачальноеВремя,ТекущаяВременнаяОтметка,СледующаяВременнаяОтметка);
...
&НаКлиенте
Функция ПолучитьСтркуДлительностиТитра(Знач НачальноеВремя,Знач ТекущаяВременнаяОтметка,Знач СледующаяВременнаяОтметка)
	ДержатьТитрМС = 2000;
...
&НаКлиенте
Процедура ЗакончитьЗаписьВидеоОдногоСценария()
	...
	ПараметрыФайла = ПрочитатьСтруктуруИзJSONФайлаСервер(ФайлПараметров);
	...
	ПреобразоватьСубтитрыВФорматSRT(ИмяФайлаСубтитров,ПараметрыФайла["subsfile"]);
	
	ВыполнитьКомандуОбработатьСценарийВидео(ТекИД_СценарияВМассиве);
	
	
	
	
	
	//Файл = Новый Файл(ФайлРезультат);
	//НовоеИмяФайла = Файл.Путь + "\S" + ПолучитьФорматированныйНомер(ТекИД_СценарияВМассиве,3) + Файл.Расширение;
	//Если ФайлСуществуетКомандаСистемы(НовоеИмяФайла) Тогда
	//	УдалитьФайлыКомандаСистемы(НовоеИмяФайла);
	//КонецЕсли;	 
	//ПереместитьФайлКомандаСистемы(ФайлРезультат,НовоеИмяФайла);
	//Отладка("Файл переименован в " + НовоеИмяФайла);
...
&НаКлиенте
Функция СоздатьФайлПараметровПоСценарию(ТекСценарий,ИД_СценарияВМассиве)
	ВременныйФайл = ПолучитьИмяВременногоФайла("json");
	ЗТ = Новый ЗаписьТекста(ВременныйФайл,"UTF-8",,Истина); 
	ЗТ.ЗаписатьСтроку("{"); 
...
&НаКлиенте
Процедура ВыполнитьКомандуОбработатьСценарийВидео(ИД_СценарияВМассиве)
	Если БылаОшибкаСкриптовСборкиВидео Тогда
		Возврат;
	КонецЕсли;	 
	...
	ФайлПараметровСценария = СоздатьФайлПараметровПоСценарию(ТекСценарий,ИД_СценарияВМассиве);
...
&НаКлиенте
Процедура НачатьЗаписьВидео(ТекШаг)
...
&НаКлиенте
Процедура ЗаписатьЛогЗаписиВидео(СтрокаТитров)
	Если НЕ Объект.ИдетЗаписьВидео Тогда
		Возврат;
	КонецЕсли;	 
	...
	Если ТекИД_ШагаВМассиве >= 0 Тогда
		...
		Если НЕ ТекШаг.ЭтоШагКонтекста Тогда
			...
			Если ТекШаг.ШагСПараметрамиВТаблице = Истина Тогда
				...
				КонецЦикла;
			КонецЕсли;	 
			ЗаписатьЛогЗаписиВидео(СтрокаТитров);
		КонецЕсли;	 
	КонецЕсли;	 
...
Процедура ВыполнитьСценарии(ТекСценарий = Неопределено) Экспорт
	...
	Если Объект.ДелатьСкриншотПриВозникновенииОшибки Тогда
		Если Не ЗначениеЗаполнено(Объект.КомандаСделатьСкриншот) Тогда
			СделатьСообщение("Не указана команада для выполнения скриншотов! Например <""C:\Program Files (x86)\IrfanView\i_view32.exe"" /capture=2 /convert=>");
			Возврат;
		КонецЕсли;	 
...
Процедура СоздатьФайлHTMLИнструкции()
	...
	Для каждого Сценарий Из МассивСценариев Цикл
		...
		Для каждого Шаг Из МассивШагов Цикл
			...
			Страница.ДобавитьСтроку("<p align=""left"" style=""font-size:30px"">" + ПолучитьФорматированныйНомер(НомШага,3) + ". " + Шаг.ИмяШага + "</p>");
		...
		Если XML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Если ИмяУзла = "click" Тогда
				...
				ИначеЕсли МассивОбъектов[ИдВМассиве-1] = "FormButton" Тогда	
					...
					ИмяОбъекта = "с заголовком """ + ЗаголовокОбъекта + """";
				...
				ИначеЕсли (МассивОбъектов[ИдВМассиве-1] = "FormField") или (МассивОбъектов[ИдВМассиве-1] = "FormDecoration") Тогда	
					...
					Иначе	
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме я нажимаю на гиперссылку с заголовком """ + ИмяОбъекта + """");
					КонецЕсли;	 
				КонецЕсли;	 
			ИначеЕсли ИмяУзла = "close" Тогда
				ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " Я закрываю окно с заголовком """ + МассивСвойств[ИдВМассиве-1]["caption"] + """");
			...
			ИначеЕсли ИмяУзла = "startChoosing" Тогда
				Если ТекущаяТЧ <> "" Тогда
					...
					Иначе	
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме в ТЧ """ + ТекущаяТЧ + """ я нажимаю кнопку выбора у реквизита с заголовком """ + ИмяОбъекта + """");
					КонецЕсли;	 
				Иначе	
					ИмяОбъекта = МассивСвойств[ИдВМассиве-1]["title"];
					Если (СокрЛП(ИмяОбъекта) <> "") и (Не ЕстьДублиЗаголовковУПоля(МассивСтруктурФорм,ТекущаяФорма,ИмяОбъекта,"ПолеФормы")) Тогда
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме я нажимаю кнопку выбора у поля с заголовком """ + ИмяОбъекта + """");
					Иначе	
						ИмяОбъекта = МассивСвойств[ИдВМассиве-1]["name"];
			...
			ИначеЕсли ИмяУзла = "clear" Тогда
				ИмяОбъекта = МассивСвойств[ИдВМассиве-1]["title"];
				Если (СокрЛП(ИмяОбъекта) <> "") и (Не ЕстьДублиЗаголовковУПоля(МассивСтруктурФорм,ТекущаяФорма,ИмяОбъекта,"ПолеФормы")) Тогда
					ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме я нажимаю кнопку очистить у поля с заголовком """ + ИмяОбъекта + """");
				Иначе	
					ИмяОбъекта = МассивСвойств[ИдВМассиве-1]["name"];
			...
			ИначеЕсли ИмяУзла = "open" Тогда
				Если МассивОбъектов[ИдВМассиве-1] = "FormField" Тогда
					...
					Иначе	
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме я нажимаю на кнопку открытия поля с заголовком """ + ИмяОбъекта + """");
					КонецЕсли;	 
				КонецЕсли;	 
			...
			ИначеЕсли ИмяУзла = "executeChoiceFromChoiceList" Тогда
				Если МассивОбъектов[ИдВМассиве-1] = "FormField" Тогда
					Если ТекущаяТЧ <> "" Тогда
						...
						Иначе	
							ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме в ТЧ """ + ТекущаяТЧ + """ из выпадающего списка с заголовком """ + ИмяОбъекта + """ я выбираю """ + МассивСвойств[ИдВМассиве]["presentation"] + """");
						КонецЕсли;	 
					Иначе	
						ИмяОбъекта = МассивСвойств[ИдВМассиве-1]["title"];
						Если СокрЛП(ИмяОбъекта) = "" Тогда
							...
							//ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме в ТЧ """ + ТекущаяТЧ + """ из выпадающего списка с именем """ + ИмяОбъекта + """ я выбираю """ + МассивСвойств[ИдВМассиве]["presentation"] + """");
						Иначе	
							ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме из выпадающего списка с заголовком """ + ИмяОбъекта + """ я выбираю """ + МассивСвойств[ИдВМассиве]["presentation"] + """");
							//ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме в ТЧ """ + ТекущаяТЧ + """ из выпадающего списка с заголовком """ + ИмяОбъекта + """ я выбираю """ + МассивСвойств[ИдВМассиве]["presentation"] + """");
						КонецЕсли;	 
					КонецЕсли;	 
				Иначе	
			...
			ИначеЕсли ИмяУзла = "executeChoiceFromDropList" Тогда
				...
				Если ТекущаяТЧ <> "" Тогда	
					...
					Иначе	
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме в ТЧ """ + ТекущаяТЧ + """ я выбираю значение реквизита с заголовком """ + ИмяОбъекта + """ из формы списка");
					КонецЕсли;	 
				Иначе	
					Если СокрЛП(ИмяОбъекта) = "" Тогда
						ИмяОбъекта = МассивСвойств[ИдВМассиве-1]["name"];
			...
			ИначеЕсли ИмяУзла = "activate" Тогда
				Если МассивОбъектов[ИдВМассиве-1] = "FormGroup" Тогда
					...
					Иначе	
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме я перехожу к закладке с заголовком """ + ИмяОбъекта + """");
					КонецЕсли;	 
			...
			ИначеЕсли ИмяУзла = "expand" Тогда
				Если МассивОбъектов[ИдВМассиве-1] = "FormGroup" Тогда
					...
					Иначе	
						...
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме я разворачиваю группу с заголовком """ + ИмяОбъекта + """");
					КонецЕсли;	 
				КонецЕсли;	 
			...
			ИначеЕсли ИмяУзла = "collapse" Тогда
				Если МассивОбъектов[ИдВМассиве-1] = "FormGroup" Тогда
					...
					Иначе	
						...
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме я сворачиваю группу с заголовком """ + ИмяОбъекта + """");
					КонецЕсли;	 
				КонецЕсли;	 
			...
			ИначеЕсли ИмяУзла = "selectOption" Тогда
				Если МассивОбъектов[ИдВМассиве-1] = "FormField" Тогда
					Если СокрЛП(МассивСвойств[ИдВМассиве-1]["title"]) <> "" Тогда
						ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме я меняю значение переключателя с заголовком """ + МассивСвойств[ИдВМассиве-1]["title"] + """ на """ + МассивСвойств[ИдВМассиве]["presentation"] + """");
					Иначе	
				...
				Если МассивОбъектов[ИдВМассиве-1] = "FormField" Тогда
					...
					Если ТекущаяТЧ <> "" Тогда	
						Если СокрЛП(ЗаголовокПоля) <> "" Тогда
							ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме в ТЧ """ + ТекущаяТЧ + """ в поле с заголовком """ + МассивСвойств[ИдВМассиве-1]["title"] + """ я ввожу текст """ + МассивСвойств[ИдВМассиве]["text"] + """");
						Иначе	
					...
					Иначе	
						Если (СокрЛП(ЗаголовокПоля) <> "") и (Не ЕстьДублиЗаголовковУПоля(МассивСтруктурФорм,ТекущаяФорма,ЗаголовокПоля,"ПолеФормы")) Тогда
							ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме в поле с заголовком """ + МассивСвойств[ИдВМассиве-1]["title"] + """ я ввожу текст """ + МассивСвойств[ИдВМассиве]["text"] + """");
						Иначе	
			...
			ИначеЕсли ИмяУзла = "setCheck" Тогда
				Если МассивОбъектов[ИдВМассиве-1] = "FormField" Тогда
					...
					Если ТекущаяТЧ <> "" Тогда		
						Если СокрЛП(МассивСвойств[ИдВМассиве-1]["title"]) <> "" Тогда
							ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме в ТЧ """ + ТекущаяТЧ + """ я изменяю флаг с заголовком """ + МассивСвойств[ИдВМассиве-1]["title"] + """");
						Иначе	
					...
					Иначе	
						Если СокрЛП(МассивСвойств[ИдВМассиве-1]["title"]) <> "" Тогда
							ТекстРезультат.ДобавитьСтроку(ПолучитьПервоеКлючевоеСлово(СчетчикДействий) + " В открытой форме я изменяю флаг с заголовком """ + МассивСвойств[ИдВМассиве-1]["title"] + """");
						Иначе	
...
Процедура ЗапуститьСеансTestClient(ДопПараметр = "")
	...
	СтрокаЗапуска = """" + СтрокаЗапуска1с + """ ENTERPRISE " + КаталогБазы + " /TESTCLIENT /RunModeManagedApplication " + ДопПараметр;
...
&НаКлиенте
Процедура Подождать(Знач ВремяСекунды) 
	Если ЭтоLinux Тогда 
		КомандаСистемы("ping -i "+Строка(ВремяСекунды)+" 127.0.0.1 -c 2");
	КонецЕсли;
...
Процедура ФорматироватьМассивСтрокКакТаблицу(МассивСтрок)
	...
	Для каждого Стр Из МассивСтрок Цикл
		...
		Для Ккк = 0 По МассивДлин.Количество()-1 Цикл
			ДлинаСтроки = СтрДлина(СокрЛП(МассивСтолбцов.Получить(Ккк)));
			Если ДлинаСтроки > МассивДлин[Ккк] Тогда
				МассивДлин[Ккк] = ДлинаСтроки;
			КонецЕсли;	 
...
Функция ПолучитьСтрокуТабличныйДокументФормаTestClient(ТабДок)
	...
	Для каждого СтрокаТабДок Из ТабДок Цикл
		...
		Для каждого КолонкаТабДок Из СтрокаТабДок Цикл
			СтрокаТЧ = СтрокаТЧ + " '" + КолонкаТабДок + "' |";
...
Процедура ПереместитьКурсорМышиКАктивномуОбъекту() Экспорт
	...
	Путь = Объект.КаталогСкриптовСборкиВидео + "\uihelper\MouseMove\MouseMove.exe -movemousetoactivecontrol mousespeed=500";
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		ВыполнитьКомандуОСБезПоказаЧерногоОкна(Путь);	
	Иначе
		ЗапуститьПриложение(Путь,,Истина);
