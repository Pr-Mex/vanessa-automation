...
Функция СформироватьОписаниеОшибки(Знач Стр, ИмяФайла, ИмяПроцедуры,ИнфоОбОшибке)
	...
	Стр = Стр + Символы.ПС + "Технические данные:"
          + Символы.ПС + "        ИмяФайла: " + ИмяФайла
          + Символы.ПС + "        ИмяПроцедуры: " + ИмяПроцедуры 
          + Символы.ПС + "        ВерсияVB: " + ПолучитьВерсиюОбработкиКлиент() 
          + Символы.ПС + "        ВерсияПлатформы: " + ВерсияПриложения 
          + Символы.ПС + "        РежимСовместимостиTestManager: " + РежимСовместимости;
...
Процедура ВыполнитьШаг()
	...
	Исключение
		...
		Иначе
			...
			ОписаниеОшибкиШага = "Шаг (" + ТекШаг.Имя + ") не выполнен.";
			СделатьСообщение(ОписаниеОшибкиШага);
			ИнфоОбОшибке = Неопределено;
