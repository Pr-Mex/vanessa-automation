		...
		Пока Истина Цикл
			...
			Если НашлиТегExportScenarios Тогда
				Если Лев(СтрНРег,9) = "сценарий:" Тогда
					...
					ТаблицаУжеСуществующихСценариев.Добавить(ПолучитьСтруктуруДляСценария(НРег(ИмяСценария),ИмяФайла,ТаблицаШагов));
					Продолжить;
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		Если ЗначениеЗаполнено(ЗнКаталогСкриптовСборкиВидео) Тогда
			Объект.КаталогСкриптовСборкиВидео = ПреобразоватьПутьСТочкамиКНормальномуПути(ЗнКаталогСкриптовСборкиВидео);
		КонецЕсли;	 
...
Процедура ПодготовитьСозданиеИнструкцииHTML()
	...
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		ОписаниеОповещения = Вычислить("Новый ОписаниеОповещения(""ОбработатьЗаписьФайла"", ЭтаФорма)");
		ТД.НачатьЗапись(ОписаниеОповещения, ИмяФайлаЛогаИнструкцииHTML,КодировкаТекста.UTF8);
	Иначе 
		ТД.Записать(ИмяФайлаЛогаИнструкцииHTML,КодировкаТекста.UTF8);
	КонецЕсли;
...
Процедура ПодготовитьСозданиеИнструкцииMarkdown()
	...
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		ОписаниеОповещения = Вычислить("Новый ОписаниеОповещения(""ОбработатьЗаписьФайла"", ЭтаФорма)");
		ТД.НачатьЗапись(ОписаниеОповещения, ИмяФайлаЛогаИнструкцииMarkdown,КодировкаТекста.UTF8);
	Иначе 
		ТД.Записать(ИмяФайлаЛогаИнструкцииMarkdown,КодировкаТекста.UTF8);
	КонецЕсли;
