...
Процедура СоздатьФайлMarkdownИнструкции()
	...
	ИДСкриншота  = 0;
	Для каждого Сценарий Из МассивСценариев Цикл
		Страница.ДобавитьСтроку("## " + Сценарий.ИмяСценария);
		...
		Для каждого Шаг Из МассивШагов Цикл
			...
			Для Ккк = 0 По МассивСтрок.Количество()-1 Цикл
				Стр = МассивСтрок[Ккк];
				Если Ккк = 0 Тогда
					Страница.ДобавитьСтроку("	" + ПолучитьФорматированныйНомер(НомШага,3) + ". " + Стр);
				Иначе
					ЕстьВертЧерта = Ложь;
					Если Найти(Стр,"|") > 0 Тогда
						ЕстьВертЧерта = Истина;
			...
			Страница.ДобавитьСтроку("<img src=" + СтрЗаменить(СтрСкриншотаДляMarkdown,"\","/") + ">");
		КонецЦикла;
	КонецЦикла;
	
	
	
	ИмяФайлаДляЗаписи = Объект.КаталогOutputИнструкцияMarkdown + "/Instr_" + ТекущаяФичаИнструкцииMarkdown + ".MD";
	Сообщить("Записываю " + ИмяФайлаДляЗаписи);
	Страница.Записать(ИмяФайлаДляЗаписи);
