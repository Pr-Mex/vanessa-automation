...
Функция ПолучитьФайлMP3ИзТекста(ПараметрыВидео,Текст,ИмяФайла)
	...
	КонецЕсли;	 
	Команда = "" + ffmpeg + " -i " + ИмяWav + " -vn -ar 44100 -ac 2 -ab 192k -f mp3 -af ""volume=1.29"" " + ИмяMP3;
	ПараметрыВидео.Ванесса.Отладка(Команда);
