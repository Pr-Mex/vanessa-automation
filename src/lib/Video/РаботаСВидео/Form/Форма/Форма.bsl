...
Процедура УскоритьЗамедлитьФайлВидеоИДобавитьMp3(ПараметрыВидео,ИмяФайлВидео,ИмяФайлMP3,ДлительностьВидео,НоваяДлительностьВидео,ДлительностьMP3)
	...
	Если Прав(НРег(ИмяФайлаВидеоВременный),3) = "mp4" Тогда
		...
		Команда = "" + ПараметрыВидео.ЗаписьВидеоКомандаFfmpeg + " -i " + ИмяФайлаВидеоВременный + " -r 29.97 -qscale 1  " + НовоеИмяФайлВидео;
		ПараметрыВидео.Ванесса.Отладка(Команда);
		КомандаСистемы(Команда);
...
Процедура СобратьФинальноеВидеоИзЧастей(ПараметрыВидео,МассивЧастей)
	...
	Если СокрЛП(ИмяФайлаВодянойЗнак) = "" Тогда
		СобранноеВидео3 = СобранноеВидео1;
	Иначе
		...
		ЗТ = Новый ЗаписьТекста(ИмяВременногоФайла,"UTF-8",,Истина); 
		//ЗТ.ЗаписатьСтроку("CHCP 65001"); 
		Команда = "" + ПараметрыВидео.ЗаписьВидеоКомандаFfmpeg + " -i mix1.mpg -i " + Файл_watermark.Имя + " -qscale 1 -filter_complex overlay=main_w-overlay_w-10:main_h-overlay_h-10 -codec:a copy " + СобранноеВидео3;
		Сообщить(""+Команда);
		...
		ЗапуститьПриложение(ИмяВременногоФайла,,Истина,retCode);
		Если retCode <> 0 Тогда
			ВызватьИсключение("Команда ffmpeg по добавлению водяной отметки не выполнена!");
		КонецЕсли;	
