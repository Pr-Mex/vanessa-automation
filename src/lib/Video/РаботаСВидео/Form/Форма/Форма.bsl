	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			Если ПараметрыВидео.Ванесса.ЭтоПрерывающйВидеоШаг(Шаг) Тогда
				//надо собрать файл вставки
			Иначе
				//1. надо масштабировать файл видео, чтобы он и звук правильно соотносились
				ДлительностьВидео = ОпределитьДлительностьФайла(ПараметрыВидео,ИмяФайлаШага);
				ДлительностьMP3   = ОпределитьДлительностьФайла(ПараметрыВидео,ИмяMp3Шага);
