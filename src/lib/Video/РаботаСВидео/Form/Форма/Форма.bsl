...
Процедура СобратьФинальноеВидеоИзЧастей(ПараметрыВидео,МассивЧастей)
	...
	Иначе
		...
		КомандаВодянойЗнак = "" +  Файл_watermark.Имя + " -filter_complex overlay=main_w-overlay_w-10:main_h-overlay_h-10 ";
		
	КонецЕсли;	 
	
	Команда = "" + ПараметрыВидео.ЗаписьВидеоКомандаFfmpeg + " -i mix1.mpg -i " + КомандаВодянойЗнак + " -qscale 1 -af ""aresample=async=1:first_pts=0"" -codec:a libmp3lame -q:a 0 " + СобранноеВидео3;
	ПараметрыВидео.Ванесса.Отладка(Команда);
	...
	//добавляем музыку
	Если ПараметрыВидео.ФайлМузыки <> Неопределено Тогда
		mp3файл = ПараметрыВидео.ФайлМузыки;
...
Функция Установлен_ImageMagic(ПараметрыВидео)
	Лог = ПолучитьИмяВременногоФайла("txt");
	Команда = """" + ПараметрыВидео.ЗаписьВидеоКомандаConvert + """ -version > " + Лог;
	ПараметрыВидео.Ванесса.Отладка(Команда);
	КомандаСистемы(Команда);
