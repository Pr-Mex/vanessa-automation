...
&НаКлиенте
Функция ЭтоШагПервыйВВеткеДерева(ПараметрыВидео,Шаг,ИмяШагаВеткиДерева)
...
Процедура ЗавершитьЗаписьВидео(ПараметрыВидео) Экспорт
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			Если ЭтоШагПервыйВВеткеДерева(ПараметрыВидео,Шаг,ТекстВставкиВеткиДерева) Тогда
				ИмяФайлаШагаВетки = ПараметрыВидео.ЗаписьВидеоКаталогДляВременныхФайлов + "\" + СтрЗаменить(ИмяФайлаШага,"_main.","_ins.");
