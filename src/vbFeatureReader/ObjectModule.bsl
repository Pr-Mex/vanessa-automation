...
Процедура ДобавитьКолонкуСнипетаВТаблицаУжеСуществующихСценариев(ТаблицаУжеСуществующихСценариев)
	...
	ТаблицаУжеСуществующихСценариев.Колонки.Добавить("СтруктураОбработкиПараметров");
	
	Для каждого СтрТаблицаУжеСуществующихСценариев Из ТаблицаУжеСуществующихСценариев Цикл
		ЗначенияПараметров           = Новый Массив;
		СтруктураОбработкиПараметров = Новый Структура;
		
		СтарыйStepDefinition = ПолучитьStepDefinitionПоСтроке(СтрТаблицаУжеСуществующихСценариев.ИмяСценария,ЗначенияПараметров,,,СтруктураОбработкиПараметров);
		НовыйStepDefinition  = ПолучитьНовыйStepDefinition(СтарыйStepDefinition);
		
		//Сообщить("НовыйStepDefinition="+НовыйStepDefinition);
		
		СтрТаблицаУжеСуществующихСценариев.Снипет             = НРег(НовыйStepDefinition);
		СтрТаблицаУжеСуществующихСценариев.СтруктураОбработкиПараметров = СтруктураОбработкиПараметров;
	КонецЦикла;
...
Функция ПолучитьStepDefinitionПоСтроке(Знач Стр,ЗначенияПараметров, СтрокаПараметров = "",ЕстьПараметрыТаблицы = Ложь,СтруктураОбработкиПараметров = Неопределено) 
	...
	//Если СтруктураОбработкиПараметров <> Неопределено Тогда
	//	СтрокаОбработанныхПараметров = Стр;
	//	СтруктураОбработкиПараметров.Вставить("СтрокаОбработанныхПараметров",СтрокаОбработанныхПараметров);
	//КонецЕсли;	 
