...
Процедура ДобавитьСценарийДляДанногоПримера(СтрокаПримеров,ИменованныеПараметры)
	...
	ПараметрыКопировния.Вставить("ИменованныеПараметры",ИменованныеПараметры);
...
Процедура ОбработатьПередачуПараметровТаблицей(СтрСтрокиШагов,ТаблицаСтрокПередачаПараметровТаблицей,РазныеИменнованныеПараметры)
...
Процедура ЗакрытьПредыдущийСценарий(ИмяФичи,ПолноеИмяФичи,ТекущийТипСценария,ПредыдущееКлючевоеСлово,ТекущееКлючевоеСлово,ДеревоСтроки,ОписаниеСценария,ТаблицаШагов,ТаблицаИзвестныхStepDefinition,ТаблицаСтрокПримеров,НомерСтрокиСценария,ТаблицаУжеСуществующихСценариев,СтруктураПараметров,МассивСценариевЗащитаОтЗацикливанияКеш = Неопределено,ЭтоЗагрузкаПодчиненногоСценария)
	...
	Если (ТекущийТипСценария = "background") или (ТекущийТипСценария = "scenario") или (ТекущийТипСценария = "scenario_outline") Тогда
		...
		Для каждого СтрТаблицаШагов Из ТаблицаШагов Цикл
			...
			ОбработатьПередачуПараметровТаблицей(СтрСтрокиШагов,СтрТаблицаШагов.ТаблицаСтрокПередачаПараметровТаблицей,РазныеИменнованныеПараметры);
			...
			Если ПараметрыКопировния.Свойство("СозданиеСекцииПримеров") Тогда
				...
				Если СтрОткудаКопировать.СтрокаПараметровШагаВВидеТаблицы = Истина Тогда //тогда надо обновить параметры внутри таблицы
					Для Каждого ПараметрТаблицы Из СтрОткудаКопировать.ПараметрыТаблицы Цикл
						Для Каждого ИменнованныйПараметр Из ПараметрыКопировния.СтрокаПримеров.ИменованныеПараметры Цикл
...
Процедура ОбработатьСтрокиЭмуляцияRegExp(Стр,ПараметрыСтроки,Символ,ДелатьОберткуПараметров = Ложь);
	...
	Пока Поз > 0 Цикл
		...
		Если Поз > 0 Тогда
			...
			ПараметрыСтроки.Добавить(СтрЗаменить(ЗначениеПараметра,ПредставлениеАпостроф,"'"));
