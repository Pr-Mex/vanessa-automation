				...
				Если СтрТаблицаШагов.НачалоОбласти Тогда
					Если СтрТаблицаШагов.Родитель <> Неопределено Тогда
						Если СтрТаблицаШагов.Родитель = -1 Тогда
							СтрокиШагов = СтрДеревоСтроки.Строки;
						Иначе	
							СтрокаДерева = ДеревоСтроки.Найти(СтрТаблицаШагов.Родитель,"НомерСтрокиВФиче",Истина);
							Если СтрокаДерева <> Неопределено Тогда
								//значит явно указываем эту строку в качестве родителя
...
Процедура ДобавитьШаг(ТаблицаШагов,ИмяШагаБезКлючевогоСлова,ИмяШага,НомерСтроки,МассивТаблицПередаваемыхКакПараметр,СруктураОбласти = Неопределено)
...
Функция ФичаИмеетКорректныеОтступыВНачалеСтрок(Тзн,ИмяФайла,ИмяФайлаОригинальное)
	...
	Для Каждого СтрТзн Из Тзн Цикл
		...
		Если КлючевоеСлово = "scenario" или КлючевоеСлово = "scenario_outline" Тогда
	...
	Для СчетчикСтрокФичи = 1 По КолСтрокТаблицаТекстФичи Цикл
		...
		Если Лев(Стр,1) = "#" Тогда //Это комментарий. Его игнорируем.
			...
			Если Лев(НРег(Стр),8) = "#область" Тогда
				...
				ДобавитьШаг(ТаблицаШагов,"","",НомСтр,"",СруктураОбласти);
		...
		Если (ТекущееКлючевоеСлово = "given") или (ТекущееКлючевоеСлово = "when") или (ТекущееКлючевоеСлово = "then") или (ТекущееКлючевоеСлово = "and") или (ТекущееКлючевоеСлово = "but") Тогда
			МассивТаблицПередаваемыхКакПараметр = Новый Массив;
			ДобавитьШаг(ТаблицаШагов,ВтораяЧастьСтроки,ИмяШага,НомСтр,МассивТаблицПередаваемыхКакПараметр);
