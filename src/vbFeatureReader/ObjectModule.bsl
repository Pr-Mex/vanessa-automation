...
Процедура СкопироватьСтрокуВДереваПодчиненныеЭлементы(КудаКопировать,ОткудаКопировать,ПараметрыКопирования = Неопределено)
	...
	Если ОткудаКопировать.ШагСПараметрамиВТаблице  = Истина Тогда
		...
		ОбработатьПередачуПараметровТаблицей(КудаКопировать,ТаблицаСтрокПередачаПараметровТаблицей,РазныеИменнованныеПараметры);
	КонецЕсли;	 
...
Функция ФичаСодержитТолькоТабыВНачалеСтрокиИНеСодержитПробелов(Тзн,ИмяФайла)
	Статус = Истина;
	ВыводилИмяФичи = Ложь;
	...
	Для Каждого СтрТзн Из Тзн Цикл
		...
		Если НашелПробелВНачалеСтроки Тогда
			Если Не ВыводилИмяФичи Тогда
				...
				Сообщить("В фиче <" + ИмяФайла + "> обнаружены строки с пробелами в начале и тэг @Tree. Вместо пробелов в начале строки должны быть символы табуляции.");
			КонецЕсли;	 
...
Функция ЗагрузитьФичуВТаблицуЗначений(ИмяФайла,НадоСтроитьДерево)
	...
	Если НадоСтроитьДерево Тогда
		...
		Если Не ФичаСодержитТолькоТабыВНачалеСтрокиИНеСодержитПробелов(Тзн,ИмяФайла) Тогда
