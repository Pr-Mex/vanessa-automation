	...
	Если МассивТаблицПередаваемыхКакПараметр.Количество() > 0 Тогда
		...
		Для Каждого ТаблицаСтрокПередачаПараметровТаблицей Из МассивТаблицПередаваемыхКакПараметр Цикл
			...
			Для каждого СтрТаблицаСтрокПередачаПараметровТаблицей Из ТаблицаСтрокПередачаПараметровТаблицей Цикл
				...
				СтрокаТаблицы.ТипКартинки                      = -1;
				Если СтрТаблицаСтрокПередачаПараметровТаблицей.Стр <> "" Тогда
...
Процедура СкопироватьСтрокуВДереваПодчиненныеЭлементы(КудаКопировать,ОткудаКопировать,ПараметрыКопирования = Неопределено)
	...
	Если ОткудаКопировать.ШагСПараметрамиВТаблице  = Истина Тогда
		...
		Если ПараметрыКопирования.Свойство("ЗначенияТаблицДляЗаменыВоВложенномСценарии") Тогда
...
Функция ФичаСодержитТолькоТабыВНачалеСтрокиИНеСодержитПробелов(Тзн,ИмяФайла,ИмяФайлаОригинальное)
	Статус = Истина;
	ВыводилИмяФичи = Ложь;
	...
	Для Каждого СтрТзн Из Тзн Цикл
		...
		Для Ккк = 1 По ДлинаСтроки Цикл
			...
			Если Символ = " " Тогда
				НашелПробелВНачалеСтроки = Истина;
			ИначеЕсли Символ = Символы.Таб Тогда
				//пропускаем - т.к. должны быть табы
			Иначе
				Прервать;
			КонецЕсли;	 
		КонецЦикла; 
		
		Если НашелПробелВНачалеСтроки Тогда
			Если Не ВыводилИмяФичи Тогда
				ВыводилИмяФичи = Истина;
				Сообщить("В фиче <" + ИмяФайлаОригинальное + "> обнаружены строки с пробелами в начале и тэг @Tree. Вместо пробелов в начале строки должны быть символы табуляции.");
			КонецЕсли;	 
...
Функция ЗагрузитьФичуВТаблицуЗначений(ИмяФайла,НадоСтроитьДерево,ИмяФайлаОригинальное)
	...
	Если НадоСтроитьДерево Тогда
		...
		Если Не ФичаСодержитТолькоТабыВНачалеСтрокиИНеСодержитПробелов(Тзн,ИмяФайла,ИмяФайлаОригинальное) Тогда
