...
Функция ПолучитьПараметрыПодключения(ТабПарам)
	...
	Для ккк = 1 По ИменаПараметров.Количество() Цикл
		...
		ИначеЕсли ВРег(ИмяПараметра) = ВРег("Версия платформы") Тогда			
			Стр.ВерсияПлатформы = ЗначениеПараметра;
		КонецЕсли;	
	КонецЦикла;
	...
	Если Не ЗначениеЗаполнено(Стр.ПутьКПлатформе) Тогда
		Если ЗначениеЗаполнено(Стр.ТипКлиента) Тогда
			Если ВРег(Стр.ТипКлиента) = ВРег("Толстый") Тогда
...
&НаСервере
Процедура НайтиКаталогПлатформы(ВерсияПлатформы)
