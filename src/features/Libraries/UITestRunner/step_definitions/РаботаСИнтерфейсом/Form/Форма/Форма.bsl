...
Процедура ВОткрытойФормеЯНажимаюНаКнопкуСЗаголовком(ИмяКнопки,ИскатьПоИмени = Ложь) Экспорт
	...
	Если НЕ Нашли Тогда
		Если ДопПараметры.Свойство("ТекущееОкно") Тогда
			Если ДопПараметры.ТекущееОкно <> Неопределено Тогда
				Если ДопПараметры.ТекущееОкно.Заголовок = "1С:Предприятие" Тогда
	...
	Если НЕ Нашли Тогда
		...
		Если ДопПараметры.Свойство("ТекущееОкно") Тогда
			Если ДопПараметры.ТекущееОкно <> Неопределено Тогда
				Стр = Стр + Символы.ПС + "ТекущееОкно=" + ДопПараметры.ТекущееОкно.Заголовок;
			КонецЕсли;	 
		КонецЕсли;	 
