...
Процедура УЭлементаЯЖдуЗначения(ИмяПоля,Значение,ИскатьПоИмени = Ложь) Экспорт
	УЭлементаЯЖдуЗначенияВТеченииСекунд(ИмяПоля,Значение,10,ИскатьПоИмени);
	
	
	//ТекущееЗначение = Ванесса.ПолучитьЗначениеРеквизитаОткрытойФормыПоЗаголовку(ИмяПоля,ИскатьПоИмени);
	//
	//Если НРег(ТекущееЗначение) <> НРег(Значение) Тогда
	//	ВызватьИсключение "У элемента <" + ИмяПоля + "> ожидали значение <" + Значение + ">, а получили <" + ТекущееЗначение + ">";
	//КонецЕсли;	 
	//
КонецПроцедуры
...
Процедура ЕслиПоявилосьОкноДиалогаЯНажимаюНаКнопку(ИмяКнопки) Экспорт
	...
	Если Кнопка = Неопределено Тогда
		ВызватьИсключение "Кнопка с заголовком <" + ИмяКнопки + "> не найдена!";
	КонецЕсли;	 
