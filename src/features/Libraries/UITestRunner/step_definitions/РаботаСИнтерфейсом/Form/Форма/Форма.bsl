...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ВТаблицеЯУстанавливаюФлаг(Парам01,Парам02)","ВТаблицеЯУстанавливаюФлаг","И     в табилце ""ТабличнаяЧасть1"" я устанавливаю флаг ""Реквизит булево""", "Устанавливает флаг в Истина", "UI.Таблицы.Флаги");
...
Функция ПолучитьРасхожденияТД(ФайлМакета, ТабПарам)
	...
	Для Каждого СтрокаОжидаемая Из ТабПарам Цикл
		...
		ИначеЕсли СтрокаТаблицыМакета.Значение <> ЗначениеПараметра Тогда
			СтрокаРасхождения = СтрШаблон("Значение параметра %1: ожидали <%2>, получили <%3>;", ИмяПараметра, ЗначениеПараметра, СтрокаТаблицыМакета.Значение);
			МассивРасхождений.Добавить(СтрокаРасхождения);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьВхождениеСтрокВТабличныйДокументСервер(ФайлМакета,МассивДляПоиска,ЗначенияВходят)
	...
	Если ЗначенияВходят Тогда
		Если МассивОшибок.Количество() > 0 Тогда
			...
			КонецЦикла;	
			ВызватьИсключение "Табилчный документ не содержит нужные строки <" + СтрокаИсключения + ">";
		КонецЕсли;	 
	Иначе	
		Если МассивНенайденныхСтрок.Количество() <>  МассивДляПоиска.Количество() Тогда  
			...
			Для Каждого Элем Из МассивОшибок Цикл
				СтрокаИсключения = СтрокаИсключения + Элем + ";";
			КонецЦикла;	
			ВызватьИсключение "Табилчный документ содержит ненужные строки <" + СтрокаИсключения + ">";
		КонецЕсли;	 
	КонецЕсли;	 
