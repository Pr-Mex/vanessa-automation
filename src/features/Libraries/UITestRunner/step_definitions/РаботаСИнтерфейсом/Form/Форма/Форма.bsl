...
Процедура ЯЗапоминаюЗначениеТаблицыПоляКак(ИмяТЧ,ИмяПоля,ИмяПеременной,ИскатьПоИмени = Ложь) Экспорт
	...
	//	//попробуем найти это поле и получить значение по заголовку
	//	ПолеТаблицы = ТЧ.НайтиОбъект(,,ИмяПоля);
	//	Если ПолеТаблицы <> Неопределено Тогда
	//		Значение = ТЧ.ПолучитьТекстЯчейки(ПолеТаблицы.ТекстЗаголовка); 
	//	Иначе
	//		Массив = ТЧ.НайтиОбъекты();
	//		Для Каждого Элем Из Массив Цикл
	//			Сообщить("Элем.Имя=" + Элем.Имя);
	//		КонецЦикла;	
	//		ВызватьИсключение "Не смог получить текст ячейки таблицы <" + ИмяТЧ + "> по строке <" + ИмяПоля + ">";
	//	КонецЕсли;	 
	//КонецПопытки;
