...
// Английские синонимы параметров VAParams.json, используемых этим скриптом.
// Соответствие синонимов взято из ПолучитьКоллекциюКоманд() формы ОбщегоНазначенияVA.
Функция СоответствиеАнглийскихСинонимов()

	Синонимы = Новый Соответствие;
	Синонимы.Вставить("projectpath", "КаталогПроекта");
	Синонимы.Вставить("logpath", "ПутьКФайлуДляВыгрузкиСтатусаВыполненияСценариев");
	Синонимы.Вставить("textlogname", "ИмяФайлаЛогВыполненияСценариев");

	Возврат Синонимы;

КонецФункции

// JSON-файл параметров может содержать имена параметров как на русском,
// так и на английском (синонимы). Приводит английские имена к русским.
Процедура НормализоватьПараметрыСборки(ПараметрыСборки)

	Для Каждого Синоним Из СоответствиеАнглийскихСинонимов() Цикл
		ЗначениеПараметра = Неопределено;
		Если НЕ ПараметрыСборки.Свойство(Синоним.Значение)
			И ПараметрыСборки.Свойство(Синоним.Ключ, ЗначениеПараметра) Тогда
			ПараметрыСборки.Вставить(Синоним.Значение, ЗначениеПараметра);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗапускVA(Аргументы)
	...
	НормализоватьПараметрыСборки(ПараметрыСборки);
