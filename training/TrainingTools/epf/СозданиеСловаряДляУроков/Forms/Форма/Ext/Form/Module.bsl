...
Процедура ПрочитатьДанные(Команда)
	...
	КешДанныеРаннегоПеревода = Новый Соответствие;
	Если ЗначениеЗаполнено(ИмяФайлаПеревода) Тогда
		...
		Для Каждого Элем Из ДанныеРаннегоПеревода Цикл
			КешДанныеРаннегоПеревода.Вставить(КлючПеревода(Элем.ru), Элем);
		КонецЦикла;	
...
&НаКлиенте
Функция ВернутьЭкранированиеСимволов(Стр)
...
&НаКлиенте
Функция ЗаменитьЭкранированиеСпецСимволов(Стр)
