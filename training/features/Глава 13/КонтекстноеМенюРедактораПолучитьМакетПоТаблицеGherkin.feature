...
Сценарий: Контекстное меню редактора. Получить макет по таблице Gherkin.
	...
	* Данный пункт меню позволяет по таблице Gherkin получить табличный документ.
		...
		И Пауза 1
		И Я делаю подсветку элемента VA "СгенерированныйСценарийКонтекстноеМенюПолучитьМакетПоТаблице" "Получить макет по таблице" и перемещаю курсор

	* При выполнении команды будет открыто окно макета.
		И я делаю клик по элементу формы VA UI Automation 'ЭтотСеанс' 'СгенерированныйСценарийКонтекстноеМенюПолучитьМакетПоТаблице' UI Automation	
		И Пауза 1
