...
Сценарий: Как устроены шаги сценариев? Снипеты шагов.
	...
	* Привет! В этом уроке я расскажу тебе про то, что такое снипет шага сценария и что означает сообщение "Не найдена процедура или функция для выполнения шага". Давай откроем второй экземпляр Ванессы в режиме обучения.
	...
	* В данном случае это ключевое слово Дано
		И я делаю подсветку фрагмента текста в редакторе VA в режиме обучения "Дано" 'Ключевое слово "Дано"' подсценарий
	* Затем из шага убираются все параметры.
	* В данном примере их два.
	* Первый параметр
		И я делаю подсветку фрагмента текста в редакторе VA в режиме обучения '"Параметр1"' 'Первый параметр' подсценарий
	* И второй параметр
		И я делаю подсветку фрагмента текста в редакторе VA в режиме обучения '"Параметр2"' 'Второй параметр' подсценарий
	* То что осталось от исходной строк^и называется снипетом шага.
	* В данном случае снипет будет выглядеть вот так
		И я делаю подсветку фрагмента текста в редакторе VA в режиме обучения "ПримерШагаСПервымПараметромИВторымПараметром" 'Снипет шага' подсценарий
	...
	* Не найдена процедура или функция для выполнения шага.
	...
	* Это сообщение означает, что для данной строк^и сценария не найден epf файл, который реализует выполнение шага, находящегося в этой строке.
		И я делаю подсветку в окне сообщений по части строки 'Не найдена процедура или функция для выполнения шага' 'Сообщение: "Пустой адрес снипета у шага"' 
		И Пауза 5
		
	* Также в редакторе строка шага была раскрашена специальным цветом.
		И я делаю подсветку текста в редакторе VA в режиме обучения с 7 по 7 строку "Специальная раскраска цветом" подсценарий
