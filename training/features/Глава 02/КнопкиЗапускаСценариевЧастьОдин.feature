...
Сценарий: Основные кнопки работы со сценариями. Часть один.
	...
	* Если перед этим сделать активной строку дерева, которая принадлежит какому-либо фича файлу, то именно этот файл откроется при переходе в редактор.
		Если существует элемент UI Automation "ЭтотСеанс" с именем "Интерактивная справка. При возникновении ошибки. Наименование" и типом "" тогда
			И я запоминаю строку "Интерактивная справка. При возникновении ошибки. Наименование" в переменную "ИмяПоля"
		Иначе		
			И я запоминаю строку "Интерактивная справка. При возникновении ошибки. Description" в переменную "ИмяПоля"
