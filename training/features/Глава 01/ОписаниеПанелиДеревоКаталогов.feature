...
Сценарий: Панель "Дерево каталогов"
	...
	* Теперь дерево каталогов заполнилось. Давай откроем какой-нибудь фича файл в редакторе. 
		...
		И я делаю клик в поле с ID "$IDЭлемента$" UI Automation
		И Пауза 0.1
	...
	* Фича файл загрузился. При этом строка стала выделена жирным шрифтом, т.к. этот файл является в данный момент активным в редакторе.
		И я запоминаю элемент "Issue-34-ОтчетAllure" с типом "" процесса "ЭтотСеанс" в переменную "IDЭлемента" UI Automation
		И я делаю подсказку балун у элемента "$IDЭлемента$" UI Automation
			| 'text' | 'Активный сценарий открытый в редакторе' |

	* О его структуре и как правильно писать сценарии мы поговорим в другом уроке.
	...
	* За ними идут файлы, для которых эта строка была найдена в их тексте.
