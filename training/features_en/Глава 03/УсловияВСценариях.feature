...
* And upload a test example.
		...
		И я загружаю фичи в VA в режиме обучения "$КаталогИнструментов$\training\features_en\ПримерыПримерУсловия1.feature"

* The first point. The condition step must start with the keyword If, and end with Then.
		И я делаю подсветку фрагмента текста в редакторе VA в режиме обучения подсценарий 'If current window header is "WindowHeader" Then' 'Example of condition step'
...
* Steps that are inside the condition are separated by an additional indent.
		И я делаю подсветку фрагмента текста в редакторе VA в режиме обучения подсценарий 'And Pause 1' 'Step inside the condition'
...
* Here is an example of a common step that is not a condition.
		И я делаю подсветку фрагмента текста в редакторе VA в режиме обучения подсценарий 'Дано Табличный документ "AttributeName" равен макету "TemplateName"' 'Common step'
* And here is an example of the same step when it is used as a condition.
		И я делаю подсветку фрагмента текста в редакторе VA в режиме обучения подсценарий 'Если Табличный документ "AttributeName" равен макету "TemplateName" Тогда' 'Common step become a condition'
...
* And at the end the word Then is added to the step text
		И Я делаю подсветку фрагмента текста в редакторе VA в строке в режиме обучения подсценарий 22 "Then" 'Добавлено слово "Then" в конце шага'
...
* Let's look at one more point.
		И я загружаю фичи в VA в режиме обучения "$КаталогИнструментов$\training\features_en\ПримерыПримерУсловия2.feature"
...
* The syntax of Vanessa Automation language allows the use of the ElsIf keyword.
		И Я делаю подсветку фрагмента текста в редакторе VA в строке в режиме обучения подсценарий 12 "ElsIf" 'Keyword ElsIf'
