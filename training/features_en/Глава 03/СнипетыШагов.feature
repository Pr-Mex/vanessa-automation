...
* And upload a test example.
		...
		И я загружаю фичи в VA в режиме обучения "$КаталогИнструментов$\training\features_en\ПримерыПримерСнипетыШагов.feature"
...
* In this case, this keyword is Given
		И я делаю подсветку фрагмента текста в редакторе VA в режиме обучения "Given" 'Ключевое слово "Given"' подсценарий
...
* In this case, the snippet will look like this
		И я делаю подсветку фрагмента текста в редакторе VA в режиме обучения "ExampleStepWithFirstParameterAndSecondParameter" 'Step snippet' подсценарий
...
* No procedure or function was found to perform the step.
...
* This message means that no epf file was found for the script line that executes the step on this line.
		И я делаю подсветку в окне сообщений по части строки 'Procedure or function to perform the step is not found' 'Сообщение: "Пустой адрес снипета у шага"' 
		И Пауза 5
