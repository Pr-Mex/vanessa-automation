...
* And upload a test example.
		...
		И я загружаю фичи в VA в режиме обучения "$КаталогИнструментов$\training\features_en\ПримерыЭкспорт\ЭкспортныеСценарииСТаблицами.feature"
...
* The first subscript is designed so that it has a step that takes the Gherkin table as a parameter.
		И я делаю подсветку фрагмента текста в редакторе VA в режиме обучения подсценарий "|\'Simple table Gherkin\'|" 'Pass table into a step'
...
* Let's see what possibilities we have for transferring tables to such subscripts.
		И я загружаю фичи в VA в режиме обучения "$КаталогИнструментов$\training\features_en\ПримерыПримерПодсценарии2.feature"
...
* Therefore, this example will replace the Gherkin table only in the second step of this subscript.
		И я делаю подсветку фрагмента текста в редакторе VA в режиме обучения подсценарий "And I call a subsript and pass several tables into it" 'Calling a subscript'
