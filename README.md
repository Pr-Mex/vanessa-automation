...
* [утилита для формирования отчеттов о проверки Allure](http://allure.qatools.ru/)

Cклонируйте данный репозиторий с помощью **ms-git**
...
### Использования в режиме проверки поведения пользваотельского интерфейса
...
* зафиксировали требования к информационной системе
* создали автоматизированные сценарии проверки в виде epf файлов
* разработали управляемые формы или рабочие столы конфиугурации в режиме прототипирования
...
* архитектор или эксперт 1С - если текущий сценарии некоректно спроектированы с точки зрения метаданных
...
Для запуска в консольном режиме используется понятие *профиль консольного запуска*. Профиль консольного запуска предназначен для удобной передачи параметров. Профиль запуска представлет собой текстовый файл в формате JSON.
...
* мы используем принцип самопроверки через feature файлы, поэтому перед разработкой новой функциональности мы также - разрабатываем feature файлы, генерируем шаблоны сецнариев и наполняем их кодом для проверки. Поэтому к доработкам без feature файлов мы относимся "холодно".
