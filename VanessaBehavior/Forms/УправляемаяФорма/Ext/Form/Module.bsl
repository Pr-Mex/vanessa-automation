...
Функция ПодключитьКонтекстОбработкиКлиент(Файл, ВерсияФайла = Неопределено)
	...
	Если ФормаОбработки <> Неопределено Тогда
		...
		Исключение
			...
			ЗаписатьЛогВЖРОшибка("VanessaBehavior.ПолучитьСписокТестов", ТекстСообщения);
...
Процедура ЗапуститьКлиентаТестирования(ДанныеЗапускаКлиентаТестирования, ДопПараметр = "")
	...
	ТекстСообщения = "СтрокаЗапуска=%1";
...
Процедура СерверныеВызовыПриОткрытииФормы(ДопПараметры)
	...
	ТекстСообщенийПользователю = ТекстСообщенийПользователю(МакетПереводСообщений);
	
	ДопПараметры.Вставить("ТекстСообщенийПользователю",ТекстСообщенийПользователю);
	
	СлужебныеПеременные = Новый Структура;
	СлужебныеПеременные.Вставить("ТекстСообщенийПользователю",ТекстСообщенийПользователю);
	ОбъектСервер.УстановитьСлужебныеПеременные(СлужебныеПеременные);
КонецПроцедуры 
