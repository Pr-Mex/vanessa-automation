...
Процедура ОткрытьФичаФайл(Команда)
	...
	Если ЗначениеЗаполнено(Объект.КомандаОткрытияФичиВТекстовомРедакторе) Тогда
		...
		КомандаДляЗапуска = СтрЗаменить(КомандаДляЗапуска,"%LineNumber%",XMLСтрока(СтрокаФичи.НомерСтрокиВФиче));
	КонецЕсли;	 
...
Процедура ЗапуститьСеансСлужебнойБазы(Имябазы, ТипЗапуска, Ключи) Экспорт
	...
	Если Статус <> 0 Тогда
		ТекстСообщения = "Не смог запустить конфигуратор. СтрокаЗапускаКонфигуратора=<%1>";
...
Процедура ВыполнитьШаг()
	...
	Если Объект.ВыводитьВЛогВыполнениеШагов Тогда
		ЗаписатьЛогВТекстИнформация("" + ТекущаяДата() + ". Шаг"," " + ТекШаг.Имя); 
	КонецЕсли;	 
